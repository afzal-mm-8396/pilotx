...
var changeEventHandler = function(event1) {
    ...
    let saveAttr = _LC.directive.save.getAttr(target._specialAttributeDetails);
    ...
    renderNodes(toAppend, dynamicNodes, helperNode, options, establishBindings, returnVal, templateContent, yieldComp) {
        ...
        for(let i=0;i<dynamicNodes.length;i++) {
            ...
            if(type ===  "componentDynamic") {
                ...
                let self = this;
                processDirectiveLazy.forEach(function(obj){  //eslint-disable-line no-loop-func
                    let attr = obj.attr;
                    let node = obj.node;
                    let staticProp;
                    ...
                    if(attr.hookNode){
                        _LC.directive.infoA(self,info,dynamicN,helperNode,attr,yieldComp,options);
                        if(attr.hookName == "prop" || attr.hookName == "data" ){
                            ...
                            _LC.directive.save.addNode(self,attr, dynamicN, gonnaSave,node,staticProp,helperNode,options,dynamicNodes);
                        }
                    }
                })
                //Added now
...
//                  }
        ...
        if(options.type !== "remove") {
            ...
            for(let k = firstIndexForIteration,v=lastIndexForIteration;v>0; k++, v--) {
                ...
                node._items[k] = {"_dynamicProperty" : {}, "itemProperty" : {}, "indexProperty": {}, "propProperty" : {}, "propNodes" : {}};
...
//                  if(options.type !== "update") {
                ...
                let propArr = Object.keys(node._items[k].propProperty)
                if(propArr.length){
                    let self = this;
                    propArr.forEach(function(propValue){     //eslint-disable-line no-loop-func
                        self.afterUpdateFor(self,node,propValue, optns)
                    })
                }
            }
            if(node._fRender) {
        ...
        for(let key in node._propNodes){
            this.setData(key, node._propNodes[key]);
            let cxObj = node._cx;
            if(cxObj){
                // if(cxObj.type == "for"){
                    // let newArr = [];
                    while(cxObj){
                        if(cxObj.type == "for" || cxObj.type == "forIn"){
                            ...
                            this.beforeUpdateFor(this, cxObj.node, key, cxObj.itemIndex);
                            cxObj = null;
                        }else{
...
//                    this.updateBlockHelpers(node._helpers[itemKey], optns);
                this.executeBlockHelpers(node._helpers[itemKey],node);
                    ...
                    let propArr = Object.keys(node._items[itemKey].propProperty)
                    if(propArr.length){
                        let self = this;
...
//                		this.handleBreak1(parentNode._cx , "continue");
        ...
        if(node.ownerElement && node.ownerElement._hooksPresent){
            let saveAttr = _LC.directive.save.getAttr(node.ownerElement._specialAttributeDetails);
            if(saveAttr){
...
//        		let attr = templateAttributes.attr[i];
        ...
        if(this._triggerOnInit) {
            let triggerOnInit = this._triggerOnInit;
            let self = this;
            triggerOnInit.forEach(function(obj){
                Lyte.Component.set(obj.scope.$data ,obj.value, obj.name == "data" ? self.$data : self, {force : true});
            })
            this._triggerOnInit = undefined;
        }
        if(this.__updateProp){
            if(this._cx){
                let forNode = this._cx.node;
                if(this._cx.type == "for" || this._cx.type == "forIn"){
                    let ind = this._cx.itemIndex;
                    if(this.__updateProp.data){
                        let self = this;
                        this.__updateProp.data.forEach(function(propVal){
                            if(forNode._propNodes[propVal]){
                                addBindings(self.$data,forNode._items[ind].propProperty[propVal]); //added so that child will listn and update parent
                                Lyte.Component.set(forNode._propNodes[propVal],ind, self.$data,{force : true});
                        ...
                        this.__updateProp.prop.forEach(function(propVal){
                            if(forNode._propNodes[propVal]){
                                Lyte.Component.set(forNode._propNodes[propVal][ind], "$data",self.$data, {force : true});
                            }
                        })
                    }
                }
                ...
                if(this.__updateProp.data){
                    let self = this;
                    this.__updateProp.data.forEach(function(propVal){
                        Lyte.Component.set(self._callee.component.data, propVal, self.$data, {force : true});
                    ...
                    let self = this;
                    this.__updateProp.prop.forEach(function(propVal){
                        Lyte.Component.set(self._callee.component.data,propVal, self,{force : true});
                    })
                }
            }
        }        
        ...
        "update":function(object, property, value, options, fromStore,oldValue,setterScope, actualProperty, fromParent,storeRecord){
        	...
        	if(!oldValue){
                ...
                if(fromComponent && fromComponent.tagName !== "LYTE-YIELD") {
                    ...
                    if(!options || ((window.isLyteWidget || window.__lyteEnablePromise) && typeof options == "object" && options.skipValidation != true)){
...
//                    }
        ...
        "getCtxVal" : function(context,val,self){
            if(context != undefined){
                if(window.isLyteWidget || window.__lyteEnablePromise) {
                    let ctxVal = context[val];
                    if(context && typeof context == "object" && !Array.isArray(context) && (context.localName && context.tagName && _LC.isCustomElement(context)) || (ctxVal && ctxVal.__lyteDirective)){
                        console.log("Dummy")
        ...
        "directive" : {
            "save" : {
                ...
                addNode : function(self, attr, dynamicN, gonnaSave, node, staticProp , helperNode, options, dynamicNodes){
                                                ...
                                                if(helperNode._dirObj.prop){
                                                    if(helperNode._dirObj.prop.indexOf(nodeVal) != -1){
                                                        ...
                                                        // if(establishBindings || estuh) {
                                                            ...
                                                            _LC.establishBindings({[nodeVal] : self.getProperty(nodeVal)} ,self.component.data);    
                ...
                updateNode : function(self, node, nodeValue){
                ...
                removeAllNode : function(self,spNodes){
                    if(spNodes){
                ...
                triggerListeners : function(self,saveAttr, eleProp){
                    let prop = self._properties;
                    if(saveAttr){
                ...
                triggerAllListeners : function(self,directiveName,target,switchContext){
                    ...
                    let listenerArr = _LC.directive.save.triggerMap[saveNodeName];
                    if(listenerArr){
                        listenerArr.forEach(function(prop){
                            _LC.directive.save.triggerData(prop, target);
                        })
                        if(switchContext){
                            let propName = target.hasAttribute("lyte-directive-prop") ? target.getAttribute("lyte-directive-prop") : target.getAttribute("lyte-directive-data")
                            let contextSwitchArray = [];
                            _LC.adCx(target, contextSwitchArray);
                            if(contextSwitchArray.length){
                                _LC.affectChanges(self.getProperty(propName));
                                for(let i=contextSwitchArray.length-1; i>=0; i--){
                                    _LC.rmCx(target, [contextSwitchArray[i]], true);
                                    _LC.affectChanges(self.getProperty(propName));
                                }
                            }
                        }
                ...
                triggerData : function(eleProp, target){
...
var widgetIframe;
window.DataBindingLayer = {
    //parentOnly
    "sendDataUpdateToFrame" : function(element, keyValuePairs) {
        let actualElement = element.getRootNode().host;
        if(actualElement._m) {
            origSendDataUpdate();
        } else {
            setTimeout(function() {
        ...
        function origSendDataUpdate() {
            let shadow = actualElement.getRootNode();
            let newKeyValuePairs = {};
            for(let key in keyValuePairs) {
                let value = keyValuePairs[key];
                if(!(value instanceof HTMLElement) && typeof value !== "function" && !(value instanceof Window) && !(value instanceof Document) && !(value instanceof ShadowRoot)) {
            ...
            try {
                window.lyteWidgetFrame.contentWindow.postMessage(obj, "*");
            } catch(e) {
                console.error("Error in sending data update to frame");
            }
        }
        
    },
    //parentOnly
    "sendLbindUpdate" : function(node, attr, value) {
        let attrDetails = node._attributeDetails;
        if(attrDetails && attrDetails[attr]) {
            if(attrDetails[attr].isLbind) {
    ...
    "getData" : function(id) {
        ...
        let elmId = splitArr[0];
        let dataId = splitArr[1];
        return this.parentMap[elmId][dataId];
    },
    //parentOnly
    "getElementMap" : function(elementCnt) {
        let parentMap = this.parentMap;
        let elementMap;
        if(elementMap = parentMap[elementCnt]) {
            return elementMap;
        } else {
            elementMap = parentMap[elementCnt] = {};
            return elementMap;
        }
    },
    //parentOnly
    "processDataInParent" : function(data, elementCnt) {
        if(typeof data != "object") {
            return;
        }
        if(!elementCnt) {
            elementCnt = data.__id.split("-")[0];
        }
        let elementMap = this.getElementMap(elementCnt);
        this.processData1(data, elementMap);
    },
    //parentOnly
    "processData1" : function(data, elementMap) {
        if(data && typeof data == "object") {
            if(data.__id) {
                let __id = data.__id;
                let dataCnt = __id.split("-")[1];
                elementMap[dataCnt] = data;
            ...
            if(Array.isArray(data)) {
                data.forEach(function(item) {
                    DataBindingLayer.processData1(item, elementMap);
                })
            } else {
                for(var key in data) {
                    DataBindingLayer.processData1(data[key], elementMap);
                }
            }
        }
    },
...
;(function(Lyte) {
    let _LC = Lyte.__lc;
    document.parentListeners = {};
    window._wrapperComponents = [];
    //For now only main Lyte
    window.addEventListener("message" , eventHandlerParent);
    async function eventHandlerParent(event) {
        var data = event.data;
        var widgetObj;
        if(data.c) {
            // console.log("from child", parentWidgetMappings, parentWidgetMappings[1]? parentWidgetMappings[1].count : 0);
            var mappings;
            if(data.x) {
                widgetObj = window.parentWidgetMappings[data.x];
                mappings = widgetObj.mappings;
            }
            if(data.x1) {
                widgetObj = window.parentBodyWidgetMappings[data.x1];
                if(!widgetObj) {
                    widgetObj = window.parentBodyWidgetMappings[data.x1] = { "mappings" : {}, "count" : 2, "id" :  data.x1};
                    var config = window.Lyte.getConfig("widget");
                    if(config) {
                        widgetObj.url = config.src;
                    }
                    var div = document.createElement("div");
                    window.bodyWidgetDiv.appendChild(div);
                    var shadow = div.attachShadow({"mode" : "open"});
                    // window._docShadows.push(shadow);
                    // docEventListenerHandler.attachShadow(shadow);
                    shadow._isBodyShadow = true;
                    shadow._id = data.x1;
                    window.parentBodyWidgetMappings[data.x1].mappings[1] = shadow;
                    widgetObj.shadow = shadow;
                    if(data.x2) {
                        var mainWidgetObj = window.parentWidgetMappings[data.x2];
                        registerEventsForShadow(mainWidgetObj.element.component, widgetObj.shadow, true, widgetObj);
                    }
                }
                mappings = widgetObj.mappings;
            }
            switch(data.c) {
                //DataBindingLayer Set
                case "dC" : {
                    let pData = DataBindingLayer.getData(data.e);
                    DataBindingLayer.processDataInParent(data.g);
                    Lyte.Component.set(pData, data.f, data.g );
                }
                break;
                case "dCInit" : {
                    var elementMap = DataBindingLayer.getElementMap(data.e);
                    DataBindingLayer.processDataInParent(data.g);
                    elementMap.__node.setData(data.f, data.g);
                }
                break;
                case "eventAck" : {
                    window.clean = true;
                ...
                break;
                case "u" : {
                    var elem = mappings[data.e];
                    elem.setAttribute(data.a, data.v);
                ...
                case "full" : {
                    ...
                    console.time("diff");
                    var ss = dd.diff(window.actMappings.base.children[1], data.html);
                ...
                case "getDetails" : {
                    var actData = data;
                    var target;
                    var isWindow = actData._m == "window"; 
                    switch(actData._m) {
                        case "window" : {
                            target = window;
                        }
                        ...
                        break;
                        default : {
                            target = mappings[actData._m];
                        }
                    }
                    var params = actData.params || [];
                    params.forEach(function(item, index) {
                        try {
                            if(item.__type  == "node") {
                                let mappings;
                                if(item.x1) {
                                    mappings = window.parentBodyWidgetMappings[item.x1].mappings;
                                } else if(item.x) {
                                    mappings = window.parentWidgetMappings[item.x].mappings
                                }
                                params[index] = mappings[item._m];
                            }
                        } catch(e) {
                    ...
                    if(actData.isProp) {
                        prop = target[actData.prop];
                    } else {
                        prop = target[actData.prop].apply(target, params);
                    }
    
                    if(prop instanceof CSSStyleDeclaration) {
                        prop = JSON.parse(JSON.stringify(prop));
                    }
                    
                    // console.log(prop);
                    window.lyteWidgetFrame.contentWindow.postMessage({"p" : "getDetails", "value" : prop, "count" : actData.count, "timeStamp" : +(new Date())}, "*");
                }
                break;
                case "1" : {
                    var actData = data.data;
                    // console.log("parent changes");
                    // console.time("Parent Implementation");
                    
                    for(var i=0;i<actData.length;i++) {
                        var target = mappings[actData[i].target];
                        switch(actData[i].type) {
                            case "childList" : {
                                for(var j=0;j<actData[i].addNodes.length;j++) {
                                    var ee = actData[i].addNodes[j];
                                    switch(ee.type) {
                                        case "append" : {
                                            target.appendChild(mappings[ee._m]);
                                        }
                                        break;
                                        case "element" : {
                                            // dummyTemplate.innerHTML = ee.value;
                                            // var element = dummyTemplate.content.children[0];
                                            window.ignoreParent = true;
                                            var element;
                                            let isSvg;
                                            let shadowComp;
                                            let isParentComp = window._wrapperComponents.indexOf(ee.tagName) != -1 && ee.elementCnt;
                                            if(ee.tagName == "svg") {
                                                isSvg = true;
                                                element = document.createElementNS("http://www.w3.org/2000/svg", "svg");
                                            } else {
                                                let oldIgnore = window.ignoreParent;
                                                if(isParentComp ) {
                                                    element = document.createElement(ee.tagName);
                                                    element.attachShadow({"mode" : "open"});
                                                    docEventListenerHandler.attachShadow(element.shadowRoot);
                                                    _widgetLyte.__lc.shadow.attachEvents(element.shadowRoot);
                                                    window.ignoreParent = false;
                                                    window.__lyteConfig.isWidgetComponent = true;
                                                    shadowComp = document.createElement(ee.tagName);
                                                    shadowComp.setAttribute("comp-in-parent", "");
                                                    element._shadowComp = shadowComp;
                                                    element.shadowRoot.appendChild(shadowComp);
                                                    handleInitPropertiesParent(shadowComp, ee);
                                                    window.ignoreParent = oldIgnore;
                                                    shadowComp.setAttribute("lyte-widget", "");
                                                    window.__lyteConfig.isWidgetComponent = false;
                                                } else {
                                                    element = document.createElement(ee.tagName);
                                                    // handleInitPropertiesParent(element, ee);
                                                }
                                            }
                                            window.ignoreParent = false;
                                            ...
                                            for(var key in ee.attrs) {
                                                if(key == "node-id") {
                                                    oldElement.setAttribute(key, ee.attrs[key])
                                                } if(key == "src" || key == "href") {
                                                    let _url;
                                                    if(ee.attrs[key].startsWith("blob:") || ee.attrs[key].startsWith("mailto:") || ee.attrs[key].startsWith("http://") || ee.attrs[key].startsWith("https://")) {
                                                        _url = ee.attrs[key] 
                                                    } else {
                                                        _url = widgetObj.url + "/" + ee.attrs[key];
                                                    }
                                                    if(isSvg) {
                                                        element.setAttributeNS(null, key, _url);
                                                    } else {
                                                        element.setAttribute(key, _url);
                                                    }
                                                } else if(key == "lyte-src" || key == "lyte-href") {
                                                    let actualAttrName = key.substring(5);
                                                    if(ee.attrs[key].startsWith("blob:") || ee.attrs[key].startsWith("mailto:") || ee.attrs[key].startsWith("http://") || ee.attrs[key].startsWith("https://")) {
                                                        element.setAttribute(actualAttrName, ee.attrs[key]);    
                                                    } else {
                                                        element.setAttribute(actualAttrName, widgetObj.url + "/" + ee.attrs[key]);    
                                                    }
                                                } else if(key.startsWith("on-") && isParentComp) {
                                                    let x = data.x;
                                                    // var methodName = key;
                                                    var methodCamelCase = _LC.String.toCamelCase(key);
                                                    let elm1 = element;
                                                    element.setMethods(methodCamelCase, function() { //eslint-disable-line no-loop-func
                                                        let cnt = window._mC++;
                                                        let methodMap = window._mCMap[cnt] = {};
                                                        var promise = new Promise(function(resolve, reject) {//eslint-disable-line no-loop-func
                                                            methodMap.resolve = resolve;
                                                            methodMap.reject = reject;
                                                        });
                                                        var args = Array.from(arguments);
                                                        var sendArgs = [];
                                                        args.forEach(function(item) {//eslint-disable-line no-loop-func
                                                ...
                                                } else if(key != "xmlns" && key != "xml" && key  != "xmlns:xlink" && key != "xml:space") {
                                                    if(isSvg) {
                                                        element.setAttributeNS(null, key, ee.attrs[key]);
                                                    } else {
                                                        //default handler
                                                        if(typeof ee.attrs[key] != "object") {
                                                            element.setAttribute(key, ee.attrs[key]);
                                                        } else {
                                                            var attrDetails = element._attributeDetails = element._attributeDetails || {};
                                                            attrDetails[key] = {"isLbind" : true};
                                                            element.setAttribute(key, ee.attrs[key].value);
                                                        }
                                                    }
                                                }
                                            }
                                            element = oldElement;
                                            element._m = widgetObj.count++;
                                            iterateNodesParent(element, ee.childNodes, widgetObj);
                                            if(ee.prev) {
                                                var previousSibling = mappings[ee.prev];
                                                previousSibling.parentNode.insertBefore(element, previousSibling.nextSibling);
                                            } else {
                                                target.appendChild(element);
                                            }
                                            // element._m = ee._m;
                                            mappings[element._m] = element;
                                        }
                                        break;
                                        case "node" : {
                                            var text = document.createTextNode(ee.value);
                                            text._m = widgetObj.count++;
                                            mappings[text._m] = text;
                                            if(ee.prev) {
                                                var previousSibling = mappings[ee.prev];
                                                previousSibling.parentNode.insertBefore(text, previousSibling.nextSibling);
                                            } else {
                                ...
                                for(var k=0;k<actData[i].removedNodes.length;k++) {
                                    var remNode = actData[i].removedNodes[k];
                                    var actualNode = mappings[remNode.node];
                                    actualNode.remove();
                                }
                            }
                            break;
                            case "attributes" : {
                                if(target._shadowComp) {
                                    target = target._shadowComp;
                                }
                                if((target.tagName == "INPUT" || target.tagName == "TEXTAREA") && actData[i].attributeName == "value") {
                                    target.value = actData[i].attributeValue;
                                } 
                                if(actData[i].attributeName != "onmouseover") {
                                    var attrName = actData[i].attributeName;
                                    if(attrName == "lyte-src" || attrName == "lyte-href") {
                                        attrName = attrName.substr(5);
                                    }
                                    if(actData[i].remove) {
                                        target.removeAttribute(attrName);
                                    } else {
                                        target.setAttribute(attrName, actData[i].attributeValue);	
                                    }
                            ...
                            break;
                            case "characterData" : {
                                target.nodeValue = actData[i].value;
                            }
                        }
                    }
                    // console.timeEnd("Parent Implementation")
                    // var dd = new diffDOM.DiffDOM();
                    // console.time("diff");
                    // var ss = dd.diff(window.actMappings.base.children[1], data.full);
                    // console.timeEnd("diff");
                    // console.log(ss);
                }
                break;
                case "1ar" : {
                    window.Lyte.Widget.__acMap[data.re].resolve();
                }
                break;
                case "2" : {
                    Lyte._allowReset = true;
                    widgetObj.mappings[data.y].reset();
                    Lyte._allowReset = false;
                } 
                break;
                // Init of frame completed;
                case "3" : {
                    var iframe = window.lyteWidgetWindows.get(event.source);
                    let config = window.Lyte.getConfig("widget");
                    if(!config.deferInit) {
                        iframe.onloadNew();
                    }
                    // window.childWin.onloadNew();
                }
                break; 
                case "4" : {
                    var target = mappings[data.z];
                    target.getRootNode()._addEventListener(target, data.y, data.w);
                }
                break; 
                // Document event listener addition
                case "4a" : {
                    // if(data.y == "click") {
                    //     return;
                    // }
                    var eventTypeObj = document.parentListeners[data.y];
                    if(!eventTypeObj) {
                        eventTypeObj = document.parentListeners[data.y] = {
                            "mainFunction" : function(event) {
                                if(event._doneAlready) {
                                    return;
                                }
                                var eventObj = {};
                                for(var key in event) {
                                    if(!(event[key] instanceof HTMLElement) && !(event[key] instanceof SVGElement) && typeof event[key] !== "function" && !(event[key] instanceof Window) && !(event[key] instanceof Document) && key !== "sourceCapabilities" ) {
                                        eventObj[key] = event[key];
                                    }
                                }
                                window.lyteWidgetFrame.contentWindow.postMessage({"p" : "documentListener", "type" : data.y, "eventObj" : eventObj}, "*");
                            }, 
                            "instances" : {}
                        }
                        document.addEventListener(data.y, eventTypeObj.mainFunction);
                    }
                    
                    eventTypeObj.instances[data.z] = true;
                }
                break;
                case "4aR" : {
                    Object.keys(document.parentListeners)
                }
                break;
                case "5" : {
                    var target = mappings[data.z];
                    target.getRootNode()._removeEventListener(target, data.y, data.w)
                }
                break;
                case "5a" : {
                    let eventTypeObj = document.parentListeners[data.y];
                    if(eventTypeObj) {
                        delete eventTypeObj.instances[data.z];
                        if(!Object.keys(eventTypeObj.instances).length) {
                            document.removeEventListener(data.y, eventTypeObj.mainFunction);
                        }
                    }
                }
                break;
                case "6" : {
                    widgetIframe ? widgetIframe.remove() : undefined;
                }
                break;
                case "7" : { 
                    // console.log("Error occurred in widget ", widgetObj? widgetObj.element : "", data.y);
                    try {
                        if(widgetObj.element) {
                            widgetObj.element.component.executeMethod("onInternalError", data.y);
                        } else {
                            console.log("Error occurred in widget ", data.y); 
                        }
                    } catch(e) {
                ...
                case "8" : {
                    var iframe = window.lyteWidgetWindows.get(event.source);
                    iframe.globalResourcesResolve(data.done);
                }
                break; 
                case "9" : {
                    try {
                        if(widgetObj.element) {
                            widgetObj.element.component.executeMethod("onReady");
                        } else {
                ...
                break;
                case "14r" : {
                    let mapEntry = window._mCMap[data.r1];
                    delete window._mCMap[data.r1];
                    if(data.s == 0) {
                        mapEntry.reject();
                    } else {
                        mapEntry.resolve(data.r);
                    }
                }
                break;
                case "changeVal" : {
                    var prop = data.v;
                    var target = mappings[data.y];
                    target[data.v] = data.z;
                }
            }
        }
    }
    window.parentWidgetMappings = {};
    window.parentBodyWidgetMappings = {};
    window.lyteWidgetCnt1 = 0;
    var bodyWidgetDiv = window.bodyWidgetDiv= document.createElement("div");
    function registerEventsForShadow(self, shadow, flag, subwidgetObj) {
        // var url = window.parentWidgetMappings[self.id].url;
        shadow =  shadow || self.widgetShadow;
        var childWin = self.iframe.contentWindow;
        shadow._eL = {};
        // For adding listener for a specific element inside the shadow
        shadow._addEventListener = function(target, type, id) {
            shadow._eL[id] = function(event) {
                var eventObj = { "button": event.button };
                childWin.postMessage({"p" : "3" , "x" : self.id, "1" : target._m, "2" : type, "3" : target.value, "4" : eventObj},"*");
            }; 
            target.addEventListener(type, shadow._eL[id]);
        };
        shadow._removeEventListener = function(target, type, id) {
            target.removeEventListener(type, shadow._eL[id]);
        }
        shadow.addEventListener("click", function(event) {
            if(event.target.tagName == "INPUT" && event.target.getAttribute("type") == "file") {
                ...
                if(window.clean || true) {
                    event._doneAlready = true;
                    var eventObj = {"button" : event.button};
                    childWin.postMessage({"p" : "3", "x" : self.id , "x1" : flag ? subwidgetObj.id : undefined, "1" : event.target._m, "2" : "click", "timeStamp" : new Date(), "4" : eventObj}, "*");
                    // childWin.postMessage({"type" : "event", "element" : event.target._m, "evType" : "click", "timeStamp" : +(new Date())}, "*");
                    
                }
            }
            
        });
        var changeEvents = ["change" , "input"];
        changeEvents.forEach(function(item) {
            shadow.addEventListener(item, function(event) {
                var valueComp;
                var target = event.target;
                if(target.tagName == "INPUT") {
                    ...
                    switch(tagType) {
                        case "file" : {
                            valueComp = target.files;
                        }
                        break;
                        case "radio" : 
                        case "checkbox" : {
                            valueComp = target.checked;
                        }
                        break;
                        default : 
                        valueComp = target.value;
                    }
                } else {
                    valueComp = target.value;
                }
                // if(event.target.tagName == "INPUT" && event.target.getAttribute("type") == "file") {
                // 	childWin.postMessage({"p" : "3" , "x" : self.id, "1" : event.target._m, "2" : "changeFile", "3" : event.target.files}, url)
                // 	// childWin.postMessage({"type" : "event" , "element" : event.target._m, "evType" : "changeFile", "value" : event.target.files, "timeStamp" : +(new Date())}, "*")
                // } else {
                // 	childWin.postMessage({"p" : "3" , "x" : self.id, "1" : event.target._m, "2" : "change", "3" : event.target.value}, url)
                // 	// childWin.postMessage({"type" : "event" , "element" : event.target._m, "evType" : "change", "value" : event.target.value, "timeStamp" : +(new Date())}, "*")
                // }
                childWin.postMessage({"p" : "3" , "x" : self.id, "1" : target._m, "2" : item, "3" : valueComp}, "*")
                
            });
        })
    
        var preventedEvents = ["submit", "reset"];
        preventedEvents.forEach(function(item) {
            shadow.addEventListener(item, function(event) {
                if(event.type == "reset" && Lyte._allowReset) {
                    return;
                }
                event.preventDefault();
                // childWin.postMessage({"p" : "3" , "x" : self.id, "1" : event.target._m, "2" : event.type, "3" : event.target.value},url)
            });
        })
        
        var arr = ["blur", "focus", "focusin", "focusout", "keydown", "keyup", "keypress", /* "mousemove",*/ "mousedown", "mouseup", "mouseout", "mouseover", "mouseleave","dragstart", "drop" ,"dragover"];
        // var arr= [];
        for(var i=0;i<arr.length;i++) { //eslint-disable-line no-loop-func
            shadow.addEventListener(arr[i], function(event) { //eslint-disable-line no-loop-func
                if(event.type == "dragover") {
                    event.preventDefault();
                }
                if(event.type == "drop") {
                    console.log("drop in parent");
                }
                let eventObj = {};
                for(var key in event) {
                    if(!(event[key] instanceof HTMLElement) && typeof event[key] !== "function" && !(event[key] instanceof Window) && !(event[key] instanceof Document) && key !== "sourceCapabilities" && !(event[key] instanceof ShadowRoot)) {
                        eventObj[key] = event[key];
                    }
                }
                let passObj = {"p" : "3", "1" : event.target._m, "2" : event.type, "3" : event.target.value, "4" : eventObj};
                if(event.target.getRootNode()._isBodyShadow) {
                    passObj.x1 = event.target.getRootNode()._id;
                } else {
                    passObj.x = self.id;
                }
                if(passObj["4"]) {
                    ["relatedTarget", "toElement", "srcElement", "target", "fromElement"].forEach(function(item,index) { //eslint-disable-line no-loop-func
                        passObj["4"][item] = passObj["4"][item] ? passObj["4"][item]._m : undefined;
                    })
                }

                childWin.postMessage(passObj,"*")
                // childWin.postMessage({"type" : "event" , "element" : event.target._m, "evType" : event.type, "value" : event.target.value, "timeStamp" : +(new Date())},"*")
            }, (arr[i].indexOf("focus") != -1) || arr[i] == "blur" ? true : false);
        }
    }
    /*disable async function*/
    function handleInitPropertiesParent(element,ee) {
        element.__widgetP = true;
        element.setData(ee.initProperties);
        element.__elmCnt = ee.elementCnt;
        var elementMap = DataBindingLayer.getElementMap(ee.elementCnt);
        elementMap.__node = element;
        DataBindingLayer.processDataInParent(ee.initProperties, ee.elementCnt);
    }
    console.log("abc");
    /*disable async function*/
    function iterateNodesParent(element, childNodes, widgetObj, isSvgParent) {
        ...
        childNodes.forEach(function(item) {
            if(item.type == "element") {
                ...
                let shadowComp;
                window.ignoreParent = true;
                let isParentComp = window._wrapperComponents.indexOf(item.tagName) != -1 && item.elementCnt;
                if(item.tagName == "svg" || isSvgParent) {
                    isSvgParent = true;
                    newElement = document.createElementNS("http://www.w3.org/2000/svg", item.tagName);
                } else {
                    let oldIgnore = window.ignoreParent;
                    if(isParentComp) {
                        newElement = document.createElement(item.tagName);
                        newElement.attachShadow({"mode" : "open"});
                        docEventListenerHandler.attachShadow(newElement.shadowRoot);
                        _widgetLyte.__lc.shadow.attachEvents(newElement.shadowRoot);
                        window.ignoreParent = false;
                        window.__lyteConfig.isWidgetComponent = true;
                        shadowComp = document.createElement(item.tagName);
                        shadowComp.setAttribute("comp-in-parent", "");
                        newElement._shadowComp = shadowComp;
                        newElement.shadowRoot.appendChild(shadowComp);
                        handleInitPropertiesParent(shadowComp, item);
                        window.ignoreParent = oldIgnore;
                        newElement.setAttribute("lyte-widget", "");
                        window.__lyteConfig.isWidgetComponent = false;
                    } else {
                        newElement = document.createElement(item.tagName);
                        // handleInitPropertiesParent(newElement, item);
                    }
                }
                // element.appendChild(newElement = document.createElement(item.tagName));
                element.appendChild(newElement);
                window.ignoreParent = false;
                let oldElement = newElement;
                if(isParentComp) {
                    newElement = shadowComp;
                }
                if(item.tagName != "SCRIPT") {
                    for(var key in item.attrs) {
                        var attrUrl = item.attrs[key];
                        switch(key) {
                            case "lyte-src" : 
                            case "lyte-href" : {
                                    // Check if the url is absolute using the url protocol.
                                    var actualAttrName = key.substr(5);
                                    if(!(attrUrl.startsWith("http://") || attrUrl.startsWith("https://"))) {
                                        newElement.setAttribute(actualAttrName, widgetObj.url + "/" + item.attrs[key]);    
                                    } else {
                                        newElement.setAttribute(actualAttrName, item.attrs[key]);    
                                    }
                            }
                            break;
                            case "node-id" : {
                                oldElement.setAttribute(key, item.attrs[key]);
                            }
                            break;
                            case "xmlns" : 
                            case "xml" : 
                            ...
                            default : { 
                                if(isSvgParent) {
                                    newElement.setAttributeNS(null, key, item.attrs[key]);
                                } else {
                                    if(key.startsWith("on-") && isParentComp) {
                                        let _id = widgetObj.shadow._id;
                                        // var methodName = key;
                                        var methodCamelCase = _LC.String.toCamelCase(key);
                                        let elm1 = newElement;
                                        newElement.setMethods(methodCamelCase, function() {
                                            let cnt = window._mC++;
                                            let methodMap = window._mCMap[cnt] = {};
                                            var promise = new Promise(function(resolve, reject) {
                                                methodMap.resolve = resolve;
                                    ...
                                    try {
                                        // default handler
                                        if(typeof item.attrs[key] != "object") {
                                            newElement.setAttribute(key, item.attrs[key]);
                                        } else {
                                            var attrDetails = newElement._attributeDetails = newElement._attributeDetails || {};
                                            attrDetails[key] = {"isLbind" : true};
                                            newElement.setAttribute(key, item.attrs[key].value);
                                        }
                                        // newElement.setAttribute(key, item.attrs[key]);
                                    } catch(e) {
                ...
                newElement._m = widgetObj.count++;
                mappings[newElement._m] = newElement;
                iterateNodesParent(newElement, item.childNodes, widgetObj, isSvgParent);
                element.appendChild(newElement);
            } else {
                var textNode = document.createTextNode(item.value);
                element.appendChild(textNode);
                textNode._m = widgetObj.count++;
                mappings[textNode._m] = textNode;
            }
        })
    }
   
    window.lyteWidgetWindows = new Map();
    Lyte.domContentLoaded(function() {
    document.body.appendChild(bodyWidgetDiv);
    window.Lyte.Component.register("lyte-widget-child", {
        _template:"<template tag-name=\"lyte-widget-child\"> <lyte-yield yield-name=\"yield\"></lyte-yield> </template>",
        _dynamicNodes : [{"type":"insertYield","position":[1]}]
        });
    var loadedWidgets = {};
    
    window.Lyte.Component.register("lyte-widget", {
            _template:"<template tag-name=\"lyte-widget-child\"> <lyte-yield yield-name=\"yield\"></lyte-yield> </template>",
            _dynamicNodes : [{"type":"insertYield","position":[1]}],
            _observedAttributes :["data","name","baseComponent","src"],
            data : function(){
                return {
                    data : Lyte.attr("object", {"watch" : true}), 
                    name : Lyte.attr("string"), 
                    baseComponent : Lyte.attr("string"),
                    src : Lyte.attr("string"),
                    resources : Lyte.attr("array")
                }		
            }, 
            "methods" : {
                "onInternalError" : function(error) {
                    var printError = true;
                    if(this.getMethods("onError")) {
                        var retVal = this.executeMethod("onError", error);
                        if( retVal === false) {
                            printError = false;
                        }
                    } 
                    if(printError) {
                        console.log("Error occurred in widget ", this.$node, error);
                    }
                }, 
                "onReady" : function() {
            ...
            init : function() {
                this.$node.removeWidget = function() {
                    _LC.ignoreDisconnect = ltCf.ignoreDisconnect = true;
                    this.remove();
                    _LC.ignoreDisconnect = ltCf.ignoreDisconnect = false;
                    this.__widgetRemoved = true;
                    return this;
                }
                this.$node.restoreWidget = function(outlet) {
                    if(!this.__widgetRemoved) {
                        console.log("Widget hasn't been removed yet.");
                    } else {
                        if(outlet instanceof HTMLElement) {
                            outlet.appendChild(this);
                        } else {
                            console.log("Outlet specified for restoring the widget isn't an valid element");
                        }
                    }
                }
                
                if(!window.lyteWidgetFrame) {
                    createWidgetIframe(window.Lyte);
                }
                this.iframe = widgetIframe = window.lyteWidgetFrame;
            },
            observeAttrs : function(changeObj) {
                if(changeObj.type == "deepChange") {
                    if(changeObj.path.startsWith(".")) {
                        path = changeObj.path.substring(1);
                    } else {
                        path = changeObj.path;
                    }
                    this.iframe.contentWindow.postMessage({
            ...
            didConnect : function() {	
                var lyteConfig = window.Lyte.getConfig("widget");
                // if(!lyteConfig || !lyteConfig.resources || !lyteConfig.resources[this.getData("name")]) {
                    // console.error("Given widget is not present");
                    // return;
                // }
                this.childDiv = this.$node.children[0];
                this.widgetShadow = this.childDiv.attachShadow({"mode" : "open"});
                // window._docShadows.push(this.widgetShadow);
                // docEventListenerHandler.attachShadow(this.widgetShadow);
                // _widgetLyte.__lc.shadow.attachEvents(this.widgetShadow);
                // _LC.shadow.attachEvents(this.widgetShadow);
                this.mappings = {};
                if(this.iframe.loadSuccessful) {
                    this.iframeFunc();
                } else {
                    this.iframe.pendingWidgets.push(this);
                }
            },
            didDestroy : function() {
                var iframe = this.iframe;
                var widgetObj = window.parentWidgetMappings[this.id];
                if(iframe.loadSuccessful) {
                    iframe.contentWindow.postMessage({
                        "p" : "9",
                        "1" : this.id
                    }, "*");
                } else {
                    var pendingWidgets = iframe.pendingWidgets;
                    pendingWidgets.splice(pendingWidgets.indexOf(this), 1);
                }
                let indexOf = docEventListenerHandler.docShadows.indexOf(this.$node.children[0].shadowRoot);
                docEventListenerHandler.docShadows.splice(indexOf, 1);
                
            },
            getDependentWidgets : function(name, dependencies, retArr) {
                let widgets;
                if(widgets = dependencies[name]) {
                    for(var i=0;i<widgets.length;i++) {
                        if(retArr.indexOf(widgets[i])) {
                            retArr.push(widgets[i]);
                            this.getDependentWidgets(widgets[i], dependencies, retArr);
                        }
                    }
                }
            ...
            iframeFunc : function() {
                this.id =  this.widgetShadow._id = ++window.lyteWidgetCnt1;
                var widgetObj = window.parentWidgetMappings[this.id] = {};
                var widgetName = this.getData("name");
                widgetObj.widgetName = widgetName;
                widgetObj.element = this.$node;
                widgetObj.shadow = this.widgetShadow;
                widgetObj.mappings = {};
                widgetObj.mappings[1] = this.widgetShadow;
                widgetObj.url = this.getData("src");
                this.widgetShadow._m = 1;
                widgetObj.count = 2;
                // Initial registration of the widget in the child.a
                var config = window.Lyte.getConfig("widget");
                if(!widgetObj.url && config.src) {
                    widgetObj.url = config.src;
                }
                var widgetsToLoad = [widgetName];
                this.getDependentWidgets(this.getData("name"), config.dependencies || {}, widgetsToLoad);
                var self = this;
                widgetsToLoad.forEach(function(item,index) {
                    
                    let wName = item;
                    // config = config && config[wName] ? config[wName] : undefined;
                    var fObj = {
                        "p" : index == 0 ? "1" : "0",
                        "1" : self.id,
                        "w" : wName
                    };
                    // var resources = this.getData("resources") ? this.getData("resources") : (config && config.resources )? config.resources : [];
                    var resources = config.resources ? (config.resources[wName] || []) : [];
                    if(!loadedWidgets[wName] && resources && resources.length){
                        fObj.src = resources;
                    }
                    self.iframe.contentWindow.postMessage(fObj, "*");
                });
                ...
                // if(config.dependencies && config.dependencies[wName]) {
                //     var dependencies = config.dependencies[wName];
                //     for(var i=0;i<dependencies.length;i++) {
                //         let wName = dependencies[i];
                //         // config = config && config[wName] ? config[wName] : undefined;
                //         let fObj = {
                //             "p" : "0",
                //             "w" : wName
                //         };
                //         // var resources = this.getData("resources") ? this.getData("resources") : (config && config.resources )? config.resources : [];
                //         let resources = config.resources ? (config.resources[wName] || []) : [];
                //         if(!loadedWidgets[wName] && resources && resources.length){
                //             fObj.src = resources;
                //             this.iframe.contentWindow.postMessage(fObj, "*");
                //         }   
                //     }
                // }
                var self = this;
                registerEventsForShadow(this);
                setTimeout(function() {
                    // To construct the base component along with the data 
                    self.iframe.contentWindow.postMessage({
                        "p" : "2",
                        "x" : self.id,
                        "1" : self.getData("baseComponent"),
                        "2" : self.getData("data"),
                        "w" : widgetName
                    }, "*");
                });
...
})(window._widgetLyte || window.Lyte);
window._wrapperComponents = [];
function createWidgetIframe(Lyte) {
    var iframe = document.createElement("iframe");
    var config = window.Lyte.getConfig("widget");
    var resourcesInParent = config.resourcesInParent;
    if(resourcesInParent && resourcesInParent.length) {
        var styles = [];
        var scripts = [];
        resourcesInParent.forEach(function(item) {
            let url = item;
            let type; 
        ...
        Lyte.injectResources(scripts).then(function(data) {
            iframe.resourcesInParentResolve(true);
        }, function(data) {
            iframe.resourcesInParentResolve(false);
        });

        styles.forEach(function(style) {
            docEventListenerHandler.registerStyle(style);
        })            
    }
    window._mC = 0;
    window._mCMap = {};
    
    iframe.pendingWidgets = [];
    // iframe.setAttribute("sandbox", "allow-scripts allow-forms allow-scripts ")
    // iframe.setAttribute("style" , "display:none");
    if(config && config.src){
        iframe.setAttribute("src", config.src);
        // iframe.setAttribute("sandbox","allow-scripts allow-same-origin");
        // iframe.setAttribute("style", "height:1000px;width:1000px;");
        iframe.setAttribute("style", "opacity:0;height:0px;width:0px;position:absolute;left:700px;top:0px");
        window.lyteWidgetFrame = iframe;
        function iframeOnLoad() {
            ...
            if(config.globalResources) {
                if(!config.globalResources.files) {
                    console.error("No global Resources mentioned");
                } else {
                    setTimeout(function() {
                        iframe.contentWindow.postMessage({
                            "p" : "12",
                            "1" : config.globalResources.files
                        }, "*")
                    },100)   
                }
            }
            
            window.lyteWidgetWindows.set(iframe.contentWindow, iframe);
        }
        if(!config.deferInit) {
            iframe.onload = iframeOnLoad;
        } else {
            window.Lyte.Widget.init = function() {
                iframeOnLoad();
                iframe.onloadNew();
            }
        }
        iframe.resourceDownloadsDone =  new Promise(function(mainResolve, mainReject) {
            let resources = ["resourcesInParent", "globalResources"];
            let promiseArray = [];
            resources.forEach(function(item) {
                if(config[item]) {
                    let prom = new Promise(function(resolve, reject) {
                        iframe[item+"Resolve"] = function(success) {
                            if(success) {
                                resolve();
                                if(config[item].success) {
                                    config[item].success();
                                }
                            } else {
                                reject();
                                if(config[item].failure) {
                                    config[item].failure();
                                }
                            }
                        }
            ...
            if(!promiseArray.length) {
                ...
                Lyte.resolvePromises(promiseArray).then(function() {
                    mainResolve();
                },function() {
                    mainReject();
                });
        ...
        iframe.onloadNew = function() {
            iframe.loadSuccessful = true;
            iframe.resourceDownloadsDone.then(function() {
                let wrapperComps = Object.keys(_widgetLyte.__lc._registeredComponents);
                for(var i=0;i<wrapperComps.length;i++) {
                    wrapperComps[i] = wrapperComps[i].toUpperCase();
                }
                iframe.contentWindow.postMessage({
                    "p" : "parentComps", 
                    "1" : wrapperComps
                }, "*");
                for(var i=0;i<iframe.pendingWidgets.length;i++) {
                    iframe.pendingWidgets[i].iframeFunc();
                }
            }, function() {
                console.error("Error in Global Resource Downloads / Resource in Parents of Widgets");
            });
        }
...
window.addEventListener("loadWidgetIframe", function() {
    createWidgetIframe(_widgetLyte);
});


// Execute it in parent Lyte Scope
(function(Lyte) {
    var globalWidgetObj = Lyte.Widget = {};
    globalWidgetObj.configureGlobals = function(conf) {
            var widgetConfig = window.Lyte.getConfig("widget");
    ...
    Lyte.Widget.addComponent = function(widgetName, resources) {
        ...
        let acMap = this.__acMap = this.__acMap || [];
        fObj[1] = acMap.length;
        window.lyteWidgetFrame.contentWindow.postMessage(fObj, "*");
        let promiseObj = {};
        acMap.push(promiseObj);
        var promise = new Promise(function(resolve, reject) {
    ...
    Lyte.Widget.remove = function(arg1) {
        if(window.isLyteWidget) {
           this.destroy(arg1, true);
        } else {
            if(!arg1) {
                console.error("No widget name is specified");
                return;
            }
            var widgetConfigGlobal = window.Lyte.getConfig("widget");
            this.destroy(arg1, true);
    ...
    function widgetAdd(name, config){
        ...
        // widgetConfig[name] = config;
    }
    Lyte.Widget.destroy = function(data, fromRemove){
        if(typeof data == "string"){
            widgetDestroy(data, fromRemove);
        }
        else if(Array.isArray(data)){
            data.forEach(function(itm){
                widgetDestroy(itm, fromRemove);
            })
        }
    ...
    function widgetDestroy(name, fromRemove){
        if(window.isLyteWidget){
            if(name){
                ...
                widg ? widg.destroy(fromRemove) : undefined;
            }
            else{
