...
var widgetScopeMap = new Map(), widgetStyles = new Map(), widgetShadows = new Map(), widgetMap = new Map(), prxWkMap = new WeakMap();
    window.widgetScopeMap = widgetScopeMap;
    window.widgetMap = widgetMap;
    var isWidget = window.isLyteWidget, widgetConfig = {}, oldProtoSet;
    var windowEventListeners = [];
    var oldDocumentEventListener = document.addEventListener;
...
if(isWidget) {
    ...
    document.body.appendChild = function(node, bypass) {
        ...
        if(node instanceof Node && (node.getRootNode() != document)) {
            ...
            if(!rootNode._bodyShadow) {
                ...
                var bodyWidgetObj = {"shadow" : rootNode._bodyShadow, "mappings" : {}, "count" : 2};
                bodyWidgetObj._id = window.bodyShadowCount++;
                ...
                var styles = rootNode.querySelectorAll("link");
            ...
            // originalBodyAppendChild.call(bodyShadow, node);  
    ...
    if(window.isLyteWidget) {
        ...
        window.getDetailsFromParent = async function (element, property, isProp, parameters) {
            ...
            if(parameters) {
                for(let i=0;i<parameters.length;i++) {
                    ...
                    if(item instanceof Node) {
                        if(!item._m) {
                            return element.__origFuncs ? element.__origFuncs[property].apply(element, parameters) : undefined;
                        }
                        parameters[i] = {"__type" : "node", "_m" : item._m}
            ...
            var obj = {"c":"getDetails", "prop" : property, "isProp" : isProp, "count" :  window.getDetailsCount++, "params" : parameters, "_m" : element_m};
            if(isAnyElement) {
                if(shadow._isBodyShadow) { 
                    obj.x1 = shadow._id;
                } else {
        ...
        HTMLElement.prototype["getBoundingClientRect"] = async function() {
             return $Lt._aW(await window.getDetailsFromParent(this, "getBoundingClientRect", false, []));
        }
        ...
        windowFuncsFromParent.forEach(function(item) {
            ...
            window[item] = async function() {
                return $Lt._aW(await window.getDetailsFromParent(window, item, false, Array.from(arguments)));
            }
        });

        var windowPropsFromParent = ["pageXOffset", "pageYOffset", "innerHeight", "innerWidth"];
        windowPropsFromParent.forEach(function(item) {
            Object.defineProperty(window, item, {
                "get" : async function() {
                    return $Lt._aW(await window.getDetailsFromParent(window, item, true, []));
                }
            })
        });

        var documentElementProps = ["scrollTop", "scrollLeft", "clientWidth", "clientHeight"];
        documentElementProps.forEach(function(item) {
            Object.defineProperty(document.documentElement, item, {
                "get" : async function() {
                    return $Lt._aW(await window.getDetailsFromParent(document.documentElement, item, true, []));
                }
            })
        })

        // var oldDocumentEventListenerAdd = document.addEventListener;
        // document.addEventListener = function() {
        //     var arr = Array.from(arguments);
        //     var oldFunction = arr[1];
        //     arr[1] = function(event) {
        ...
        //             }
        //         })
        //         oldFunction.apply(this, [eventProxy]);
        //     }
        //     arr[1]._oldF = oldFunction;
        //     return oldDocumentEventListenerAdd.apply(this, arr);
        // }
...
window.DataBindingLayer = {
    ...
    //childOnly
    "updateInitProperties" : function(node, key, value) {
        if(node.__elmCntW) {
            var elementCounter = this.getElementCounter(node.__elmCntW);
            this.addIdsNew(value, elementCounter);
            window.parent.postMessage({"c" : "dCInit", "e" : node.__elmCntW, "f" : key, "g" : value}, "*");
            this.removeIdsNew(value);
        }
    },
    "elmCnt" : 1,
    //childOnly
    ...
    "valueSet" : function(object, property, value) {
        ...
        if(setInfo) {
            setInfo.forEach(function(item) {
                ...
                window.parent.postMessage({"c" : "dC", "e" : item, "f" : property, "g" : value}, "*");
    ...
    "getElementCounter" : function(elementCnt) {
        ...
        var elmCounterObj = {
            elmCnt : DataBindingLayer.elmCnt++,
            dataCnt : 1
        };
    ...
    "childMap" : {},
    //childOnly
    "sendDc" : function(id, property, value) {
        window.parent.postMessage({"c" : "dC", "e" : id, "f" : property, "g" : value}, "*");
    },
    //childOnly
    /*disable async function*/
...
;(function(Lyte) {
    let _LyteComponent = Lyte.Component;
    let _LC = Lyte.__lc;;
    var __stack = [];
    var __stackStatus = 0;
    window.execStack1 = function() {
        __stackStatus = 1;
        try {
            eventHandlerChild(__stack.shift());
        } catch(e) {
            console.error(e);
        }
        __stackStatus = 0;
    ...
    function errorFunction(errorEvent) {
        window.parent.postMessage({
            "c": "7",
            "x": _LyteComponent.currentWidgetId, 
            "y" : errorEvent.type == "error"? errorEvent : errorEvent.reason
        }, "*");
    }
    window.addEventListener("error", errorFunction );
    window.addEventListener("unhandledrejection", errorFunction);
    window.addEventListener("message", function(event) {
        // if(event.data.timeStamp) {
            // console.log("Time taken", new Date() - event.data.timeStamp);
    ...
    async function eventHandlerChild(event) {
        ...
        var widgetObj;
        if(data.p) {
            if(!document._lyteResetAdded) {
                // document.addEventListener("reset", function(event) {
                //     setTimeout(function() {
                //         if(!event.defaultPrevented) {
                //             event.target.dispatchEvent(new Event("lyteReset"));
                //         }
                //     },0);
                // }, true);
                // document.__lyteResetAdded = true;
            }
            if(data.x) {
                _LyteComponent.currentWidgetId = data.x;
                widgetObj = window.childWidgetMappings[data.x];
            }
            if(data.x1) {
                widgetObj = window.childBodyWidgetMappings[data.x1];
            }
            var param1 = data[1];
            var param2 = data[2];
            var param3 = data[3];
            ...
            switch(data.p) {
                case "0" : {
                    if(data.src && !loadedWidgets[data.w]){
                        await loadWidget(data.w, data.src);
                        loadedWidgets[data.w] = true;
                    }
                }
                break;
                case "1" : {
                    // Param1 is widgetId.
                    // window.makeChangesForChildWidget();
                    var widgetObj = window.childWidgetMappings[param1] = {};
                    var div = document.createElement("div");
                    div.setAttribute("id" , param1);
                    document.body.appendChild(div, true);
                    var shadow = widgetObj.shadow = div.attachShadow({"mode" : "open"});
                    var wShadows = widgetShadows.get(data.w);
                    if(!wShadows) {
                        widgetShadows.set(data.w, (wShadows=[]));
                    }
                    wShadows.push(shadow);
                    document._addShadowForGlobalRegistry(shadow);
                    // var shadow = widgetObj.shadow = div;
                    shadow._id = param1;
                    shadow._eL = [];
                    // shadow._eLCount = 0;
                    widgetObj.mappings = {};
                    widgetObj.mappings[1] = shadow;
                    shadow._m = 1;
                    widgetObj.count = 2;
                    if(data.src && !loadedWidgets[data.w]){
                        await loadWidget(data.w, data.src);
                    ...
                    initializeMutations(widgetObj);
                    initStylesToWidget(shadow, data.w);
                } 
                break;
                case "1a" : {
                    await loadWidget(data.w, data.src);
                ...
                break;
                case "2" : {
                    // param1 is base component name.
                    // param2 is data of base component. 
                    if(data.w){
                        _LyteComponent.currentWidget = data.w;
                    }
                    var component = document.createElementLyte(param1);
                    component.addEventListener("onReady", function() {
                        window.parent.postMessage({
                            "c": "9",
                            "x": data.x
                        }, "*");
                    });
                    component.setData(param2);
                    widgetObj.shadow.appendChild(component);
                    if(data.w){
                ...
                case "3" : {
                    ...
                    // param2 is eventType
                    // param3 is value
                    // Note : dispatchEvent doesn't trigger anchor tag's default click. 
                    var target = widgetObj.mappings[param1];
                    switch (param2) {
                        // case "changeFile" : {
                        //     target.files = param3;
                        //     // data.value.arrayBuffer().then(function(abc) {
                        //     // 	console.log(abc);
                        //     // });
                        //     // window.sssd = data.value;
                        // }
                        // break;
                        case "click" : {
                            // console.log("Clicked", new Date() - event.data.timeStamp);
                            var clickEvent = new Event(param2, {"bubbles" : true, "cancelable" : true });
                            clickEvent.button = param4.button;
                            clickEvent.__lyteCreated = true;
                            if(!target) {
                                return;
                            }
                            target.dispatchEvent(clickEvent); 
                            var typeName = target.getAttribute("type");
                            if(!clickEvent.defaultPrevented) {
                                if(typeName == "submit"  || typeName == "image" || (target.tagName == "BUTTON" && typeName != "button" && typeName != "reset" && typeName))  {
                                    if(target.form) {
                                        var submitEvent = new Event("submit", {"bubble" : true, "cancelable" : true});
                                ...
                                if(typeName == "reset") {
                                    // var resetEvent = new Event("reset", {"bubble" : true, "cancelable" : true});
                                    // target.form.dispatchEvent(resetEvent);
                                    var resetEvent;
                                    if(!target.form.__lyteResetAdded) {
                                        target.form.addEventListener("lyteReset", function(event) {
                                            // setTimeout(function() {
                                            //     if(!event.defaultPrevented) {
                                            //         alert("reset done")
                                            //     }
                                            // }, 0)
                                            window.parent.postMessage({"c" : "2", "x" : widgetObj.shadow._id, "y" : event.target._m}, "*");
                                        }, true);
                                        target.form.__lyteResetAdded = true;
                                    }
                                    target.form.reset();
                        ...
                        default : {
                            ...
                            if(param2 == "drop") {
                                console.log("Drop event called in child")
                            }
                            var customEvent = new Event(param2, {"bubbles" : true, "cancelable" : true});
                            customEvent.__lyteCreated = true;
                            if(param4) {
                                delete param4["isTrusted"];
                                for(var key in param4) {
                                    customEvent[key] = param4[key];
                                }
                            }
                            if(target) {
                ...
                case "8" : {
                    //Change of data
                    widgetObj.shadow.querySelector("[lyte-rendered]").setData(param1);
                    // widgetObj.shadow.children[0].setData(param1);
                }
                break;
                case "9" : {
                    var widgetObj = window.childWidgetMappings[param1];
                    widgetObj.observer.disconnect();
                    widgetObj.shadow.querySelectorAll("[comp-in-parent]").forEach(function(item) {
                        delete DataBindingLayer.childMap[item.__elmCntW];
                    });
                    // widgetObj.shadow.remove();
                    widgetObj.shadow.host.remove();
                    delete window.childWidgetMappings[param1];
                }
                break;
                case "10" : {
                    Lyte.Component.set(widgetObj.shadow.querySelector("[lyte-rendered]").component.data, param1, param2);
                }
                case "11" : {
                    Lyte.Widget.destroy(param1);
                    break;
                }
                case "12" : {
                    Lyte.injectResources(param1, function(event) {
                        ...
                        window.parent.postMessage({"c" : "8", "done" : success}, "*");
                        // window.parent.postMessage({})
                    });
                }
                break;
                case "13" : {
                    Lyte.Widget.destroy(param1, true);
                ...
                case "14" : {
                    var target = widgetObj.mappings[param1];
                    let meth;
                    if(target && target._initMethods && (meth = target._initMethods[param2])) {
                        let retValue = meth.apply({"$node" : target}, param3);
                        window.parent.postMessage({"c" : "14r", "r1" : param4, "r" : retValue}, "*");
                    } else {
                        window.parent.postMessage({"c" : "14r", "r1" : param4, "s" : 0}, "*");
                    }
                }
                break;
                case "15" : {
                    ...
                    if(target.$data) {
                        for(let key1 in param2) {
                            Lyte.Component.set(target.$data, key1, param2[key1]);
                        }
                    } else {
                        target.setData(param2, param3);
                    }
                }
                break;
                case "getDetails" : {
                    var count = data.count;
                    var promiseObj = window.getDetailsPromiseObj[count];
                    delete window.getDetailsPromiseObj[count];
                    // console.log("In child getDetails", data.value);
                    promiseObj.resolve(data.value);
                }
                break;
                case "documentListener" : {
                    var type = data.type;
                    var eventObj = data.eventObj;
                ...
                break;
                case "lbindUpdate" : {
                    let element = DataBindingLayer.childMap[param1];
                    let contextSwitchArray;
                    let callee = element._callee;
                    if(element) {
                        contextSwitchArray = [];
                        _LC.adCx(element, contextSwitchArray);
                    }
                    let dynamicValue = element._attributeDetails[param2].dynamicValue;
                    let obj = _LC.getNew(callee.component.data, dynamicValue);
                    if(obj.context) {
                        let lastKeyIndex = +obj.lastKey;
                        if(Array.isArray(obj.context) && typeof lastKeyIndex == "number") {
                            _LC.aF(obj.context, lastKeyIndex < obj.context.length ? "replaceAt" : "insertAt", lastKeyIndex, value);
                        } else {
                            _LC.set(obj.context, obj.lastKey, param3);
                        }
                    }
                    if(contextSwitchArray) {
                        _LC.rmCx(element, contextSwitchArray);
                    }
                }
                break; 
                case "parentComps" : {
                    window.parentWrapperComps = param1;
                }
            }
            _LyteComponent.currentWidgetId = undefined;
        }
    }
    window.childWidgetMappings = {}; 
    window.childBodyWidgetMappings = {};
    ...
    function handleInitProperties(node, dataManipulation) {
        ...
        return {"iP" : initProperties, "eC" : elementCount.elmCnt};
    }
    function loadWidget(name, src){
        widgetStyles.set(name, []);
        var prmArr = [];
        return _lyteWidgetInit(name, src);
        // src.forEach(function(itm){
        //     prmArr.push(Lyte.widgetInit(name, itm));
        // });
        // return Promise.all(prmArr);
    }
    function initStylesToWidget(shadow, widgetName) {
        var styles = widgetStyles.get(widgetName) || [];
        var docFrag = document.createDocumentFragment();
        for(var i=0;i<styles.length;i++) {
            // var styleTag = document.createElement("style");
            // styleTag.innerHTML = styles[i].string;
            // styleTag.setAttribute("filename" , styles[i].name);
            // docFrag.appendChild(styleTag);
            let linkTag = document.createElement("link");
            linkTag.setAttribute("href", styles[i]);
    ...
    function initializeMutations(widgetObj, isWidgetBody) {
        ...
        const callback = function(mutationsList, observer) {
            ...
            // console.time("Child Mutation", targetNode);
            let refAttrObj = {};
                    ...
                    mutation.addedNodes.forEach(function(node) {
                        if(!node._m) {
                            if(node.nodeType == 1) {
                                ...
                                obj.addNodes.push({"type" : "element" , "tagName" : node.tagName, "_m" : node._m, "childNodes" : childNodeProcessed, "prev" : node.previousSibling ? node.previousSibling._m : undefined, "attrs" : attrs, "initProperties" : initPropReturn.iP, "elementCnt" : initPropReturn.eC});
                                // someArray.push(node);
                                
                            } else {
                                node._m = widgetObj.count++;
                                mappings[node._m] = node;
                                obj.addNodes.push({"type" : "node", "value" : node.nodeValue,  "prev" : node.previousSibling ? node.previousSibling._m : undefined});
                            }
                        } else {
                            obj.addNodes.push({"type" : "append", "_m" : node._m});
                        }
                    });	
                    mutation.removedNodes.forEach(function(item) {
                        // delete mappings[item._m];
            ...
            // console.timeEnd("Child Mutation");
            
            
            // window.parent.postMessage({"t" : "checking", "html" : document.querySelector("welcome-comp").outerHTML, "timeStamp" : + (new Date())}, "*");
            // console.log(list);	
            if(!isWidgetBody) {
                // postToBody();
                window.parent.postMessage({"c" : "1", "x" : targetNode._id, "data" : list, "timeStamp" : + (new Date())}, "*");    
                // window.parent.postMessage({"c" : "1", "x" : targetNode._id, "data" : list, "timeStamp" : + (new Date())}, "*");    
            } else {
                window.parent.postMessage({"c" : "1", "x1" : targetNode._id, "x2" : targetNode._mX, "data" : list, "timeStamp" : + (new Date())}, "*");
            }
            list = [];
            ...
            // window.parent.postMessage({"t" : "full", "html" : document.querySelector("welcome-comp").outerHTML }, "*");
        };
        var timeoutValue;
        function postToBody() {
            // console.log("postToBody");
            clearTimeout(timeoutValue);
            // timeoutValue = setTimeout(function() {
                var newList = list;
                list = [];
                // console.log("postToBody - inner");
        ...
        function iterateNodesDelete(node, widgetObj) {
            node.childNodes.forEach(function(item) {
                if(item.nodeType == 1) {
                    // delete widgetObj.mappings[item._m];
                    iterateNodesDelete(item, widgetObj);
                } 
                // else {
                // 	// delete widgetObj.mappings[item._m];
        ...
        function iterateNodes(node, targetNode, widgetObj, dataManipulation) {
            ...
            node.childNodes.forEach(function(item) {
                ...
                if(item.nodeType == 1) {
                    ...
                    item._m = item._m || widgetObj.count++;
                }
                childNodes.push(obj);
            });
            return childNodes;
    ...
    window.makeChangesForChildWidget = function() {
        ...
        HTMLElement.prototype.setAttribute = function(attrKey, attrValue) {
            ...
            protoSet.apply(this, [attrKey, attrValue]);
        }
    }
    ...
    if(window.isLyteWidget) {
        // initializeBodyMutations(bodyDiv);
        var bodyWidgetObj = {"shadow" : window.bodyShadow, "mappings" : {}, "count" : 2};
        window.bodyShadowCount = 1;
        bodyWidgetObj._id = window.bodyShadowCount++;
        window.childBodyWidgetMappings[bodyWidgetObj._id] = bodyWidgetObj;
        window.bodyShadow._id = bodyWidgetObj._id;
        window.bodyShadow._m = 1;
        setTimeout(function() {
            window.bodyShadow.appendChild(document.createElement("div"));
        },3000);
        initializeMutations(bodyWidgetObj, true);
        document._addShadowForGlobalRegistry(window.bodyShadow);
        window.initializeMutations = initializeMutations;
        setTimeout(function() {
            window.parent.postMessage({
                "c" : "3"
            }, "*");    
...
let _LyteComponent = Lyte.__lc;
var oldQuerySelector = document.querySelector;
document.querySelector = function() {
    // if(arguments[0] == 'lyte-button#demo') { 
    //     return document.body.querySelector("[id='2']").shadowRoot.children[1].children[3];
    // } 
    return oldQuerySelector.apply(this, arguments);
}
var oldDocumentEventListenerAdd = document.addEventListener;
var oldShadowEventListener = ShadowRoot.prototype.addEventListener;
ShadowRoot.prototype.addEventListener  = HTMLElement.prototype.addEventListener = document.addEventListener = function() {
    var arr = Array.from(arguments);
    var oldFunction = arr[1];
    arr[1] = function(event) {
        var eventObj = {};
        ...
        let proxyObj = $Lt ? $Lt.oldProxy : Proxy;
        var eventProxy = new proxyObj(event, {
            /*disable async function*/
            "get" : /*disable async function*/function(target, prop) {
    ...
    arr[1]._oldF = oldFunction;
    oldFunction._newF = arr[1];
    if(this instanceof ShadowRoot) {
        return oldShadowEventListener.apply(this, arr);
    } else {
        if(arr[0] == "mouseenter") {
            return;
        }
...
let oldDocumentEventListenerRemove = document.removeEventListener;
let oldShadowEventListenerRemove = ShadowRoot.prototype.removeEventListener;
ShadowRoot.prototype.removeEventListener = HTMLElement.prototype.removeEventListener = document.removeEventListener = function() {
    var arr = Array.from(arguments);
    if(!arr[0] || !arr[1]) {
        console.error("Remove Event Listener params not working")
        return;
    }
    arr[1] = arr[1]._newF;
    
    if(this instanceof ShadowRoot) {
        return oldShadowEventListenerRemove.apply(this, arr)
    } else {
        return oldDocumentEventListenerRemove.apply(this, arr)
    }
};
...
_LyteComponent.shadow.shadowList.push(wrapper.shadowRoot);
wrapper.shadowRoot._shadowChild = [];
wrapper.shadowRoot._lyteShadow = true;
wrapper.shadowRoot._compList = [];
// wrapper.shadowRoot._id = id;
...
wrapper.instantiated = true;
_LyteComponent.shadow.attachEvents(wrapper.shadowRoot);

defProp(HTMLElement.prototype, 'getData', { //af check
    configurable : true, 
    writable : true,
    value : function(arg0) {
        if(this._initProperties) {
            if(arg0) {
                return this._initProperties[arg0];
            } else {
...
(async function(){
    ...
    function replaceFuncCons(func){
        ...
        if(proto){
            proto.constructor = function(){ return function(){} };
        }
    }
    /*disable async function*/
     ...
     * readProps -> [string] ==> can only get the props
     * writeProps -> [string] ==> can write or read the props
     * cloneProps -> [string] ==> if this ins value is returned from get / function it will be isCloned
     * functions -> [string] ==> only the given functions will be executed
     * copyClone -> boolean ==> whether to have the reference of proxy object in the instance for further clone
     * class -> Class ==> class reference
     * isNode -> boolean ==> whether its a type of Node
     */
    //Math, Intl, internalError, 
    const builtInObjs = ["AggregateError","Array","ArrayBuffer","BigInt","BigInt64Array","BigUint64Array","Boolean","DataView","Date","Error","EvalError","FinalizationRegistry","Float32Array","Float64Array","Function","Int16Array","Int32Array","Int8Array","Map","Number","Object","Promise","Proxy","RangeError","ReferenceError","Reflect","RegExp","Set","SharedArrayBuffer","String","Symbol","SyntaxError","TypedArray","TypeError","Uint16Array","Uint32Array","Uint8Array","Uint8ClampedArray","URIError","WeakMap","WeakRef","WeakSet","WebAssembly"];
    const cloneOpts = {
        "Selection" : {
            "type" : "Selection",
            "readProps" : ["anchorOffset", "baseOffset", "extentOffset", "focusOffset", "isCollapsed", "rangeCount", "type"],
            "writeProps" : [],
            "cloneProps" : ["anchorNode", "basNode", "extentNode", "focusNode"],
            "functions" : [],
            "copyClone" : true
        },
        "Component" : {
            ...
            "writeProps" : [], 
            "cloneProps" : ["$node"],
            "functions" : ["getData", "getMethods", "setData", "setMethods", "executeMethod" , "throwEvent"],
            "copyClone" : true
        },    
        "Window" : {
            "type"	: "Window",
            "class" : window,
            "targetProps": ["setTimeout","setInterval"],
            "readProps" :  ["Component", "JSON","Promise","Lyte","closed","console","document","devicePixel","innerHeight","innerWidth","length","name","outerHeight","outerWidth","Math"],
            "writeProps" : ["$Lt","__LCObj","__lyteWidget", "__getClone__","__winproxy"],
            "cloneProps" : ["location","navigator","console", "event", "Node"],
            "functions" : ["fetch" , "addEventListener", "getSelection","unescape","clearInterval","clearTimeout","alert","prompt","confirm","print", "prop"],
            "copyClone" : false
        }, 
        "Document" : {
            "type" : "Document",
            "class" : document,
            "isNode" : true,
            "readProps" : ["__window", "characterSet","compatMode","contentType","doctype","designMode","documentElement","visibilityState","designMode","head","title","readyState","lastModified","visibilityState","hidden","screenX","screenY","screenLeft","screenTop","scrollX","scrollY","status","btoa","atob","screen","nodeType"],
            "readLocalProps" : [""],
            "readLocalFunctions" : ["querySelector", "querySelectorAll", "getElementById", "getElementsByTagName", "getElementsByClassName", "getElementsByName"],
            "writeProps" : ["__window", "__getClone__"],
            "cloneProps" : ["body","implementation","location", "head"],
            "functions" : ["addEventListener","createAttribute","createCDATASection","createComment","createDocumentFragment","createElement","createEvent","createNodeIterator","createTextNode","createTreeWalker","getElementById","getElementsByClassName","getElementsByTagNameNS","getElementsByName","getElementsByTagName","hasFocus","querySelector","querySelectorAll","removeEventListener"],
            "copyClone" : false
        },
        "Node" : {
            "type":"Node",
            "class": Node,
            "isNode": true,
            "readProps" : ["align","title","lang","translate","dir","dataset","hidden","tabIndex","accessKey","draggable","spellcheck","autocapitalize","contentEditable","isContentEditable","inputMode","offsetParent","offsetTop","offsetLeft","offsetWidth","offsetHeight","style","innerText","outerText","nonce","namespaceURI","innerHTML","outerHTML","prefix","nodeValue","textContent","localName","tagName","id","className","classList","scrollTop","scrollLeft","scrollWidth","scrollHeight","clientTop","clientLeft","clientWidth","clientHeight","lookupPrefix","lookupNamespaceURI","isDefaultNamespace","isConnected","baseURI","nodeType","nodeName"],
            "writeProps" : [],
            "cloneProps" : ["component","children","firstChild","firstElementChild","lastChild","parentNode","previousSibling","nextSibling","lastElementChild","childNodes"],
            "functions" : ["setData","cloneNode","appendChild","addEventListener", "setMethods", "scrollIntoViewIfNeeded","scrollIntoView","webkitMatchesSelector","matches","getClientRects","getBoundingClientRect","hasAttributes","getAttributeNames","getAttribute","getAttributeNS","hasAttribute","hasAttributeNS","setAttribute","setAttributeNS","removeAttribute","removeAttributeNS","toggleAttribute","querySelector","querySelectorAll","getElementById","getElementsByName","getElementsByClassName","getElementsByTagName"],
            "copyClone" : true,
            "instance": {
                "property":{
                    nodeName : {
                        equals : ["BODY","HEAD","HTML"],
                        allowedMethods : ["appendChild", "removeChild", "addEventListener", "removeEventListener", "dispatchEvent"],
                        allowSet : false,
                        allowGet : true
                    }
                }
            }
            //functions depriciated : scrollIntoViewIfNeeded
        }, 
        "NodeTop" : {
            "type" : "Node",
            "readProps" : ["align","title","lang","translate","dir","dataset","hidden","tabIndex","accessKey","draggable","spellcheck","autocapitalize","contentEditable","isContentEditable","inputMode","offsetParent","offsetTop","offsetLeft","offsetWidth","offsetHeight","style","innerText","outerText","nonce","namespaceURI","innerHTML","outerHTML","prefix","nodeValue","textContent","localName","tagName","id","className","classList","scrollTop","scrollLeft","scrollWidth","scrollHeight","clientTop","clientLeft","clientWidth","clientHeight","lookupPrefix","lookupNamespaceURI","isDefaultNamespace","isConnected","baseURI","nodeType","nodeName"],
            "functions" : ["appendChild", "removeChild", "addEventListener", "removeEventListener", "dispatchEvent"],
            "writeProps" : [],
            "cloneProps" : ["component","children","firstChild","firstElementChild","lastChild","parentNode","previousSibling","nextSibling","lastElementChild","childNodes"]
        },
        "Event" : {
            "type" : "Event",
            "readProps" : ["cancelBubble","returnValue","composed","timeStamp","defaultPrevented","cancelable","bubbles","eventPhase","type","NONE","CAPTURING_PHASE","AT_TARGET","BUBBLING_PHASE","which","sourceCapabilities","isTrusted","screenX","screenY","clientX","clientY","ctrlKey","shiftKey","altKey","metaKey","button","buttons","pageX","pageY","x","y","offsetX","offsetY","movementX","movementY","layerX","layerY","view","detail"],
            "writeProps" : ["cancelBubble"],
            "cloneProps" : ["relatedTarget","fromElement","toElement","srcElement","currentTarget","target"],
            "functions" : ["getModifierState","initMouseEvent","toLocaleString","propertyIsEnumerable","preventDefault","initEvent","stopPropagation","stopImmediatePropagation","initUIEvent"],
            "copyClone" : true
            //depriciated functions //initMouseEvent ,initEvent,initUIEvent
        },
        "Location" : {
            "type" : "Location",
            "readProps" : ["href","host","hostname"],
            "writeProps" : [],
            "cloneProps" : [],
            "functions" : [],
            "copyClone" : true
        ...
        "Console":{
            "type" : "Console",
            "readProps" : ["log","error","time","timeEnd","warn"],
            "writeProps" : [],
            "cloneProps" : [],
            "functions" : [],
            "copyClone" : true
        ...
        "Implementation":{
            ...
            "writeProps":[],
            "cloneProps":[],
            "functions" : ["createDocument", "createDocumentType", "createHTMLDocument", "hasFeature"],
            "copyClone" : true
        }
    };  
    ...
    const mapOpts = {
        checkIfWidgetNode : {
            Document : {
                methods : ["querySelector", "querySelectorAll", "getElementById", "getElementsByTagName", "getElementsByClassName", "getElementsByTagNameNS", "getElementsByName"]
            }
        },
        saveInWidgets : {
            Node : {
    ...
    Object.defineProperty(cloneOpts, "__cloneReturn", {
        value : false
    });
    ...
    // var shadowOverrides = ["getElementsByTagName", "getElementsByName", "getElementsByClassName"];
    // shadowOverrides.forEach(function(item) {
    //     var func;
    //     switch(item) {
    //         case "getElementsByTagName" : {
    //             func = function(arg1) {
    //                 return this.querySelectorAll(arg1)
    //             }
    //         }
    //         break;
    //         case "getElementsByName" : {
    //             func = function(arg1) {
    //                 return this.querySelectorAll(`[name='${arg1}']`);
    //             }
    //         }
    //         break;
    //         case "getElementsByClassName" : {
    //             func = function(arg1) {
    //                 return this.querySelectorAll(`[class='${arg1}']`);
    //             }
    //         }
    //     }
    //     ShadowRoot.constructor.prototype[item] = func;
    // });
    var widgetUtils = {
    ...
    class Widget{
        static addEventListener(eventName, func, options, widgetName, listenerId) {
            ...
            listenerArr.push({"func" : func, "options" : options, "widget" : widgetName, "id" : listenerId});
            return listenerId;
        }
        ...
        static removeEventListener(listenerId, widgetName) {
            ...
            // let actualListenerId = listenerIdSplits[1];
        ...
        constructor(widgetName, widIns){
            ...
            this.win_prox = this.getClone( widIns ? widIns.win_prox : window, this.doc_prox, true, undefined, widIns ? "Window" : undefined);
            // this.prox = this.getFinalWindowProxy(widgetName, this.doc_prox, this.win_prox, this.getClone);
        ...
        destroyInstance(){
            ...
            setTimeout(function() {
                for(var i=0;i<widgetComps.length;i++) {
                    Lyte.Component.unregisterComponent(widgetComps[i], {"__forceRemove" : true});
                }
            },100);
        ...
        removeComponent(ins){
            ...
            if(!this.components.size){
                ...
                // var keepAlive = config && config.hasOwnProperty("keepAlive") ? config.keepAlive : false;
                // if(this.toDestroy){
                //     this.destroyInstance();
                // }
                // else if(!keepAlive){
                //     Lyte.Widget.destroy(this.widgetName);
                // }
            }
        }
        static getCloneType(item) {
            ...
            //since with constructor callback, the function replaces component's constructor, couldn't check the constructor.name else using this now
            ...
            // }
                ...
                cloneType = "Navigator";
            // }else if(item instanceof NetworkInformation) {
            // 	cloneType = "Connection";
        ...
        getClone(item, doc, forceClone, scope, type) {
            if(!item) {
                return;
            }
            ...
            if(cloneType == "Window"){
                ...
                Object.defineProperty(this.Lyte.$, "__Lyte__", {
                    value : this.Lyte
                });
                this.Lyte.__widget = this.widgetName; // for debugging
                ...
                this.Lyte.injectServices = function(data){
                    var toInjectService = function(name,key){
                      ...
                      for(var comp of registeredComponents){
                        ...
                        // component.compInstances.forEach(function(compIns){
                        //     self.Lyte.Component.set(compIns.component.data,key,self.Lyte.registeredServices[name]);
                        // })
                      }
                    }
                    data.forEach(function(itm){
                ...
                Object.defineProperty(this.Lyte.Service, "__Lyte__", {
                    value: this.Lyte
                });
                Object.defineProperty(this.Lyte.Mixin, "__Lyte__", {
                    value: this.Lyte
                });
                obj.Lyte = this.Lyte;
                obj.$Lt = this.$Lt = { oldProxy : $Lt.oldProxy, "__defaultProps" : $Lt.__defaultProps};
                ...
                obj.document.__window = obj.window; // obj.window is undefined
        ...
        ItemClone(item, type, scope, commonScp) {
            ...
            let handlers = {
                "get" : function(target, prop, receiver) {
                    ...
                    // if(target == Object && prop == "defineProperty") {
                    //     return mainDefineProperty;
                    // }
                    ...
                    } else if(itemDetails && itemDetails.type == "Component" && Object.keys(target.constructor.prototype).includes(prop)) {
                        return target[prop];
                    } 
                    return result;
                }.bind({window:window, document:document}), 
                ...
                // 		target.hasOwnProperty("scope") ? target.scope[key] = value : undefined;
                    }
    ...
    function createClonedScope(widgetName){
        ...
        obj.window.eval = (function(obj){
            ...
            Object.defineProperty(obj, "eval", {
                value : safeEval
            });
            return safeEval;
        })(obj);
        ...
        Object.defineProperty(obj, "setTimeout", {
            value : obj.window.setTimeout
        });
        ...
        Object.defineProperty(obj, "setInterval", {
            value : obj.window.setInterval
        });
        win_prox.scope = obj;
    ...
    var commonWidget;
    function addToWidget(fStr, widgetName, fileName, type){
        // if(type == "style") {
        //     var widgetStylesArr = widgetStyles.get(widgetName);
        //     widgetStylesArr.push({"name" : fileName, "string" : fStr});
        //     return;
        // }
    ...
    _lyteWidgetInit = function(name, arr){
        return new Promise(function(resolve, reject){
            // if(!widgetScopeMap.get(name)){
            //     var widget = new Widget(name);
            //     prx = widget.win_prox;
            //     widgetMap.set(name, widget);
            //     widgetScopeMap.set(name, prx);
            //     var sBoxP = sandbox(prx);
            //     sBoxP(overridewidget);
            // }
            ...
            arr.forEach(function(itm){
                ...
                if(type == "style") {
                    count++;
                    var widgetStylesArr = widgetStyles.get(name);
                    ...
                    oldProtoSet.apply(scrpt, ["src", itm]);
...
if(window.isLyteWidget) {
    ...
    //             var id = rootNode._id;
    //             window.parent.postMessage({"c" : "changeVal", "x" : id, "y" : event.target._m, "z" : event.target.value}, "*");
    //         }
    //     });
    // });
    var inputProtoProps = ["value"];
    inputProtoProps.forEach(function(item) {
        var descriptor = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, item);
        Object.defineProperty(HTMLInputElement.prototype, item, {
            set : function(val) {
                descriptor.set.apply(this, arguments);
                if(window.___ignoreVal) {
                    window.___ignoreVal = false;
                    return;
                }
                if(this._m) {
                    window.parent.postMessage({"c" : "changeVal", "x" : this.getRootNode()._id, "y" : this._m, "z" : val, "v" : item}, "*");
                }
            }
        });
    })
