    ...
    visible_obs : function( arg ){
        ...
        if( removed_len ){
            for( var i = removed_len - 1; i >= 0; i-- ){
                ...
                for( var j = cur_index; j < len; j++ ){
                    Lyte.objectUtils( visibleStatus, 'replace', j, visibleStatus[ j + 1 ] );
                }
        ...
        if( inserted_len ){
            for( var i = 0; i < inserted_len; i++ ){
                var cur_index = __index + i;
                for( var j = cur_index; j < len; j++ ){
                    Lyte.objectUtils( visibleStatus, 'replace', j + 1, visibleStatus[ j ] );
                }
                Lyte.objectUtils( visibleStatus, 'replace', cur_index, {} );
            }
        }
