				...
				if(_handleElement){
					...
					if(data && data._isDown){
						...
						if(!data._isMoved){
							...
							elem.style.width = calculateWidth(elem,elemOffset) +'px';
							elem.style.height = calculateHeight(elem,elemOffset)  +'px';
							elem.style.top = elemOffset.top - data.transformdiff  - parentOffset.top /*- parseInt(cellSpacing)*/ - parseInt(_marginTop) + _scrollTop + 'px';
							elem.style.left = elemOffset.left - data.transformdiff - parentOffset.left - parseInt(_marginLeft) + _scrollLeft + 'px';
						...
						if(returnVal){
							...
							if(orientation === "vertical"){
								...
								elem.style.left = data._mousePosition.x - data.transformdiff - _offset[0] - parentOffset.left - parseInt(_marginLeft) + _scrollLeft + 'px';
								elem.style.top = data._mousePosition.y - data.transformdiff - _offset[1] - parentOffset.top - parseInt(_marginTop)  + 'px';
								elemOffset = elem.getBoundingClientRect();
								if(data.scrollDivY){
									var scrollDivOffset = data.scrollDivY.getBoundingClientRect();
									if((elemOffset.left <= scrollDivOffset.right) && (elemOffset.right >= scrollDivOffset.left)){
