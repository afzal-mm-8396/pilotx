// $Id$
	 ...
	 * @namespace
	 */
    var renderingUtils = typeof renderingUtils != "object" ? {} :	renderingUtils ;
    renderingUtils.fieldsJSONArray = [];
    $( document ).ready(function() {// eslint-disable-line  @zoho/zstandard/no-body-events
        renderingUtils.windowHeight = $(window).height(); // eslint-disable-line @zoho/zohocrm/no-window-dimensions
        renderingUtils.windowWidth = $(window).width(); // eslint-disable-line @zoho/zohocrm/no-window-dimensions
        renderingUtils.windowOuterHeight = $(window).outerHeight();// eslint-disable-line @zoho/zohocrm/no-window-dimensions
        renderingUtils.windowOuterWidth = $(window).outerWidth();// eslint-disable-line @zoho/zohocrm/no-window-dimensions
    });
    ...
    renderingUtils.getTargetElem = function(e){//No I18n
         ...
         * @param {String} toId - This provides the id of the elem that has to be displayed.
         * @param {Event} ev - This provides the event that is to be triggered. i.e, like quick create popup to be constucted.
    ...
    renderingUtils.positionArrow = function(currentElement, toId, ev, arrowElemId, posObj) {//No I18N
        ...
        var hideArrow = Lyte.registeredMixins["crm-emailconversation-view-mixin"] && Lyte.registeredMixins["crm-emailconversation-view-mixin"].isEmailViewOpen(); //No I18N
        if ( totViewPortWidth - clientX - 15 > showElemWidth) { //If true right side space available
            if(toId == "wid_calCreate_opts_menu"){
                if ( bottomSpc > neededHeight + 40) {
                    topVal = pageY - 60;
                } else {
                    topVal = pageY - (neededHeight - bottomSpc + 150);
                }
            }
            else{
                ...
                topVal = pageY - 30;
                    ...
                    topVal = pageY - (neededHeight - bottomSpc + 25);
                }
            }
            left = pageX + 20;
        ...
        left = left < 0 ? 0 : left;
        elemToShow.css({"top": topVal -6 , "left": left});//No I18n
        ...
        if ( arrowElemId === "qcpopuparrow") {
            ...
            if(!($('.ziaCallAnalyticswrapper').length || $('.etTrendsWrapper').length)){
                arrElemId.attr({"class": arrowClass}).css({"left":"", "top": pageY - topVal - 3});  //No I18N
            }
        } else {
        ...
        if(curElem.hasClass("eventActionsMenu") || curElem.hasClass("quickCreateLink")){
            ...
            $(".eventActionsMenu span, .quickCreateLink span").removeClass("quickCreateLinkAct");
         ...
         * => //This method would trigger the event click on the element 'test' irrespective of the call type. i.e, even if it is onclick,Jquery(.click) etc..
          */
    renderingUtils.triggerEvent = function(eventType, Jelem, source){
        if(Jelem[0]) {
            if(Jelem[0][eventType]){
                ...
                if(source){
                    Jelem.eq(0).trigger(eventType, source);
                }
                else{
    ...
    renderingUtils.sectionSorting = function(){//No I18N
          ...
          sortable.multiselectable({
            ...
            if(item.hasClass("pinField")){
                $('ul#selectedColumns li.unPinField.selectedElem').removeClass("selectedElem");
              }
            else if(item.hasClass("unPinField")){
                $('ul#selectedColumns li.pinField.selectedElem').removeClass("selectedElem");
              }
                }
          });
          sortable.multisortable({
              items:"> li:not(.restrictSort)",//NO I18N
                selectedClass:"selectedElem",//No I18N
              cancel:".nosort",//No I18N
              start:function(event,ui){
                  ...
                  if(parentId === "selectedColumns" && !selElem.hasClass("pinField")){
                      selElem.children(".pinIcon").hide(); //No I18N
                  }
              ...
              beforeStop:function(event,ui){
                  ...
                  if(targetId === "availableList" && placeholderId === "availableList"){
                    //Removing draggable image
                      ...
                      items.stop(true,true).animate({ //eslint-disable-line @zoho/webperf/no-animate
                      ...
                      var scrollToElem;
                        ...
                        var pinField = $(".pinField"); //No I18N
                        var pinFieldLastEle = pinField.last(),pinFieldLen = pinField.length;
                        ...
                        ;
                        if(!itemIndex.hasClass("pinField")){
                            crmListView.originalOrder.splice(crmListView.originalOrder.indexOf(itemIndex.attr("field_id")),1);
                        }
                        if(itemIndex.attr("mandatory") || itemIndex.hasClass("pinField")){
                            if(selectedClm[0].classList.contains("pinnedUl")){
                                if(itemIndex.hasClass("pinField") && pinFieldLen === 1){
                                    selectedClm.prepend(itemIndex);
                                }
                                else{
                                    pinFieldLastEle.after(itemIndex);
                                }
                            } else {
                                selectedClm.prepend(itemIndex);
                            }
                            if(itemIndex.attr("mandatory") && !itemIndex.hasClass("pinField")){
                                crmListView.originalOrder.unshift(itemIndex.attr("field_id")); //No I18N
                            }
                        }
                        else {
                              ...
                              itemIndex.removeClass("unPinField");
                              itemIndex.children().filter(".pinIcon").empty(); //No I18N
                              itemIndex.children(".pinIcon").attr({"class" : "pinIcon dIB fR pR", "id" : "pin" , "onClick" : "crmListView.pinUnpinAction(this,event)", "style" : "display : none; cursor : pointer"}); //No I18N
                              scrollToElem = itemIndex;
                              itemIndex.attr("style", "margin-left : 11px; margin-right : 11px;"); //NO I18N
                          }
                          commonUtils.addToMandatory();
                      });
                      if(scrollToElem){$L(".availableCol").scrollTo(scrollToElem);}
                    }
                  else if(targetId === "availableList" && placeholderId === "selectedColumns")
                  {	  
                      var kanbanViewPopup = $("#KanbanViewPopup"); //No I18N
                      var selectedClm = $("#selectedColumns");
                      $.each(items, function(index){
                          var itemIndex = $(items[index]);
                          var selected = itemIndex.children().find(".selectionClass");
                           if(selected.length > 0){
                             $.each(selected, function(index1){
                                ...
                                selectIndex.replaceWith($ESAPI.encoder().encodeForHTML(txt));
                             });
                          }
                          if(!kanbanViewPopup.length) {
                                var pinElem = itemIndex.find(".pinIcon");
                                itemIndex.addClass("unPinField"); //No I18N
            ...
            update:function(){ //eslint-disable-line @zoho/zohocrm/no-unused-vars
            ...
            stop:function(event,ui){
                ...
                var placeholderId = items.parent().attr("data-cid"); //No I18N
                if(sameDivDrag || (targetId === "availableList" && placeholderId === "selectedColumns")){
                    var previousElem = $('ul#selectedColumns').children()[items.index() > 0 ? items.index()-1 : 0];
                    var reorderIndex = items.index() === 0 ? 0 : crmListView.originalOrder.indexOf($(previousElem).attr("field_id"))+1;
                    var itemsLen = items.length , reorderElem = [];
                    for(var i=0;i<itemsLen;i++){
                        if(!items[i].classList.contains("pinField")){
                            if(crmListView.originalOrder.indexOf(items[i].getAttribute("field_id")) !== -1) {
                                crmListView.originalOrder.splice(crmListView.originalOrder.indexOf(items[i].getAttribute("field_id")),1);
                            }
                            reorderElem.push(items[i].getAttribute("field_id"));
                        }
                    }
                    crmListView.originalOrder = crmListView.originalOrder.slice(0, reorderIndex).concat(reorderElem, crmListView.originalOrder.slice(reorderIndex));
                }
                //eslint-disable-next-line @zoho/webperf/directly-select-with-id
    ...
    renderingUtils.showCustomConfirmMessageNewModel = function(msg, confirmBtnObj, cancelBtnObj){
        ...
        var resetAndClosePopup = function(btnObj, event) {
            ...
            setTimeout(function() {
                var $confirmCancelBtnClass = container.find("[data-zcqa='confirm_cancelbtn']"); // eslint-disable-line @zoho/webperf/no-attribute-selectors
                var $confirmDeleteBtnClass = container.find("[data-zcqa='confirm_deletebtn']"); // eslint-disable-line @zoho/webperf/no-attribute-selectors
        ...
        $confirmBtn.off('click').click(function(event) { //No I18N
            ...
            if(featuresAvailable.NEXTGENUI_ENABLED && confirmBtnObj.val==='Back to Settings'){
        ...
        $cancelBtn.off('click').click(function(event) { //No I18N
            ...
            if(featuresAvailable.NEXTGENUI_ENABLED && cancelBtnObj.val==='Back to Home'){
                Lyte.Router.transitionTo("crm.tab.module.begin",'Home');
            }
        });
    ...
    renderingUtils.showCustomConfirmMessage = function(id,msg, button){
        $("#" + id).remove();//eslint-disable-next-line @zoho/webperf/no-multipleDOMLookup
        if($("#" + id).length === 0 ){ 
            var div = $("#confirmMsg1")[0].cloneNode(true);
            div.id = id;
            var ln = msg.length
            for(var i = 0;i < ln;i++){
                $(div).find("#confirmMessage" + i).html(msg[i]); //eslint-disable-line @zoho/webperf/directly-select-with-id
            }
            ln = button.length
            for(var i = 0;i < ln;i++){
            $(div).find("#confirmButton" + i).val(button[i].val); //eslint-disable-line @zoho/webperf/directly-select-with-id
                if(button[i].fn){
                    //eslint-disable-next-line @zoho/webperf/no-multipleDOMLookup
                    $(div).find("#confirmButton" + i).click(button[i].fn); //eslint-disable-line @zoho/webperf/directly-select-with-id 
                }
            }
            
            $(div).addClass("w100_per");
            
            document.body.appendChild(div) // eslint-disable-line @zoho/zstandard/no-body-events
        }
        else{
            div = $("#"+id)[0]; //eslint-disable-line @zoho/webperf/no-multipleDOMLookup
        }
        renderingUtils.positionCenter(id);
        crmui.modalZIndex('nonLyte', div, 'open');//No I18N
        return div;
    }
    ...
    renderingUtils.positionCenter = function(id){
        ...
        if($('.eventPop').hasClass('ziaForEmailsCheck')){
            givenId.css("z-index","316"); //no i18n
        }
    }
        ...
        for(var i = 0;i < ln;i++){
            ...
            if(callBack){
                li.click( function() { // eslint-disable-line no-loop-func
                    callBack(this);
                    div.hide();// eslint-disable-line @zoho/webperf/no-closure
                });
            }
        if(cT.length){
            var len = cT.length;
    ...
    renderingUtils.bindEventForCreateHTML = function(ev){//Removed from inside the loop and added out to avoid codecheck //No I18N
        ...
        * @example  
        * renderingUtils.createHTML({"name":"div", "attr":{"id":"NotFoundAlertDiv","class":'w100_per pT30'},"onclick":"crmDeduplicate.hideMessagePopup(\"" + module + "\")"});
        * => <div id="NotFoundAlertDiv" class="w100_per pT30" onclick='crmDeduplicate.hideMessagePopup(\"" + module + "\")'></div>
        ...
        if($freezeLayerDiv){
            ...
            $freezeLayerDiv.click(function(){
                ...
                if(!QuickActionSelObj && ($window.scrollTop() + renderingUtils.windowHeight - parseInt(el.css("top")) < 0 || $window.scrollTop() - (parseInt(el.css("top")) + parseInt(el.height())) > 0)){ 
                    $L("#"+divId).scrollTo({axis:'y',duration:1000,offset:-80,onAfter:function(){renderingUtils.shakeAnimation(divId,dir,dist);}}); //No I18N
                }else{
            ...
            if($('.dItimesdiv').length){
                $('.dItimesdiv #sdiv').hide();//eslint-disable-line @zoho/webperf/directly-select-with-id
            ...
            switch(dir){
                case "left" :
                    if(doc.style.left.indexOf("%") !== -1){var at = parseInt(tem / 10);
                        ...
                        $("#" + divId).animate({left:"+=" + at + "%"},100); //eslint-disable-line @zoho/webperf/no-animate
                    }
                    else{
                        $("#" + divId).animate({left:"+=" + tem},100);//eslint-disable-line @zoho/webperf/no-animate
                    }
                    break;
                case "right" :
                    if(doc.style.right.indexOf("%") !== -1){
                        var at = parseInt(tem / 10);
                        $("#" + divId).animate({right:"+=" + at + "%"},100);//eslint-disable-line @zoho/webperf/no-animate
                    }
                    else{
                        $("#" + divId).animate({right:"+=" + tem},100);//eslint-disable-line @zoho/webperf/no-animate
                    }
                    break;
                case "up":
                    $("#" + divId).animate({top:"+=" + tem},100);//eslint-disable-line @zoho/webperf/no-animate
                    break;
            }
        }
            elVisible ? el.hide() : null;//No I18N
    ...
    renderingUtils.dummyBackground = function(hideOnClick, elemIdToFocus) {//No I18n
        ...
        var scrlTop = $L(window).scrollTop();
    ...
    renderingUtils.removeDummyBackground = function() {//No I18n
        ...
        var createEditCallDiv = $L("#createEditCallDiv"),eventCreatePopup = $L("#eventCreatePopup");
        if((!createEditCallDiv || createEditCallDiv.length === 0 || createEditCallDiv[0].style.display === 'none') && (!eventCreatePopup || eventCreatePopup.length === 0 || eventCreatePopup[0].style.display === 'none')){
    ...
    renderingUtils.displayErrorDiv = function (fldName , fldLabel, message, fldObj){
           ...
           if(!fldObj || $(fldObj).length === 0){
                    fldObj = getObj(fldName); //eslint-disable-line @zoho/zohocrm/no-deprecated-fnc
            }
            ...
            if(fldLabel=='pipelineStage' || fldLabel=='pipelineName'){
                div.style.top = "auto";
            ...
            if( quickConfig || imapConfigpageID){
             ...
             if((imapConfigpageID || quickConfig) && (div.id === "displayContent_imapPort" || div.id === "displayContent_smtp_port") ){
                //eslint-disable-next-line @zoho/webperf/layout-thrashing
            ...
            $(div).empty();
            div.textContent = I18n.getValueForFields(message,fldLabel);
            ...
            if(div.id === "displayContent_similarityCreationName") {
                var fldObjEle = $(fldObj);
                var fldObjParent = fldObjEle.parent();
                fldObjParent.append(div);
                var similarity = $('#displayContent_similarityCreationName');
                similarity.css('top', '130%');  //No I18n
                similarity.parents('.similarityValidation').addClass('mB25');  //No I18n
            }
            ...
            if(div.id === "displayContent_imapServer" || div.id === "displayContent_smtp_server" || div.id === "displayContent_smtp_emails" || div.id === "displayContent_roleName" || div.id === "displayContent_reportsTo" || div.id === "displayContent_taskSubject" || div.id === "displayContent_days"  || div.id === "displayContent_assignTo"){
                ...
                if(fldLabel === "SameFieldName"){
                    ...
                    $("#WFTriggerFieldsDIV").find(".wsNowrap").eq(0).append(div);//No I18N
                }
                else{
                    // eslint-disable-next-line @zoho/zstandard/no-commoncode-in-ifelse 
                    $("#WFTriggerFieldsDIV").find(".wsNowrap").eq(0).append(div);//No I18N
            ...
            {
                //eslint-disable-next-line @zoho/webperf/no-multipleDOMLookup
                $(fldObj).next().css({"width":"90%"})
                //eslint-disable-next-line @zoho/webperf/no-multipleDOMLookup
                $(fldObj).next().append(div);
            }
    ...
    renderingUtils.displayAlert = function (fldName, fldLabel, alertMsg, currObj, alertType){
        if(alertType === "inline"){
         ...
         /**
         * This method is used to remove the dummy_Body loader and leftMenuLoader
         * @returns {void}
          */
    renderingUtils.removeDefaultLoader=function(){
         $L("html").addClass("page_initial_loading_hide");
         $L(".dummy_Body").hide();
         var leftMenuLoader = $L('#leftMenuLoader');
         if(leftMenuLoader && leftMenuLoader.length > 0 && !leftMenuLoader[0].classList.contains('crm-initldt-primhide')){
                leftMenuLoader[0].classList.add("crm-initldt-primhide");
                leftMenuLoader.hide();
         }
         $L('.crmAppTopPanelIniLoader').hide();
    }
        /**
         ...
         * @param {Boolean} icon - This provides a boolean which is set to true , to enable the icon and false to disable
         * @param {Function} onCloseCallBack - This provides a function that has to be called on closing the popup
         * @returns {void}
          */
    renderingUtils.displayPermissionDenied	= 	function(Response, callBack, reason, zIndex,icon, onCloseCallBack , prop){
        /*
        ...
        //to hide loader if any in new UI
        renderingUtils.removeDefaultLoader();
        Lyte.triggerEvent("showOrHideContextualLoader"); //No I18n
        
        var currentTransObj = Lyte.Router.getRouteInstance().transition,title;
        icon = icon === undefined && true;
        ...
        if(Response && typeof Response !== "undefined"){
            try{//eslint-disable-next-line @zoho/webperf/directly-select-with-id
        ...
        if(reason){
            ...
            if("MODULE_NOT_FOUND" === reason){//NO I18N
                reason = I18n.getMsg("crm.workflow.alert.moduleNotPresent");
                title = I18n.getMsg("crm.followsettings.sandbox.module.not.available");
                icon = true;	
            }
            if(reason === "COMMANDCENTER_PROCESS_JOURNEY_DOES_NOT_EXIST") {
                title = I18n.getMsg("crm.cc.process.not.exist");
                reason = I18n.getMsg("crm.cc.journey.not.exist");
                icon = true;
            }
    
            if(reason === "COMMANDCENTER_PROCESS_PATHFINDER_DOES_NOT_EXIST") {
                title = I18n.getMsg("crm.cc.process.not.exist");
                reason = I18n.getMsg("crm.cc.pathfinder.not.exist");
                icon = true;
            }
        ...
        var defaultProp = { "cxPropReason" : reason, "cxPropTitle" : title,"cxPropIcon" : icon,"cxPropZindex" : zIndex}; //NO I18N
        if(prop){
            for(var obj in prop){
                defaultProp[obj] = prop[obj];
            }
        }
        var component = Lyte.Component.render("crux-permission-denied-alert",defaultProp,"body");
    ...
    //					renderingUtils.removeFreezeLayerPD();
        ...
        component.eventLis = Lyte.addEventListener("beforeRouteTransition", function(item){
            ...
            if(comp && comp.getData('cxPropShow')){
                var viewRoutes = ["crm.tab.module.custom-view.list","crm.tab.module.custom-view.zia", "crm.tab.module.custom-view.kanban","crm.tab.module.custom-view.canvas","crm.tab.module.custom-view.map"];//no i18n
                ...
                if(!preservePopup && !(kanbanviewEle && kanbanviewEle.value === "true")){ //eslint-disable-line @zoho/webperf/layout-thrashing
                    comp.setData('cxPropShow',false); //no i18n
                }
            }
        })
        $L("#Warningdiv").hide();	//To hide when we delete a contact
        if($L('#appointmentCreate').is(':visible') || $L('#serviceAjaxEdit').is(':visible') || $L('#appointmentRescheduleDiv').is(':visible') || $L('#appointmentCancelDiv').is(':visible') || $L('#appointmentCompletePopup').is(':visible') || ( zIndex===undefined && $L('#sbl_subformBulkCreate').is(':visible'))){//eslint-disable-line @zoho/webperf/layout-thrashing
    ...
    renderingUtils.removeFreezeLayerPD = function(){
        if($L('#topBandPopup').is(':visible') || $L('#appointmentCreate').is(':visible') || $L('#serviceAjaxEdit').is(':visible') || $L('#appointmentRescheduleDiv').is(':visible') || $L('#appointmentCancelDiv').is(':visible') || $L('#appointmentCompletePopup').is(':visible')){
            ...
            if($L('#topBandPopup:visible').length){
                //$('#FreezeLayer').css('z-index',''); //eslint-disable-line @zoho/zohocrm/no-freezeLayer
                var freezelayer = $L('#FreezeLayer')[0];
                if(freezelayer){
    ...
    renderingUtils.ajaxErrorBackForLyte = function() {
        if(!Lyte.Router.getRouteInstance().transition.preservePopup && crmHistory.loaded && crmHistory.handleAjaxErrorLyte) {
            ...
            if(Lyte.Router.getRouteInstance().transition.abort)
            {
                Lyte.Router.getRouteInstance().transition.abort();
            }
            if(!app.prevTransition.route || app.historyLength <= 1){
                if(featuresAvailable.NEXTGENUI_ENABLED  && !featuresAvailable.NEXTGENUI_NEW_PANEL_ENABLED)
                {
                    Crm.executeLeftMenuAction("action", "renderModules"); //No I18n
                }
                else{
                if(clientPortalName){
                    Lyte.Router.getRouteInstance().replaceWith("crm.tab.module",Crm.userDetails.CLIENT_TYPE);//NO I18N
                }else{
    ...
    renderingUtils.getHeightFromTabLayer = function(){
        var height = renderingUtils.windowHeight - renderingUtils.tabLayerOuterHeight();
    ...
    renderingUtils.getScrollBarWidth = function() {
        ...
        document.body.appendChild(outer);// eslint-disable-line @zoho/zstandard/no-body-events
        var w1 = inner.offsetWidth;// eslint-disable-line @zoho/webperf/layout-thrashing
        outer.style.overflow = 'scroll';//No I18N
        var w2 = inner.offsetWidth;// eslint-disable-line @zoho/webperf/layout-thrashing
        if (w1 == w2){
            w2 = outer.clientWidth;// eslint-disable-line @zoho/webperf/layout-thrashing
        }
    ...
    renderingUtils.listviewFixedColumn = function(){
        var listLyteComp = ["list","users","list-view"].contains(Lyte.Router.getRouteInstance().routeName) ;//No I18N
        ...
        var listViewTableHig = $("#listviewtablescroll").height();	//eslint-disable-line @zoho/webperf/layout-thrashing
    ...
    renderingUtils.listviewFixedTable = function(){
        var listviewtablescroll = $L('#listviewtablescroll');
    ...
    renderingUtils.tableSorter = function(){
        ...
        renderingUtils.generateColumnHeader = function() {
            ...
            var th = originalTable.find('#listviewHeaderRow td');//eslint-disable-line @zoho/webperf/directly-select-with-id
            var fixedHeaderTr = $('#fixedHeader .tr');
            var docfragCol = document.createDocumentFragment();
            fixedHeaderTr.empty();
            var td;
            for( var i=0; i<th.length; i++) { //eslint-disable-line @zoho/zstandard/proper-usage-of-loop
                td = $(th[i]).clone();
                td.removeClass('th').addClass('td fixedHeaderTd');
                $(docfragCol).append(td);
                
            }
            fixedHeaderTr.append(docfragCol);//eslint-disable-line @zoho/zohocrm/check-html-usage
        }
    
        /**
         * This method is used to set the Row Height of the list view table.
         * @param {Number} fixedColumnLVFWidth - This provides the width of the fixed column. 
         * @returns {void}
    ...
    renderingUtils.fixedRowHeaderWheelBind = function() {
        ...
        if(negRowH < 0) {
            fixRHead.on('wheel', function(e) {
                var fixedRowHeaderDOM = $('#fixedRowHeaderTable');
                var origTableCont = $L('#listviewtablescroll');
                e.stopPropagation();
         ...
         * @returns {void}
         * @param {HTMLElement} obj - This provides the Element whose parent element has to be prevented from scrolling.
          */
    renderingUtils.scrollPrevent = function(obj){
        $( obj ).on( 'wheel', function (e) {//No I18N
    ...
    renderingUtils.freezeProperty = function(obj, property){        
         ...
         * @param {Object} transInfo - This provides the object which contains  
    ...
    renderingUtils.forceTransitionTo = function(transInfo){
        ...
        if(transInfo.route == curInfo.route){
            ...
            var queryParam_same = $u.isEqual(transInfo.queryParams,curInfo.queryParams,false);
            var dynamicParam_same = $u.isEqual(transInfo.dynamicParams,curInfo.dynamicParams,false);
    ...
    renderingUtils.getSingularLabel = function(module){
        ...
        return I18n.getMsg(Crm.moduleInfo[module].translatedSingularLabel);
    }
    ...
    renderingUtils.getPluralLabel = function(module)
    {
        ...
        return I18n.getMsg(Crm.moduleInfo[module].translatedPluralLabel);
    }
    ...
    renderingUtils.showLyteErrorMessage = function(message,borderElem,id){
        ...
        if(elemId){
            ...
            if(borderElem.outerWidth() < 200){ //eslint-disable-line @zoho/webperf/layout-thrashing
    ...
    renderingUtils.removeMultiSpacestoSingleSpace = function(val){
         ...
         * @returns {HTMLElement} By skipping all the Textnodes which were the child of the HTMLElement and returns the First Child
          */
    ...
    renderingUtils.highlightUrlInText = function(content){//No i18n
        if(content){
            var urlRegex = /((https?|ftp)(:|&#x3a;)(\/\/|&#x2f;&#x2f;))((www\.){0,1}[a-zA-Z0-9-_]+(\.[a-z0-9-_]+)+)((:|&#x3a;)[0-9]*(\.[a-z0-9]+)*)*(((\/|&#x2f;)[^\s]*)*)|www\.([a-zA-Z0-9-_]+(\.[a-z0-9-_]+)+)((:|&#x3a;)[0-9]*(\.[a-z0-9]+)*)*(((\/|&#x2f;)[^\s]*)*)/g;
            //var urlRegex = /(((https?|ftp)(:|&#x3a;)(\/\/|&#x2f;&#x2f;))((www\.){0,1}[a-zA-Z0-9-_]+(\.[a-z0-9-_]+)+)((:|&#x3a;)[0-9]*(\.[a-z0-9]+)*)*(((\/|&#x2f;)[^\s]*)*(?=&gt;|>))|www\.([a-zA-Z0-9-_]+(\.[a-z0-9-_]+)+)((:|&#x3a;)[0-9]*(\.[a-z0-9]+)*)*(((\/|&#x2f;)[^\s]*)*(?=&gt;|>)))|(((https?|ftp)(:|&#x3a;)(\/\/|&#x2f;&#x2f;))((www\.){0,1}[a-zA-Z0-9-_]+(\.[a-z0-9-_]+)+)((:|&#x3a;)[0-9]*(\.[a-z0-9]+)*)*(((\/|&#x2f;)[^\s]*)*)|www\.([a-zA-Z0-9-_]+(\.[a-z0-9-_]+)+)((:|&#x3a;)[0-9]*(\.[a-z0-9]+)*)*(((\/|&#x2f;)[^\s]*)*))/g;
    ...
    renderingUtils.showLyteErrorMessageWithParentScope = function(message,id,parentScopeElement){
        ...
        var elemId = id;	
        var $parentScopeElement = $(parentScopeElement).find('#' + id);//eslint-disable-line @zoho/webperf/directly-select-with-id
        var borderElem = $parentScopeElement.find('lyte-drop-button');
        if(!borderElem.length){
            var borderElem =  $parentScopeElement.find('.lyteField');
        }
        if(!borderElem.length){
            var borderElem =  $parentScopeElement.find('.cxFakeDropButton');//Added to show error msg for crux-dropdown if data are not loaded in it.
        }
    ...
    renderingUtils.tabLayerTop = function(){
        var tabLayer = $L("#tabLayer");
        var crmNextGenTopMenu = $L('#crmNextGenTopMenu');
        var tabLayerTop = 0;
        if(featuresAvailable.NEXTGENUI_NEW_PANEL_ENABLED && crmNextGenTopMenu.length > 0) {
            tabLayerTop = crmNextGenTopMenu[0].getBoundingClientRect().top;
        }else if(tabLayer.length > 0 && !featuresAvailable.NEXTGENUI_ENABLED){
            tabLayerTop = tabLayer[0].getBoundingClientRect().top;
       } 
        return tabLayerTop;
    }
    renderingUtils.tabLayerOuterHeight = function(){
        var tabLayer = $L("#tabLayer");
        var crmNextGenTopMenu = $L('#crmNextGenTopMenu');
        var tabLayerOuterHeight = 0;
        if(featuresAvailable.NEXTGENUI_NEW_PANEL_ENABLED  && crmNextGenTopMenu.length > 0) {
            tabLayerOuterHeight = crmNextGenTopMenu.outerHeight();
        } else if(tabLayer.length > 0 && !featuresAvailable.NEXTGENUI_ENABLED){
            tabLayerOuterHeight = tabLayer.outerHeight();
        }     
        return tabLayerOuterHeight;
    }
    
    /**
     * This method is used for showing custom conformation popup.
     * @param {String} title - This provides the title that has to be shown.
     * @param {String} message - This provides the message content that has to be shown.
     * @param {Array} buttons- This provides the list of buttons which will be shown
     * 
     * buttons = [
     * 	{
     * 		"id": {String} <button id>
     * 		"label": {String} <button label>,
     * 		"appearance": {String} <button appearance>,
     * 		"callBackFunction": {Function} <callback function>
     * 	}
     * ]
     */
    /*renderingUtils.showConformationPopup = function (title, message, buttons) {
        var $component = document.getElementById("crm-conformation-popup-selector");
        if (!$component) {
            var componentResource = networkUtils.returnDependencyFiles(["crm-conformation-popup.js"], crmConstants.jsStaticPathForLyte); // no i18n
            var componentCss = networkUtils.returnDependencyFiles(["crm-conformation-popup.css"], crmConstants.lessStaticPathForLyte); // no i18n
    
            var staticFiles = componentResource.concat(componentCss);
            Lyte.injectResources(staticFiles, undefined, function () {
                var $element = Lyte.Component.render(
                    "crm-conformation-popup",	//no i18n
                    {
                        title: title,
                        message: message,
                        buttons: buttons,
                ...
                $element.setAttribute("id", "crm-conformation-popup-selector");//no i18n
            });
        } else {
            $component.setData("title", title);//no i18n
            $component.setData("message", message);//no i18n
            $component.setData("buttons", buttons);//no i18n
            $component.setData("showPopup", true);//no i18n
        }
    };*/
                
        /**
         * This method is used to find the left position relative to the parent 
         * @param {HTMLElement} obj - This provides the element whose offset left has to be returned
         * @returns {Number} - This returns the left position relative to the parent 
         */
    renderingUtils.findPosX = function(obj) {
        var curleft = 0;
        ...
        /**
         * This method is used to find the top position relative to the parent 
         * @param {HTMLElement} obj - This provides the element whose offset top has to be returned
         * @returns {Number} - This returns the top position relative to the parent 
         */
    renderingUtils.findPosY = function(obj) {
        var curtop = 0;
        if (document.getElementById || document.all) {
        ...
        /**
         * This method is used to load a tooltip for the element which is passed as an argument
         * @param {HTMLElement} elem - This provides the element for which the tooltip has to be created.
         * @returns {void}
         */
    renderingUtils.loadToolTip = function(elem) {
        ...
        renderingUtils.zctt.showtt(revertSplChrs(toolTip),undefined,undefined,undefined,"false");
    }
    /**
     * This method is used to set and display tooltip for a elements
     * 
     */
    renderingUtils.zctt = function(){
        var tt, mw=400,tops=0,left=0,doctt=document,timeout=0;
    ...
    //			var e = (!e)? window.event:e;
                ...
                tt.setAttribute('class', defCls);
                        ...
                        //renderingUtils.zctt.setpos(e,theme);
                ...
                h = parseInt(tt.offsetHeight) + tops;
                w = parseInt(tt.offsetWidth) + left + 20;
                if(timeout!=0){
                    clearTimeout(timeout);
                }
                var el = document.getElementById("tooltip-zc");//NO I18N
                addEventInCrm(el,'mouseenter',function() {//NO I18N
                    tt.style.display = 'block';clearTimeout(timeout)
                });
                addEventInCrm(el,'mouseleave',//No I18n
                    function(){
                        if (waitTime) {
                        ...
                        if (moveOnMouseMove) {
                            if (!e) {
                                ...
                                doctt.onmousemove = renderingUtils.zctt.setpos;
                            }else {
                                renderingUtils.zctt.setpos(e,theme);
                            }
                        }
            ...
            setpos:function(e,theme){
                ...
                if (theme) {
                    var targetElem = renderingUtils.getTargetElem(e);
                ...
                if(u<0){
                    tt.style.top = tops + 'px';//No I18N
                    tt.style.bottom ="";
                }else if(isTopp&&isTopp=="true"){
                    tt.style.top="";
                    tt.style.bottom = ((ch-u)+tops) + 'px';//No I18N
                }else if(isTopp&&isTopp=="false"){
                    tt.style.bottom = "";
                    tt.style.top = (u+tops) + 'px';//No I18N
                }
                else if((u+h+left)>ch){
                    u = u + 50;
                    tt.style.top="";
        ...
        /**
         * This method is used to freeze the background layer
         * @returns {void}  
         */
    renderingUtils.showFreezeLayer = function(){
        var freezeBackGround = $('#freezeBackGround');
        freezeBackGround.css({'display':'block'}); //No I18N
        if ($L('crm-twitter-post-create').length) {
            freezeBackGround.css('z-index','301'); //No I18N
        }
    }
    
        /**
         * This method is used to remove the frozen background layer
         * @returns {void}  
         */
    renderingUtils.hideFreezeLayer = function(){
        if($(".temp_freezelayer") && !($('.ziaCallAnalyticswrapper').length || $('.etTrendsWrapper').length)){
            var tempFreezeLayer = $(".temp_freezelayer.emailAI");
            if(tempFreezeLayer){
                tempFreezeLayer.remove();
            }
        }
        $('#freezeBackGround').css({"display":"none", "zIndex":"30"});
    }
    
    /**
     * This method is used to set the width for the label of the fields
     * @param {String} modules - This provides with the name of the module in which the width has to be set.
     * @param {String} id - This provides the name of the field for which the width has to be set
     * @returns {void}
     */
    renderingUtils.labelWidthFixed = function(modules, id){
        //console.time("time");
        var array1 = [];
        var array2 = [];
        if(id === undefined){
        ...
        return{column1Width:maxWidth1,column2Width:maxWidth2};
    
    }
    
    /**
     * This method is used to freeze the background 
     * @returns {void}
     */
    renderingUtils.freezeBackground = function(){
        $('#ajax_load_tab').css({"top": "0", "position":"fixed"}); //eslint-disable-line @zoho/zohocrm/no-ajaxloadtab
        var oFreezeLayer = document.createElement("DIV");
        oFreezeLayer.id = "FreezeLayer";
        oFreezeLayer.className = "freezeLayer";
        var scrOfX = 0;
        if( document.body && ( document.body.scrollLeft || document.body.scrollTop ) ) { //eslint-disable-line @zoho/webperf/layout-thrashing
            scrOfX = document.body.scrollWidth;//eslint-disable-line @zoho/webperf/layout-thrashing
          } else if( document.documentElement && ( document.documentElement.scrollLeft || document.documentElement.scrollTop ) ) {//eslint-disable-line @zoho/webperf/layout-thrashing
            scrOfX = document.documentElement.scrollWidth;//eslint-disable-line @zoho/webperf/layout-thrashing
          }
        var windowHeight = renderingUtils.windowHeight;
        if(document.body.offsetHeight > windowHeight){//eslint-disable-line @zoho/webperf/layout-thrashing
                oFreezeLayer.style.height = document.body.offsetHeight + "px";
        }else{
                oFreezeLayer.style.height = windowHeight + "px";
        }
        if(scrOfX !== 0){
        ...
        if(!getObj("FreezeLayer")){//eslint-disable-line @zoho/zohocrm/no-deprecated-fnc
            document.body.appendChild(oFreezeLayer); //eslint-disable-line @zoho/zstandard/no-body-events
        }
        if(typeof oDialog !== "undefined"){
            oDialog.style.zIndex = "30";
        }
        renderingUtils.scrollPrevent($("#FreezeLayer"));//No I18N
    }
    
        /**
         * This method is used to close the freeze layer and the element whose id has been provided.
         * @param {String} id - This provides the id of the element that has to be closed.
         * @returns {void}
         */
    renderingUtils.newcloseFreez = function(id)
    {
        $("#"+id).css({"width":"","display":"none","bottom":""});
        $( "#FreezeLayer" ).remove();
        $('body.oH').removeClass('oH');
    }
    
        /**
         * This method is used to remve the freeze Layer
         * @returns {void}
         */
    
    renderingUtils.removeFreezeLayer = function(){
        var tempFreezeLAyer = $(".temp_freezelayer");
        if(tempFreezeLAyer.length>0 && !$('.ziaCallAnalyticswrapper').length){
                        tempFreezeLAyer.remove();
                }else if(!($('.ziaCallAnalyticswrapper').length || $('.etTrendsWrapper').length || $('.etTrendsWrapperCNS').length)){
            $(".freezeLayer").remove();
        }
    }
    
    /**
     * Utility functions for handling fullscreen operations.
     */
    renderingUtils.fullscreenUtil = {
        /**
        ...
        isFullscreenEnabled: function () {
            return !!(
                document.fullscreenEnabled || // eslint-disable-line @zoho/zohocrm/browser-support
         ...
         * @returns {Element|null} The current fullscreen element, or null if there is none.
         */
        getFullscreenElement: function () {
            return (
                document.fullscreenElement || // eslint-disable-line @zoho/zohocrm/browser-support
        ...
        requestFullscreen: function (element) {
            if (element.requestFullscreen) { // eslint-disable-line @zoho/zohocrm/browser-support
                element.requestFullscreen(); // eslint-disable-line @zoho/zohocrm/browser-support
            } else if (element.mozRequestFullScreen) {
                element.mozRequestFullScreen(); // Firefox <= 68
            } else if (element.webkitRequestFullscreen) {
                element.webkitRequestFullscreen(); // Modern Safari
            } else if (element.webkitRequestFullScreen) {
                element.webkitRequestFullScreen(); // Safari <= 13 (uppercase "S") //No I18N
            } else if (element.msRequestFullscreen) {
                element.msRequestFullscreen(); // Legacy Edge
            } else if (element.webkitEnterFullscreen) {
                element.webkitEnterFullscreen(); // Very old Safari (rare)
            }
        },
        ...
        exitFullscreen: function () {
            if (document.exitFullscreen) { // eslint-disable-line @zoho/zohocrm/browser-support
                document.exitFullscreen(); // eslint-disable-line @zoho/zohocrm/browser-support
            } else if (document.mozCancelFullScreen) {
                document.mozCancelFullScreen(); // Firefox <= 68
            } else if (document.webkitExitFullscreen) {
                document.webkitExitFullscreen(); // Modern Safari
            } else if (document.webkitCancelFullScreen) {
                document.webkitCancelFullScreen(); // Safari <= 13 (uppercase "S") //No I18N
            } else if (document.msExitFullscreen) {
                document.msExitFullscreen(); // Legacy Edge
            }
        },
    
        /**
         * Toggle fullscreen for a given element.
         * @param {Element} element - The element to toggle fullscreen for.
         */
         ...
         * @param {Function} callback - The callback function to execute on fullscreen change.
         */
        onFullscreenChange: function (element, callback) {
            if (element.addEventListener) {
                element.addEventListener("fullscreenchange", callback); // eslint-disable-line @zoho/zohocrm/browser-support
                element.addEventListener("mozfullscreenchange", callback); // Firefox <= 68
                element.addEventListener("webkitfullscreenchange", callback); // Safari
                element.addEventListener("MSFullscreenChange", callback); // Legacy Edge
            } else {
                element.onfullscreenchange = callback;
                element.onmozfullscreenchange = callback; // Firefox <= 68
                element.onwebkitfullscreenchange = callback; // Safari
                element.onMSFullscreenChange = callback; // Legacy Edge
            }
        },
         ...
         */
        offFullscreenChange: function (element, callback) {
            if (element.removeEventListener) {
                element.removeEventListener("fullscreenchange", callback);
                element.removeEventListener("mozfullscreenchange", callback); // Firefox <= 68
                element.removeEventListener("webkitfullscreenchange", callback); // Safari
                element.removeEventListener("MSFullscreenChange", callback); // Legacy Edge
            } else {
                element.onfullscreenchange = null;
                element.onmozfullscreenchange = null; // Firefox <= 68
                element.onwebkitfullscreenchange = null; // Safari
                element.onMSFullscreenChange = null; // Legacy Edge
            }
        },
    
        /**
         * Set onfullscreenchange directly on an element.
         * @param {Element} element - The element to set the onfullscreenchange property on.
         * @param {Function} callback - The callback function to execute on fullscreen change.
         */
        setOnFullscreenChange: function (element, callback) {
            if (element.onfullscreenchange !== undefined) {
                element.onfullscreenchange = callback;
            } else if (element.onmozfullscreenchange !== undefined) {
                element.onmozfullscreenchange = callback; // Firefox <= 68
            } else if (element.onwebkitfullscreenchange !== undefined) {
                element.onwebkitfullscreenchange = callback; // Safari
            } else if (element.onMSFullscreenChange !== undefined) {
                element.onMSFullscreenChange = callback; // Legacy Edge
            }
        },
    
        /**
         * Clear onfullscreenchange directly on an element.
         * @param {Element} element - The element to clear the onfullscreenchange property on.
         */
        clearOnFullscreenChange: function (element) {
            if (element.onfullscreenchange !== undefined) {
                element.onfullscreenchange = null;
            } else if (element.onmozfullscreenchange !== undefined) {
                element.onmozfullscreenchange = null; // Firefox <= 68
