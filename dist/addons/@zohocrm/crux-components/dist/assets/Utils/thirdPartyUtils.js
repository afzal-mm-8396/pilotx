...
thirdPartyUtils.unbindChildPerfectScroll = function(parentObj){
	parentObj = $L(parentObj);
	var psScrolls = parentObj.find(".ps-container");
	var psScrollsLength = psScrolls!=null ? psScrolls.length : 0;
	if(  psScrollsLength>0 ){
		try{
			for(var i=0;i<psScrollsLength;i++){
				if($.isFunction(psScrolls.eq(i).perfectScrollbar)){
					psScrolls.eq(i).perfectScrollbar('destroy');//NO I18N
				}
			}
		}
		catch(e){
...
thirdPartyUtils.unbindFullcalendar = function(parentObj){
	var calendarDiv = parentObj.find("#calendarBd");//eslint-disable-line @zoho/webperf/directly-select-with-id
...
thirdPartyUtils.bindPerfectScrollUpdate = function(JElem){
	 ...
	 * @param {jQueryElement} JElem - This provides the jQueryElement for which the perfect scroll plugin is initiated.
	 * @param {Object} options - Refer {@link https://mdbootstrap.com/freebies/perfect-scrollbar/?utm_source=GitHub&utm_medium=PerfectScrollbar#options|here}
...
//	if(options){
//		if(Object.keys(options).indexOf('preventOnEnd') !== -1){
//			var value = options.preventOnEnd;
//			delete options.preventOnEnd;
//			options.wheelPropagation = !value;
//		}
//		if(Object.keys(options).indexOf('preventOnHorizontal') !== -1){
//			var value = options.preventOnHorizontal;
//			delete options.preventOnHorizontal;
//			options.suppressScrollX = value;
//		}
//	}
...
/**
 * @param {Object} data - This provides a object that contains the required properties to construct a span which is disabled.
 * @returns {String} - This returns a span element with the provided data in it.
 */
thirdPartyUtils.selectTempResult = function(data){
	var selectEmptyText = data.text;
	var ele = data.element;
	if(ele !== undefined){
		var iscolorcodeEnabled=$(ele.parentElement).data("colorcode_enabled"); //NO i18N 
		var pickMap = $(ele.parentElement).attr("data-picklistmap") ? $(ele.parentElement).attr("data-picklistmap") : $(ele.parentElement).attr("data-values"); //eslint-disable-line @zoho/webperf/no-multipleDOMLookup 
		var validJson = pickMap !== undefined ?  Crm.isValidJSON(pickMap) : false;
		$('.select2-container').removeClass('dropdown_eventcolor');
		if (validJson) {
			var colorCode = $(ele).data("colorcode");
			if(colorCode || (!colorCode &&  iscolorcodeEnabled)){
				var systemvalue=$(ele).attr("actualvalue") ? $(ele).attr("actualvalue"): $(ele).val();
				var noneClass = systemvalue=== "-None-" || systemvalue === "" ? " colorEnabledSelect2None" : "" ; //No I18N
				selectEmptyText = "<span class='disable" + data.disabled + noneClass + "'><span class='dIB activitesColor' data-zcqa='option_" + $ESAPI.encoder().encodeForHTMLAttribute(selectEmptyText) + "' style='background:" + $ESAPI.encoder().encodeForHTMLAttribute(colorCode) + "'></span><span>" + $ESAPI.encoder().encodeForHTML(data.text)  + "</span></span>";//No I18N
				$('.select2-container').addClass('dropdown_eventcolor');
				return selectEmptyText;
			}
			else{
				$('.select2-container').removeClass('dropdown_eventcolor'); // eslint-disable-line @zoho/webperf/no-multipleDOMLookup 
			}
		}
	}
...
/**
 * @param {Object} data - This provides a object that contains the required properties to construct a span which is disabled.
 * @param {HTMLElement} container - This provides the container element from which the ID is taken and necessary classes are added. 
 * @returns {String} - This returns a span element with the provided data in it.
 */
thirdPartyUtils.selectEmptyVal = function(data, container){
	 ...
	 if(selectEmptyId === '' || selectEmptyId === '-None-' &&  !container ){
		 if(container != undefined ){
	 	 ...
	 	 // Displaying the selected option for formula string return types
	 if(container && container.attr('id') === "select2-returntype-container" && CMB.fieldPopupJSON.isFormulaReturnTypeSplitEanbled && CF.formulaReturnType && CF.formulaReturnType.startsWith('string') && CF.subReturnType!==""){	//No I18N
	 	if($('#currentfield').attr('ekind')==='old'){
			var subReturnTypeString = 'crm.label.string.'+CF.subReturnType ;	//No i18n
			var stringReturnTypeCharacterLimit = CF.subReturnType==="small" ? 255 : CF.subReturnType ==="medium" ? 2000 : 32000 ; //No I18N
			var tooltipSpanClass = "<span data-zcqa='formulaType_info' class='mL5 zcicncss-info zcicn-cssIcons zcicncss13 zcicn_grey_mask peAuto' lt-prop-title='"+ $ESAPI.encoder().encodeForHTML(I18n.getMsg('crm.formula.string.maxlimit',[I18n.getMsg(subReturnTypeString),stringReturnTypeCharacterLimit])) +"'></span>";
			 return "<span class='disable" + data.disabled + "'>"+$ESAPI.encoder().encodeForHTML(I18n.getMsg('crm.label.string'))+" (" + $ESAPI.encoder().encodeForHTML(I18n.getMsg('crm.label.string.'+CF.subReturnType)) + ")"+tooltipSpanClass+"</span>";
		 }
	 	return "<span class='disable" + data.disabled + "'>"+$ESAPI.encoder().encodeForHTML(I18n.getMsg('crm.label.string'))+"<span class='crmLabelColor'> (" + $ESAPI.encoder().encodeForHTML(I18n.getMsg('crm.label.string.'+CF.subReturnType)) + ")</span></span>";
	 }
	 ...
	 * @param {JQueryElement} JElem - This provides the Select element for which Select2 has to be initiated. 
	 * @param {Object} options - This provides a list of options that can be found in the properties table.
...
//						});
				...
				if(elemId && elemId ==="returntype"){
					//add drop down icon for 'String' return type
					obj.templateResult = CF.addDropdownMenuIcon;
				}		
				...
				else if(elemId && elemId === "userDateFormat") //No i18n
				{
					obj.templateResult = function renderOptionText(state) {
				        ...
				        var result = '<span class="crmBaseColor  crm-font-bold mR5 wbBreakWord">';//NO I18N
				        result += state.text;
				        result += '</span><span class="crmNotFoundColor crm-small-font-size dIB">(';//NO I18N
				...
				var objParentParent = $(this).parent().parent();// eslint-disable-line @zoho/webperf/no-multipleDOMLookup 
				if(!($thisSelect.data("colorcode_enabled")) && objParentParent!==undefined && objParentParent.hasClass("colorEnabledPicklist"))
				{
					objParentParent.removeClass("colorEnabledPicklist");
				}				
				$(this).select2(obj);
				...
				if( isMultipleAttr ){
					...
					selectionElem.on("blur",function(){//No I18N
						...
						var selTwo_labelValCreate = selTwo.closest(".labelValCreate"); //No I18N
						var removeFocusField = true;;
						var elementFocus = $('.select2-container--open').eq(0);
						if(selTwo.closest('.businesscall') && elementFocus.length !== 0){
							removeFocusField=false;
						}
						if(selTwo_labelValCreate.length){
						...
						else if(selTwo.closest(".labelValTwo").length && removeFocusField){
							selTwo.closest(".tabDiv").removeClass("focusEleField");//No I18N
						}else if(closestTd.length) {//No I18N
							var focusEle = elementFocus;
							if(focusEle.length === 0){
...
//				$(".select2-selection__rendered").attr("title","");//No I18N
                ...
                //$(this).off('select2:open') -> since it is removing the previously set callbacks, commented this for HB bug ZCRM-814508
				$(this).on('select2:open', function () {
					var selectObj = $(this);
					if(selectObj.closest('.zc_nodarktheme').length || selectObj.closest('.zc_nodarktheme_modal').length){
						const $select2Container = selectObj.data('select2').$dropdown;//No I18n
						$select2Container.addClass('zc_nodarktheme');
					}
				});
				$(this).off('select2:opening');
				$(this).on('select2:opening', function (e) {
					var selectObj = $(this);
					var detailview = $('#dvinner');
					var detailviewVisible = detailview.is(":visible");//No I18N
					if(selectObj.data() && selectObj.data("colorcode_enabled")){
						var selectParent=selectObj.parent();
						var colorComponent=selectParent.find(".activitesColor");
						if(selectObj.val() === "" && colorComponent.length > 0){
							colorComponent.remove();
							selectParent.parent().addClass("colorEnabledPKNone");
						}
					}
					if($('#analyticalPageContainer').length === 1 || $('#createDashboardPanel').length === 1) {
						AnalyticsChart.freezePopScroll(selectObj);
					}
					if($L('.crmcanvasquicksavepop').length && Lyte.registeredMixins['crm-formview-mixin']){
						Lyte.registeredMixins['crm-formview-mixin'].freezeQuickSavepopScroll();//No I18N 
					}
					...
					if(selectObj.hasClass('ccPhoneNumber')){
						...
						if(!selectObj.is(':visible')){/* eslint-disable-line @zoho/webperf/layout-thrashing */
						 return false;
						}
					...
					//eslint-disable-next-line @zoho/webperf/layout-thrashing 
						  ...
						  var layoutConvLanding = $('#layout_landing_right');
					  	  var getScrollBarWidthValue = renderingUtils.windowHeight >= $(document).height() ? 0 : renderingUtils.getScrollBarWidthValue;//eslint-disable-line @zoho/webperf/layout-thrashing
						  ...
						  if(quickPopup.is(':visible')){//eslint-disable-line @zoho/webperf/layout-thrashing
							  setTimeout(function(){
								  quickPopup.addClass('oHimp');
							  },200);
						  }
						  var mmCriteriaContainer = $('.mmCriteriaContainer');
						  if(mmCriteriaContainer.is(":visible")){//eslint-disable-line @zoho/webperf/layout-thrashing
							  mmCriteriaContainer.css({"overflow":"hidden"});//No I18N
						  }  
						  if(layoutConvLanding.is(":visible")){//eslint-disable-line @zoho/webperf/layout-thrashing
						  	layoutConvLanding.addClass('oH');//No I18N	                                                  }
						 }
						  if(detailviewVisible){
							  detailview.addClass('oHimp');
						  }
					 }
					...
					setTimeout(function(){
					  ...
					  var lrImpactedDivAvailable = $L('.lrPicklistImpactedQCPopup:visible');
					  ...
					  if(lrImpactedDivAvailable.length){
						  openedSelect2Container.addClass('lrImpactedSelect2OpenElement');
					  }
					  if( $('.select2-results__option').length < 10 ){//No I18N
					  		$('.select2-container--open').find('.select2-search--dropdown').css({"height":"0px","overflow":"hidden"});//No I18N
				  	  }
...
//				});
				$(this).on('select2:close', function () {//No I18N
				   ...
				   var FormulaSubReturnTypeMenu= $("#FormulaSubReturnTypeMenu");
                   if(elemId && elemId ==="returntype" && FormulaSubReturnTypeMenu[0] && FormulaSubReturnTypeMenu[0].component.getData('ltPropShow')){
                      $(this).select2("open"); //eslint-disable-line @zoho/webperf/no-multipleDOMLookup
                   }
				  var layoutConvLanding = $('#layout_landing_right');
				  var detailview = $('#dvinner');
				  var detailviewVisible = detailview.is(":visible");//No I18N
				  ...
				  var $createEditCallDiv = $('#createEditCallDiv');
  				  if($createEditCallDiv.is(':visible')){//eslint-disable-line @zoho/webperf/layout-thrashing
						$("#createEditCallPopup").off("scroll.select2.select2-Crm_Calls_CALLTYPE");
				  }
				...
				$(this).on('select2:closing', function (){
					...
					if($L('.crmcanvasquicksavepop').length && Lyte.registeredMixins['crm-formview-mixin']){
						Lyte.registeredMixins['crm-formview-mixin'].rm_freezeQuickSavepopScroll();//No I18N 
					}
					
					var selectOpenElem = $('.select2-container--open');
					if(selectObj.closest('.zc_nodarktheme').length  || selectObj.closest('.zc_nodarktheme_modal').length){
						selectOpenElem.removeClass('zc_nodarktheme');
					}
...
//}
	...
	/**
	 * This method is used to bind the scroll plugin for table
	 * @returns {void}
	*/
...
//		            fixedHeaderTableElem.css({'right': -1*Math.abs(thirdPartyUtils.tabContLeftPos-Math.abs(listviewtablescrollElem.scrollLeft()))});//No i18N
        ...
        if(sortDiv.length && sortDiv.css('display') === 'block'){//eslint-disable-line @zoho/webperf/layout-thrashing
        	sortDiv.hide();
        }     
    });
}
	...
	$.fn.select2.amd.require(['select2/selection/search'], function (Search) {//No I18N
			if(this.$element && this.$element.data('removeElement'))
			{
				this.trigger('unselect', {//No I18N
					data : arguments[1] //eslint-disable-line @zoho/zstandard/no-reserved-words
				})
				return;
			}
			...
			Search.prototype.searchRemoveChoice = function () {
				oldRemoveChoice.apply(this, arguments); //eslint-disable-line @zoho/zstandard/no-reserved-words
				this.$search.val('');
				this.handleSearch();
			};
	});
...
thirdPartyUtils.closeSelect2 = function(id){
	...
	/**
	 * This method is used to initiate the perfect scroll plugin
	 * @param {String} id - This provides the id of the jQueryElement for which the perfect scroll plugin has to be initiated.
	 * @returns {void}
	 */
thirdPartyUtils.enablePerfectScroll = function(id){
	thirdPartyUtils.bindPerfectScroll('#' + id,{wheelSpeed: 1,wheelPropagation: true,suppressScrollX: false});//NO I18N
}
