        ...
        if(!fastRenderedProp) {
            ...
            for(let key in data) {
                ...
                if(obj.watch && /^(object|array)$/.test(obj.type)){
                    Lyte.establishObjectBinding(compData, key, false);
                }
            }
            compData.errors = {};
    ...
    afterConnected() {
        ...
        if(this._initProperties) {
            ...
            for(let key in initProperties) {
                ...
                if(!error) {
                    compData[key] = actVal;    
                     if(field && /^(object|array)$/.test(field.type) && field.watch){
                        Lyte.establishObjectBinding(compData, key);
                    }
                }
            }
        ...
        for(let i=0;i<dynamicNodes.length;i++) {
            ...
            if(type ===  "componentDynamic") {
            	if(options.node) {
                 ...
                 this.bindNode(dynamicN, undefined, helperNode, options, dynamicNodes[i], processLast, establishBindings,undefined,dynamicNodes._cache);
            }
            else if(type === "attr"){
				...
				for(let key in info.attr) {
                    ...
                    if(attr && (attr.dynamicValue || attr.helperInfo)) {
			            ...
			            if(!attr.globalEvent) {
                        	...
                        	var node = this.bindNode(dynamicN.attributes.getNamedItem(attrName), toBeRemoved, helperNode, options, attr, undefined, establishBindings,undefined,dynamicNodes._cache);
...
//            this._template.content = frag;
            ...
            try{
                for(let i=0;i<attributesT.length;i++) {
                    if(!this._templateAttributes.attr[attributesT[i].name] && attributesT[i].name !== "tag-name" && attributesT[i].name !== "use-strict") {
                        ...
                        this._template.removeAttribute(attributesT[i].name);
                        i--;
                    }
                }    
            } catch(e) {
    ...
    bindNode(node, toBeRemoved, helperNode, options, nodeInfo, processLast, establishBindings, isTemplate,cache) {
        ...
        "set" : function(object, property, value, fromStore, fromParent) {
            ...
            if(Array.isArray(object._scp)){
                object._scp.forEach(function(obj){
                    var recObj = Lyte.nestScp[obj.id], model, field, rec, attr, wobj = {};
                    if(recObj.model){
                        attr = recObj.attr;
            ...
            if(property instanceof Object){
            	if(_Lyte.isRecord(object) && !fromStore && !object.$.isCloned) {
                        ...
                        let record = store.$.setData(object.$, property, undefined, undefined, undefined, shareObj);
                        if(record.$.isError){
                            return record;
                        }
                ...
                if(_Lyte.isRecord(object) && !fromStore && !object.$.isCloned) {
                    ...
                    let record = store.$.setData(object.$, property,value, undefined, undefined, shareObj);
                    if(record.$.isError){
                        return record;
                    }
...
//                                          this.establishBindings(item._propBindingObject, value);
                ...
                if(watch && watch.length){
                    watch.forEach(function(wObj){
                        ...
                        deletedItems.forEach(function(itm){
...
"callDeepObservers" : function (data, args, property){
            var type = args.type;
            data._scp.forEach(function(scp){
                var recObj = Lyte.nestScp[scp.id], attr, rec, isRec = false;
                if(recObj.model){
                    attr = recObj.attr;
                    rec = store.peekRecord(recObj.model, recObj.pK);
                    isRec = true
            ...
            if(_Lyte._ms) {
                ...
                if (info[def].dynamicNodes) {
                    ...
                    if(info[def]._ht) {
                        ...
                        if(contentNewCompile) {
                            contentNewCompile.innerHTML = info[def]._ht;
                            constr.splitTextNodes(contentNewCompile);
                        }
                        constr.splitTextNodes(content);
                    }
...
var store = {
	...
	$:{ 
		...
		nestScp : {},
		nestScpId : 1, 
		changeRelPkMaps : function(data, oldPk, newPk){
		...
		establishLink:function(forward,backward,data,relatedRecord,index,ignorePartial, isDuplicate){
			if(!relatedRecord){
				return "ERR13";
			}
		...
		handleArrOp:function(data,type,obj,pos,len){
			...
			var toBind = typeof Lyte.arrayUtils != "undefined" ? true : false, ret;
			switch(type){
				case "push": {
        ...
        checkResponse:function(data , model , response , pK , partialObj, status){
			...
			if(data.$.isNew){
				...
				if(model.data._recMap){
					model.data._recMap.delete(typeof dirtyId == "object" ? dirtyId : dirtyId.toString());
					model.data._recMap.set(typeof newPk == "object" ? newPk : newPk.toString(), data);
				}
		...
		value : function value(modelName, pKey, isDeleted){
			...
			if(isDeleted === true){
				...
				if(!isComp && model.data._recMap){
					...
					var data = model.data, pK = this.model[modelName]._pK;
