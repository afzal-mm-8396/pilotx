 ...
 */
		...
		function customElementPrototype() {
			...
			if (!fastRenderedProp) {
				...
				for (var _key5 in data) {
					var obj = data[_key5];
					compData[_key5] = obj[def];
					if (obj.watch && /^(object|array)$/.test(obj.type)) {
						Lyte.establishObjectBinding(compData, _key5, false);
					}
				}
				compData.errors = {};
			...
			value: function afterConnected() {
				...
				if (this._initProperties) {
					...
					for (var _key8 in initProperties) {
						var actVal = void 0;
						var _field = compInstance.__data[_key8];
						if (_field && _field.type !== _LC.getDataType(initProperties[_key8]) && (initProperties[_key8] !== undefined || _field.type === "boolean")) {
						...
						if (!error) {
							...
							if (_field && /^(object|array)$/.test(_field.type) && _field.watch) {
								Lyte.establishObjectBinding(compData, _key8);
							}
						}
					}
        ...
        content = toAppend.cloneNode(true, "lyte");
				...
				for (var _i8 = 0; _i8 < dynamicNodes.length; _i8++) {
					...
					if (type === "componentDynamic") {
						...
						this.bindNode(dynamicN, undefined, helperNode, options, dynamicNodes[_i8], processLast, establishBindings, undefined, dynamicNodes._cache);
					} else if (type === "attr") {
					...
					if (this._templateAttributes && this._templateAttributes.attr) {
						...
						try {
							for (var _i45 = 0; _i45 < attributesT.length; _i45++) {
								if (!this._templateAttributes.attr[attributesT[_i45].name] && attributesT[_i45].name !== "tag-name" && attributesT[_i45].name !== "use-strict") {
									this._templateAttributes.attr[attributesT[_i45].name] = { "name": attributesT[_i45].name, "staticValue": attributesT[_i45].value };
									this._template.removeAttribute(attributesT[_i45].name);
									_i45--;
								}
							}
						} catch (e) {
			...
			for (var _key26 in def) {
				if (def[_key26] && def[_key26].type === "observer") {
					...
					if (def[_key26].observes) {
						def[_key26].observes.fnName = _key26;
   ...
   */
		...
		"aF": function aF() {
          ...
          continue;
										...
										for (var _i58 = index, _j5 = 0; _i58 < finalIndex; _i58++, _j5++) {
											_LC.removeSelectedBindingDeep(_defineProperty({}, itemValue, helper._items[_i58].itemProperty), _defineProperty({}, itemValue, deletedItems[_j5]));
										}
										var contextSwitchArray = [];
          ...
          continue;
										...
										for (var _i60 = _index2, _j7 = 0; _i60 < _finalIndex; _i60++, _j7++) {
											_LC.removeSelectedBindingDeep(_defineProperty({}, _itemValue, _helper._items[_i60].itemProperty), _defineProperty({}, _itemValue, _deletedItems[_j7]));
										}
          ...
          continue;
										...
										for (var _i62 = _index3, _j9 = 0; _i62 < _finalIndex2; _i62++, _j9++) {
											_LC.removeSelectedBindingDeep(_defineProperty({}, _itemValue2, _helper2._items[_i62].itemProperty), _defineProperty({}, _itemValue2, _deletedItems2[_j9]));
										}
		...
		"callArrayObservers": function callArrayObservers(array, args) {
			...
			if (array._bindings) {
				...
				for (var _i66 = 0; _i66 < objbind.length; _i66++) {
					var binding = objbind[_i66];
					var path = objbind[_i66]._path;
		...
		"establishSelectedBinding": function establishSelectedBinding(property, actualData, node) {
			...
			for (var _i68 = 0; _i68 < props.length; _i68++) {
				...
				currentProp = currentProp[props[_i68 + 1]];
				currentValue = currentValue[props[_i68 + 1]];
			}
		},
		"establishBindings": function establishBindings(properties, actualData) {
						...
						for (var j = 0; j < nodes.length; j++) {
							...
							if (helper && helper.tagName === "TEMPLATE" && helper.getAttribute("is") === "for") {
								if (helper._items) {
									...
									for (var _i69 = 0; _i69 < helper._items.length; _i69++) {
										var data = actualData[_i69];
										var _item11 = helper.getAttribute("item");
										if (data) {
											if (_typeof(helper._items[_i69]) === "object") {
												this.establishBindings(helper._items[_i69].itemProperty, _defineProperty({}, _item11, data));
											}
										}
									}
								}
			...
			for (var _i70 in properties) {
				...
				if (!actData || typeof actData === "string" || typeof actData === "number" || typeof actData === "boolean") {
					...
					actData._bindings.add(properties[_i70]);
					if (_typeof(properties[_i70]) === "object") {
						this.establishBindings(properties[_i70], actData);
					}
				}
			}
		},
		"removeSelectedBindingDeep": function removeSelectedBindingDeep(binding, actualData) {
			...
			for (var _i71 in binding) {
				...
				if (actData && actData._bindings) {
					...
					//                        delete actData._bindings;
					//                    }
					deleteBindingCheckSize(actData, "_bindings", binding[_i71]);
				}
				if (_typeof(binding[_i71]) === "object" && actData) {
					this.removeSelectedBindingDeep(binding[_i71], actData);
				}
			}
			if (binding._forHelpers) {
				...
				for (var _i72 = 0; _i72 < objbind.length; _i72++) {
					...
					if (fH.getAttribute("is") === "for") {
						...
						for (var _i73 = 0; _i73 < itemCases.length; _i73++) {
							this.removeSelectedBindingDeep(itemCases[_i73].itemProperty, items[_i73]);
						}
					} else {
		...
		"affectChanges": function affectChanges(item, contextAlreadySwitched, oldValue, setterScope, newValue) {
			if (item._dynamicNodes) {
				for (var _i74 = 0; _i74 < item._dynamicNodes.length; _i74++) {
					item._dynamicNodes[_i74]._callee.updateNode(item._dynamicNodes[_i74], item._path);
				}
			}
			...
			if (item._helperNodes) {
				...
				for (var _s3 = 0; _s3 < itemHelperNodes.length; _s3++) {
					...
					if (itemHelperNodes[_s3].getAttribute("is") === "for" && itemHelperNodes[_s3]._items) {
						...
						for (var _i75 = 0; _i75 < itemHelperNodes[_s3]._items.length; _i75++) {
							callee.component.data[itemValue] = items[_i75];
							callee.component.data[indexValue] = _i75;
							callee._properties[itemValue] = itemHelperNodes[_s3]._items[_i75].itemProperty;
							if (itemHelperNodes[_s3]._items[_i75]._dynamicProperty[propPath]) {
								nodes = itemHelperNodes[_s3]._items[_i75]._dynamicProperty[propPath];
						...
						if (nodes.length) {
							...
							for (var _i78 = 0; _i78 < nodes.length; _i78++) {
								nodes[_i78]._callee.updateNode(nodes[_i78], item._path);
							}
			...
			if (Array.isArray(item)) {
				...
				for (var _key36 in item) {
					var oldV = oldValue ? oldValue[_key36] : oldValue,
					    newV = newValue ? newValue[_key36] : newValue;
					if (_key36 == "*" && item[_key36].hasOwnProperty("_observers")) {
						oldV = oldValue;
						newV = newValue;
					}
					this.affectChanges(item[_key36], true, oldV, setterScope, newV);
				}
			}
		},
		...
		"get": function get(context, path, ac, cache) {
			...
			if (cache && cache.hasOwnProperty(path)) {
				...
				try {
					...
					for (var _i80 = 1; _i80 < arr.length; _i80++) {
						var locVal = arr[_i80];
				...
				case "attr":
					{
						...
						for (var _i81 = 0; _i81 < node.attributes.length; _i81++) {
							if (node.attributes[_i81].nodeValue.indexOf("{{") !== -1) {
								var _val7 = node.attributes[_i81].nodeValue;
								...
								if (actObj && (actObj.name === "action" || actObj.name === "method") && /^(onfocus|onfocusin|onfocusout|onresize|onscroll|onclick|ondblclick|onmousedown|onmouseup|onmousemove|onmouseover|onmouseout|onchange|onselect|onsubmit|onkeydown|onkeypress|onkeyup|oncontextmenu)$/.test(node.attributes[_i81].name)) {
									attr[node.attributes[_i81].name.substr(2)] = { name: node.attributes[_i81].name.substr(2), camelCase: _LC.String.toCamelCase(node.attributes[_i81].name.substr(2)), helperInfo: actObj, newHelperInfo: Lyte.Compile.getHelper(actValue, true), globalEvent: true };
									...
									if (dynNewCompile) {
										dynNewCompile.setAttribute(node.attributes[_i81].name.substr(2), componentName + " => " + actString);
									}
						...
						if (toBeRemoved.length) {
							for (var _i82 = 0; _i82 < toBeRemoved.length; _i82++) {
								...
								if (dynNewCompile) {
									dynNewCompile.removeAttribute(toBeRemoved[_i82]);
								}
							}
						}
						if (dynNewCompile) {
				...
				case "switch":
					{
						...
						if (_Lyte._ms) {
							...
							for (var _key39 in info.cases) {
								...
								if (content.tagName == "TEMPLATE") {
									info.cases[_key39]._ht = content.innerHTML;
								}
								if (info.cases[_key39]._ht) {
									content.innerHTML = info.cases[_key39]._ht;
									if (_contentNewCompile2) {
										_contentNewCompile2.innerHTML = info.cases[_key39]._ht;
								...
								if (info.cases[_key39]._ht) {
									info.cases[_key39]._ht = content.cloneNode(true);
									constr._helperTemplate.content.append(info.cases[_key39]._ht);
									content.innerHTML = "";
								}
							}
							if (info[def].dynamicNodes) {
	...
	function getArgString(name, array) {
		...
		for (var _i84 = 0; _i84 < array.length; _i84++) {
			if (array[_i84] && _typeof(array[_i84]) === "object") {
				array[_i84] = getArgString(array[_i84].value.name, array[_i84].value.args);
			}
		}
		if (name) {
	...
	Lyte.Compile.getTrimmedContent = function (content, position, node) {
		...
		for (var _i85 = 0; _i85 < position.length; _i85++) {
			for (var j = content.childNodes.length - 1; j > position[_i85]; j--) {
				content.childNodes[j].remove();
			}
			content = content.childNodes[position[_i85]];
		}
		return dummyContent;
	};
	...
	Lyte.Compile.getHelperArgs = function (str, newCompile) {
		...
		for (var _i87 = 0; _i87 < str.length; _i87++) {
			if (!stack.length && str.charAt(_i87) === ",") {
				var _toPush2 = str.substr(from, _i87 - from);
				...
				args.push(_toPush2);
				from = _i87 + 1;
			} else if (str.charAt(_i87) === "(") {
				if (stack[stack.length - 1] != "'" && stack[stack.length - 1] != "\"") {
					stack.push(str.charAt(_i87));
					lastPushed = str.charAt(_i87);
				}
			} else if (str.charAt(_i87) === ")") {
				if (stack[stack.length - 1] === "(") {
					stack.pop();
				}
			} else if (str.charAt(_i87) === "'" && str.charAt(_i87 - 1) !== "\\") {
				if (stack[stack.length - 1] === "'") {
					stack.pop();
