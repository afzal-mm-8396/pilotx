; (function () {
    if ($L) {
        ...
        $L.eyeDropper.initialize = function (obj) {
            var options = obj || {};
            options.onPick = obj.onPick ? obj.onPick : undefined;
            // options.query = obj.query;
            $L.eyeDropper.options = options;;
            if (window.EyeDropper == undefined) {
                // EyeDropper API is not supported on this platform
                lyteEyeDropper(options);
            }
            else {
                eyeDropperApi(options);
            }
        }
        ...
        function eyeDropperApi(obj) {
            ...
            $L(query)[0].addEventListener('click', e => {
                ...
                eyeDropper.open()
                    .then(colorSelectionResult => {
                        ...
                        if (obj.onPick) {
                            obj.onPick(colorSelectionResult.sRGBHex);
                        }
                    })
                    .catch(error => {
                        console.error(error);
        ...
        function getPixelInfo(event) {
            ...
            if ($L.eyeDropper.options.onPick) {
                $L.eyeDropper.options.onPick(color);
            }
