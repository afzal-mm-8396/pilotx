	...
	if( lytedom ){
		...
		function mouseup( evt ){
			...
			if( data.moved || anchorClick ){
				...
				if( target && !document.contains( target[0] ) ){
			...
			if(!data.anchorClick){//happ only once
				...
				[ 'mousemove', 'mouseup', 'moved', 'clientY', 'clientX', 'tempElement', 'con_x', 'con_y', 'element', 'pos', 'anchorClick', 'shapeDim' ].forEach( function( item ){
					delete data[ item ];
				});
			}
		}

		function getLineCurve(lineOffset, start, end, ref_x, ref_y, data) {
			...
			var bendStartX = (start.x - ref_x),
				...
				lineOffsetValue = {},
				lineModifiers = [{ x: bendStartX, y: bendStartY, name: "M" }],
				cornerRadius = data.lineRadius,
				path = "";
			...
			lineOffsetValue.start && lineModifiers.push({ x: bendStartX + lineOffsetValue.start.inc * (lineOffsetValue.start.hori ? lineOffset.start : 0), y: bendStartY + lineOffsetValue.start.inc * (lineOffsetValue.start.hori ? 0 : lineOffset.start), name: "L" });
			lineOffsetValue.end && lineModifiers.push({ x: bendEndX + lineOffsetValue.end.inc * (lineOffsetValue.end.hori ? lineOffset.end : 0), y: bendEndY + lineOffsetValue.end.inc * (lineOffsetValue.end.hori ? 0 : lineOffset.end), name: "L" });
			lineModifiers.push({ x: bendEndX, y: bendEndY, name: "L" });

			for (var i = 0; i < lineModifiers.length; i++) {
				var point = lineModifiers[i];

				if (i == 0) {
					path += "M " + point.x + " " + point.y;
				} else if (!cornerRadius || (i == lineModifiers.length - 1)) {
					path += " L " + point.x + " " + point.y;
				} else {
					var prev = lineModifiers[i - 1],
						next = lineModifiers[i + 1],
						...
						effectiveRadius = Math.min(cornerRadius, dist1 / 2, dist2 / 2);

					if (effectiveRadius > 0 && dist1 > 0 && dist2 > 0) {
						var beforeX = point.x - effectiveRadius * (dx1 / dist1),
							beforeY = point.y - effectiveRadius * (dy1 / dist1),
							afterX = point.x + effectiveRadius * (dx2 / dist2),
							afterY = point.y + effectiveRadius * (dy2 / dist2);

						path += " L " + beforeX + " " + beforeY;
						path += " Q " + point.x + " " + point.y + " " + afterX + " " + afterY;
					} else {
						path += " L " + point.x + " " + point.y;
					}
				}
		...
		function draw_curve( svg, start, end, data, _this ){
			...
			$svg = $L( svg ),
			$data = $svg.data() || {}, 
			svgDataOptions = $data.options || {},
			type = svgDataOptions.connection_type || data.connection_type,
			scroll = data.getScroll(),
			curve_offset = /*Math.min( */svgDataOptions.curve_offset == undefined ? data.curve_offset : svgDataOptions.curve_offset/*, width / 3 )*/,
			_module = data.module,
			...
			is_src_hidden = !start.width && !start.height && start.elem,
			is_target_hidden = !end.width && !end.height && end.elem;

			if( !is_src_hidden && !is_target_hidden ){
				switch( type ){
					case "line": {	
						if(lineOffset.start || lineOffset.end){
							path = getLineCurve(lineOffset, start, end, ref_x, ref_y, data);
						} else {
					...
					break;
					case "elbow" : {
						var avoidOverlap = data.centerCommonItems && $data.options && $data.options.modifiers,
						avoid_line = data.avoid_line && ignore_break && (svgDataOptions.configs ? svgDataOptions.configs.avoidLine != false : true) && !avoidOverlap ,
						arc = data.elbow_arc && ignore_break,
						adjustPoints = ignore_break && $data.adjustPoints;

						if (data.textbox_overlap_check && !data.textBoxRanges) {
							data.textBoxLength = 0;
							$L(data.wrapperElement.parentElement).children('lyte-textbox').each(function (index, item) {
								var $txtBox = $L(item);
								if (!$txtBox.hasClass('lyteConnectHiddenElem')) {
									$txtBox.data('connector').data().isGoingToUpdate || addTextboxRanges($txtBox, data);
									data.textBoxLength++;
								}
							});
						}

						path = $L.elbow( svg, start, end, data, avoid_line && arc, avoidOverlap );
			...
			if( text_box ){
				...
				if (type == "curvyLine" || type == "linear_curve") {
				...
				} else if(type == 'advanced_curve' || type == 'curve'){
					var elem = paths[0],
						mid_co_ord = elem.getPointAtLength(elem.getTotalLength() / 2),
						mid_left = (ref_x - scroll.left) + mid_co_ord.x,
						mid_top = (ref_y - scroll.top) + mid_co_ord.y;
					...
					var pos =  $L.elbow.textbox.call(_this, $data.absolute_points, data, text_box );
					$text.css({
						left : pos.x,
						top : pos.y
					}).data( 'position', pos );

					if (data.textbox_overlap_check) {
						if ((data.textBoxRanges ? data.textBoxRanges.length + 1 : 1) == data.textBoxLength) {
							delete data.textBoxRanges;
							delete data.textBoxLength;
						} else {
							$data.isGoingToUpdate && addTextboxRanges($text, data);
						}
						delete $data.isGoingToUpdate;
					}
				}
				...
				$L.elbow.textbox && $L.elbow.textbox.call(_this, $data.absolute_points, data, void 0 );
			}
		...
		function addTextboxRanges($txtBox, data) {
			var txtBcr = $txtBox[0].getBoundingClientRect(),
				scale = data.getScale(),
				width = txtBcr.width / scale,
				height = txtBcr.height / scale;

			data.textBoxRanges = data.textBoxRanges || [];

			data.textBoxRanges.push({
				dimension: { width: width, height: height },
				position: { left: parseFloat($txtBox.css('left')) - width / 2, top: parseFloat($txtBox.css('top')) - height / 2 }
			});
		}
		...
		function mousemove( evt ){
			...
			if (!$L(data.element).hasClass('lyteConnectAnywhereAnchor') && data.element && connection_data.connectShortest) {
				var connectItem = data.element.closest(connection_data.module),
					shapeDim = data.shapeDim,
					result = find_position( connectItem.getBoundingClientRect(), { left: evt.clientX, top: evt.clientY, width: 2, height: 2 }, { target_position: { x: 0.5, y: 0.5 } }, undefined, connectItem, connection_data ).src_position;

				if (shapeDim.__x != result.x || shapeDim.__y != result.y) {
					var angle = 180;
					data.con_x = shapeDim.left + (shapeDim.__x = result.x) * shapeDim.width;
					data.con_y = shapeDim.top + (shapeDim.__y = result.y) * shapeDim.height;
			...
			connection_data.onConnectionDrag && connection_data.onConnectionDrag(data.element, data.tempElement, evt, elem);
		...
		function calc_offset( elem, ns, scale, data = {} ){
			var off_parent = data.sequentialFlow ? data.wrapperElement.closest('.lyteConnectWrapper') : elem.offsetParent;
		...
		function mousedown( evt ){
			...
			if( close_com && is_reselect ){
				...
				if( is_anchor ){
					...
					__parent = close_module,
					par_bcr = __parent.getBoundingClientRect(),
					is_not_left;

					if (isNaN(__x) || isNaN(__y)) {
						__x = ((bcr.x + (bcr.width / 2)) - par_bcr.x) / __parent.offsetWidth;
						__y = ((bcr.y + (bcr.height / 2)) - par_bcr.y) / __parent.offsetHeight;
					}

					is_not_left = __x != 0 && __x != 1;

					shapeDim = { width: (par_bcr.width / __scale), height: (par_bcr.height / __scale) };

					near = calc_offset( __parent, 'left', __scale ) + scroll.left + __x * shapeDim.width;
					_height = calc_offset( __parent, 'top', __scale ) + scroll.top + __y * shapeDim.height;
					...
					if (data.connectShortest) {
						shapeDim = {
							left: near - __x * shapeDim.width,
							top: _height - __y * shapeDim.height,
		...
		function adjust_bcr( _module, elem, bcr, scroll, scroll_elem, form_module, __scale, data ){
			...
			if( _module == elem){
				elem = {
					...
					offsetTop : 0,
					offsetWidth : elem.offsetWidth,
					offsetHeight : elem.offsetHeight * (elem.closest('.collapsingGroup') ? getComputedStyle(elem.closest('.collapsingGroup')).transform.split('(')[1].split(')')[0].split(',')[3] : 1)
				};
			}
			...
			__fn = function( ns, __elem ){
				...
				var parent = __elem.offsetParent,
					border = parent instanceof HTMLElement ? parseFloat(getComputedStyle(parent)['border' + ns + 'Width']) : 0;
				return __fn( ns, parent ) + __elem[ 'offset' + ns ] + border;
			},
			obj = {
				width : elem.offsetWidth,
				height : elem.offsetHeight,
				left : calc_offset( _module, 'left', __scale, data ) + group_off.left + __fn( 'Left', elem ) + scroll.left - ( scroll_elem ? scroll_elem.scrollLeft : 0 ),
				top : calc_offset( _module, 'top', __scale, data ) + group_off.top + __fn( 'Top', elem ) + scroll.top - ( scroll_elem ? scroll_elem.scrollTop : 0 ),
				scroll : scroll
			};
		...
		function adjust_position( obj, bcr, element, data, __elem, other_elem ){
			...
			_module = data.sequentialFlow ? element : element.closest( __module + ",lyte-textbox" ),
			x = obj.x,
			y = obj.y,
			...
			isXpx = typeof x == "string" && x.indexOf( "px" ) != -1,
			isYpx = typeof y == "string" && y.indexOf( "px" ) != -1;
			...
			var adjWidth = isXpx ? parseFloat(x) : x * bcr.width,
			adjHeight = isYpx ? parseFloat(y) : y * bcr.height,
			_left = bcr.left,
			_top = bcr.top,
			fn = function( name ){
				if( avoid_module ){
			...
			ret_obj.x = adjWidth + _left;
			ret_obj.y = adjHeight +_top;
			ret_obj.pos_x = adjWidth + ( avoid_module ? ( _left - fn( 'left' ) ) : 0 );
			ret_obj.pos_y = adjHeight + ( avoid_module ? ( _top - fn( 'top' ) ) : 0 );
		...
		function find_position( bcr1, bcr2, options, target_elem, src_elem, data ){
			...
			if( target_elem && /lyte-textbox/i.test( target_elem.tagName ) && !Object.keys( target_position ).length ){
				...
				if (!src || !target) {
					(src ? [[src_x, src_y]] : src_elem.getData('ltPropDefaultAnchors')).forEach(function (_anchor) {
						var anchor = Array.isArray(_anchor) ? _anchor : _anchor.point,
							srcBcr = { value: anchor, left: bcr1.left + (bcr1.width * anchor[0]), top: bcr1.top + (bcr1.height * anchor[1]) };
						(target ? [[target_x, target_y]] : target_elem.getData('ltPropDefaultAnchors')).forEach(function (_anchor) {
							var anchor = Array.isArray(_anchor) ? _anchor : _anchor.point,
								targetBcr = { value: anchor, left: bcr2.left + (bcr2.width * anchor[0]), top: bcr2.top + (bcr2.height * anchor[1]) },
		...
		function update_individual_connector( item ){
			...
			this.data(connection_data_str).textbox_overlap_check && (item.data().isGoingToUpdate = true);
		...
		function updateConnectionElement(data, connectionElem) {
			var getElem = function (param) {
				...
				srcElem = getElem(data.src),
				trgElem = getElem(data.target),
				...
				setVal = function (elem, elemType, _class, itemClass) {
					if (elem && connElem) {
						var prevElem = conData[elemType],
							oldQuery = conData[elemType + '_query'];
						conData['active_' + elemType] = elem;
						conData[elemType + '_query'] = data[elemType];
						...
						get_connections(prevElem)[elemType].length || prevElem.removeClass(_class);
						retObj[elemType] = {old : oldQuery, new : data[elemType]};
						updated = true;
					}
				};
		...
		*/
		...
		function merge_modifiers(original, copy) {
			...
			var first = original[0],
				firstDir = first.x == original[1].x ? 'y' : 'x',
				secondDir = firstDir == 'y' ? 'x' : 'y',
				changed,
				last = $L.extend({}, original[original.length - 1]);

			copy.forEach(function (item, index) {
				var start = index + 1,
					prop = index % 2 == 0 ? firstDir : secondDir;

				if (start < original.length - 1) {
					if (JSON.stringify(item).indexOf('%') != -1) {
						var match = JSON.stringify(item).match(/(?:(\d+)\s*%\s*([-+])\s*(\d+))/);
						if (match) {
							var percent = parseInt(match[1]),
								second = parseInt(match[2] + 1 * match[3]);

							item = Math.abs(first[prop] - original[original.length - 1][prop]) * percent / 100 + second;
						}
					}
					original[start][prop] = original[start + 1][prop] = first[prop] + item;
					( start + 1 == original.length - 1 ) && original.push($L.extend({}, last));
					changed = true;
				}
			});
			...
			if (changed) {
				...
				for (var i = 1; i < original.length; i++) {
					original.__length += Math.sqrt(Math.pow(original[i].x - original[i - 1].x, 2) + Math.pow(original[i].y - original[i - 1].y, 2));
				}
			};
		}
		...
		function remove_copy_pts( pts ){
			...
			for( var i = 0; i < len - 1; i++ ){
				...
				if (Math.abs(next.x - cur.x) < 0.01 && Math.abs(next.y - cur.y) < 0.01) {
					pts.splice( 1 + i--, 1 );
					len--;
				}
		...
		function splitRangesByTextbox(ranges, data, svg) {
			var scale = data.getScale(),
				txtBox = $L(svg).data('text_box').getBoundingClientRect().width / scale,
				ck = 40 / scale;
			(data.textBoxRanges || []).forEach(function (item) {
				var rngCpy = $L.extend(true, {}, item);
				rngCpy.position.left -= (txtBox / 2) + ck;
				rngCpy.dimension.width += txtBox + ck;
				data.splitIndiv(ranges, rngCpy);
			});
		}
		...
		$L.elbow = function( svg, start, end, data, ignore, avoidOverlap ){
			var $svg = $L( svg ),
			...
			customAngle = options.angle,
			start_angle = (customAngle && customAngle.start != undefined ) ? customAngle.start : find_angle( start ),
			end_angle = (customAngle && customAngle.end != undefined ) ? customAngle.end : find_angle( end ),
			radius = data.connector_radius,
			allow_break = data.check_break && !data.ignore_break && (options.configs ? options.configs.checkLineBreak != false : true),
			...
			if( existing_point && existing_point.length ){
				...
				if( allow_break ){
					...
					data.textbox_overlap_check && splitRangesByTextbox(ranges, data, svg);
					delete $svg.data().adjustPoints;
					modifier = find_common_modifier( start_angle, end_angle, start, end, type, ck )
					if(!avoidOverlap && (!data.elbowAvoidance || checkValidModifier( modifier, start, end, data, ck ))){
...
;( function(){
	...
	function find_position( points, textbox ){
		var txtOptions = $L(textbox).data('connector').data('options').textBox,
		customPoint = txtOptions.segmentIndex,
		index = Math.floor( points.length / 2 ),
		pos = {},
		pt1 = points[ customPoint ? customPoint - 1 : index ], 
        pt2 = points[ customPoint ? customPoint : index - 1 ],
		modifiers = txtOptions.modifiers;

		if (modifiers) {
			var modifiersfit, inc, enforceMin = modifiers[1],
				modifier = modifiers[0];
			pt1 = points[0];
			pt2 = points[1];

			modifiers = parseFloat(modifier) * ((typeof modifier == 'string' && modifier.includes('%')) ? points.__length / 100 : 1);

			if (pt1.x == pt2.x) {
				var lengthDif = Math.abs(pt1.y - pt2.y),
					halfLen = lengthDif / 2;
				if(enforceMin && modifiers > halfLen){
					modifiers = halfLen;
				}
				modifiersfit = lengthDif > modifiers;
				inc = pt1.y < pt2.y;
				pos.x = (pt1.x + pt2.x) / 2;
				pos.y = modifiersfit ? pt1.y + (inc ? 1 : -1) * modifiers : (pt1.y + pt2.y) / 2;
			} else {
				var lengthDif = Math.abs(pt1.x - pt2.x),
					halfLen = lengthDif / 2;
				if(enforceMin && modifiers > halfLen){
					modifiers = halfLen;
				}
				...
				inc = pt1.x < pt2.x;
				pos.y = (pt1.y + pt2.y) / 2;
				pos.x = modifiersfit ? pt1.x + (inc ? 1 : -1) * modifiers : (pt1.x + pt2.x) / 2;
			}
 
			pos.lineIndex = index - 1;
		}else if( customPoint || points.length % 2 == 0 ){
           pos.x = ( pt1.x + pt2.x ) / 2;
           pos.y = ( pt1.y + pt2.y ) / 2;
	...
	function check_avoid( exp_pos, points, data, textbox, frm_self ){ 
		...
		var customSegment = $L(textbox).data('connector').data('options').textBox.segmentIndex,
		alterPoints = customSegment ? points.slice(customSegment - 1, customSegment + 1) : points,
		bcr_obj = textbox.__bcr,
		bcr = bcr_obj.bcr,
		dimension = bcr_obj.dimension,
		ranges = create_range_frm_pt( dimension, alterPoints ),
		...
		checking.forEach( function( item ){
			...
			if( bcr == item.bcr || isNaN(item.position.left) || isNaN(item.position.top) || (data.textbox_overlap_check && item.item && $L(item.item).data('connector').data().isGoingToUpdate) ){
				return;
			}
	...
	$L.elbow.textbox = function( points, data, textbox, type, frm_self ){
		if( type == "curvyLine"){
			...
			if( textbox ){
				var pos = find_position( points, textbox ),
				textOverlap = false,
