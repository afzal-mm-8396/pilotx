	...
	screenGrab.prototype = {
		...
		convertUrls : function( dom, font_family, ignore ){
			...
			try{
				...
				is_not_ignore_image = !__this.options.ignoreImage;
			             ...
			             if( tagName == 'svg' ){
			             	...
			             	promiseArr.push( new _window.Promise( function( res ){
		             	 	...
		             	 	promiseArr.push( __this.constructHTML( item ).then( function( lent, stringArr, string ){
								...
								void_rgx = /^(area|base|br|col|embed|hr|img|input|link|meta|param |source|track|wbr)$/i;
								...
								if( void_rgx.test( tagName ) ){
									_str += '/>';
								} else {
		             	 			_str +=  ( '>' + ( __options.maintainScroll ? __this.getScrollWrapper( string, sleft, stop ) : string ) + '</' + tagName + '>' );
								}
