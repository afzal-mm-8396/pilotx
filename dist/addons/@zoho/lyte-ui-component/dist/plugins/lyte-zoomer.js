(function () {
    $L.zoomer = function (params) {
                ...
                body.mouseMove = function(e){
                    ...
                    function renderContainer() {
                        ...
                        if(eventTarget.shadowRoot) {
                           eventTarget = eventTarget.shadowRoot.elementsFromPoint(e.clientX, e.clientY)[0];
                        }
                        if (check) {
                            foundNodeObj = $L(eventTarget).nodeFinder();
                        } else {
                            body.resetContainer();
                            return
                        }
                    ...
                    // }
                     function checkIfOverText(e) {
                        ...
                        if (target.shadowRoot) {
                            target = target.shadowRoot.elementsFromPoint(e.clientX, e.clientY)[0];
                        }
                        range.selectNode(target);
                        
                        let rects = range.getClientRects(), suitableRect;
                        if (rects.length > 150) {
                            return [false, target.getBoundingClientRect()]
                        } else if (rects.length == 1) {
                            return [true, target.getBoundingClientRect()]
                        }
        ...
        } else if (params === "destroy" || params === "Destroy") { 
