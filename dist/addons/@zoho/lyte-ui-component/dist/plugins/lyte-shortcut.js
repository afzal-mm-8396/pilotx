...
; (function (window, document) {
  ...
  var _registeredKeyCodeShortcuts = {};
  var _timeoutID;
  var _allPressed = []
  var _allCodePressed = []
  var _allKeyCodePressed = []
  ...
  var codeToKeyCode = {
    ...
    // Extended F-keys (supported on some keyboards)
    "f13": "124",
    "f14": "125",
    "f15": "126",
    "f16": "127",
  ...
  /**
   * Convert keys to keyCode equivalent
   */
  function convertKeysToKeyCode(shortcutString) {
    if (!shortcutString) return "";

    // detect joiner from input
    var joiner = shortcutString.indexOf("+") !== -1 ? "+" : " ";

    var parts = shortcutString.trim().split(/[+\s]+/);
    var result = [];

    // modifiers that should remain unchanged
    var modifiers = {
      meta: true,
      shift: true,
      alt: true,
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      // If it's a modifier, keep as-is
      if (modifiers[part]) {
        result.push(part);
        continue;
      }

      // convert key → keyCode if mapping exists
      if (codeToKeyCode[part]) {
        result.push(codeToKeyCode[part]);
      } else {
        result.push(part);
      }
    }
  ...
  /**
   * Convert keyCodes (strings) to keys
   * Example: "control+65+shift" → "control+keya+shift"
   */
  function convertKeyCodeToKeys(shortcutString, pressTogether) {
    if (!shortcutString) {
      return "";
    }
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      // Keep modifiers unchanged
      if (modifiers[part]) {
        result.push(part);
        continue;
      }

      // Convert keyCode string → key
      let matchedKey = null;
      for (const key in codeToKeyCode) {
        if (codeToKeyCode[key] === part) {
          matchedKey = key;
          break;
        }
      }

      result.push(matchedKey ?? part);
    }
    if (pressTogether) {
      return result.join("+");
    }
  ...
  function getAllRegisteredShortcutKeys() {
    ...
    Object.assign(allRegisteredShortcutKeys, _registeredKeyCodeShortcuts)
  ...
  function increaseCharPointerKeyCode(currentPressedCharacter) {
    var allShortcutKeys = _registeredKeyCodeShortcuts;
    for (var shortcutKey in allShortcutKeys) {
      var keyArray = _splitArray(shortcutKey)
      var progress
      codeShortcutCounter[shortcutKey] = progress = codeShortcutCounter[shortcutKey] ? codeShortcutCounter[shortcutKey] : 0
          ...
          if (item.modifier.length <= 0) {
            ...
            if (item.pressTogether) {
              if (!allModifier.includes('meta')) {
                ...
                else if (_keyShortcutCounter[keys] === _splitArray(keys).length && item.modifier.sort().join('+') === allModifier.sort().join('+') && _allPressed.join("+").includes(keys)) {
                  item._invokedKey = item.modifier.sort().length !== 0 ? item.modifier.sort().join('+') + "+" + keys : keys
                  allValues.push(item)
                  foundMatches[keys] = true;
                }
                ...
                else if (_keyShortcutCounter[keys] === _splitArray(keys).length && item.modifier.sort().join('+') === allModifier.sort().join('+') && _allPressed.join("+").includes(keys) && allModifier.length === item.modifier.length) {
                  item._invokedKey = item.modifier.sort().length !== 0 ? item.modifier.sort().join('+') + "+" + keys : keys
                  allValues.push(item)
                  foundMatches[keys] = true;
                }
  ...
  function getMatchedElementsKeyCode(event) {
    var allValues = [], foundMatches = {};
    var allModifier = _getModifiers(event)
    // _invokedKey writes in the global object but it won't cause problems i guess
    var allRegisteredShortcutKeys = _registeredKeyCodeShortcuts;
    for (var keys in codeShortcutCounter) {
      if (allRegisteredShortcutKeys[keys]) {
        for (var i = 0; i < allRegisteredShortcutKeys[keys].length; i++) {
          var item = allRegisteredShortcutKeys[keys][i]
          if (item.modifier.length <= 0) {
            if (item.pressTogether) {
              if (codeShortcutCounter[keys] === _splitArray(keys).length && _allKeyCodePressed.join("+").includes(keys) && allModifier.length === 0) {
                item._invokedKey = keys
                allValues.push(item)
                foundMatches[keys] = true;
              }
              ...
              if (_splitArray(keys).length !== 1 && codeShortcutCounter[keys] === _splitArray(keys).length && _allKeyCodePressed.length === _splitArray(keys).length && _allKeyCodePressed.join(" ").includes(keys) && allModifier.length === 0) {
                item._invokedKey = keys
                allValues.push(item)
                foundMatches[keys] = true;
              }
              else if (!event.metaKey && !event.ctrlKey && _splitArray(keys).length === 1 && codeShortcutCounter[keys] === _splitArray(keys).length && _allKeyCodePressed.join(" ").includes(keys) && allModifier.length === 0) {
                item._invokedKey = keys
                allValues.push(item)
                foundMatches[keys] = true;
              }
            ...
            if (item.pressTogether) {
              if (codeShortcutCounter[keys] === _splitArray(keys).length && item.modifier.sort().join('+') === allModifier.sort().join('+') && _allKeyCodePressed.join("+").includes(keys)) {
                item._invokedKey = item.modifier.sort().length !== 0 ? item.modifier.sort().join('+') + "+" + keys : keys
                allValues.push(item)
                foundMatches[keys] = true;
              }
              ...
              if (codeShortcutCounter[keys] === _splitArray(keys).length && _allKeyCodePressed.join(" ").includes(keys) && item.modifier.sort().join('+') === allModifier.sort().join('+')) {
                item._invokedKey = keys
                allValues.push(item)
                foundMatches[keys] = true;
              }
  ...
  function _handleKeyPress(event) {
    ...
    // RegisterKeyCode
    //this should be stringified keyCode
    var keyCode = event.keyCode;
    _allKeyCodePressed.push(keyCode)
    keyCode = keyCode.toString();
    increaseCharPointerKeyCode(keyCode)
    if (Object.keys(codeShortcutCounter).length) {
      var matchedElements = getMatchedElementsKeyCode(event)
      _createGracePeriod()
  ...
  function _processString(keys, callback, options) {
    var isValid;
    var userDefinedKey = keys;
    options.userDefinedKeys = userDefinedKey;
    keys = removeWhiteSpace(keys)
    var useKey = options.useKey
    var useCode = options.useCode
    var useKeyCode = options.useKeyCode;
    if (!useKey && !useCode && !useKeyCode) {
      keys = userToBrowserArrowKeys(keys)
    }
    ...
    if (useCode) {
      ...
      var convertedKeys = convertKeysToKeyCode(keys);
      shortcut.pushKeyCode({
        newKey: convertedKeys,
        keys: keys,
        oldKey: undefined,
        value: callback,
        options: options
  ...
  function userToBrowserKeys(keys) {
    ...
    //alt will be replaced by control
    if (keys.search("/option/i")) {
      keys = keys.replaceAll("option", "alt")
    }
    ...
    if (keys.search("/esc/i")) {
      keys = keys.replace(/\besc\b/g, "escape");
    }
    return keys;
  }
  function userToBrowserArrowKeys(keys) {
    //arrow keys will be replaced
    if (keys.search("/left/i")) {
      keys = keys.replaceAll(/\bleft\b/g, "arrowleft")
    }
    if (keys.search("/right/i")) {
      keys = keys.replaceAll(/\bright\b/g, "arrowright")
    }
    if (keys.search("/up/i")) {
      keys = keys.replaceAll(/\bup\b/g, "arrowup")
    }
    if (keys.search("/down/i")) {
      keys = keys.replaceAll(/\bdown\b/g, "arrowdown")
    }
    return keys;
  }
 ...
 */
  ...
  function replaceShortcut(entry, shortcutCache, index) {
    ...
    var returnedObject = segregateKeys(entry.newKey)
    var wait, preventDefault, useCode, excludeElements, validate, hovercard, useKeyCode, userDefinedKeys;
    if (entry.options) {
      ...
      useKeyCode = entry.options.useKeyCode ? entry.options.useKeyCode : false
      ...
      userDefinedKeys = entry.options.userDefinedKeys ? entry.options.userDefinedKeys : undefined;
    }
    else {
      wait = 0
    ...
    if (hovercard) {
      var keys = entry.options.userDefinedKeys;
   ...
   */
  _lyteUiUtils.addGlobalEventListener('mousemove', function (event) {
  ...
  Object.defineProperty(shortcut, "pushKeyCode", {
    writable: false,
    value: function (entry) {
      removeFromCache(entry)
      // undefined or empty string cases or empty object
      ...
      if (entry.useKeyCode === undefined) {
        entry.useKeyCode = true
      }
      storeToCache(entry, _registeredKeyCodeShortcuts)
    }
  })
  /**
  ...
  Object.defineProperty(shortcut, "hasRegisteredShortcut", {
    ...
    value: function (keys) {
      ...
      var pressTogether = processedKeys.pressTogether;
      var allRegisteredShortcutKeys = getAllRegisteredShortcutKeys();
      ...
      for (var keys in allRegisteredShortcutKeys) {
        var convertedKeys = convertKeyCodeToKeys(keys, pressTogether);
        if ((convertedKeys === processedKeys.newKey) || (keys === processedKeys.newKey)) {
          for (var i = 0; i < allRegisteredShortcutKeys[keys].length; i++) {
            var item = allRegisteredShortcutKeys[keys][i]
  ...
  Object.defineProperty(shortcut, "registerKeyCode", {
    writable: false,
    value: function (keys, callback, options) {
      if (options) {
        if (options.type) {
    ...
    value: function (keys) {
      if (typeof keys === 'object') {
        for (var i = 0; i < keys.length; i++) {
          ...
          _key = userToBrowserArrowKeys(_key)
  ...
  Object.defineProperty(shortcut, "unregisterKeyCode", {
    writable: false,
    value: function (keys) {
      if (typeof keys === 'object') {
        for (var i = 0; i < keys.length; i++) {
          var _key = keys[i]
          _key = convertKeysToKeyCode(_key);
          _key = removeWhiteSpace(_key)
          _removeKey(_key, _registeredKeyCodeShortcuts);
        }
      }
      else {
        keys = removeWhiteSpace(keys)
        keys = convertKeysToKeyCode(keys);
        _removeKey(keys, _registeredKeyCodeShortcuts);
      }
    }
  })
  /**
  ...
  Object.defineProperty(shortcut, "unregisterAll", {
    ...
    value: function () {
      ...
      for (var key in _registeredKeyCodeShortcuts) {
        delete _registeredKeyCodeShortcuts[key]
      }
      clearTimeout(_timeoutID)
      _allKeysPressed = []
      _allKeyCodePressed = []
