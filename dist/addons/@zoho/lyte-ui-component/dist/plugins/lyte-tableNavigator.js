(function () {
  if ($L) {
    $L.prototype.betaTableNavigator = function (params) {
      ...
      function getTableData() {
        ...
        colLimit = tableData[0] ? tableData[0].length : 0;
      }
      ...
      var row = 0, col = 0;
      var focusableElems = [], focusableElemIndex = 0;
      var inCellNavigation = false;
      ...
      function focusCell() {
        ...
        if (tableData[row] && tableData[row][col]) {
          tableData[row][col].focus();
          focusChange();
          cellChange();
        }
      }

      function focusInCell(direction) {
        if (!tableData[row] || !tableData[row][col]) {
          return;
        }
        getCellChildren(tableData[row][col]);
      ...
      function moveToCell(newRow, newCol, direction) {
      ...
      function moveLeft() {
        if (inCellNavigation) {
          if (focusableElems.length > 0 && focusableElemIndex > 0) {
            focusableElemIndex--;
            focusableElems[focusableElemIndex].focus();
            focusChange();
            return;
          }
          moveToCell(row, col - 1, "left");
          return;
        }
        moveToCell(row, col - 1, "left");
      }
      ...
      function moveRight() {
        if (inCellNavigation) {
          if (focusableElems.length > 0 && focusableElemIndex < focusableElems.length - 1) {
            focusableElemIndex++;
            focusableElems[focusableElemIndex].focus();
            focusChange();
            return;
          }
          moveToCell(row, col + 1, "right");
          return;
        }
        moveToCell(row, col + 1, "right");
      }
      ...
      function spaceKeyPressed() {
        inCellNavigation = true;
        focusInCell("right");
      }

      function escapeKeyPressed() {
        inCellNavigation = false;
        focusCell();
      }

      function tabKeyPressed(e) {
        for (let _row = 0; _row < tableData.length; _row++) {
          for (let _column = 0; _column < tableData[_row].length; _column++) {
            if (tableData[_row][_column] === e.target) {
              row = _row;
              col = _column;
            }
          }
      ...
      if (!table.navBetaTableKeyDown) {
        table.navBetaTableKeyDown = function (e) {
          getTableData();
      ...
      if (!table.navBetaTableMouseClick) {
        table.navBetaTableMouseClick = function (e) {
          getTableData();
          ...
          try {
            ...
            getCellChildren(tableData[row][col]);
            if (focusableElems.length > 0 && focusableElems.indexOf(e.target) >= 0) {
              inCellNavigation = true;
              focusableElemIndex = focusableElems.indexOf(e.target);
            } else {
              inCellNavigation = false;
            }
          } catch (err) {
      ...
      if (params !== "destroy") {
        table.addEventListener("keydown", table.navBetaTableKeyDown);
        table.addEventListener("click", table.navBetaTableMouseClick);
      } else if (params === "destroy") {
        table.removeEventListener("keydown", table.navBetaTableKeyDown);
        table.removeEventListener("click", table.navBetaTableMouseClick);
        tableData.forEach(function (element) {
          element.forEach(function (elem) {
            elem.tabIndex = -1;
          });
        });
        focusableElems = tableRows = rowLimit = colLimit = null;
        focusableElemIndex = inCellNavigation = null;
