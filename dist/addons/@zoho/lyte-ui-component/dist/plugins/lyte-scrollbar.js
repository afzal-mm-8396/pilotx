/**
 * Known issue - to fix
 * when appendTo is container and scrollable element is having border extra scroll is happening
 */
...
;( function( cb ){
    ...
    if( $L ) {
        ...
        function appendDiv( className, obj, dir, is_parent ) {
            ...
            if( obj.aria ){
                $L( div ).attr({
                    ...
                    "aria-live" : "polite",
                    tabindex : obj.scrollbarTabindex || Math.max( this.tabIndex, obj.tabIndex || 0 )
        ...
        function mouseenter( evt, frm_reset ) {
            ...
            obj.vertbcr = this._vertDiv ?  
                            bcr_cal( this._vertDiv ) : 
                            {};
            obj.horbcr = this._horiDiv ?  
                            bcr_cal( this._horiDiv ) : 
                            {};
        ...
        function mouseleave( evt ){
            ...
            var bars = $L( this ).children( '.lyteScrollContainer' ), 
            scrlDiv = this._scrolldiv || this,
            __scrolldata = scrlDiv._scrollData,
            is_native_scroll = __scrolldata.nativeScroll;
            ...
            if( bars.length ) {
                ...
                if( evt.type ){
                    ...
                    delete scrlDiv._prevPosY; 
                    delete scrlDiv._mouseleave;
                    delete scrlDiv._prevPosX; 
                    delete scrlDiv._wheelEvt;
                }
        ...
        function outerDivClick( evt ) {
            ...
            var elem = this._scrolldiv;

            if( !elem._scrollData ){
                return;
            }

            var inn = this.children[ 0 ], 
            outBcr = bcr_cal( this ), 
            inBcr = bcr_cal( inn ),
            obj = { 
                scrollLeft : Math.round( elem.scrollLeft ), 
                scrollTop : Math.round( elem.scrollTop ), 
            ...
            var scramt = evt.type != 'click' ? 
                        ( evt[ cY ] - ( this.prev || evt[ cY ] ) ) : 
                        ( evt[ cY ] - ( inBcr[ top1 ] + inBcr[ hgt ] / 2 ) ), 
        ...
        function fitForScroll( mode, obj, frm_reset ) {
            ...
            sW = 'scrollHeight', 
            wd = 'height', 
            elem = mode ? this._vertDiv : this._horiDiv,
            scrolldata = this._scrollData,
        ...
        function scroll( evt ) {
            var a, 
            b, 
            issafIE = isSaf.isIE11Lyte || isSaf.safari || isSaf.isIpad, 
            isIe = isSaf.isIE11Lyte,
            scroll_data = this._scrollData,
            is_native_scroll = scroll_data.nativeScroll;

            if( is_native_scroll && scroll_data.showOn == 'scroll' ){
                if( !this._enableScroll ){
                    mouseenter.call( this, true );
                }
                clearTimeout( this._scrollplugin )  
                this._scrollplugin = setTimeout(mouseleave.bind( scroll_data.appendTo == "parent" ? this.parentElement : this, {} ), scroll_data.tOut )
            }
            ...
            if( issafIE && evt && evt.type == 'scroll' && ( this._alive && !evt._byFunc ) ) {
                ...
                if( this.prevScrlLeft != obj.scrollLeft ) {
                    ...
                    if( is_native_scroll ){
                        $L.fastdom.mutate( updatePos.bind( this, false, a , evt, obj ) );
                    } else {
                ...
                if( this.prevScrlTop!= obj.scrollTop ) {
                    ...
                    a = 0;
                    if( is_native_scroll ){
                        $L.fastdom.mutate( updatePos.bind( this, true, b , evt, obj ) );
                    } else {
        ...
        function removeScroll(){
            ...
            for( var i = 0; i < elements.length; i++ ) {
                ...
                elem.classList.remove( 'lyteScrollBar', 'lyteScrollNativeScrollImpl' );
                ...
                clearTimeout( elem._scrollplugin );
                delete elem._scrollplugin;
                delete elem._tchtime;
                ...
                delete elem._wheelObj; 
                delete elem._vertDiv; 
                delete elem._horiDiv;
                delete elem._scrollFun; 
                delete elem._alivetime; 
                delete elem._alive; 
                delete elem._wheelObj;
                delete elem.resetScrollbar; 
                delete wrap._scrolldiv; 
                delete elem._tchtime;
                ...
                delete elem._tabindex; 
                delete elem._wheelObj;
                delete elem._prevPosY; 
                delete elem._mouseleave;
                delete elem._prevPosX; 
                delete elem._wheelEvt;
            }
            return this;
        }
        ...
        lyteDomObj.prototype.removeScroll = $L.prototype.removeScroll = function(){
            // console.warn( 'removeScroll deprecated. Use scroll("destroy") instead' );
            return removeScroll.call( this );
        }

        lyteDomObj.prototype.scroll = $L.prototype.scroll = function( obj ) {
            ...
            obj.nativeScroll = obj.nativeScroll || false;
            ...
            var elements = this,
            is_initial_native_scroll = obj.nativeScroll;
            ...
            for( var i = 0; i < elements.length; i++ ) {
                ...
                if( !id ){
                    elem.id = id = "LyteScrollbar_" + Date.now() + '_' + parseInt( Math.random() * 10000 );
                }

                if( !is_initial_native_scroll && default_values.nativeScroll ){
                    if( wrp.tagName == "LYTE-TABLE" ){
                        var to_set = true;

                        if( wrp.ltProp( 'infiniteScroll' ) ){
                            to_set = false;
                        } else if( wrp.ltProp( 'preventScrollbar' ) || wrp.ltProp( 'stickyTable' ) ){
                            to_set = true;
                        } else {
                            to_set = false;
                        }
                        obj.nativeScroll = to_set;
                    }
                }

                if( obj.appendTo == "container" && wrp.tagName == "LYTE-TABLE" ){
                    /**
                     * For lyte-table forcefully setting appendTo to parent
                     */
                    obj.appendTo = "parent";
                    is_parent = true;
                }   
                ...
                if( !obj.nativeScroll ){
                    ...
                    elem.classList.add( 'lyteScrollNativeScrollImpl' );
                }
        ...
        lyteDomObj.prototype.destroyLyteScroll = $L.prototype.destroyLyteScroll = function(){
            // console.warn( 'destroyLyteScroll deprecated. Use scroll("destroyScroll") instead' );
            return destroy.call( this );  
        }

        lyteDomObj.prototype.resetScrollbar = $L.prototype.resetScrollbar = function(){
