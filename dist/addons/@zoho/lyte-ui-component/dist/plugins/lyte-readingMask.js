        ...
        $L.prototype.readingMask = function (params) {
            ...
            if (!element.maskOver) {
                element.maskOver = function (e) {
                    if (element.mask.classList.contains('lyteReadingMaskTransition')) { 
                        if (element.timerId) { 
                            clearTimeout(element.timerId)
                        }
                        element.timerId = setTimeout(function () {
                            element.mask.classList.remove('lyteReadingMaskTransition')
                        }, 100)
                    }
                    if (element.once) { 
                        element.maskStyle.display = "block";
            ...
            if (!element.maskMove) { 
                element.maskMove = function (e) {
                    element.tabPressed = false;
                    // element.setMaskDimensions();

                    if (params && params.event && element.inital) {
                        element.cursorX = params.event.clientX;
                        element.cursorY = params.event.clientY;
                        element.inital = false;
                    } else if ((!params || !params.event) && element.inital) {
            ...
            if (!element.maskLeave) { 
                element.maskLeave = function (e) {
                    ...
                    if (hideMaskOnLeave) {
                        ...
                        element.mask.classList.add('lyteReadingMaskTransition')
                    }
                }
            }
            // function to set the mask dimensions based on user input or default values
            if (!element.setMaskDimensions) {
                element.setMaskDimensions = function (dimensions) {
                    if (dimensions) {
                        maskHeight = dimensions.height;
                        maskWidth = dimensions.width;
                    }
                    element.maskStyle.height = (typeof maskHeight === "number" ? maskHeight + "px" : maskHeight) || "100px";
                    element.maskStyle.width = (typeof maskWidth === "number" ? maskWidth + "px" : maskWidth) || "500px";
                }
            }
            ...
            if (params !== "destroy") {
                ...
                function setInitialMaskVariables(params){
                        ...
                        if (params && !params.hideFreezeOnLeave) { 
                            hideFreezeOnLeave = params.hideFreezeOnLeave;
                        } else { hideFreezeOnLeave = true; }
                        if (params && !params.hideMaskOnLeave) { 
                            hideMaskOnLeave = params.hideMaskOnLeave;
                        } else { hideMaskOnLeave = true; }
                ...
                createMaskandFreeze();
                setInitialMaskVariables(params);
                setMaskAttributes();
                setFreezeLayerAttributes();
                element.setMaskDimensions();
                
                // on entering the element, the mask and the freeze layers are displayed
                window.addEventListener('mouseover', element.maskOver);
                // moving the mask layer along with the mouse move
                window.addEventListener('mousemove',element.maskMove);
                // on leaving the element, the mask and the freeze layers are disappeared
                $L(document).on('mouseleave',element.maskLeave);
                // listening tab keypress
                $L('body').on('keyup', element.tabKeyUp);
                    initialMaskRender();
            }
                // destroy functionalities to remove all eventlisteners and layers
            else if (params === 'destroy') { 
                window.removeEventListener('mouseover', element.maskOver);
                window.removeEventListener('mousemove', element.maskMove);
                $L(document).off('mouseleave', element.maskLeave);
