...
//for now added simple eventmousedown to all the clonetags of month view's event resize
    //add class checks after cloning tags
    //replace action methods like onSchedulerEventMouseDown of clone tags with new actions

;(function(){
    var intervalIdY, intervalIdX, boundedMouseUp;

    var mouseMoveHandler = function( e ){
        var int = this.getData('ltPropMultiUserView') && this.getData('ltPropSchedulerView')==='day' && !this.getData('ltPropUserBasedView') ? 10 : Math.ceil(this.getData('ltPropTimeLine')/4);
        // checkAndAddCursorClass( this, e, 5 );

        checkAndIncreaseScrollY( this, e, mouseMoveHandler, int );
        checkAndIncreaseScrollX( this, e, mouseMoveHandler, int );
        ...
        var parentTr;
        var newParentTr;
        if( view === 'day' && isMultiUserView && !isUserBased){
            parentTr = resTd;
        }
        else{
        ...
        if( dragDir === 'top' ){
            ...
            if( ((!this.getData('ltPropMultiUserView') && type === 'week') || (type === 'day' /*&& (!this.getData('ltPropMultiUserView') || this.getData('ltPropUserBasedView'))*/)) && (!isAllDayEvent( tagDiv ) )){
                ...
                if(newParentTr === 'next'){
                    ...
                    var count = 1;
                    if( type=='day' && this.getData('ltPropMultiUserView') && !this.getData('ltPropUserBasedView')){
                        count = Math.ceil(Math.abs( e.pageX - window.scrollX - tagDiv.getBoundingClientRect().left ) / this.getData('quadrant')); 
                        count = count > 0 ? count : 1;
                    }
                    newStartDate.add(int*count,'minutes');
                }
                    ...
                    newStartDate.subtract(int*count,'minutes');
                    //corner case
                    ...
                    if( (type == 'day'||type == 'week') && newStartDate.format('DD-MM-YYYY') !== $L.moment(eventObj.start, format).format('DD-MM-YYYY')){
                        newStartDate.set('hours',0);
                        newStartDate.set('minutes',0);
                    }
                }
            }
            else{
                ...
                var timeDiv = newParentTr.querySelector('.scheduler-event-div');
                newStartDate = $L.moment( oldStartDate.format('DD-MM-YYYY') + ' ' + timeDiv.getAttribute('data-time') , "DD-MM-YYYY hh:mm A");
            }
            ...
            if( ((!this.getData('ltPropMultiUserView') && type === 'week') || (type === 'day' /*&& (!this.getData('ltPropMultiUserView') || this.getData('ltPropUserBasedView'))*/)) && (!isAllDayEvent( tagDiv ) )){
                ...
                if(newParentTr === 'next'){
                    var count;
                    if( type=='day' && this.getData('ltPropMultiUserView') && !this.getData('ltPropUserBasedView')){
                        count = Math.ceil(Math.abs( e.pageX - window.scrollX - tagDiv.getBoundingClientRect().right ) / this.getData('quadrant')); 
                        count = count > 0 ? count : 1;
                    }
                    else{
                        count = Math.ceil(Math.abs( e.pageY - window.scrollY - tagDiv.getBoundingClientRect().bottom ) / this.getData('quadrant')); 
                        count = count > 0 ? count : 1;
                    }
                    ...
                    if( (type == 'day'||type == 'week') && newEndDate.format('DD-MM-YYYY') !== $L.moment(eventObj.start, format).format('DD-MM-YYYY')){
                        newEndDate.set('hours',0);
                        newEndDate.set('minutes',0);
                    }
                }
                else if(newParentTr === 'prev'){
                    var count;
                ...
                var timeDiv = newParentTr.querySelector('.scheduler-event-div');
                newEndDate = $L.moment( oldEndDate.format('DD-MM-YYYY') + ' ' + timeDiv.getAttribute('data-time') , "DD-MM-YYYY hh:mm A");
            }
            ...
            if( !elem ){return;}
            var newStartDate = $L.moment(elem.getAttribute('data-date') + ' ' + oldTime , "DD-MM-YYYY hh:mm A");
            var isUpdated = updateEventObj( tagDiv, eventObj, newStartDate, this );
            if( !isUpdated ){
                return;
            }
            ...
            var elem = newParentTr.querySelector('.scheduler-event-div');
            if( !elem ){return;}
            var newEndDate = $L.moment( elem.getAttribute('data-date') + ' ' + oldTime, "DD-MM-YYYY hh:mm A");
            var isUpdated = updateEventObj( tagDiv, eventObj, newEndDate, this );
            if( !isUpdated ){
                return;
            }
        ...
        else if( type === 'day' && isMultiUserView && !isUserBased){
        ...
        if( this.getMethods('onEventResizeMove') ){
            this.executeMethod('onEventResizeMove', e, this, tagDiv, this.genreateActualEvent(tagDiv.getData('ltPropEvent')), eventTagHeight);
        }
    ...
    var mouseUpHandler = function( e ){
        ...
        if( isResizing ){
            if( this.getMethods('onEventResizeEnd') ){
                this.executeMethod('onEventResizeEnd', e, this, eventTag, this.genreateActualEvent(eventTag.getData('ltPropEvent')));
            }
        }
    ...
    $L.schedulerEventResize = function( event, _this, eventObj, eventTag ){
        ...
        if( type === 'month' || ( type === 'week' && (isAllDayEvent( eventTag )||_this.getData('ltPropMultiUserView')) ) ){
            ...
            if( type === 'day' && _this.getData('ltPropMultiUserView') && !_this.getData('ltPropUserBasedView')){
                if( Math.abs(startX - right ) < diff ){
        ...
        if( type === 'month' || ( type === 'week' && (isAllDayEvent( eventTag )||_this.getData('ltPropMultiUserView')) ) ){
            ...
            if( type === 'day' && _this.getData('ltPropMultiUserView') && !_this.getData('ltPropUserBasedView')){
                ...
                firstTrBound = firstTr.getBoundingClientRect();
                _this.setData('breakpoint',firstTrBound.width/2);
                _this.setData('quadrant',firstTrBound.width/6);
            }
            else{
    ...
    function checkAndIncreaseScrollY( _this, event, mouseMoveHandler, int ){
        if( event.clientY ){
            _this.setData('tempResizeClientY',event.clientY);
        }

        if( !_this.getData('isResizing') || intervalIdY ){
            return;
        }
        ...
        var shouldScrollTop = event.clientY <= wrapperBound.top + checkDiff;
        var shouldScrollBottom = event.clientY >= wrapperBound.bottom - checkDiff;

        if( !shouldScrollTop && !shouldScrollBottom ){
            return;
        }

        var dir = shouldScrollBottom ? 1 : -1;

        intervalIdY = setInterval(function(){    //need to change this to requestAnimationFrame method!
            ...
            var tempY = _this.getData('tempResizeClientY');
            var stopScroll = !_this.getData('isResizing') || !intervalIdY;
             if( dir === 1 ){
                stopScroll = stopScroll || tempY < wrapperBound.bottom - checkDiff;
            }
            else{
                stopScroll = stopScroll || tempY > wrapperBound.top + checkDiff;
            }
            ...
            wrapper.scrollTop += scrollDiff * dir;
            ...
            _this.setData('curPgY',_this.getData('curPgY')-(dir * int)); 
            mouseMoveHandler.bind( _this )( eveSim );
            
        }, intervalDuration);
    }

    function checkAndIncreaseScrollX( _this, event, mouseMoveHandler, int ){
        if( !(_this.getData('ltPropSchedulerView') === 'day') &&_this.getData('ltPropMultiUserView')&& !_this.getData('ltPropUserBasedView') ){
            return;
        }

        if( event.clientX ){
            _this.setData('tempResizeClientX',event.clientX);
        }

        if( !_this.getData('isResizing') || intervalIdX ){
            return;
        }
        ...
        var checkDiff = 20;
        var intervalDuration = 200;
        var shouldScrollRight = event.clientX >= wrapperBound.right - checkDiff;
        var shouldScrollLeft = event.clientX <= wrapperBound.left + checkDiff;

        if( !shouldScrollLeft && !shouldScrollRight ){
            return;
        }

        var dir = shouldScrollRight ? 1 : -1;
        ...
        intervalIdX = setInterval(function(){
            ...
            var tempX = _this.getData('tempResizeClientX');
            var stopScroll = !_this.getData('isResizing') || !intervalIdX;
            if( dir === 1 ){
                stopScroll = stopScroll || tempX < wrapperBound.right - checkDiff;
            }
            else{
                stopScroll = stopScroll || tempX > wrapperBound.left + checkDiff;
            }
            ...
            wrapper.scrollLeft += scrollDiff * dir;
            ...
            _this.setData('curPgX', _this.getData('curPgX') - (dir * int));
            mouseMoveHandler.bind( _this )( eveSim );
    ...
    function updateEventObj( eventTag, eventObj, newDate, _this ){
        ...
        var isMultiUserView = _this.getData( 'ltPropMultiUserView' );
        var isUserBased = _this.getData('ltPropUserBasedView');
        var type = _this.getData( 'ltPropSchedulerView' );
        var interval = (isMultiUserView && type==='day' && !isUserBased) ?  60 : _this.getData('ltPropTimeLine');

        if( !(eventObj && key && newDate) ){
            return false;
        }
        if( key === 'top' ){         
            if( type === 'day' && isMultiUserView && !isUserBased ){
    ...
    function getTrOfCurDate( e, eventObj, retParTr, _this, tagDiv ){
        ...
        if( dragDir === 'top' || dragDir === 'bottom' ){
            ...
            var timeFormat = currDate.format('hh') + ':' + res + currDate.format(' A');

            if( type === 'day' && isMultiUserView && !isUserBased){
                currTr = _this.$node.querySelector('td>div[data-time="' + timeFormat + '"]');
            }
            else{
                currTr = _this.$node.querySelector('tr[data-time="' + timeFormat + '"]');
            }
    ...
    function getNextOrPrevToBreakPoint( e, eventObj, parentTr, _this ){
        ...
        if( dragDir === 'top' ){
            ...
            var newDate = $L.moment( div.getAttribute('data-date') + ' ' + oldTime, 'DD-MM-YYYY hh:mm A');
            var diff = currDate.fromNow( newDate );
            // if( currDate.format("hh:mm A") === parentTr.getAttribute('data-time')){
            //     return false;
            // }
