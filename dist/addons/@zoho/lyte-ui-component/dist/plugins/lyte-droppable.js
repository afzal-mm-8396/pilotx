...
;(function( window ) {
	...
	if($L){	
		...
		$L.prototype.managerDD = {
			...
			_checkIntersects : function(event,draggable,droppable,toleranceMode){
				...
				if(draggable.ownerDocument.defaultView != droppable.ownerDocument.defaultView){
					...
					var currentWin = event.target.ownerDocument ? (event.target.ownerDocument.defaultView || event.target.ownerDocument.parentWindow) : null;
					while(currentWin && currentWin.frameElement){
						var frameElementOffset = currentWin.frameElement.getBoundingClientRect();
			...
			_getOverflowParent : function(droppables){
				...
				var Parentoverflow = '/hidden|auto|scroll|clip/g';
				while(( !css.overflow.match(Parentoverflow) ) && parent.tagName !== 'BODY'){
					...
					if(!data.disabled  && !(managerDD._isRelated(draggable, droppables[i])) && managerDD._checkAcceptable(draggable,data.accept) && managerDD._isNotRejected(draggable,data.reject)){
						...
						if(managerDD._checkIntersects(event,draggable,droppables[i],data.tolerance) && managerDD._checkIsVisible(droppables[i],draggable)){
							if(!data.entered){
								...
								if(prevDroppable && prevDroppable != droppables[i]){
									...
									if(prevDroppableData.onLeave){
										prevDroppableData.onLeave(draggable._draggableData._element,prevDroppable,draggable._draggableData._origin);
									}
								...
								if(data.onEnter){
									data.onEnter(draggable._draggableData._element,droppables[i],draggable._draggableData._origin);
								}							if(data.onDropEnter){
								data.onDropEnter(draggable._draggableData._element,droppables[i],draggable._draggableData._origin);
							}								data.entered = true;
								draggable._draggableData.currDroppable = droppables[i];
							}
							if(data.onDrag){
								if(event.type == "mousemove" || (draggable._draggableData.aria && event.type == 'keydown')){
									mousePosition = {
										x : event.clientX,
										y : event.clientY
									};
								...
								data.onDrag(draggable._draggableData._element,droppables[i],event,mousePosition,draggable._draggableData._origin);
							}
							if(data.hoverClass && !$L(droppables[i]).hasClass(data.hoverClass)){
								...
								if(data.onHover){
									data.onHover(draggable._draggableData._element,droppables[i],draggable._draggableData._origin);
								}
							}
							break;
						}
							...
							if(data.entered){
								...
								if(prevDroppableData.onLeave){
									prevDroppableData.onLeave(draggable._draggableData._element,prevDroppable,draggable._draggableData._origin);
								}							if(prevDroppableData.onDropLeave){
								prevDroppableData.onDropLeave(draggable._draggableData._element,prevDroppable,draggable._draggableData._origin);
							}								if(prevDroppableData.onDropLeave){
									prevDroppableData.onDropLeave(draggable._draggableData._element,prevDroppable,draggable._draggableData._origin);
								}
								if(prevDroppableData.hoverClass && $L(prevDroppable).hasClass(prevDroppableData.hoverClass)){
			...
			_scrollAniamtionEnd : function(event, mousePosition){
				var droppables = this._droppables;
				var draggable = this._current;
				if(draggable && draggable._draggableData){
					for(var i = 0; i < droppables.length; i++){
						var data = droppables[i]._droppableData;
						if(data && !data.disabled && data.onAnimationCancel){
							data.onAnimationCancel(draggable._draggableData._element, droppables[i], event, mousePosition, draggable._draggableData._origin);
							break;
						}
					}
				}
			},
			_scroll : function(scrollDiv, mousePosition, event){
				var droppables = this._droppables;
				var draggable = this._current;
				if(draggable && draggable._draggableData){
					for(var i = 0; i < droppables.length; i++){
						var data = droppables[i]._droppableData;
						if(data && !data.disabled && data.onScroll){
							data.onScroll(draggable._draggableData._element, droppables[i], event, mousePosition, draggable._draggableData._origin, scrollDiv);
							break;
						}
					}
				}
			...
			_drop : function(event){
				...
				for(var i=0; i< droppables.length ; i++){
					...
					if(!(managerDD._isRelated(draggable, droppables[i]))){
						...
						if(!data.disabled && managerDD._checkAcceptable(draggable,data.accept) && managerDD._isNotRejected(draggable,data.reject)){
							if(data.ignoreIntersect || ( managerDD._checkIntersects(event,draggable,droppables[i],data.tolerance) && managerDD._checkIsVisible(droppables[i],draggable) )){
								if(data.onDrop){
									data.onDrop(draggable._draggableData._element,droppables[i],draggable._draggableData._origin,event);
								}
								break;
							}
						}
					...
					if(iframes){
						for(var i = 0; i<iframes.length ; i++){
							try {
								...
								// "Accessing iframe of different domain is not allowed."
							}
						}
					}
				}
		...
		$L.prototype.droppable = function(object, properties) {
			...
			data.ignoreIntersect = data.ignoreIntersect ? data.ignoreIntersect : false;
			...
			if(managerDD._droppables.indexOf(droppableEle) == -1){
				...
				if(managerDD._droppables.length > 1 && data.sortDroppables && !data.preventSort){
