	...
	if( typeof $L != "undefined" ){
			...
			isOver : function(event, data, fromDraggable){
				...
				if(fromDraggable){
					...
					if(prevOffset && (clientX < prevOffset.left || clientX > prevOffset.right || clientY < prevOffset.top || clientY > prevOffset.bottom)){
						...
						var sortables = _lyteUiUtils.querySelectorAll(data.connectToSortable+".sortable-parent");
						if(sortables.length){
		...
		$L.prototype.sortable = function(object, utilOptions) {
			...
			if(element._sortableParentData){
				...
				data.onkeydown = data.onkeydown ? data.onkeydown : _sortableData.onkeydown;
				...
				data.scrollDivX = data.scrollDivX == undefined ? _sortableData.scrollDivX : typeof data.scrollDivX === "string" ? document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ](data.scrollDivX) : data.scrollDivX;
				data.scrollDivY = data.scrollDivY == undefined ? _sortableData.scrollDivY : typeof data.scrollDivY === "string" ? document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ](data.scrollDivY) : data.scrollDivY;
			...
			var bindMouseEvents = function( event, data, divOffset ){
				if(event.type == "mousedown"){
					...
					_lyteUiUtils.addEventListenerGlobal('touchmove',data._parentElem.__mouseMoveEvent, {capture : true, passive : false});
					_lyteUiUtils.addEventListenerGlobal('touchend',data._parentElem.__mouseUpEvent, true);
				}
			}
			...
			function findNextSortElem(sortableElems,data,count,event){
				...
				if(SortableElem && SortableElem !== currentEle ){
					if(count > 0){
						Lyte.Component.insertAfter(SortableElem,currentEle);
					}else{
						Lyte.Component.insertBefore(SortableElem,currentEle);
					}
			...
			function keyDownEvent( event ){
				...
				if(data.onkeydown){
					data.onkeydown( event, sortableElems[data.focusIndex] );
				}
		}
		function mouseDownEvent( event ){
			...
			var createPlaceholder = function(  _placeholder, _div, width, height, data ){
				...
				$L(_placeholder).addClass(data.placeholder.replace(/\./g,' '));
				if (!data.updateOnDrop) {
								...
								var measure2 = $L.fastdom.measure(function(){
									...
									var mutate2 = $L.fastdom.mutate(function(){
										...
										if(droppablePlace && AppendToDroppableParentCheck()){
											_lyteUiUtils.appendChild(droppablePlace,_placeholder);
										...
										if(droppablePlace && checkDroppable(droppablePlace,_sortableElem.parentElement,_sortableElem,data.connectedWith,data.containmentDimensions,_mousePosition)){
											...
											if(_elemBelow && ($L(_elemBelow).hasClass('sortable-parent')) && checkForIntersect(_elemBelow,_mousePosition) && checkParentDroppable(_elemBelow,_sortableElem.parentElement,_sortableElem,data.connectedWith,data.containmentDimensions,_mousePosition) && (!data.preventDropAtStart && !firstChildNotRestricted(data, _elemBelow) && iselemCanDropOnTop(data,_elemBelow,_sortableElem))){												
												_lyteUiUtils.insertBefore(getChildren(data, _elemBelow)[0], _placeholder);
												callPlaceholder(data, _elemBelow, droppablePlace);
											}
			...
			var mouseUpEvent = function(event){
				...
				if(event.type == "touchend"){
					_lyteUiUtils.removeEventListenerGlobal('touchmove',mouseMoveEvent, {capture : true, passive : false});
				...
				if(_sortableElem){
					...
					if(data._isDown){
						...
						data._isDown = false;
						_div = data._div;
						_placeholder = _lyteUiUtils.querySelectorAll('#dummy').length == 1 ? _lyteUiUtils.querySelectorAll('#dummy')[0] : data._placeholder;
			...
			var callPlaceholder = function(data, _elemBelow, droppablePlace){
				var _$LPlaceholder = $L(_placeholder);
				var _placeholderClass = data.placeholder.replace(/,/g,' ');
				if(!data.onPlaceholder || checkValidDroppable(data,_elemBelow || droppablePlace.parentElement)){
					...
					_$LPlaceholder.addClass(_placeholderClass);
				}
				else{
					if(_$LPlaceholder.hasClass(_placeholderClass)){
						_$LPlaceholder.removeClass(_placeholderClass);
					}
					_$LPlaceholder.addClass(_placeholderClass);
				}
					...
					if(_maxScrollHeight - scrollTop > data.scrollSpeedCondition){
						if(isRelativeY && scrollDiv.contains(data._div)){
							if(checkElemContainment(data.containmentDimensions,divOffset,'bottom',data.containment)){
								...
								data._div.style.top =  data.containmentDimensions.bottom - data._div.offsetHeight - parent.top + (_maxScrollHeight - scrollTop >= data.scrollSpeed ?( data.scrollSpeed + scrollTop) : _maxScrollHeight )  +'px';
							}
						...
						if(isRelativeY && scrollDiv.contains(data._div)){
							data._div.style.top = _mousePosition.y - _offset[1] - parent.top + _maxScrollHeight + "px";
			...
			var onDrag = function(data,event){
				var placeholder = _lyteUiUtils.querySelectorAll('#dummy').length == 1 ? _lyteUiUtils.querySelectorAll('#dummy')[0] : data._placeholder;
			...
			var checkForItems = function(data,targetElem){
				if(data.items.length > 0){
					for(var i = 0 ; i<data.items.length ; i++){
						var elements = _lyteUiUtils.querySelectorAll(data.items[i]);
