...
( function() {
	...
	var reg_sinks = {
		...
		'line_break': /(\n|\r\n|\r)/,
		'special': /[{}*$#\-_~!`()\\\[\]\.|"]/,
		'spl_except_forw_slsh': /[*_~!`]|^www|^ht' + 'tps:|^ht' + 'tp:/,
		'space': /[ ]/,
		'number': /\d/
	};

	var curNumOfLeadingSpaces = 0;
	var lastNumOfLeadingSpaces = 0;
	...
	function tokenList() {
		...
		this.getTokens = function() {	//for debugging
			return tokens;
		}
	}
	...
	*/
	function buildConfig( totalConfiguration ) {
		var attributes = [ 
						...
						'ol', 'ul', 'li',
						'table', 'thead', 'tbody', 'tr', 'th', 'td', 'code', 'break' ],
	...
	function _rDP( list, totalConfiguration ) {
		...
		function addAllAttributes( node, configName ) {
			...
			for( key in attributes ) {
				if( key !== 'ltPropPreventSecurityPrefix' && key !== 'styleAttributes' ){
		...
		function checkForBreakTag( ) {
			var shouldAllow = userConfig.getAttribute( 'break', 'allowBrTags' );

			return shouldAllow == true;
		}

		function shouldAllowSpecialCodeBlocks() {
			return totalConfiguration && totalConfiguration.allowSpecialCodeBlocks === true;
		}

		function shouldAllowTextColoring() {
			return totalConfiguration && totalConfiguration.allowTextColoring === true;
		}
		...
		var buildMarkdown = function() {
			...
			while( list.peek().get( 'type' ) !== '' ) {
				...
				if( !checkForBreakTag() ){
					...
					var flag = false;
					while( list.peek().get( 'type' ) === 'line_break' ) {
						if( !flag ){
							flag = true;
						}
						else{
							appendElement( div, document.createElement( 'br' ) );
						}
						list.eat();
					}
				}
		...
		var buildBasedOnFirstCharacter = function() {
			...
			if( isStartOfHeader() ) {
				...
				curNumOfLeadingSpaces = 0;
				lastNumOfLeadingSpaces = 0;
			}
		...
		var buildOrderedList = function( subLevel ) {
			var parent = document.createElement( 'ol' );
			var mem, type;

			do {
				if( subLevel && ( curNumOfLeadingSpaces - subLevel < 0 ) ){
					backtrack(mem);
					addAllAttributes( parent, 'ol' );
					return parent;
				}
				var subParent = $L(parent).find('li').eq(-1) ;

				if( ( curNumOfLeadingSpaces - lastNumOfLeadingSpaces > 1 ) && subParent.length > 0 ){
					appendElement( subParent[0], buildRespectiveList(  type ) );
				}
				else{
		...
		var buildUnorderedList = function( subLevel ) {
		...
		var buildRespectiveList = function( type ) {

			if( type === 'ord' ){
				return buildOrderedList( curNumOfLeadingSpaces );
			}
			else if( type === 'unord' ){
				return buildUnorderedList( curNumOfLeadingSpaces );
			}
			else{
				return null;
			}
		...
		var isStartOfList = function() {
			...
			if( isOrderedListStart() ){
				res = 'ord';
			}
			else if( backtrack( mem ), isUnorderedListStart() ){
				res = 'unord';
			}
			else{
				res = false;
		...
		var isOrderedListStart = function() {
			...
			var tempCurNum = consumeLeadingWhiteSpaces();
			...
			if( !isValidStart ) {
				...
				lastNumOfLeadingSpaces = curNumOfLeadingSpaces;
				curNumOfLeadingSpaces = tempCurNum;
			}
		...
		var para = function( parent ) {
			...
			// Found a possible text color syntax
			else if( isTextColorStart() ) {
				token = list.eat();
				token_aft = list.peek();
				mem = capture();
				ret = getTextColor();
			...
			// Found highlight character
				...
				token_aft = list.peek();
				mem = capture();
				ret = isCodeBlock(2) ? getCodeBlock(2) : getHighlight();
		...
		var getCodeBlock = function(itr) {
			var code, ret, styleAttributes = userConfig.getAllAttributes( 'code' ).styleAttributes || {};
			...
			if( cs.isEmpty() ) {
				ret = consumeAsTextForCodeBlock() || "";

				if( ret.trim().length !== 0 && isCodeBlock(3) ) { 
					for(var i=0;i<3;i++){
						list.eat();
					}
					code = document.createElement( 'code' );
					var shouldRenderSpecial = shouldAllowSpecialCodeBlocks();
					var specialCodeObject = shouldRenderSpecial ? parseSpecialCodeBlock( ret ) : null;
					var specialCodeNode = specialCodeObject ? getSpecialCodeNode( specialCodeObject.language, specialCodeObject.code ) : null;

					if( specialCodeNode ) {
						code.appendChild( specialCodeNode );
					}
					else {
						var div = document.createElement( 'div' );
						div.style.whiteSpace = styleAttributes.whitespace || 'pre';
						div.textContent = ret;
						code.appendChild( div );
					}
					addAllAttributes( code, 'code' );
		...
		var parseSpecialCodeBlock = function( rawCodeBlockText ) {
			var str, match, language, code;

			if( typeof rawCodeBlockText !== 'string' ) {
				return false;
			}

			/* Backward compatibility: current codeblock parser may leave one opening backtick in the buffer. */
			str = rawCodeBlockText.replace( /^\s*`/, '' ).replace( /^\r?\n/, '' );
			match = str.match( /^([^\n\r]+)\r?\n([\s\S]*)$/ );
		...
		var getSpecialCodeNode = function( type, code ) {
			var builder, result, snippet, wrapper, fragment;

			if( !$L || !$L.snippets || typeof $L.snippets.getBuilder !== 'function' ) {
				return false;
			}

			try {
				builder = $L.snippets.getBuilder( type, code );
				...
				snippet = result && ( result.snippets || result.snippet );
				...
				wrapper.className = 'lyteMarkdownCodeSnippet';

				if( typeof snippet === 'string' ) {
					fragment = document.createElement( 'div' );
					fragment.innerHTML = snippet;

					while( fragment.firstChild ) {
						wrapper.appendChild( fragment.firstChild );
					}
				}
				else if( snippet && snippet.nodeType ) {
		...
		var isCodeBlock = function(itr) {
			var mem = capture();
			var token_bef = list.peekback();
			var token = list.peek();

			if(  token_bef.get( 'value' ) === '\\' ){
				return false
			}
		...
		var consumeAsTextForCodeBlock = function() {
			var str = '';

			while( !isCodeBlock(3) 
				&& (!delimiter() || list.size() - capture() > 1 )
			) {
		...
		var isTextColorStart = function() {
			var token = list.peek(),
			token_bef = list.peekback();

			if( !shouldAllowTextColoring() ) {
				return false;
			}
		...
		var consumeUntilSpecialCloseBrace = function() {
			var str = '';
		...
		var getTextColor = function() {
			var slashToken, colorToken, openColorToken, colorValue,
			openTextToken, textValue, closeTextToken, closeDollarToken,
			span;

			slashToken = list.eat();
			colorToken = list.eat();

			if( !slashToken || slashToken.get( 'value' ) !== '\\' ) {
				return false;
			}

			if( !colorToken || colorToken.get( 'value' ) !== 'color' ) {
				return false;
			}

			openColorToken = list.eat();

			if( !openColorToken || openColorToken.get( 'type' ) !== 'special{' ) {
				return false;
			}

			colorValue = consumeUntilSpecialCloseBrace();

			if( !colorValue || list.eat().get( 'type' ) !== 'special}' ) {
				return false;
			}

			openTextToken = list.eat();

			if( !openTextToken || openTextToken.get( 'type' ) !== 'special{' ) {
				return false;
			}

			textValue = consumeUntilSpecialCloseBrace();
			closeTextToken = list.eat();
			closeDollarToken = list.eat();

			if( !closeTextToken || closeTextToken.get( 'type' ) !== 'special}' ) {
				return false;
			}

			if( !closeDollarToken || closeDollarToken.get( 'type' ) !== 'special$' ) {
				return false;
			}

			span = document.createElement( 'span' );
			span.style.color = colorValue.trim();
