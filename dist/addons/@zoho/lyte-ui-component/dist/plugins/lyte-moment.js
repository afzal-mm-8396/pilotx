;( function(){
	// if( typeof $L != "undefined" ){
			if( typeof $L == "undefined" ){
				$L = {
					extend : function( is_deep, obj1, obj2 ){

						if( typeof is_deep != "boolean" ){
							obj2 = obj1;
							obj1 = is_deep;
						}
			...
			timezone_fallback = {
				...
				"ADT" : "America/Halifax",
				/**
				 * This can be Atlantic Standard Time or Alaska Standard Time
				 * Here i am choosing Alaska Standard Time here -> for CRM's case
				 */
			...
			function convert_frm_i18n( format, arg, number_conversion, callback ){
				...
				parsed.forEach( function( item ){
					if( item.format.str ){
						parsed_str.unshift( item );
					} else {
						parsed_non_str.unshift( item );
					}
				} );

				parsed_str.concat( parsed_non_str ).forEach( function( item ){
					...
					Array.from( arr ).reverse().forEach( function( _item ){
						var conv = fn ? fn.i18n( __map[ _item ] || _item ) : _item ;
			...
			function lyteMoment( arg, format, uL, lL ){
				...
				if( uL && uL.constructor == Object ){
					...
					case_sensitive = uL.case;
					__maintain_time = uL.maintain_time;
					uL = uL.uL;
				}
				...
				this.maintain_time = __maintain_time == void 0 ? maintain_time : __maintain_time;
				...
				this._isValid = validate.call( this, arg, format, case_sensitive );
			}
			...
			function find( array, val, case_sensitive ){
				array = array || [];

				var crct, i,
				__length = array.length,
				is_case_sens = case_sensitive == false,
				regex_tail = is_case_sens ? "i" : void 0;

				for( i = 0; i < __length; i++ ) {
					var __cur = array[ i ];

					if( new RegExp( "^" + __cur + "$", regex_tail ).test( val ) ) {
						crct = is_case_sens ? val : __cur;
						break;
					}
				}
			...
			function getMonth( val, suff, str, long, case_sensitive ) {
				var mon;
				if( str ) {
					var ret = find( long ? longMon : shortMon, val, case_sensitive );
					mon = ret.mon;
					val = ret.index;
					if( !mon || val > 11 ){
						val = "Invalid";
			...
			function __getWeekReverse( dobj, ignore ){
			    ...
			    if( month == 11 ){
			        ...
			        exp_end = date + ( endday - __week_end + 7 ) % 7;
	 	...
	 	// 		return { week : Math.ceil( ( total + startday + date ) / 7 ), day : dobj.getDay() + 1};
			...
			function valFormat( arg, format, case_sensitive ){
				var copyFormat = {}, 
				ret, 
				date,
				__new = new Date(),
				...
				for( var i = 0; i < prseVal.length; i++ ) {
					...
					switch( cur.type ) {
						...
						case 'month' : {
							if( cur.regex.test(arg) ) {
								ret = getMonth( arg.match(  cur.regex )[ 0 ], cur.suff, cur.str, cur.long, case_sensitive );
						...
						case 'longdate' : {
							if( cur.regex.test(arg) ) {
								if( cur.str ) {
									ret = find( cur.array, arg.match(  cur.regex )[ 0 ], case_sensitive );
								} else {
				...
				if( validate.call( this, date ) ){
					if( isDef( copyFormat.timezone ) ){
						...
						date.setTime( date.getTime() + copyFormat.timezone * 1000 * 60 );
						...
						machine_old_tz = date.getTimezoneOffset();

						copyFormat.timezone =  ori_timezone - machine_old_tz;
						
						/**
						 * setting the difference in setMinutes directly takes the time.
						 * Like machine is in Sydney/Australia timezone. for reading "Apr 7 2024" in Asia/Calcutta timezone adding the 330 difference changes the hour & minute to 5:30+10:00( previous value +11:00)
						 * But its a DST point. it should be 4:30 +10:00. So here i am directly adding time in ms
						 */
						// date.setMinutes( date.getMinutes() + copyFormat.timezone );
						date.setTime( date.getTime() + copyFormat.timezone * 1000 * 60 );
						...
						//    date.setMinutes( date.getMinutes() - diff );
							date.setTime( date.getTime() - diff * 1000 * 60 );
						}
			...
			function setTimezoneName(date_obj, _this) {
				var mod_obj,
				__timezone = _this._timezone,
				is_error;

				try {  
					if( _this._timezone ){ 
						mod_obj = date_obj.toLocaleString('en-US', { timeZone: timezone_fallback[ __timezone ] || __timezone, timeZoneName: "long" }).split(/AM|PM/)[1];
					} else {
				...
				if( is_error || !mod_obj ){
					if( __timezone ){
						var input = _this._arg;
						if( typeof input == "string" ){
							/**
							 * In Firefox it does not support some old timezones. So here i am returning the timezone offset value directly with GMT;
							 */
							var match = input.match( /([\+\-])(\d{2}):(\d{2})/ );
							if( match ){
								return "GMT" + match[ 1 ] + match[ 2 ] + ":" + match[ 3 ];
							}
						}
						/**
						 * In this case i can't do anything as i dont have any info regarding the argument except timezone name. If the time falls on DST it will fail here
						 */
						return __timezone;
					}
					mod_obj = new Date().toTimeString().match(/\(([^)]+)\)/)[1];
				...
				if( date_obj ){
					var __trimmed_value = mod_obj.trim(),
					replacements = { 
						"Central European Standard Time" : "Central European Time",
						"Alaska Standard Time" : "AKST",
						"Alaska Daylight Time" : "AKDT" 
					};

					/**
					 * Temp. fix. Need to add a proper fix here
					 */
					if( /[\+\-]\d{2}:\d{2}/.test( __trimmed_value ) ){
						return __trimmed_value;
					}

					__trimmed_value = replacements[ __trimmed_value ] || __trimmed_value;

					return __trimmed_value.match(/\b\w/g).join('');
				} 
				return mod_obj;
			}
			...
			function validate( arg, format, case_sensitive ) {
				var cons = arg.constructor;
				...
				if( cons == Date || cons == ( new Date().constructor ) ) {
					...
					if( format ) {
						var ret = valFormat.call( this, arg, format, case_sensitive );
					...
					// need to check this. some low time may have 10 digit value
					...
					this.maintain_time = arg.maintain_time;

					return validate.call( this, new Date( arg._dateObj.getTime() ) );
				}
			}
			...
			function convert_timezone_DLS( date_obj, name, short, frm_self ){
				...
				try {
					return ( this.dls_check_map = ( name ? date_obj.toLocaleString( "en-US", { timeZone: timezone_fallback[ name ] || name, timeZoneName: short } ) : create_dls( date_obj, short ) ) );
				} catch( err ) {
					/**
					 * When used some non standard timezones like 'PST' in firefox it throws error. So here i am using some fallback timezones for the passed one
					 */
					var fallback_timezone_name = timezone_fallback[ name ];
					if( frm_self || !fallback_timezone_name ){
						throw new Error( err );
					}
					return convert_timezone_DLS.call( this, date_obj, fallback_timezone_name, short, true );
				}
			}

			$L.extend( lyteMoment.prototype, {
				...
				parseDate : function( str, date_obj, timezone ){
					...
					/**
					 * for firefox where timezone offset is not proper for older years.
					 * Eg. before 1906 India has three timezones( non standard ).
					 * Firefox gives timezones with decimal values. So i am removing the decimals as of now for matching this with chrome behavior.
					 */
					str = str.replace( /\.\d+$/, '' );
			...
			$L.moment.maintain_time_on_modification = function( value ){
				maintain_time = value;
			}
		// }
	})();
;( function( ){
	if( $L.moment ) {
		...
		$L.extend( proto, {
			...
			add : function( val, prop, cyclic ) {
				...
				if( this.validate() ) {
					if( this.isDef( prop ) ) {
						...
						var ret = modify_props( val, prop ),
						maintain_time = this.maintain_time,
						tz_off = maintain_time ? this.timezoneOffset( this.toDate() ) : 0;
						...
						if( maintain_time ){
							handle_diff.call( this, this.toDate(), tz_off, prop );
						}
			...
			fromNow : function( arg, to_accurate, milliseconds ){
				if( this.validate() ){
					var today = this,
					...
					diff = parseInt( ( new_time - old_time ) / ( milliseconds ? 1 : 1000 ) ),
					past = diff < 0,
					limits = { years : { val : 320 * 24 * 60 * 60, conv : 365 * 24 * 60 * 60 }, months : { val : 26 * 24 * 60 * 60, conv : 30 * 24 * 60 * 60 }, days : { val : 22 * 60 * 60, conv : 24 * 60 * 60 }, hours : { val : 45 * 60, conv : 60 * 60 }, minutes : { val : 44, conv : 60 }, seconds : { val : milliseconds ? 1 : 0, conv : 1 } },
					...
					fn = function( _diff, obj ){
						...
						for( key in limits ){
							if( _diff >= (limits[ key ][ ns1 ] * (milliseconds ? 1000 : 1)) ) {
								var convValue = limits[ key ].conv * (milliseconds ? 1000 : 1);
								val = Math[ ns2 ]( _diff / convValue );
								value = val;

								var new_diff = _diff - val * convValue;
								if( new_diff > 0 ){
									fn( new_diff, obj );
								}
								break;
					...
					milliseconds && (limits.milliseconds = { val : 0, conv : 1/1000 });
		...
		function i18N( format, callBack, convert_numbers ){
			if( this.validate() ){
				...
				ui_utils = typeof _lyteUiUtils != "undefined" ? _lyteUiUtils : void 0;

				if( convert_numbers ){
					convert_numbers = ui_utils ? _lyteUiUtils.i18n( "1" ) != "1" : false;
				}
				...
				parsed.forEach( function( item ){
					...
					mapped_value = __map[ formatted ] || formatted,
					i18ned = ui_utils && !/^[0-9]$/.test( formatted || "" ) ? _lyteUiUtils.i18n( mapped_value ) : formatted,
					index = item.index;

					if( i18ned == mapped_value ){
						i18ned = formatted;
					}

					if( convert_numbers && !__format.str ){
						for( var i = 0; i < 10; i++ ){
							i18ned = i18ned.replace( new RegExp( i, "g" ) , _lyteUiUtils.i18n( i.toString() ) );
						}
					}
		...
		function modify_props( val, prop ){
			switch( prop ){
				case 'hours' : {
		...
		function handle_diff( date, old_off, prop ){
			var new_off = this.timezoneOffset( date ),
			diff = new_off - old_off;

			if( diff && prop != 'time' ){
				date.setTime( date.getTime() + diff * 60 * 1e3 );
			}
		}
