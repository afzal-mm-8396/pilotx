;( function(){
    ...
    function mousedown( ev ){
        ...
        if( isTch ){
            ...
            $L.extend(evt, ev.touches[ 0 ]);
        }
        var elem = this,
        targetElem = $L( this ).data( 'element' ),
        data = targetElem._resizeData,
        directions = $L( elem ).data( 'directions' );
        if(window._freeflowActive){//elem._freeflowActive is not used because, down is made on one handler and up on another error occurs
            return;
        }
        ...
        if( data.component == "table" ){
         ...
         if(data.boundaryScroll && data.boundary){
            boundElem = $L( targetElem ).closest( data.boundary ).get( 0 );
            targetElem.parScrollTop = boundElem.scrollTop;
            targetElem.parScrollLeft = boundElem.scrollLeft;
         }
         ...
         $L.fastdom.mutate( function(){
            ...
            if(targetElem.parScrollTop || targetElem.parScrollLeft){
                boundElem.scrollTop = targetElem.parScrollTop;
                boundElem.scrollLeft = targetElem.parScrollLeft;
            }
            delete targetElem._oheight; delete targetElem._owidth; delete targetElem._mwidth;
            delete targetElem.parScrollTop; delete targetElem.parScrollLeft;
            
            this._mousedown && data.onSelect && data.onSelect( elem, targetElem, ev, elem._clone );
        ...
        var scaledElem = data.scaledParent || targetElem,
            style = window.getComputedStyle(scaledElem),
            transform = style.transform || style.webkitTransform || style.mozTransform;

        if (transform && transform !== 'none') {//this will work only, when scale applied to parent
            ...
            if (matrix && matrix[1]) {
                ...
                if (matrixValues.length >= 6) {
                    [a, b, c, d] = matrixValues;

                    this._scaleX  = Math.round(Math.hypot(a, b) * 1000) / 1000;
                    this._scaleY = Math.round(Math.hypot(c, d) * 1000) / 1000;
        ...
        data.boundaryScroll && data.boundary && evt.preventDefault();
    }
    ...
    function tableDown( ev ){
        ...
        isLastColumnResize = !direction && preventTable && data.resizeLastColumn && !element.nextElementSibling,
        nextstyle,
        cells = toArray( row.children ),
        final = [],
        ...
        if( !preventTable || isLastColumnResize ){
    ...
    function mousemove( ev, custom ){
        var evt = ev,
        isTch = /touch/i.test( ev.type ),
        elem = window._resizeelem,
        targetElem = $L( elem ).data( 'element' ) || elem,
        directions = $L( elem ).data( 'directions' ),
        data = targetElem._resizeData,
        boundaryScroll = data.boundaryScroll && data.boundary;
        if( isTch ){
            if( evt.touches.length > 1 ){
                return;
            }
        ...
        if( window._resizeelem._mousedown || (data.freeflow && data.preventDraggable && !isTch && ev.buttons != 0)){
            return;
        }
        if (boundaryScroll) {
            if (!custom && this.customScroll && (this.customScroll.x || this.customScroll.y)) {
                var blockX = directions.width && (evt.clientX <= 0 || evt.clientX >= window.innerWidth - 1),
                    blockY = directions.height && (evt.clientY <= 0 || evt.clientY >= window.innerHeight - 1),
                    corner = directions.width && directions.height,
                    noChange = { x: true, y: true };

                if (corner ? ((this.customScroll.x && blockX) && (this.customScroll.y && blockY)) : ((!this.customScroll.x || blockX) && (!this.customScroll.y || blockY))) {
                    return;
                }

                if (this.customScroll.x) {
                    noChange.x = false;
                    if (!blockX) {
                        this.oldClient.x = evt.clientX;
                        this.customScroll.x = false;
                    } else if (corner && !this.customScroll.y && evt.clientY != this.oldClient.y) {
                        this.oldClient.x = evt.clientX;
                    } else {
                        noChange.x = true;
                    }
                ...
                if (this.customScroll.y) {
                    noChange.y = false;
                    if (!blockY) {
                        this.oldClient.y = evt.clientY;
                        this.customScroll.y = false;
                    } else if (corner && !this.customScroll.x && evt.clientX != this.oldClient.x) {
                        this.oldClient.y = evt.clientY;
                    } else {
                        noChange.y = true;
                    }
                ...
                if (noChange.x && noChange.y) {
                    return;
                }
            }
            cancelAnimationFrame(this.mousemoveRaf);
        }
        var oldClient = this.oldClient,
        winwidth = window.innerWidth,
        ...
        xIncr =  newX - (boundaryScroll ? (oldClient ? oldClient.x : newX) : ( is_preserve_aspect ? 0 : rtlfunc( 'right', bcr, winwidth ) )) - (boundaryScroll ? 0 : elem._clientX),
        yIncr = newY - (boundaryScroll ? (oldClient ? oldClient.y : newY) : ( is_preserve_aspect ? 0 : bcr.bottom )) - (boundaryScroll ? 0 : elem._clientY),
        clone = targetElem._clone,
        ...
        scaleX = elem._scaleX || 1,
        scaleY = elem._scaleY || 1,
        spacing = data.spacing || {},
        oldIncr, bbcr, lockIncr = {}, 
        boundary_elem = data.boundary && $L( targetElem ).closest( data.boundary ).get( 0 );
        ...
        if( boundaryScroll ? is_top || is_left : (data.boundary && !next) ){
            
            oldIncr = {x: xIncr, y: yIncr};

            if( data.boundary.constructor == String ){
               var bcr1 = getBcr( boundary_elem );
               bbcr = {
                  left : bcr1[ rtl_left ] + ( spacing.left || 0 ) * scaleX,
                  right : bcr1[ rtl_right ] - ( spacing.right || 0 ) * scaleX,
                  top : bcr1.top + ( spacing.top || 0 ) * scaleY,
                  bottom : bcr1.bottom - ( spacing.bottom || 0 ) * scaleY,
                  height : bcr1.height,
                  width : bcr1.width
               };
            ...
            if( is_preserve_aspect || angle ){
                ...
                if (boundaryScroll) {
                    newX = (bcr.left + bcr.width / 2 * scaleX) + xIncr;
                    newY = (bcr.top + bcr.height / 2 * scaleY) + yIncr;
                } else {
                    var marginal_diff_x = ( elem._clientX + bcr.width / 2 * scaleX ),
                        marginal_diff_y = ( elem._clientY + bcr.height / 2 * scaleY );
                    newX -= marginal_diff_x;
                    newY -= marginal_diff_y;
                }

                if( !boundaryScroll && newX > bbcr[ rtl_right ] ){
                ...
                if( !boundaryScroll && newY > bbcr.bottom ){
        ...
        if (boundaryScroll && oldIncr) {//to get fix at top/right side
            if (yIncr != oldIncr.y) {
                yIncr = oldIncr.y;
                lockIncr.y = true;
            }
            ...
            if (!lockIncr.y && !lockIncr.x && yIncr == 0 && xIncr == 0) {
                this.oldClient = { x: ev.clientX, y: ev.clientY };
                return;
            }
        }

        if( is_width ){
            if( !preventTable || ( isTable && data.resizeLastColumn && !next ) ){
        ...
        if( is_height ){
            ...
            isTable && (elem.style.height = ( parseFloat( elem.style.height ) + yIncr ) + 'px')
        }
        ...
        if( angle ){
            ...
            if( is_left ) {
                if (boundaryScroll && parseFloat(targetElem.style.left) == 0 && (xIncr * xFact) < 0 && parseFloat(targetElem.style.width) >= bbcr.width) {
                    lockIncr.x = true;
                }
                targetElem.style[ rtl_left ] = lockIncr.x ? 0 : ( parseFloat( targetElem.style[ rtl_left ] ) + xIncr ) + 'px';
            }
            
            if( is_top ) {
                if (boundaryScroll && parseFloat(targetElem.style.top) == 0 && (yIncr * yFact) < 0 && parseFloat(targetElem.style.height) >= bbcr.height) {
                    lockIncr.y = true;
                }
                targetElem.style.top = lockIncr.y ? 0 : ( parseFloat( targetElem.style.top ) + yIncr ) + 'px';
            }
        }
        ...
        if (boundaryScroll) {
            !is_top && is_height && (boundary_elem.scrollTop = boundary_elem.scrollHeight);
            !is_left && is_width && (boundary_elem.scrollLeft = boundary_elem.scrollWidth);
            this.oldClient = { x: evt.clientX, y: evt.clientY };
            this.customScroll || (this.customScroll = {});
            var atTop = evt.clientY <= 0,
                atBottom = evt.clientY >= window.innerHeight - 1,
                atLeft = evt.clientX <= 0,
                atRight = evt.clientX >= window.innerWidth - 1,
                custMoveValue = {};

            this.mousemoveRaf = requestAnimationFrame(function () {
                if ((is_top && atTop) || (!is_top && atBottom) || ((atTop || atBottom) && parseFloat(targetElem.style.height))) {// top bottom
                    this.customScroll.y = true;
                    var currentScrollY = window.pageYOffset || document.documentElement.scrollTop,
                        scrollStep = atTop ? -5 : 5,
                        trgPos = (is_top ? targetElem.getBoundingClientRect().top : targetElem.getBoundingClientRect().bottom) + scrollStep,
                        overFlow = trgPos - window.innerHeight;


                    (atTop ? trgPos <= 0 : overFlow > 0) && window.scrollTo({
                        top: currentScrollY + (atTop ? trgPos : overFlow),
                        behavior: 'auto'
                    });
                    custMoveValue.y = evt.clientY + scrollStep;
                }

                if ((is_left && atLeft) || (!is_left && atRight) || ((atLeft || atRight) && parseFloat(targetElem.style.width))) {// left right
                    this.customScroll.x = true;
                    var currentScrollX = window.pageXOffset || document.documentElement.scrollLeft,
                        scrollStep = atLeft ? -5 : 5,
                        trgPos = is_left ? targetElem.getBoundingClientRect().left : targetElem.getBoundingClientRect().right,
                        overFlow = trgPos - window.innerWidth;

                    (atLeft ? trgPos <= 0 : overFlow > 0) && window.scrollTo({
                        left: currentScrollX + (atLeft ? trgPos : overFlow),
                        behavior: 'auto'
                    });
                    custMoveValue.x = evt.clientX + scrollStep;
                }

                if (custMoveValue.x != undefined || custMoveValue.y != undefined) {
                    mousemove.call(this, {
                        type: 'mousemove',
                        clientX: custMoveValue.x != undefined ? custMoveValue.x : evt.clientX,
                        clientY: custMoveValue.y != undefined ? custMoveValue.y : evt.clientY,
    ...
    function mouseup( evt, frm_remove ){
        ...
        if(data.freeflow && !isTch && !window._freeflowActive && !elem._moved && Object.keys(evt).length > 0 && !frm_remove){
            window._freeflowActive = true;
            return;
        }
        ...
        delete elem._scaleX;
        delete elem._scaleY;
        delete this.oldClient;
        delete this.customScroll;
        delete this.mousemoveRaf;
        delete window._freeflowActive;
    ...
    function tableMove( evt ){
        ...
        if( _lyteUiUtils.isWidget && !document.contains( evt.target ) ){
            return;
        }
        ...
        if( data.preventTable && isHori && !data.resizeLastColumn ){
    ...
    function update_prop( obj ){
        var len = this.length;

        for( var i = 0; i < len; i++ ){
            var cur = this.get( i ),
            __data = cur._resizeData;
    ...
    $L.prototype.enableResize = function( obj ){
        if( obj == 'destroy' ){
            ...
            return update_prop.call( this, arguments[ 1 ] );
        }
        ...
        obj.resizeLastColumn = obj.resizeLastColumn || false;
        bind.call( this, obj );
        return this;
    }

    document[ "addEventListener" + ( _lyteUiUtils.isWidget ? "Global" : "" ) ]( 'click', function( evt ){
        if( !window._freeflowActive ){
            var active = Array.from( document.getElementsByClassName( selection_class ) ),
