	...
	var buildTokenizer = function( language, config ) {
		...
		if (alias && alias.length > 0) {
			for (var i = 0; i < alias.length; i++) {
				var aliasLanguage = alias[i];

				registeredTokenizers[ aliasLanguage] = registeredTokenizers[language];
			}
		}
		...
		registeredTokenizers[ language ].prototype.buildTokens = function( code, tokenConfig, buildUnmatchedToken ) {
			...
			__fn = function( token ) {
				...
				currentMatchIndex;
				var preceededBy = token.preceededBy || undefined;
				...
				if (preceededBy) {
					if (result.length > 0) {
						...
						if (!preceededBy.includes(lastTokenType)) {
							return;
						}
					}
				}
