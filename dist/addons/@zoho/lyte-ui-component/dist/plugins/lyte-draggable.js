...
;(function( window ) {
	...
	if($L){
		$L.prototype.manageDraggable = {
			init : false,
			draggedEle : null,
			isRestrictedClass : function(restrictClass,element){
				if(element.classList.contains(restrictClass)){
					return true;
				}
			},
			isRestricted : function(restrict,element){
				...
				for(var i = 0; i<restrict.length; i++){
					var elements = _lyteUiUtils.querySelectorAll(restrict[i]);
		...
		$L.prototype.draggable = function(object) {
			...
			if(this.length > 1){
				var elemArray = this;
				for(var i = 0; i<elemArray.length; i++){
					var drag_element = $L(elemArray[i]);
					var flag = data.preventReapply && drag_element.hasClass('draggable-element');
					if(!flag){
						drag_element.draggable(Object.assign({},data));
					}
				}
				return;
			}
			//Parent Element
			if((data.restrict && manageDraggable.isRestricted(data.restrict,this[0])) || (data.restrictClass && manageDraggable.isRestrictedClass(data.restrictClass,this[0]))){
				return;
			}
			data._element = this[0];
			manageDraggable.addClassOrAttr(data.attribute,data._element,'draggable-element');
			var Elem_css = getComputedStyle($L(this)[0]);
			var _handleElement;
			var _initClientX;
			var _initClientY;
			var _initialPos = {};
			...
			data.restrictClass = data.restrictClass ?  data.restrictClass : '';
			...
			data.DragElemAppendTo = data.DragElemAppendTo ? data.DragElemAppendTo : false;
			data.bubbles = data.bubbles === undefined ? true : data.bubbles;
			data.mouseUpBubbles = data.mouseUpBubbles ? data.mouseUpBubbles : false;
			data.scrollDivX = data.scrollDivX ? (typeof data.scrollDivX == "string" ? document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ](data.scrollDivX) : data.scrollDivX) : undefined;
			data.scrollDivY = data.scrollDivY ? (typeof data.scrollDivY == "string" ? document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ](data.scrollDivY) : data.scrollDivY) : undefined;
			data.preventScrollHeightCalculation = data.preventScrollHeightCalculation ? data.preventScrollHeightCalculation : false;
			...
			data.staticDimensions = data.staticDimensions ? data.staticDimensions : false;
			...
			data.istransform = data.istransform ? data.istransform : false;
			data.aria = data.aria ? data.aria : false;
			data.keyNavSpeed = data.keyNavSpeed ? data.keyNavSpeed : 20;
			data.scale = data.scale ? data.scale : {x: 1, y: 1};
			data.isPositionFixed = data.isPositionFixed ? data.isPositionFixed : false;
			data.zIndex = data.zIndex ? data.zIndex : 200000;
			data.userSelect = data.userSelect ? data.userSelect : false;
			data.reCalulateOffsetOnDragStart = data.reCalulateOffsetOnDragStart ? data.reCalulateOffsetOnDragStart : false;
			...
			function scalePosition( position, scaleVal ){
				return position / scaleVal;
			}
			function mouseDownEvent(event){
				...
				if(_handleElement){
					...
					if(data.isPositionFixed){
						...
						var Elem_css = getComputedStyle(data._element);
						_borderVal = {
							left : parseFloat(Elem_css.borderLeft.split(' ')[0]),
							right : parseFloat(Elem_css.borderRight.split(' ')[0]),
							top : parseFloat(Elem_css.borderTop.split(' ')[0]),
							bottom : parseFloat(Elem_css.borderBottom.split(' ')[0])
						}
					}
					...
					var returnVal = true;
					data.transformdiff = 0; 
					if(cs.transform !== 'none' && data.istransform){
						var transform = cs.transform;
						elem.style.transform = 'none';
						var originalOffset = elem.getBoundingClientRect();
						elem.style.transform = transform;
						data.transformdiff = elemOffset.top - originalOffset.top;
					}
					data._element.modifyScaleValue = function(scale){
						data.scale = scale;
						data._element._draggableData.scale = scale;
					}
					//Callback fired
					if(data.onStart){
			...
			function initializeDragEvents (elem,data,_handleElement,event){
				...
				_initialPos = {
					...
					if(event.type == "touchstart"){
						...
						_lyteUiUtils.addEventListenerGlobal('touchmove',mouseMoveEvent, true);
						_lyteUiUtils.addEventListenerGlobal('touchend',mouseUpEvent, true);
					...
					if(event.type == "mousedown"){
						_lyteUiUtils.addEventListenerGlobal('mousemove',mouseMoveEvent);
						_lyteUiUtils.addEventListenerGlobal('mouseup',mouseUpEvent);
					...
					if(data.userSelect && document.body){
						document.body.classList.add('lyteDraggableBodyWrapper');
					}
			}
				...
				if(_handleElement){
					...
					if(data && data._isDown){
						...
						data._element.addScrollDivXY =  function(scrollDivX,scrollDivY){
							var scrollDivX = scrollDivX ? (typeof scrollDivX == "string" ? document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ](scrollDivX) : scrollDivX) : undefined;
							var scrollDivY = scrollDivY ? (typeof scrollDivY == "string" ? document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ](scrollDivY) : scrollDivY) : undefined;
						...
						if(!data._isMoved){
							...
							if(typeof data.helper == "string"){
								if(data.helper == "clone"){
									...
									helper.modifyScaleValue = data._element.modifyScaleValue;
								...
								if(helper){
									...
									var elem_offset = elem.getBoundingClientRect();
									elemOffset.width = elem_offset.width;
									elemOffset.height = elem_offset.height;
									elem._draggableData.helperElem = elem;
								}else if(data.DragElemAppendTo){
									Lyte.Component.appendChild($L(data.appendTo)[0],elem);
									parent = data._offParent = elem.offsetParent;
									parentOffset = parent.getBoundingClientRect();
								}
							}
							...
							if(data.onDragStart){
								...
								if(data.reCalulateOffsetOnDragStart){
							...
							if(!data.staticDimensions){
								elem.style.width = calculateWidth(elem,elemOffset) / data.scale.x +'px';
								elem.style.height = calculateHeight(elem,elemOffset) / data.scale.y +'px';
							}
							elem.style.top = getTop( data, elemOffset.top, parentOffset, _marginTop, _scrollTop) + 'px';
							elem.style.left = getLeft( data, elemOffset.left, parentOffset, _marginLeft, _scrollLeft) + 'px';
							if(data.scale.x){
								elem.style.marginTop = scalePosition(parseInt(_marginTop) ,data.scale.x)  + 'px';
							}
							if(data.scale.y){
								elem.style.marginLeft = scalePosition(parseInt(_marginLeft) ,data.scale.y)  + 'px';
							}
						...
						if(returnVal){
							...
							if(orientation === "vertical"){
								elem.style.top = getTop(data, data._mousePosition.y, parentOffset, _offset, _marginTop, _scrollTop) + 'px';
								if(data.scrollDivY){
								...
								elem.style.left = getLeft( data, data._mousePosition.x, parentOffset, _offset, _marginLeft, _scrollLeft ) + 'px';
								if(data.scrollDivX){
								...
								elem.style.left = getLeft( data, data._mousePosition.x, parentOffset, _offset, _marginLeft, _scrollLeft ) + 'px';
								elem.style.top = getTop(data, data._mousePosition.y, parentOffset, _offset, _marginTop, 0)  + 'px';
								elemOffset = elem.getBoundingClientRect();
								if(data.scrollDivY){
									var scrollDivOffset = data.scrollDivY.getBoundingClientRect();
									if((elemOffset.left <= scrollDivOffset.right) && (elemOffset.right >= scrollDivOffset.left)){
								...
								if(data.connectToSortable){
									...
									if(droppablePlace && droppablePlace.parentElement._sortableParentData.droppable /* && checkDroppable(droppablePlace,_sortableElem.parentElement,_sortableElem,data.connectedWith,data.containmentDimensions,_mousePosition)*/){
										...
										if(!checkPlaceholderBelow(_elemBelow) && !($L(_elemBelow).hasClass('sortable-parent') && checkParentDroppable(_elemBelow,data)) && _placeholder && _placeholder.parentElement && !_placeholder.parentElement.contains(_elemBelow)){
							...
							if(data.hasContainment && returnVal){
								...
								if( !data.isPositionFixed && (cs.position === 'relative' || cs.position === 'absolute' || cs.position === 'fixed'  || cs.position === 'static') ){
								...
								if(elem.getBoundingClientRect().top <= top){
									elem.style.top = scalePosition(top - offsetTop  + _borderVal.top ,data.scale.y) + scrollTop + 'px';
								}
								if(elem.getBoundingClientRect().bottom >= (top + height) && !data.restrictLeftTopOnly){
									elem.style.top = scalePosition((top + height) - offsetTop - ElemRect.height + _borderVal.bottom ,data.scale.y) + scrollTop + 'px';
								}
								if((elem.getBoundingClientRect().top + minheight)  >= (top + height) && data.restrictLeftTopOnly){
									elem.style.top = scalePosition((top + height - minheight) - offsetTop + _borderVal.bottom ,data.scale.y) + scrollTop + 'px';
								}
								if(elem.getBoundingClientRect().left <= left){															
									elem.style.left = scalePosition(left + _borderVal.left - offsetLeft,data.scale.x) + 'px';
								}
								if(elem.getBoundingClientRect().right >= (left + width) && !data.restrictLeftTopOnly){
									elem.style.left = scalePosition((left - offsetLeft + width) - ElemRect.width + _borderVal.right ,data.scale.x) + scrollLeft + 'px';
								}
								if((elem.getBoundingClientRect().left + minWidth)  >= (left + width) && data.restrictLeftTopOnly){
									elem.style.left = scalePosition((left  + width - minheight) - offsetLeft + _borderVal.left,data.scale.x) + 'px';
								}
							}
						}
			...
			var getTop = function( data, elemTop, parentOffset, _offset, _marginTop, _scrollTop ){
				var top = 0;
				if( data.isPositionFixed ){
					top = scalePosition( elemTop - data.transformdiff - _offset[1] - parseInt(_marginTop), data.scale.y );
				}else{
					top = scalePosition( elemTop - data.transformdiff - _offset[1] - parentOffset.top - parseInt(_marginTop), data.scale.y ) + _scrollTop; 
				}
				return top;
			}
			var getLeft = function( data, elemLeft, parentOffset, _offset, _marginLeft, _scrollLeft ){
				var left = 0;
				if( data.isPositionFixed ){
					left = scalePosition( elemLeft - data.transformdiff - _offset[0] - parseInt(_marginLeft), data.scale.x );
				}else{
					left = scalePosition( elemLeft - data.transformdiff - _offset[0] - parentOffset.left - parseInt(_marginLeft), data.scale.x ) + _scrollLeft;
				}
				return left;
			}
			...
			var mouseUpEvent = function(event){
				...
				if(document.body){
					document.body.classList.remove('lyteDraggableBodyWrapper');
				}
				// console.log(event.type, event.target);
				// console.log("calling mouseup from draggable");
				// event.preventDefault();
				// _handleElement = event.target.closest('.selected-element') ? event.target.closest('.selected-element') : document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ]('.selected-element');
				// var target = event.target;
				// _handleElement = null;
				// while(target && target != document){
				// 	if($L(target).hasClass("selected-element")){
				...
				if(event.type == "mouseup"){
					_lyteUiUtils.removeEventListenerGlobal('mousemove',mouseMoveEvent);
					_lyteUiUtils.removeEventListenerGlobal('mouseup',mouseUpEvent);
				}
				//Unbinding touch events
				if(event.type == "touchend"){
					_lyteUiUtils.removeEventListenerGlobal('touchmove',mouseMoveEvent, true);
					_lyteUiUtils.removeEventListenerGlobal('touchend',mouseUpEvent, true);
				}
						...
						if(data._isMoved){
							...
							if(!returnVal){
								...
								if(_initialPos.pos === "absolute"){
									elem.style.left = scalePosition(_initialPos.x,data.scale.x) + "px";
									elem.style.top = scalePosition(_initialPos.y,data.scale.y) + "px";
								}
								else{
									elem.style.left = "";
									elem.style.top = "";
			...
			var onStop = function(data,event){
				data._element.classList.add('lyteDraggableHandlePointerEventsNone');
				if(event.type == "mouseup"){
				...
				data._element.classList.remove('lyteDraggableHandlePointerEventsNone');
			...
			var keyDownEvent = function(event){
				...
				if(handle.classList.contains('selected-element')){
					if(event.keyCode == 39){
						event.clientX = handle.getBoundingClientRect().left + data.keyNavSpeed;
						...
						event.clientY = handle.getBoundingClientRect().top - data.keyNavSpeed;
						mouseMoveEvent(event);
			...
			var calculateHeight = function(element,elementOffset) {
				...
				var elementHeight = elementOffset.height - paddingY - borderY;
				return elementHeight;
			};

			var calculateWidth = function(element,elementOffset) {
				var cs = getComputedStyle(element);
				if(cs.boxSizing == 'content-box'){
				...
				var elementWidth = elementOffset.width - paddingX - borderX;
				return elementWidth;
			};
			...
			var callForScrollY = function(data,scrollDiv,parentOffset,_maxScrollHeight,isRelativeY,_mousePosition,_offset,parent,event){
				...
				var height = scrollDiv !== document.documentElement ? data.scrollDivY.offsetHeight : data.scrollDivY.clientHeight;
				if(!data.preventScrollHeightCalculation){
					_maxScrollHeight = data.scrollDivY.scrollHeight - height;
				}
				...
				if( !isNegativeScroll ){
					if((divOffset.top - parseInt(data._marginTop) <= scrollDivTop ) && (scrollTop > 0)){
						data.enableScrollAniamtion = true;
						if(isRelativeY){
							data._element.style.top = scalePosition(_mousePosition.y - _offset[1] - parent.top  ,data.scale.y) - (_maxScrollHeight - scrollTop >= data.scrollSpeed ? data.scrollSpeed : _maxScrollHeight - scrollTop) + scrollTop + "px";
						}
						else if(isWindowScroll(scrollDiv) && !data.isPositionFixed){
							data._element.style.top = scalePosition(data._element.offsetTop ,data.scale.y) - (_maxScrollHeight - scrollTop >= data.scrollSpeed ? data.scrollSpeed : _maxScrollHeight ) - scrollTop  + 'px';
						}
						...
						if(managerDD._scroll){
							managerDD._scroll(scrollDiv, _mousePosition, event);
						}
					}
						...
						if(_maxScrollHeight - scrollTop > data.scrollSpeed){
							data.enableScrollAniamtion = true;
							if(isRelativeY){
								data._element.style.top = scalePosition(_mousePosition.y - _offset[1] - parent.top  ,data.scale.y) + data.scrollSpeed + scrollTop + "px";
							}
							else if(isWindowScroll(scrollDiv) && !data.isPositionFixed){
								data._element.style.top = scalePosition(data._element.offsetTop ,data.scale.y) + data.scrollSpeed + 'px';
							}
							scrollDiv.scrollTop += data.scrollSpeed;
                            if(data.onScroll){
                                data.onScroll(data._element,scrollDiv,_mousePosition);
							...
							data.enableScrollAniamtion = true;
							if(isRelativeY){
								data._element.style.top = scalePosition(_mousePosition.y - _offset[1] - parent.top ,data.scale.y) + _maxScrollHeight - scrollTop + "px";
							}else if(isWindowScroll(scrollDiv) && !data.isPositionFixed){
								data._element.style.top = scalePosition(data._element.offsetTop  ,data.scale.y) + (_maxScrollHeight - scrollTop )    + 'px';
							}
						...
						if( (scrollTop == 0 || scrollTop == _maxScrollHeight) && data.enableScrollAniamtion ){
							if( data.onAnimationCancel ){
								data.onAnimationCancel(data._element,scrollDiv,_mousePosition,event);
							}
							if(managerDD._scrollAniamtionEnd){
								managerDD._scrollAniamtionEnd(event, _mousePosition)
							}
							data.enableScrollAniamtion = false;
						}
						cancelAnimationFrame(_requestId1);
						_animationFrameFired1 = false;
						_requestId1 = null;
					...
					if((divOffset.top - parseInt(data._marginTop) <= scrollDivTop ) && (scrollTop > -(_maxScrollHeight))){
						...
						if(_maxScrollHeight - scrollTop > data.scrollSpeed){
							if(isRelativeY){
								...
								data._element.style.top =  getTop(data, _mousePosition.y, parent.top, scrollTop)  + "px";
							}
			...
			var callForScrollX = function(data,parentOffset,_maxScrollWidth,_mousePosition,_offset,parent,isRelative,event){
                ...
                if( ( !isNegativeScroll && !_lyteUiUtils.getRTL() )|| (!_lyteUiUtils.isNegativeScroll() && _lyteUiUtils.getRTL() && detectBrowser() == "chrome" && !isRelative) ){
                    if((divOffset.right >= parentOffset.right - 2) && (_scrollLeft < _maxScrollWidth)){
                        if((_maxScrollWidth - _scrollLeft) >= data.scrollSpeed){
                            ...
                            if(isRelative){
                                data._element.style.left = scalePosition(_mousePosition.x - _offset[0] - parent.left  ,data.scale.x) + data.scrollSpeed + _scrollLeft + "px";
                            }
                            if(data.bindDroppable){
                                managerDD._drag(event);
                            }
                            if(managerDD._scroll){
                                managerDD._scroll(data.scrollDivX, _mousePosition, event);
                            }
                            if(data.onScroll){
                            ...
                            if(isRelative){
                                data._element.style.left = scalePosition(_mousePosition.x - _offset[0] - parent.left ,data.scale.x) + (data.scrollSpeed - (_maxScrollWidth - _scrollLeft)) + _scrollLeft + "px";
                            }
                            if(data.bindDroppable){
                                managerDD._drag(event);
                            }
                        ...
                        if(isRelative){
                            data._element.style.left = scalePosition(_mousePosition.x - _offset[0] - parent.left  ,data.scale.x) - ((_maxScrollWidth - _scrollLeft) > data.scrollSpeed ? data.scrollSpeed : (data.scrollSpeed - (_maxScrollWidth - _scrollLeft))) + _scrollLeft + "px";
                        }
                        if(data.bindDroppable){
                            managerDD._drag(event);
                        }
                    ...
                    if( (!_lyteUiUtils.isNegativeScroll() || !isNegativeScroll) && detectBrowser() == "chrome" && isRelative){
                        if((divOffset.right >= parentOffset.right) && (_scrollLeft < _maxScrollWidth)){
                            if((_maxScrollWidth - _scrollLeft) >= 5){
                                ...
                                if(isRelative){
                                    data._element.style.left = scalePosition(_mousePosition.x - _offset[0] - parent.left ,data.scale.x) + 5 + (_scrollLeft - _maxScrollWidth) + "px";
                                }
                                if(data.bindDroppable){
                                    managerDD._drag(event);
                                }
                                ...
                                if(isRelative){
                                    data._element.style.left = scalePosition(_mousePosition.x - _offset[0] - parent.left ,data.scale.x) + (5 - (_maxScrollWidth - _scrollLeft)) + _scrollLeft + "px";
                                }
                                if(data.bindDroppable){
                                    managerDD._drag(event);
                                }
                            ...
                            if(isRelative){
                                data._element.style.left = scalePosition(_mousePosition.x - _offset[0] - parent.left ,data.scale.x) - ((_maxScrollWidth - _scrollLeft) + 5) + "px";
                            }
                            data.scrollDivX.scrollLeft -= 5;
                            if(data.bindDroppable){
                                managerDD._drag(event);
                    ...
                    else if(((_lyteUiUtils.isNegativeScroll() || isNegativeScroll) && detectBrowser() == "chrome") || detectBrowser() == "firefox" || detectBrowser() == "safari"){
                        if((divOffset.right >= parentOffset.right) && (_scrollLeft < 0)){
                            if((_maxScrollWidth + _scrollLeft) >= 5){
                                data.scrollDivX.scrollLeft += 5;
                                if(isRelative){
                                    data._element.style.left = scalePosition(_mousePosition.x - _offset[0] - parent.left ,data.scale.x) + 5 + _scrollLeft + "px";
                                }
                            }
                            else{
                                data.scrollDivX.scrollLeft += (5 - (_maxScrollWidth + _scrollLeft));
