...
;(function(){
    ...
    _lyteUiUtils.addGlobalEventListener('keydown', keyDownHandler);
    _lyteUiUtils.addGlobalEventListener('mousemove', mouseMoveHandler );
    ...
    function findValidFirstElem( configurationObject ){
        ...
        var firstElem=listOfChildren[0];
    ...
    function getRelativeOffsetTop( elem, parent ){
        if( !elem || !parent ){
            return 0;
        }
        var elemRect = elem.getBoundingClientRect();
        var parentRect = parent.getBoundingClientRect();
        return elemRect.top - parentRect.top + parent.scrollTop;
    }
    ...
    function checkViewScroll( elem, parent ){
        ...
        var elemTop = getRelativeOffsetTop( elem, parent );
        var elemBottom = elemTop + elem.offsetHeight;

        if( elemTop < parent.scrollTop ){
            parent.scrollTop = elemTop;
        }
        else if( elemBottom > parent.scrollTop + parent.offsetHeight ){
            parent.scrollTop = elemBottom - parent.offsetHeight;
        }
    }
    ...
    function inView( elem, parent ){
        ...
        var elemTop = getRelativeOffsetTop( elem, parent );
        var elemBottom = elemTop + elem.offsetHeight;
        var parentBottom = parent.scrollTop + parent.offsetHeight;

        if( elemTop >= parent.scrollTop && elemBottom <= parentBottom ){
            return true;
        }
