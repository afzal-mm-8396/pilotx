Lyte.Mixin.register( 'lyte-table-utils',{
        ...
        if( __target == this.scrollDiv ){
            ...
            if( __intersections.length  ){
                ...
                $L.fastdom.measure( function(){
                    var scrollTop = __target.__$scrollTop == void 0 ? __target.scrollTop : __target.__$scrollTop,
                    scrollLeft =  __target.__$scrollLeft == void 0 ?  __target.scrollLeft : __target.__$scrollLeft;
    ...
    createFakeColum : function( cell ){
        ...
        _lyteUiUtils.appendChild( __doc.body, originaldiv, { referenceNode : this.$node } );
        this._moved = true;
    },
    ...
    sorthorizontalscroll : function( bcr, scrollLeft, isIncrement ){
        ...
        isLeft = !isIncrement && ( this.rtlfunc( 'left', bcr, ww ) < Math.max( (this._fixedWidth || 0) + this.rtlfunc( 'left', _scrolldivBcr, ww ), 0 ) ),
            ...
            if( scrollIncrement ){
                ...
                this._measure = fastdom.measure( () => {
                    ...
                    this._reqId = fastdom.mutate( () => {
                        delete this._reqId;
                        this.sortmousemove( { clientX : this._prevx, frmScroll : true }, true, isRight );
                    });
                });
            }
         }
    ...
    swapColumnsInDom : function( startIndex, endIndex ){
        ...
        rows.forEach( row => {
            Lyte.Component[ startIndex < endIndex ? 'insertAfter' : 'insertBefore' ]( this.get_nth_cell( row, endIndex ), this.get_nth_cell( row, startIndex ) );
        });
    ...
    sortmouseup : function( evt ){
        ...
        if( this.isSortEnabled() ){
            ...
            ( this.data.ltPropRole || this.data.ltPropAria ) && selectedCell.focus({
                preventScroll : true
            });
        }
    ...
    removeIntersection : function( intersection, table ){
        ...
        if( table && table.getData( 'fromListView' ) ){
            ...
            Lyte.objectUtils( status, 'delete', parseInt( intersection._cell.getAttribute( 'index' ) ) );
        }
    ...
    destroy_map : function(){
        ...
        if( __data.ltPropCellIntersection ){
            ...
            // if( force_reset ){
            //     map.forEach( function( __, cell ){
            //         cell.parentNode.setAttribute( 'lyte-intersection-ignore', '' );
            //         if( cell.__children ){
            ...
            //             frag.append.apply( frag, cell.__children );
            ...
            // } else {
                ...
                is_cell = this.data.ltPropIntersectionType == "cell";
                ...
                if( is_cell ){
                    map.forEach( function( _, cell ){
                        // if( !cell.isConnected ){
                            var row = cell.parentNode;

                            row && row.setAttribute( 'lyte-intersection-ignore', '' );

                            if( cell.__children ){
                                var frag = new DocumentFragment();
                                frag.append.apply( frag, cell.__children );
                                ( cell.isConnected ? cell : fake_div ).appendChild( frag );
                            }
                            delete cell.__children;
                            delete cell.__dummy;
                        // }
                    ...
                    map.forEach( function( value, row ){
                        row.setAttribute( 'lyte-intersection-ignore', '' );
                        row.innerHTML = '';
    ...
    cell_intersection : function( ints ){
        ...
        if( is_cell ){
            ...
            ints.forEach( function( item ){
                ...
                if( !row || row.hasAttribute( 'lyte-intersection-ignore' ) ){
                    return;
                }
                ...
                if( is_visible ){
                    if( cell.__children ){
                        ...
                        Lyte.objectUtils( row_visible, 'add', cell_index, is_visible );
                    }
                    __map.set( cell, cell.__dummy )
                } else {
                    if( row_visible[ cell_index ] ){
                        ...
                        cell.appendChild( __frag );
                        __map.set( cell, cell.__children );
                    } 
                }
            }); 
            Lyte.Component._setIgnoreDisconnect( false );
        } else {
            this.row_intersection( ints );
        }
    },

    reset_visible : function(){
        var __obj = {},
        __data = this.data;
        ...
        this.setData( 'visibleStatus', __obj );

        if(  this.__cell_inter ){
            this.__cell_inter.disconnect();
            
            if( this.data.ltPropIntersectionType == "cell" ){
                Array.from( this.$node.getElementsByTagName( 'lyte-th') ).forEach( function( item ){
                    this.bindIntersection( item );
                }.bind( this ) );
            }
        }
    },

    visible_obs : function( arg ){
        ...
        if( this.data.ltPropIntersectionType == "cell" ){
            if( removed_len ){
                for( var i = removed_len - 1; i >= 0; i-- ){
                    ...
                    Lyte.objectUtils( visibleStatus, 'delete', cur_index );
                    
                    for( var j = cur_index; j < len; j++ ){
                        Lyte.objectUtils( visibleStatus, 'add', j, visibleStatus[ j + 1 ] );
                    }
                    Lyte.objectUtils( visibleStatus, 'delete', len - 1 );
                }
            }

            if( inserted_len ){
                var tbody_rows = this.getTbody()[ 0 ].children;

                for( var i = 0; i < inserted_len; i++ ){
                    var cur_index = __index + i;
                    for( var j = cur_index; j < len; j++ ){
                        Lyte.objectUtils( visibleStatus, 'add', j + 1, visibleStatus[ j ] );
                    }
                    Lyte.objectUtils( visibleStatus, 'add', cur_index, {} );
                    this.bind_row_inter( tbody_rows[ cur_index ] );
                }
            }
        }
    ...
    removeCellInter : function( index ){
    ...
    enable_inter : function( row ){
        this.toggle_inter( row, true );
    },

    disable_inter : function( row ){    
        this.toggle_inter( row, false );
    },

    toggle_inter : function( row, enable ){
        var cells = Array.from( row.children ),
        __attr = 'lyte-intersection-ignore',
        row_index = Array.from( row.parentNode.children ).indexOf( row ),
        visibleStatus = this.data.visibleStatus[ row_index ],
        len = cells.length,
        __fn = function( value ){
            for( var i = 0; i < len; i++ ){
               Lyte.objectUtils( visibleStatus, 'add', i, value );
            }
        },
        is_row_inter = this.data.ltPropIntersectionType == "row";

        if( enable ){
            row.removeAttribute( __attr );
        }

         var __map = cells.map( function( cell ){
            return { 
                target : is_row_inter ? row : cell, 
                intersectionRatio : 1
            };
        });
        
        this.cell_intersection( __map );

        if( !enable ){
            /**
             * this is purly for gowtham.mp@zohocorp.com's case[ CRM SubForm ]. He is editing a row and expects both original and dummy content needs to be updated.
             * So i am forcefully setting the visible status to false for rendering the dummy content.
             * When editing is done dummy content will be updated and visible status will be set to true to render the original content
             */
            row.setAttribute( __attr, "" );
            !is_row_inter && __fn( false );
        }
    },

    header_obs : function(){
        this.reset_visible();
    }.observes( 'ltPropCellIntersection' ),

    add_row_inter : function( row ){
        if( this.data.ltPropCellIntersection ){
            this.__cell_inter.observe( row );
            Lyte.objectUtils( this.data.visibleStatus, 'add', Array.from( row.parentNode.children ).indexOf( row ), {} );
        }
    },

    remove_row_inter : function( row ){
        if( this.data.ltPropCellIntersection ){
            this.__cell_inter.unobserve( row );
            // Lyte.objectUtils( this.data.visibleStatus, 'removeAt', Array.from( row.parentNode.children ).indexOf( row ) );
        }
    },

    render_row : function( row, row_visible ){
        if( !row ){
            return;
        }

        var already_rendered = row_visible && Object.keys( row_visible ).length;
        ...
        if( !already_rendered ){
            ...
            cells.forEach( function( _, index ){
                Lyte.objectUtils( row_visible, 'add', index, true );
            });
        }
    },

    restore_row : function( row ){
        var  map = this.__map,
        cont = map.get( row );

        $L.fastdom.clear( row.__lazy_fast );
        delete row.__lazy_fast;

        row.classList.remove( 'lyteRowIntersectionPlaceholder' );
        if( cont ){
            row.innerHTML = '';
            Lyte.Component._setIgnoreDisconnect( true );
            ...
            window.cancelAnimationFrame( this.__lazy_scroll_raf );
            this.__lazy_scroll_raf = window.requestAnimationFrame( function(){
                this.$node[ this.data.ltPropStickyTable ? 'reset' : 'scrollTable' ]();
            }.bind( this ) );
        }
    },

    hide_row : function( row ){
        if( !row || row.__lazy_visible_status ){
            return;
        }
        ...
        if( exst || row.__lazy_fast ){
            return;
        }

        var frag = new DocumentFragment(),
        __child = Array.from( row.childNodes ),
        len = row.children.length,
        td = document.createElement( 'td' );

        td.classList.add( 'lyteIntersectionPlaceholderCell' );
        td.colSpan = len;
        row.classList.add( 'lyteRowIntersectionPlaceholder' );

        row.__lazy_fast = $L.fastdom.measure( function(){
    ...
    row_intersection : function( ints ){
        var visibleStatus = this.data.visibleStatus,
        buffer_dist = this.data.ltPropIntersectionBuffer,
        __first = Infinity,
        __last = -Infinity,
        __rows = [];
        ...
        ints.forEach( function( item ){
            var row = item.target, 
            is_visible = item.intersectionRatio >= 0.01,
            rows = __rows = row.parentNode.children;
            row_index = Array.from( rows ).indexOf( row );

            if( row.hasAttribute( 'lyte-intersection-ignore' ) ){
                return;
            }

            row.__lazy_visible_status = is_visible;

            if( is_visible ){                
                for( var i = -buffer_dist; i < buffer_dist; i++ ){
                    var __index = row_index + i;
                    this.render_row( rows[ __index ], visibleStatus[ __index ] );
                }

                __first = Math.min( __first, row_index );
                __last = Math.max( __last, row_index );
            } else {
                var bcr = item.boundingClientRect,
                root_bcr = item.rootBounds,
                __fact = 1;
               
                if( bcr.top > root_bcr.top ){
                    __fact = -1;
                }

                while( true ){
                    var other = __rows[ row_index + __fact ];
                    if( other ){
                        if( other.__lazy_visible_status ){
                            row_index += __fact;
                            __first = Math.min( __first, row_index );
                            __last = Math.max( __last, row_index );
                            continue;
        ...
        while( true ){
            var __prev = __rows[ __first - 1 ];

            if( __prev ){
                if( __prev.__lazy_visible_status ){
                    __first--;
                    continue;
                }
            }
        ...
        while( true ){
            var __next = __rows[ __last + 1 ];

            if( __next ){
                if( __next.__lazy_visible_status ){
                    __last++;
                    continue;
                }
            }
        ...
        if( __first != Infinity ){
            for( var i = __first - buffer_dist; i >= 0; i-- ){
                this.hide_row( __rows[ i ], visibleStatus[ i ] );
            }    
        }

        if( __last != -Infinity ){
            var __len = __rows.length;
            for( var i = __last + buffer_dist; i < __len; i++ ){
                this.hide_row( __rows[ i ], visibleStatus[ i ] );
            }
        }
    }
...
Lyte.createCustomElement( "lyte-tr", {
		static: {
			observedAttributes : {
        ...
        connectedCallback: function(){
            this.connected = true;
            var table = this.closest( 'lyte-table' );

            if( !table || this.parentNode.tagName == "LYTE-THEAD" || table.ltProp( 'intersectionType' ) != "row" ){
                return;
            }

            this.__table = table.component;
            
            this.__raf = window.requestAnimationFrame( this.__table.add_row_inter.bind( this.__table, this ) );
        },
        ...
        disConnectedCallback: function(){
            ...
            if( table ){
                delete this.__table;
                table.remove_row_inter( this );
                $L.fastdom.clear( this.__lazy_fast );
                window.cancelAnimationFrame( this.__raf );
                delete this.__raf;
                delete this.__lazy_fast;
            }
        }
});
