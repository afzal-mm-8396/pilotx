;Lyte.Mixin.register( 'lyte-connect-group-sort',{
	...
	check_drop_grp : function( elem, evt ){
		...
		if( match ){
			...
			if( $match.hasClass( 'lyteConnectGroupShape' ) ){
				ret = this.check_other_grp_merge( elem, match, evt ) == false ? false : true;
			} else {
	...
	check_other_grp_merge : function( elem, match, evt ){
		...
		if( $L( elem ).hasClass( 'lyteConnectInnerItem' ) ){
			...
			if( this.getMethods( 'onBeforeInsertIntoGroup' ) && this.executeMethod( 'onBeforeInsertIntoGroup', elem, match, this.$node ) == false ){
				return false;
			}
		...
		bind_sortable : function( $node ){
			...
			__$node.sortable({
				...
				onSelect : function( element, index, grp, evt ){
					...
					if( this.data.ltPropLockGroupItems || __target.closest( '.lyteConnectInnerItem' ).length == 0 || __target.hasClass( 'lyteConnectAnchorPoint' ) ){
						return false;
					}
				...
				onBeforeDrop : function( src, target, grp, src_index, target_index, src_grp, target_grp, evt ){
					...
					if( __target == __wrapper || __target == this.$node || __target.closest('.lyteConnectionMarker') ){
