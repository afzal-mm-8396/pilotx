;Lyte.Mixin.register( 'lyte-connect-undo', {
	...
	do_modifierChange : function( last, frm_undo ){
		setTimeout( function(){
	...
	do_insertField : function( last, frm_undo ){
		if( frm_undo ){
			this.do_deleteField( last );
		} else {
			var data = this.parse( last.data );
			this.$node.insertField(last.parentId, data, last.index );
		}
	},

	do_deleteField : function( last, frm_undo ){
		if( frm_undo ){
			this.do_insertField( last );
		} else{
			var data = this.parse( last.data );
			this.$node.deleteField( data.id, last.parentId );
		}
	},
	...
	do_updateConnectionOrigin: function ( last, frm_undo ) {
		var parsed = this.parse( last.data );
		this.$node.updateConnectionOrigin( parsed.connector, frm_undo ? parsed.oldSrc : parsed.src, frm_undo ? parsed.oldTrg : parsed.trg );
	},

	do_insertSequentialElement : function( last, frm_undo ){
		if( frm_undo ){
			this.do_removeSequentialElement( last );
		} else {
			var data = this.parse( last.data ),
				location = this.getSequentialElemConn(data.position, true);
			this.$node.insertSequentialItem( data.data, {parentData: location.data, index: location.position.ind, branchIndex: location.position.groupInd} );
		}
	},

	do_removeSequentialElement : function( last, frm_undo ){
		if( frm_undo ){
			this.do_insertSequentialElement( last );
		} else {
			var data = this.parse( last.data );
			this.$node.deleteSequentialItem( this.getSequentialElemConn(data.position).data );
		}
	},
