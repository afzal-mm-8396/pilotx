...
_template:"<template tag-name=\"lyte-tooltip\" lyte-tooltip=\"\"> </template>",
_dynamicNodes : [],
_observedAttributes :["ltPropId","ltPropClass","ltPropKeepAlive","ltPropTooltipConfig","ltPropIgnoreClass","ltPropAria"],
_observedAttributesType :["string","string","boolean","object","string","boolean"],

  _lyteUtilFunctions : [ "trigger" ],
  init : function(){
  ...
  data : function(){
    ...
    return {
       ...
       * @input
       */
      ltPropKeepAlive : Lyte.attr('boolean', {default : default_values.keepAlive || false, input : true }),
      ...
      ltPropIgnoreClass : Lyte.attr( 'string', { default : default_values.ignoreClass || "lyteTooltipIgnore", input : true } ),
  ...
  focusev : function( evt ){
    ...
    __target = evt.target.correspondingElement || evt.target;
     
    fastdom.clear( __target.toolfdm );
      __target.toolfdm = fastdom.mutate( function(){
          delete __target.toolfdm;
          this.$node.trigger( evt.type == "focus" ? this.get_target( evt ) : document.body );
      }.bind( this ) );
    },
  ...
  didConnect : function(){
      ...
      document[ ns ]( 'mousemove', this._mousemove, true ); 

      if( _lyteUiUtils.isWidget ){
          _lyteUiUtils.addGlobalEventListener( 'scroll', this._toolscroll , true );
      } else {
          window.addEventListener('scroll', this._toolscroll , true);
      }
      
      document[ ns ]( 'keydown', this._keydown, true);
      document[ ns ]( 'mousedown', this._mousedown, true );
      document[ ns ]( 'touchstart', this._mousedown );
  ...
  bind_aria : function(){
         ...
         [ 'focus', 'blur' ].forEach( function( item ){
            document[ ns ]( item, this._focusev, true );
         }.bind( this ) );
  },
  ...
  mousedown : function( evt ){
      ...
      var target = this.get_target( evt ),
      $target = $L( target ),
      ...
      if( target.closest && target.closest( '[draggable=""],[draggable="true"]' ) ){
         delete this._mousedownFlag;
         this.__dragging = true;
      } else {
        delete this.__dragging;
         document[ "addEventListener" + ( _lyteUiUtils.isWidget ? "Global" : "" ) ]( isTch ? 'touchend' : 'mouseup', this._mouseup, true );
      }
  },

  mouseup : function( evt ){
      delete this._mousedownFlag;
      document[ "removeEventListener" + ( _lyteUiUtils.isWidget ? "Global" : "" ) ]( evt.type, this._mouseup, true );
  ...
  didDestroy : function(){
        ...
        if( _lyteUiUtils.isWidget ){
          _lyteUiUtils.removeGlobalEventListener( 'scroll', this._toolscroll , true );
        } else {
  ...
  _followcursor : function( evt, span ){
      if( !this.__tooltips[ span._random ] ){
        return;
      }
  ...
  nonFollowcursor : function(event, span, position, prevent){
        ...
        if( !span._callbackHandled ) {
         ...
         $L.fastdom.measure(function(){
            ...
            span_node = span.nodeName1,
            __boundary = __config.boundary,
            boundary_elem = typeof __boundary == 'string' ? span_node.closest( __boundary ) : void 0,
            __innerWidth = window.innerWidth,
            __innerHeight = window.innerHeight,
            boundary_bcr = boundary_elem ? boundary_elem.getBoundingClientRect() : { left : 0, top : 0, right : __innerWidth, width : __innerWidth, height : __innerHeight },
            ...
            innWidth = boundary_bcr.width,  
            innHeight= boundary_bcr.height,
            spanClientRect = span.getBoundingClientRect(),
            nodeClientRect = span_node.getBoundingClientRect(),
            left = this.rtlfunc( 'left', nodeClientRect, __innerWidth ) + xscroll,
            topPos = nodeClientRect.top + yscroll,
            margin = Math.min( parseInt( __config.margin ), 20 ),
            toolwid = span.offsetWidth,
            tool_hgt = span.offsetHeight,
            divWidth = nodeClientRect.width,
            wid = nodeClientRect.height,
            lt = this.rtlfunc( 'left' ),
            sty = getComputedStyle( span ),
            ...
            xscroll += Math.max( 0, this.rtlfunc( "left", boundary_bcr, __innerWidth ) );
            yscroll += Math.max( 0, boundary_bcr.top );

            if( appearance ) {
                calloutstyle = getComputedStyle( span, ':before' );
                ddct = parseFloat( calloutstyle.getPropertyValue( 'width' ) ) / 2 * 1.414;
                if( isNaN( ddct ) || !ddct ) {
                    ddct = parseFloat( getComputedStyle( span, position == "left" ? ":before" : ":after" ).getPropertyValue( 'border-left-width' ) )
                }
            } else {
                ddct = 0;
            }
                ...
                case 'bottom' :
                  {
                    ...
                    newLeft = this.horicheck( newLeft, xscroll, innWidth, spanClientRect, span, nodeClientRect, lt, ddct, appearance, this.rtlfunc( "left", boundary_bcr, __innerWidth ), __innerWidth );
                    span.style[ lt ] = newLeft + 'px';
...
horicheck : function( newLeft, xscroll, innWidth, spanClientRect, span, nodeClientRect, lt, ddct, appearance, __left, __innerWidth ){
  ...
  var flag, flag2
    if( newLeft < xscroll ){
     ...
     if( ( flag || flag2 ) && appearance ){
       var inn = this.createsupp( span ), 
       nodeleft = this.rtlfunc( 'left', nodeClientRect, __innerWidth ),
       leftVal;
       if( flag ){
          var lft = this.rtlfunc( 'left', nodeClientRect, __innerWidth ) - __left;
          inn.style[ lt ] =  ( Math.min( spanClientRect.width, nodeClientRect.width ) + Math.min( lft , 0 ) )/ 2 + Math.max( 0, lft ) + 'px';
       } else {
          var rgt = innWidth - this.rtlfunc( 'right', nodeClientRect, __innerWidth ) + __left;
...
removeTooltip : function( span ){
      ...
      if( __parent ){
          ...
          delete this.__tooltips[ span._random ];
      }
},
  ...
  get_target : function( evt ){
    var __target = evt.target.correspondingElement || evt.target,
    shadow_root = __target.shadowRoot;

    if( !_lyteUiUtils.isWidget && shadow_root && __target.closest( 'lyte-widget' ) ){
        return __target;
    }

    if( !document[ _lyteUiUtils.isWidget ? "containsGlobal" : "contains" ]( __target ) ){
        __target = window.document.documentElement;
    }

    if( shadow_root && evt.type ){
        try{
          var act_target = shadow_root.elementFromPoint( evt.clientX, evt.clientY );
          return act_target;
        } catch( e ){
          return __target;
        }
  ...
  mousemove : function(event){
        ...
        var nodeName1 = this.get_target( event );
        ...
        while(nodeName1 && nodeName1.tagName != 'BODY' && nodeName1 != document && nodeName1.tagName != 'HTML' ){
            ...
            if( title ){
                  ...
                  if( this.prevTooltipNode != nodeName1 && ( !tooltip.tooltipSpan || is_reuse ) ){
                      ...
                      !is_really_exist && _lyteUiUtils.appendChild( document.body, span, { referenceNode : this.$node } );
...
( function(){
   ...
   __fn = function(){
      ...
      if( !doc.querySelector( name ) ){
          // _lyteUiUtils.appendChild( doc.body, doc.createElement( name ) );
          Lyte.Component.render( "lyte-tooltip", {}, document.body );
      }
   };
