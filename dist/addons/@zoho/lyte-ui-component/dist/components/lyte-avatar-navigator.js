...
_observedAttributesType :["array","string","boolean","boolean","string","string","object","string","string","string","string","string","boolean","array","string","object","boolean","string","array","number","number","string","boolean","array","boolean","boolean"],

	_lyteUtilFunctions: [ 'reset' ],
	data: function() {
		return {
			/**
			...
			"ltPropImages": Lyte.attr( 'array', { 'default': [], 'input': true } ),
			...
			"ltPropUrlValue": Lyte.attr( 'string', { 'default': 'image', 'input': true } ),
			...
			"ltPropAvatarYield": Lyte.attr( 'boolean', { 'default': false, 'input': true } ),
			...
			"ltPropIconYield": Lyte.attr( 'boolean', { 'default': false, 'input': true } ),
			...
			"ltPropImageWidth": Lyte.attr( 'string', { 'default': '0px', 'input': true } ),
			...
			"ltPropSelectedClass": Lyte.attr( 'string', { 'default': _lyteUiUtils.resolveDefaultValue( 'lyte-avatar-navigator', 'selectedClass', 'lyteAvatarSelectedItem' ), 'input': true } ),
			...
			"ltPropPreviousIconClass": Lyte.attr( 'string', { 'default': _lyteUiUtils.resolveDefaultValue( 'lyte-avatar-navigator', 'previousIconClass', 'lyteAvatarPrevIcon' ), 'input': true } ),
			...
			"ltPropNextIconClass": Lyte.attr( 'string', { 'default': _lyteUiUtils.resolveDefaultValue( 'lyte-avatar-navigator', 'nextIconClass', 'lyteAvatarNextIcon' ), 'input': true } ),
			...
			"ltPropAltValue": Lyte.attr( 'string', { 'default': '', 'input': true } ),
			...
			"ltPropAlt": Lyte.attr( 'string', { 'default': _lyteUiUtils.resolveDefaultValue( 'lyte-avatar-navigator', 'alt', '' ), 'input': true } ),

			/**
			 * @componentProperty {boolean} ltPropCyclic
			 * @default true
			 * @input
			 */


			"ltPropCyclic": Lyte.attr( 'boolean', { 'default': _lyteUiUtils.resolveDefaultValue( 'lyte-avatar-navigator', 'cyclic', true ), 'input': true } ),
			...
			"ltPropTooltipValue": Lyte.attr( 'string', { 'default': '', 'input': true } ),
			...
			"ltPropTooltip": Lyte.attr( 'object', { 'default': _lyteUiUtils.resolveDefaultValue( 'lyte-avatar-navigator', 'tooltip', 
				 ...
				 * @input
				 */
   			"ltPropPreload": Lyte.attr( 'boolean', { 'default': _lyteUiUtils.resolveDefaultValue( 'lyte-avatar-navigator', 'preload', false ), 'input': true } ),
			 ...
			 */
   			...
   			"ltPropAriaLabelValue": Lyte.attr( 'string', { 'default': '', 'input': true } ),
	...
	init: function() {
		...
		this.$node.reset = function() {
			...
			that.setImageDimensions().then( function() {
	...
	setup: function() {
		var displayIndex, block, that = this;

		if( !this.isEmpty() ) {
			this.setImageDataType();
			this.setImageDimensions().then( function() {
				that.setImagesPerBlock();
				displayIndex = that.setDisplayIndex();
				that.setContainerWidth();
				block = that.buildImageBlock( displayIndex );
				Lyte.objectUtils( block, 'add', 'translate', 0 );
				that.preloadImages();
	...
	cyclicObserver: function() {
		this.toggleNavigation();
	}.observes( 'ltPropCyclic' ),
	...
	wrapAroundDelay: function( fn, timeout ) {
		var isWidget = Lyte.isWidgetBuild;
	...
	setImageDimensions: function() {
		...
		// Dumb fix for an issue where the image width was not calculated properly in widget environment
		var prom = new Promise( function( resolve ) {
			that.wrapAroundDelay( function() {
				var imageWidth = that.getImageWidth(), item;

				that.setDummyImage();
				item = that.$node.querySelector( 'lyte-avatar-navigator-item' );

				if( imageWidth === 0 ) {
					that.setData( 'ltPropImageWidth', $L( item ).outerWidth( true ) + 'px' );
				}
				
				that.setData( 'imageHeight', $L( item ).outerHeight( true ) + 'px' );
				that.removeDummyImage();
	...
	buildImageObject: function( image, index ) {
		...
		if( isString ) {
			...
			obj[ this.getAltValue() ] = this.getData( 'ltPropAltValue' ) ? image[ this.getData( 'ltPropAltValue' ) ] : ( alt || '' );
	...
	markBlocksForRemoval: function( blocks ) {
		var isPreload = this.getData( 'ltPropPreload' );
	...
	fireOnSelect: function( event, navItem ) {
		...
		if( this.getMethods( 'onSelect' ) ) {
			/**
			 * This callback is fired when an avatar navigator item is selected.
			 * @method onSelect
	...
	fireNavigationCallback: function( event, direction ) {
		if( direction === 'previous' ) {
			if( this.getMethods( 'onPrevious' ) ) {
				/**
				 * This callback is fired when previous arrow is pressed
				 * @method onPrevious
				 * @author ananthapadmanaban.n@zohocorp.com
				 * @param { object } event
				 * @param { object } avatarNavigatorContext
				 * @param { boolean } isSelectedVisible
				 */
			...
			if( this.getMethods( 'onNext' ) ) {
				/**
				 * This callback is fired when next arrow is pressed
				 * @method onNext
				 * @author ananthapadmanaban.n@zohocorp.com
				 * @param { object } event
				 * @param { object } avatarNavigatorContext
	...
	removeMarkedBlocks: function( blocks ) {
		for( var i = 0; i < blocks.length; i++ ) {
			if( blocks[ i ].remove ) {
		...
		removeClass: function( event, block ) {
			...
			if( !preload && this.deleteOtherBlocks ) {
				if( this.curCount === this.totalCount ) {
					this.curCount = this.totalCount = 0;
					this.deleteOtherBlocks = false;
					this.removeMarkedBlocks( blocks );
				}
			}
		},
...
_template:"<template tag-name=\"lyte-avatar-navigator-item\"> <img src=\"{{src}}\" onload=\"{{action('hideIcon',event)}}\" onerror=\"{{action('hideIcon',event)}}\" title=\"\" alt=\"{{alt}}\" lt-prop-title=\"{{title}}\" lt-prop-tooltip-config=\"{{config}}\"> <div class=\"lyteAvatarLoadingCont\"> <template is=\"if\" value=\"{{ltPropLoadingIconYield}}\"><template case=\"true\"> <lyte-yield yield-name=\"loadingIconYield\"></lyte-yield> </template><template case=\"false\"> <div class=\"{{ltPropLoadingIconClass}}\"></div> </template></template> </div> </template>",
...
_observedAttributesType :["object","boolean","string","string"],
