...
_observedAttributesType :["array","object","array","object","string","string","string","string","string","string","string","number","string","boolean","boolean","number","string","boolean","boolean","boolean","string","boolean","number","string","string","boolean","number","string","boolean","boolean","number","boolean","boolean","array","boolean","string","string","object","object","string","object","object","number","object","string","string","boolean","boolean","number","number","boolean","boolean","object","string","number","number","object"],
  ...
  data: function() {
    return {
      ...
      ltPropMouseOverDuration : Lyte.attr('number' , {
        default : 3000
      }),
      ...
      'htInitialClientX' : Lyte.attr('number' , {
        default : 0
      }),
      'htInitialClientY' : Lyte.attr('number' , {
        default : 0
      }),
      'draggedHorizontalObj' : Lyte.attr('object' , {
        default : {}
      })
    }
  },
  ...
  init: function() {
    ...
    this.$node.expandAll = function(param) {
      ...
      if(this.getData('ltPropStructureType') === 'hierarchy'){
        var treeData = this.component.data.ltPropData 
        function openHorizontalTreeFun(treeData){
          for(var i=0;i<treeData.length;i++){
            ...
            if(treeData[i][_this.component.data.ltPropChildrenValue] && treeData[i][_this.component.data.ltPropChildrenValue].length > 0){
              openHorizontalTreeFun(treeData[i][_this.component.data.ltPropChildrenValue])
            }
          }
          return
        }
        openHorizontalTreeFun(treeData);
      } else {
        ...
        if ($L(_this).find('.lyteTreeCollapsed').length > 0) {
    ...
    this.$node.collapseAll = function() {
      ...
      if(this.getData('ltPropStructureType') === 'hierarchy'){
        var treeData = this.component.data.ltPropData 
        function closeHorizontalTreeFun(treeData){
          for(var i=0;i<treeData.length;i++){
            if(treeData[i].collapsed === undefined || treeData[i].collapsed === false){
              Lyte.objectUtils(treeData[i] , 'add' , 'collapsed' , true)
            }
            if(treeData[i][_this.component.data.ltPropChildrenValue] && treeData[i][_this.component.data.ltPropChildrenValue].length > 0){
              closeHorizontalTreeFun(treeData[i][_this.component.data.ltPropChildrenValue])
            }
          }
          return
        }
        closeHorizontalTreeFun(treeData);
      } else {
        if (!this.getData('collapsedAll')) {
  ...
  didConnect: function() {
    ...
    var _thisTree = this;
    var isParentTree = !_thisTree.$node.classList.contains('lyteTreeChildrenLevel');
    if(isParentTree){
      this.$node.classList.add('lyteTreeParentLevel');
    }
    ...
    if(!$L(this.$node).attr('index-var') && this.getData('ltPropShortcut') ){
    ...
    if(this.getData('ltPropSortable')){
      ...
      if(this.getData('ltPropStructureType') === 'hierarchy'){
        $L(this.$node)[0].addEventListener('mousedown' , this.mouseDownFunctionHorizontalTree) 
      } else {
    ...
    if(this.getData('ltPropStructureType') === "file"){
      if( this.getData('ltPropData') && this.getData('ltPropData')[0] && this.getData('ltPropData')[0].exclusiveTree){
    ...
    if(this.getData('ltPropStructureType') === 'hierarchy' && this.getData('ltPropOrientation') === 'topToBottom'){
      if(!$L(this.$node).hasClass('lyteVerticalTree')){
        $L(this.$node).addClass('lyteVerticalTree')
      }
    }
    ...
    if($L(eve.target).hasClass('lyteTreeTopParent')){
      ...
      if(treeContent){
        if(treeComponent.data.exeDrop){
          ...
          if(currentClickedBodyInd.split(' ').length < 2){
          ...
          if(allowDrop){
            if(eve.clientY < (treeContentDim.top + topHeight)){
              ...
              if(beforeDrop !== false){
                Lyte.arrayUtils( prevDataArr , 'removeAt' , treeComponent.getCurrentIndex(currentClickedBodyInd) , 1 )
                if(sibInsertIndex < 0){
                  sibInsertIndex = 0;
                }
              ...
              if(beforeDrop !== false){
                ...
                if(sibInsertIndex > parseInt(treeComponent.getCurrentIndex(currentClickedBodyInd))) {
                  Lyte.arrayUtils( currentTreeArr , 'insertAt' , sibInsertIndex , treeComponent.getData('currentSortObject'));  
                  Lyte.arrayUtils( prevDataArr , 'removeAt' , treeComponent.getCurrentIndex(currentClickedBodyInd) , 1 )
                } else if(sibInsertIndex < parseInt(treeComponent.getCurrentIndex(currentClickedBodyInd))){
                  Lyte.arrayUtils( prevDataArr , 'removeAt' , treeComponent.getCurrentIndex(currentClickedBodyInd) , 1 )
                  Lyte.arrayUtils( currentTreeArr , 'insertAt' , sibInsertIndex , treeComponent.getData('currentSortObject'));  
                }
  ...
  mouseDownFunctionHorizontalTree : function(eve){

    var lyteTreeContent = $L(eve.target).closest('.lyteVerticalTreeNodeContent')
    lyteTreeContent.addClass('lyteTreeNodeCurrentlyDragged')

    if(lyteTreeContent[0]){
      this.setData('htInitialClientX' , eve.clientX)
      this.setData('htInitialClientY' , eve.clientY)

      
      var lyteTreeContentNode = $L(eve.target).closest('.lyteVerticalTreeNode')
      lyteTreeContentNode.addClass('lyteTreeNodeCurrentlyDraggedNode')

      var dimObj = {}
      dimObj.left = eve.clientX - lyteTreeContent[0].getBoundingClientRect().left 
      dimObj.top = eve.clientY - lyteTreeContent[0].getBoundingClientRect().top

      this.setData('initialHorizontalDim' , dimObj)

      var draggedData = $L('.lyteTreeNodeCurrentlyDragged').closest('lyte-tree')[0].getData('ltPropData');
      var dataIndex = parseInt($L(eve.target).closest('.lyteVerticalTreeNodeContent').attr('data-index'))
      
      var retObj = {
        draggedArray : draggedData,
        draggedObject : draggedData[dataIndex]
      }

      var dragStart = this.component.executeMethod('onDragStart' , retObj)

      if(dragStart !== false){
        _lyteUiUtils.addEventListenerGlobal ('mousemove' , this.component.mouseMoveFunctionHorizontalTree)
        _lyteUiUtils.addEventListenerGlobal ('mouseup' , this.component.mouseLeaveFunctionHorizontalTree)
      }
  ...
  mouseMoveFunctionHorizontalTree : function(eve){
    var topTree = $L('.lyteTreeNodeCurrentlyDragged').closest('.lyteTreeTopParent')
    var treeComponent = topTree[0].component
    $L('.lyteHorizontalTreePlaceHolder').removeClass('lyteHorizontalTreePlaceHolder')

    if((Math.abs(treeComponent.data.htInitialClientX - eve.clientX) > 5)||(Math.abs(treeComponent.data.htInitialClientY - eve.clientY) > 5)){

      var treeContent;
      if(eve.target.tagName === ".lyteVerticalTreeNodeContent"){
        treeContent = eve.target
      } else {
        treeContent = $L(eve.target).eq(0).closest('.lyteVerticalTreeNodeContent')[0]
      }

      if(!$L('.lyteTreeCurrentSortElem')[0]){
          var data = $L(eve.target).closest('lyte-tree')[0].getData('ltPropData');
          var dataIndex = parseInt($L(eve.target).closest('.lyteVerticalTreeNodeContent').attr('data-index'))
          treeComponent.setData('draggedHorizontalObj' , data[dataIndex])
          var dummyTreeContent = treeContent.cloneNode(true);
          $L(dummyTreeContent).addClass('lyteTreeCurrentSortElem')
          $L(dummyTreeContent).removeClass('lyteTreeNodeCurrentlyDragged');
          dummyTreeContent.style.position = "absolute";
        ...
        currentSortElem.style.top = (eve.clientY - treeComponent.getData('initialHorizontalDim').top)+'px'
        currentSortElem.style.left = (eve.clientX - treeComponent.getData('initialHorizontalDim').left)+'px'

        if($L(eve.target).closest('.lyteTreeNodeCurrentlyDraggedNode')[0]){
          $L(eve.target).closest('.lyteTreeNodeCurrentlyDraggedNode').eq(0).addClass('lyteTreeSortableDropNotAllowed')
        } else {
          $L(eve.target).closest('.lyteTreeSortableDropNotAllowed').removeClass('lyteTreeSortableDropNotAllowed')
          if($L(eve.target).closest('.lyteVerticalTreeNodeContent')[0]){
            $L(eve.target).closest('.lyteVerticalTreeNodeContent').eq(0).addClass('lyteHorizontalTreePlaceHolder')
          }
        }
  ...
  mouseLeaveFunctionHorizontalTree : function(eve){
    var topTree = $L('.lyteTreeNodeCurrentlyDragged').closest('.lyteTreeTopParent')
    var treeComponent = topTree[0].component


    if(!$L(eve.target).closest('.lyteTreeNodeCurrentlyDraggedNode')[0] && $L(eve.target).closest('.lyteVerticalTreeNodeContent')[0]){
      ...
      var retObj = {
        draggedArray : draggedData,
        draggedObject : treeComponent.getData('draggedHorizontalObj'),
        droppedArray : data
      }

      var beforeDrop = treeComponent.executeMethod('onBeforeDrop')

      if(beforeDrop !== false){
        if(data[dataIndex].children){
          var test = data[dataIndex]
          Lyte.arrayUtils(draggedData , 'removeObjects' , treeComponent.getData('draggedHorizontalObj'))
          Lyte.arrayUtils(test.children , 'push' , treeComponent.getData('draggedHorizontalObj'))
        } else {
          var test = data[dataIndex]
          Lyte.objectUtils(data[dataIndex] , 'add' , 'children' , [])
          Lyte.arrayUtils(draggedData , 'removeObjects' , treeComponent.getData('draggedHorizontalObj'))
          Lyte.arrayUtils(test.children , 'push' , treeComponent.getData('draggedHorizontalObj'))
        }
        retObj = {
          droppedArray : $L(eve.target).closest('lyte-tree')[0].getData('ltPropData'),
          draggedObject : treeComponent.getData('draggedHorizontalObj'),
          draggedArray : draggedData
        }
        
        treeComponent.executeMethod('onDrop' , retObj)
      }
      
      treeComponent.setData('draggedHorizontalObj' , {})

    }

    var currentSortElem = $L('.lyteTreeCurrentSortElem')[0]
    if($L('.lyteTreeCurrentSortElem')[0]){
      currentSortElem.remove()
    }

    treeComponent.setData('htInitialClientX' , 0)
    treeComponent.setData('htInitialClientY' , 0)
    treeComponent.setData('initialHorizontalDim' , {})
    $L(treeComponent.$node).find('.lyteHorizontalTreePlaceHolder').removeClass('lyteHorizontalTreePlaceHolder')
    $L(treeComponent.$node).find('.lyteTreeNodeCurrentlyDragged').removeClass('lyteTreeNodeCurrentlyDragged')
    $L(treeComponent.$node).find('.lyteTreeSortableDropNotAllowed').removeClass('lyteTreeSortableDropNotAllowed')
    $L(treeComponent.$node).find('.lyteTreeNodeCurrentlyDraggedNode').removeClass('lyteTreeNodeCurrentlyDraggedNode')
    
    
    _lyteUiUtils.removeEventListenerGlobal('mousemove' , treeComponent.mouseMoveFunctionHorizontalTree)
    _lyteUiUtils.removeEventListenerGlobal('mouseup' , treeComponent.mouseLeaveFunctionHorizontalTree)
  },
  ...
  methods: {
	...
	actions : {
    treeMouseEnterFunction : function(evt , currentNode){
      var _this = $L(currentNode).closest('.lyteTreeTopParent')[0].component
      var currentTree = $L(currentNode).closest('lyte-tree')[0]
      var currentComponent = currentTree.component
      if($L(currentNode).closest('.lyteTreeTopParent').eq(0).hasClass('lyteTreeDragRunning')){
        _this.treeHovered = setTimeout(function(){
          var hoveredDataArr = $L(currentNode).closest('lyte-tree-body').attr('data-index').split(' ')

          var returnObj = {
            hoveredNode : currentNode,
            hoveredTree : currentTree,
            mainTreeNode : _this.$node,
            hoveredData : currentTree.getData('ltPropData')[hoveredDataArr.length-1],
            hoveredDataArray : currentTree.getData('ltPropData')
          }
          if(_this.getMethods('onHover')){
            _this.executeMethod('onHover' , returnObj)
          }
          clearTimeout(_this.treeHovered)
        },_this.getData('ltPropMouseOverDuration'))
      }
    },
    treeMouseLeaveFunction : function(evt , currentNode){
      var _this = $L(currentNode).closest('.lyteTreeTopParent')[0].component
      if($L(currentNode).closest('.lyteTreeTopParent').eq(0).hasClass('lyteTreeDragRunning')){
        clearTimeout(_this.treeHovered)
      }
    ...
    openHorizontalTree : function(th){
      ...
      if(currentTreeData.collapsed === undefined ||
        currentTreeData.collapsed === true){
          /**
           * This callback is fired before a tree node is opened.
           * @method onBeforeOpen
           * @author surendran.m@zohocorp.com
           * @param { object } currentTreeObject
          ...
          if($L(th).find('lyte-tree-icon') && $L(th).find('lyte-tree-icon').length>0){
            ...
            /**
             * This callback is fired after a tree node is opened.
             * @method onOpen
             * @author surendran.m@zohocorp.com
             * @param { object } currentTreeObject
             * @param { object } treeNode
          ...
          /**
           * This callback is fired before a tree node is closed.
          ...
          /**
           * This callback is fired after a tree node is closed.
           * @method onClose
           * @author surendran.m@zohocorp.com
           * @param { object } currentTreeObject
           * @param { object } treeNode
    ...
    openVerticalTree : function(th , data){
      if(!data.collapsed || data.collapsed === false){
        /**
         * This callback is fired before a tree node is opened or closed.
         * @method onToggle
         * @author surendran.m@zohocorp.com
         * @param { string } 'open'||'close'
         * @param { object } currentTreeObject
         * @param { object } treeNode
         */
        var onToggle = this.executeMethod('onToggle' , 'open' ,data , this.$node)
        if(onToggle !== false){
          var onBeforeOpen = this.executeMethod('onBeforeOpen' , data , this.$node)
          if(onBeforeOpen !== false){
            Lyte.objectUtils(data , 'add' , 'collapsed' , true)
            this.executeMethod('onOpen' , data , this.$node)
          }
        }
      } else {
        var onToggle = this.executeMethod('onToggle' , 'close' ,data , this.$node)
        if(onToggle !== false){
          var onBeforeClose = this.executeMethod('onBeforeClose' , data , this.$node)
          if(onBeforeClose !== false){
            Lyte.objectUtils(data , 'add' , 'collapsed' , false)
            this.executeMethod('onClose' , data , this.$node)  
          }
        }
      }
    },
  ...
  Lyte.createCustomElement("lyte-tree-icon", {
    ...
    constructor: function() {
      if($L(this) && !$L(this).hasClass('lyteTreeClonedIcon')){
          ...
          if(classComponent.getData('ltPropStructureType') !== 'hierarchy'){
            ...
            this.addEventListener('click' , function(eve){
              ...
              var isHiddenIcon = function(lyteTreeIcon){
                var clientRect = lyteTreeIcon.getBoundingClientRect();
                return clientRect.top == 0 && clientRect.left == 0 && clientRect.width == 0 && clientRect.height == 0 ;
              }
              if(window.isLyteWidget && isHiddenIcon(lyteTreeIcon)){
                var treeBody =  $L(lyteTreeIcon).closest('lyte-tree-body')[0];
                var tree = treeBody.closest('.lyteTreeParentLevel');
                tree.openRecursive(treeBody);
                return;       
              }
              ...
              function closingTransition(){
                ...
                /**
                 * This callback is fired after a tree node is opened or closed.
                 * @method onToggleEnd
                 * @author surendran.m@zohocorp.com
                 * @param { object } currentTreeBodyNode
                 * @param { object } eve
                 * @param { object } currentTree
                 * @param { object } currentClickedTreeIcon
                 * @param { object } currentClickedTreeDataArray
                 */
              ...
              if(iconClassList.indexOf('lyteIconOpened') > -1){
                ...
                if(currentTree.getData('ltPropToggleAnimation')){
                  ...
                  if(currentChildTree){
                    currentChildTree.style.height = "0px";
                    currentChildTree.style.display = "none";
                    currentTree.component.executeMethod('onClose' , currentTreeBody[0], eve, currentTree, lyteTreeIcon , currentTree.getData('ltPropData') , currentTree.getData('ltPropData')[currentIndex[currentIndex.length-1]]);
                  }
                ...
                if(currentTree.getData('ltPropToggleAnimation')){
                  ...
                  if(currentChildTree){
                    currentChildTree.style.display = "block";
                    currentChildTree.style.height = "auto";
                    currentTree.component.executeMethod('onOpen' , currentTreeBody[0], eve, currentTree, lyteTreeIcon , currentTree.getData('ltPropData'),currentTree.getData('ltPropData')[currentIndex[currentIndex.length-1]]);
                  }
          ...
          } else if(classComponent.getData('ltPropStructureType') === 'hierarchy'){

            if(classComponent.getData('ltPropOrientation')  === 'leftToRight'){
...
if (!_lyteUiUtils.registeredCustomElements['lyte-tree-content']) {
  ...
  Lyte.createCustomElement("lyte-tree-content", {
    ...
    "connectedCallback": function() {
      ...
      if(currentTree.getData('ltPropStructureType') !== 'hierarchy'){
