...
_observedAttributesType :["array","number","number","number","number","number","boolean","object","boolean","number","number","object","number","number","boolean","string","boolean","boolean","boolean","number","boolean","array","object","boolean","boolean","number","boolean","number","number","object","string","string","number","boolean","boolean","boolean","number","boolean","boolean","boolean","string","boolean","number","boolean","boolean","number","boolean","array","object","boolean","boolean","boolean","boolean","boolean","boolean","string","array","array","string","boolean","boolean","number","number","boolean","array","number","boolean","object","number","string","number","string","number","boolean","boolean","boolean","boolean","boolean","number","boolean","string","string","boolean","boolean","object","boolean","number","boolean","boolean","boolean","boolean","object","boolean","boolean","boolean","object","object","boolean","number","boolean","number","boolean","number","string","string","array","array","array","object","array","boolean","string","string","array","boolean","array","string","object","array","object"],
	...
	data : function(){
		return {
			...
			ltPropLevelConnectionSpacing : Lyte.attr( 'number', { default : 20 } ),
			...
			ltPropOriginPoint : Lyte.attr( 'object', { default : {} } ),//value should be in px

			ltPropCrossLevelConnections : Lyte.attr( 'boolean', { default : false } ),

			ltPropAllowConnectionBoundary : Lyte.attr( 'boolean', { default : false } ),

			ltPropSequentialFlow : Lyte.attr( 'boolean', { default : false } ),

			ltPropSequentialAtrributes : Lyte.attr( 'object', { default : { lastLineModifier: ["100% - 10"], dropabbleBoundary: {left: 100, right: 100, top: 30, bottom: 30}, draggableSpeed: 5, autoScrollBoundaryOffset: 10, draggableTriggerThreshold: 5, collapsibleMinWidth: 200 } } ),

			ltPropAlignPosition : Lyte.attr( 'object', { default : { horizontal : "start", vertical : "start" } } ),

			ltPropTextboxOverlapCheck : Lyte.attr( 'boolean', { default : false } ),

			ltPropCommonConnectorDelay : Lyte.attr( 'number', { default : 100 } ),

			ltPropCenterCommonItems : Lyte.attr( 'boolean', { default : false } ),

			ltPropCommonChildrenSpacing : Lyte.attr( 'number', { default : 100 } ),

			ltPropAvoidCenter : Lyte.attr( 'boolean', { default : false } ),

			ltPropLoopConnectionSpacing : Lyte.attr( 'number', { default : 40 } ),
			...
			previewStyle : Lyte.attr( 'string', { default : 'width: 100%;height: 100%;' } ),

			connectionBoundary : Lyte.attr( 'object', { default : {} } ),

			sequentialData : Lyte.attr( 'array', { default : [] } ),

			commonConnector : Lyte.attr( 'object', { default : {} } )
		}		
	},
	...
	data_obs : function( arg ){
		...
		if( this.data.ltPropSequentialFlow ){
			data.forEach(function(item){
				if(!item.id){
					Lyte.objectUtils(item, 'add', 'id', this.getRandId());
				}
			}.bind(this));
			data = this.data.ltPropData;
			this.setData( 'sequentialData', data );
		}
	...
	setTranslate : function(){
		...
		this._transtime = setTimeout( this._setTranslate.bind( this ), 0 );//default diff scale works only without timeout
	},
	...
	select : function( evt, elem ){
		...
		__data = this._data,
		item = this._item,
		notSequential = !this.data.ltPropSequentialFlow,
		details, data, index;

		if( notSequential ){	
			details = this.data.details[ elem.id.replace( this.data.ltPropIdPrefix, '' ) ],
			data = details && details.data,
			index = __data.indexOf( data );
		} else {
			index = -1;
			item.every(function (item, ind) {
				if (item[0] == elem) {
					index = ind;
				} else {
					return true;
	...
	didConnect : function(){
		...
		var obj = $L.extend({ 
			...
			textbox_overlap_check : data.ltPropTextboxOverlapCheck,
			...
			sequentialFlow : data.ltPropSequentialFlow,
			centerCommonItems : data.ltPropCenterCommonItems && data.ltPropIgnoreSibling,
		...
		[ "onConnect", "onConnectionDisconnect", "onBeforeConnectionCreation", "onManualConnectionCreate", "onConnectionDrag" ].forEach( function( item ){
		...
		node.connect = function( src, target, options, ignore ){
			...
			if( __data.ltPropTextBox && textBox != false ){
				...
				__id = textBox.id = textBox.id || ( "text_box" + Date.now() + parseInt( Math.random() * 100000000 ) );
			...
			if( !__data.ltPropSequentialFlow && !this.isUndo() && !ignore ){
				this.pushToQueue({
		...
		node.disConnect = function( item, callback, ignore ){
			...
			if( !__data.ltPropSequentialFlow && is_not_undo && !ignore ){
				this.pushToQueue({
		...
		node.hideCommonConnector = function(elem, hideAlways){// hide the common connector
			this.showCommonConnector(elem, [], hideAlways, true);
		}.bind(this);
		...
		node.updateItem = this.updateItem.bind(this), //check (item and connection)overlap and update positions and dimensions
		...
		node.updateConnectionPosition = function (connector, src, trg) { // set connection position
			...
			src && (opt.src_position = (src == true ? undefined : src));
			trg && (opt.target_position = (trg == true ? undefined : trg));
			...
			if ((src || trg) && connector && connector.length) {
				...
				if (ret && !this.isUndo()) {
					if( !this.data.ltPropSequentialFlow ){
						...
						src = ret.src;
						trg = ret.target;
						src && (src.old != src.new) && this.pushToQueue({
							type: "updateConnectionOrigin",
							data: this.stringify({ connector: '#' + id, oldSrc: src.old, src: src.new })
						});
						trg && (trg.old != trg.new) && this.pushToQueue({
							type: "updateConnectionOrigin",
							data: this.stringify({ connector: '#' + id, oldTrg: trg.old, trg: trg.new })
						});
					}
					return ret;
				}
		...
		node.getShapesWithoutIncoming = function (withConnections, getDetails) {
			return this.getIsolatedShapes(undefined, undefined, getDetails, withConnections);
		}.bind(this);

		node.getIsolatedNodes = function (getDetails) {
			return this.getIsolatedShapes(true, undefined, getDetails);
		}.bind(this);

		node.isIsolated = function (node) {
			node = this.getItem(node);
			return node && this.getIsolatedShapes(true, node);
		}.bind(this);

		node.getUnreachableNodes = this.getUnreachableNodes.bind(this);

		node.getLoops = this.getLoops.bind(this);

		node.getConnectorText = function (connector) {
			connector = this.getItem(connector);
			return connector ? this.data.textBoxArray[$L(this.getConnectorTextbody(connector.id)).attr('index')].text : [];
		}.bind(this);

		node.getMatchedConnectors = function (src, trg, srcPos, trgPos) {
			src = this.getItem(src);
			trg = this.getItem(trg);

			if (src && trg) {
				var trgId = trg.id,
					ret = [];

				this.getConnections(src).src.forEach(function (item) {
					var itemSrc = item.src_position,
						itemTrg = item.target_position;
					(item.target.id == trgId) && (srcPos ? (itemSrc.x == srcPos.x && itemSrc.y == srcPos.y) : true) && (trgPos ? (itemTrg.x == trgPos.x && itemTrg.y == trgPos.y) : true) && ret.push(item);
				})
		...
		node.fitToScreen = function () {
			var data = this.data,
				boundary = data.ltPropBoundary,
				offset = data.ltPropOffset,
				screenWidth = Math.min(window.innerWidth, this.$node.offsetWidth),
				screenHeight = Math.min(window.innerHeight, this.$node.offsetHeight),
				scaleX = screenWidth / (Math.abs(boundary.left - boundary.right) + (offset * 2)),
				scaleY = screenHeight / (Math.abs(boundary.top - boundary.bottom) + (offset * 2)),
				scale = Math.min(scaleX, scaleY);

			if (Math.abs(this.data.ltPropScale - scale) > 0.0001) {
				this.setData('ltPropScale', scale);
				this.__cb = function () {
					this.moveToCenter();
				}.bind(this);
				return scale;
			} else {
				return data.ltPropScale;
			}
		}.bind(this);

		node.getShapeData = function (shape) {
			return this.data.details[this.getItem(shape).id].data;
		}.bind(this);

		node.getNextData = function (shape) {
			return $L(this.$node).connection('getNext', this.getItem(shape));
		}.bind(this);
		
		node.getPreviousData = function (shape) {
			return $L(this.$node).connection('getPrevious', this.getItem(shape));
		}.bind(this);

		node.isShapeConnected = this.isShapeConnected.bind(this);
	...
	create_marker : function( data ){
		...
		svg = $L(this.__wrapper).children( 'svg' )[ 0 ].children[ 0 ],
		fn = function( __data ){
	...
	getConnections : function( elem, allowAll ){
		var $node = $L( this.$node ),
		_this = this,
		fn = function( item ){
		...
		if( /lyte-connect-item/i.test( elem.tagName ) || (this.data.ltPropSequentialFlow && allowAll) ){
	...
	commonConnectorActions: function (evt, connection, always, alwaysAvoid) {
		var target = evt.target;

		if(target && ((!this.__moved && evt.type == 'mouseenter') || evt.type == 'mouseleave')){
			var connectioElem = target.closest('g'),
				shape = target.closest('lyte-connect-item'),
				textBox = target.closest('lyte-textbox') || (connectioElem && $L(connectioElem).data('text_box')),
				reqData = connectioElem || textBox || shape,
				details = this.data.details,
				connectedItems = [],
				getTxtPos = function (textBox) {
					var txtPos = $L.extend({ width: textBox.offsetWidth, height: textBox.offsetHeight }, $L(textBox).data('position'));

					txtPos.left = txtPos.x - txtPos.width / 2;
					txtPos.top = txtPos.y - txtPos.height / 2;
					txtPos.right = txtPos.left + txtPos.width;
					txtPos.bottom = txtPos.top + txtPos.height;

					return txtPos;
				},
				getConnPos = function (itemPos, textBoxPos) {
					var src, trg;
				...
				removeConnection = function () {
					var connElem = $L(shape ? shape : textBox).data('connection_elements');
					...
					for (var i in connElem) {
						...
						if ($conn.hasClass(shape ? 'lyteCommonShapeConnection' : 'lyteCommonConnection')) {
							if (conn._measure_fdom) {
								$L.fastdom.clear(conn._measure_fdom);
							} else {
								connectedItems.push(data.active_src.id);
							}
							this.$node.disConnect($conn);
						}
					}
				}.bind(this),
				addTxtConnection = function () {
					var textBoxPos = getTxtPos(textBox),
						connDetails = this.getConnectionDetails(connection);

					connectedItems = [];
					this.commonConnectorTimout = undefined;
					...
					for (var i in details) {
						...
						if (!avoid.includes(dataId)) {
							var connPos = getConnPos(itemPos, textBoxPos);
							connectedItems.push(dataId);
							this.$node.connect("#" + dataId, "#" + textBox.id, { src_position: connPos.src, target_position: connPos.trg, textBox: false, class: 'lyteCommonConnection' });
						}
					}
				}.bind(this),
				addShapeConnections = function () {
					var itemPos = $L.extend({}, details[shape.id].position);
					Array.from($L(this.__wrapper).children('lyte-textbox')).forEach(function (item) {
						var txtConn = $L(item).data('connector').data();
						if (txtConn.active_src.id != shape.id && txtConn.active_target.id != shape.id && !avoid.includes(item.id)) {
							var textBoxPos = getTxtPos(item),
								connPos = getConnPos(itemPos, textBoxPos);
							this.$node.connect("#" + shape.id, "#" + item.id, { src_position: connPos.src, target_position: connPos.trg, textBox: false, class: 'lyteCommonShapeConnection' });
						}
					}.bind(this));
				}.bind(this),
				addConnection = function () {
					shape ? addShapeConnections() : addTxtConnection();
					Lyte.objectUtils(commonConnector, 'add', 'commonConnectorRendered', true);
					this.getMethods('onCommonConnectorShow') && this.executeMethod('onCommonConnectorShow', reqData, connectedItems, always, this.$node);
				}.bind(this),
				commonConnector = reqData && this.data.commonConnector[reqData.id],
				avoid, connectedItems = [];

			if (commonConnector && (!commonConnector.commonConnectorAlways || always)) {
				clearTimeout(this.commonConnectorTimout);
				if (evt.type == 'mouseenter') {
					avoid = $L.extend([], alwaysAvoid || commonConnector.avoid);
					if (always) {
						removeConnection();
						addConnection();
					} else {
						var delay = this.data.ltPropCommonConnectorDelay;
						if (commonConnector.commonConnectorRendered) {
							return;
						}
						if (delay == undefined) {
							addConnection();
					...
					removeConnection();
					commonConnector.commonConnectorRendered && this.getMethods('onCommonConnectorHide') && this.executeMethod('onCommonConnectorHide', reqData, connectedItems, always, this.$node);
					Lyte.objectUtils(commonConnector, 'delete', 'commonConnectorRendered');
				}
			}
		}
	...
	showCommonConnector: function (elem, avoid = [], always, hide) {//assumes normal=>always/ only normal/ only always
		var tagName, target, conn,
			commonConnector = this.data.commonConnector,
			triggerConn = function (leave) {
				this.commonConnectorActions({ type: leave ? 'mouseleave' : 'mouseenter', target: target }, conn, always, always && avoid);
			}.bind(this),
			alreadyExist;

		elem = this.getItem(elem);
		...
		if (tagName == 'lyte-textbox') {
			...
			target = conn = elem;
		} else if (tagName == 'lyte-connect-item') {
			target = elem;
		} else {
			return;
		}

		alreadyExist = commonConnector[target.id];

		if (hide) {
			if (alreadyExist) {
				if (always) {
					if (alreadyExist.commonConnectorAlways) {
						Lyte.objectUtils(alreadyExist, 'delete', 'commonConnectorAlways');
						triggerConn(true);
						if (alreadyExist.alreadyExist) {//when always has different avoid, and normal has different avoid// so both done
							$L(target).hasClass(tagName == 'lyte-connect-item' ? 'lyteShapeHover' : tagName == 'g' ? 'lyteConnectionHover' : 'lyteTextboxHover') && (tagName == 'g' ? !$L(target).hasClass('lyteTextboxHover') : true) && triggerConn();
							Lyte.objectUtils(alreadyExist, 'delete', 'alreadyExist');
						} else {
							Lyte.objectUtils(commonConnector, 'delete', target.id);
						}
					}
				} else {
					if (alreadyExist.commonConnectorAlways) {
			...
			var value = $L.extend({ commonConnectorRendered: alreadyExist && alreadyExist.commonConnectorRendered, avoid: alreadyExist ? alreadyExist.avoid : avoid }, always ? { commonConnectorAlways: true, alreadyExist: alreadyExist } : {});
			Lyte.objectUtils(commonConnector, 'add', target.id, value);
			if (always) {
				(alreadyExist && alreadyExist.commonConnectorRendered) || triggerConn();
			}
		}
	},
		...
		var $node = $L(this.$node),
			...
			paths = function (obj, out) {
				...
				obj[dir].forEach(function (item) {
					...
					if (currId && !$L(currItem).hasClass('lyteConnectFakeHangingItem') && !$L(item.connector).hasClass('lyteLoopConnection') && retArr.indexOf(currId) == -1 && currId != _id && !$L(item.connector).hasClass(avoidClass)) {
						retArr.push(currId);
						if (corres) {
							paths(corres, out);
						}
	...
	updateItem: function (updateItem) {//check item overlap // updates position and dimensions
		var data = this.data,
			overlap = data.ltPropOverlapCheck,
			curId = $L(typeof updateItem == "string" ? '#' + updateItem : updateItem).get(0).id,
			curItem = $L("#" + curId),
			oldDetail = $L.extend({}, data.details[curId].position),
			curData = data.ltPropData[Number(curItem.attr('data-index'))],
			xInc = curItem[0].offsetLeft - oldDetail.left,
			yInc = curItem[0].offsetTop - oldDetail.top,
			newDetail;

		this.update_dimensions(curId);
		newDetail = data.details[curId].position;
		if (xInc || yInc) {
			curItem.data('position') || curItem.data('position', $L.extend(true, {}, curData.position));
			this.individual_move(undefined, curItem, xInc, yInc, this.$node.getBoundingClientRect());
			Lyte.objectUtils(curData, 'add', 'position', curItem.data('position'));
		} else if (oldDetail.width != newDetail.width || oldDetail.height != newDetail.height) {
	...
	getItem: function (node) {
		return $L(typeof node == "string" ? '#' + (node.replace('#','')) : node, this.__wrapper).get(0);
	},

	getIsolatedShapes: function (isolate, node, getDetails, withConnections) {
		var ret = [],
			details = this.data.details;
		(node ? [node] : Array.from($L('lyte-connect-item', this.__wrapper))).forEach(function (item) {
			var conn = this.$node.getConnections($L(item));
			if (isolate) {
				node && (ret = false);
				if (!conn.src.length && !conn.target.length) {
					node ? (ret = true) : ret.push(getDetails ? details[item.id] : item);
				}
			} else if ((!withConnections || conn.src.length) && !conn.target.length) {
				ret.push(getDetails ? details[item.id] : item);
			}
		}.bind(this));
		return ret;
	},

	getUnreachableNodes: function (node, getDetails) {
		node = this.getItem(node);

		if (node) {
			var connectionDetails = this.$node.getConnections(node),
				details = this.data.details,
				shapes = [],
				shapesId = [];

			Array.from($L('lyte-connect-item', this.__wrapper)).forEach(function (item) {
				if (item.id != node.id) {
					shapes.push(item);
					shapesId.push(item.id);
				}
			});

			connectionDetails.src.concat(connectionDetails.target).forEach(function (item) {
				var targetId = item.target.id,
					index = shapesId.indexOf(targetId == node.id ? item.src.id : targetId);
				if (index >= 0) {
					shapes.splice(index, 1);
					shapesId.splice(index, 1);
				}
			});

			getDetails && (shapes = shapesId.map(function (item) { return details[item] || null }));
	...
	getLoops: function (node, getDetails) {
		var ret = [],
			details = this.data.details;

		(node ? [node] : Object.keys(details)).forEach(function (item) {
			var curItem = this.getItem(item);

			if (curItem) {
				var connectionDetails = this.$node.getConnections(curItem);

				connectionDetails.src.forEach(function (src) {
					var trgId = src.target.id,
						trgElem = src.target;
					connectionDetails.target.forEach(function (trg) {
						if (trg.src.id == trgId) {
							ret.push(getDetails ? details[trgElem.id] : trgElem);//child will not have data in details
						}
					});
				});
			}
	...
	isShapeConnected: function(parent, child){
		var $child = $L(this.getItem(child)),
			$parent = $L(this.getItem(parent));
		if($parent.get(0) == $child.get(0)){
			return true;
		}
		var childConn = $child.data('connection_elements'),
			found = false;
		for(var key in childConn){
			var conn = childConn[key].connector;
			if(/^target/i.test( key ) && !conn.hasClass('lyteLoopConnection')){
				if(found = this.isShapeConnected($parent, conn.data('src'))){
					return true;
				}
			}
		}
	...
	initiate_preview : function(){
		...
		for( var key in details ){
			...
			this.getMethods( cb ) && this.executeMethod( cb, key, void 0, __data.data, false, this.$node, true );
		}
	...
	preview_click : function( evt ){
		...
		fn = function( ns, diff, width, min, max ){
			return Math.min( Math.max( data[ ns ] - ( diff / preview_bcr[ width ] * svg[ width ] ) * scale, boundary[ min ] ), boundary[ max ] );
		};
	...
	setup_viewbox : function(){
		...
		__this.___fdom = fastdom.measure( function(){
			...
			var origin = __data.ltPropOriginPoint,
			...
			noScroll = {left: origin.left != undefined && b_width < width, top: origin.top != undefined && b_height < height},
			__left = b_left - (noScroll.left ? origin.left : extra_left / 2),
			__top = b_top - (noScroll.top ? origin.top : extra_top / 2),
			...
			b_left -= (noScroll.left ? origin.left : extra_left / 2);
			b_right += (noScroll.left ? extra_left - origin.left : extra_left / 2);
			b_top -= (noScroll.top ? origin.top : extra_top / 2);
			b_bottom += (noScroll.top ? extra_top - origin.top : extra_top / 2);
			...
			fastdom.mutate( function(){
				...
				if( left_to_be < 0 ){
					obj.width = (width_perc += left_to_be) + '%';
					obj.left = 0;
				}

				if( top_to_be < 0 ){
					obj.height = (height_perc += top_to_be) + '%';
					obj.top = top_to_be = 0;
				}

				if( width_perc > 100 ){
					overall_scale_x = 100 / width_perc;
					obj.width = (width_perc = 100) + '%';
				}

				if( height_perc > 100 ){
					overall_scale_y = 100 / height_perc;
					obj.height = (height_perc = 100) + '%';
				}

				if( left_to_be + width_perc > 100 ){
					var diff = left_to_be + width_perc - 100;
					obj.width = ( width_perc -= diff ) + '%';
				}

				if( top_to_be + height_perc > 100 ){
					var diff = top_to_be + height_perc - 100;
					obj.height = ( height_perc -= diff ) + '%';
				}
				...
				__this.setData( 'previewStyle', "width:100%;height:100%;transform:scale(" + overall_scale_x + "," + overall_scale_y + ");" + extra_style );
			});
		});
	},
	...
	update_position : function( elem, only_start, elements, ignore_preview_update ){
		...
		if( this.data.ltPropPreviewImage && !ignore_preview_update ){
	...
	delete : function( id, group_id ){
		...
		if( !isUndo && !data.ltPropSequentialFlow ){
			// this.pushToQueue({
			// 	type : "deleteConnections",
	...
	insert : function( data, index, group_id ){
		...
		if( !isundo && !this.data.ltPropSequentialFlow ){
			this.pushToQueue({
				type : "insertShape",
				index : index,
	...
	setup_boundary : function( ignore_min ){
		...
		final_set = [],
		connBound = data.connectionBoundary;
		...
		data.ltPropData.forEach( function( item, index ){
			...
			var ___fn = function( item, index, children, __width, __height, ignore ){
				...
				if (item.children) {//to set childrens position
					Array.from($L(elem).children('lyte-connect-item')).forEach(function (child, index) {
						Lyte.objectUtils(item.children[index], 'add', 'position', {
							left: _left + child.offsetLeft,
							top: _top + child.offsetTop,
		...
		if( Object.keys(connBound).length ){
			var _minx = connBound.min_x,
				_miny = connBound.min_y,
				_maxx = connBound.max_x,
				_maxy = connBound.max_y;
			max_x = fn( max_x, -_minx, 'max' );
			min_x = fn( min_x, -_maxx, 'min' );
			max_y = fn( max_y, -_miny, 'max' );
			min_y = fn( min_y, -_maxy, 'min' );
			min_mid_x = fn( min_mid_x, -Math.abs( min_x - max_x ), 'min' );
			max_mid_x = fn( max_mid_x, -Math.abs( min_x - max_x ), 'max' );
			min_mid_y = fn( min_mid_y, -Math.abs( min_y - max_y ), 'min' );
			max_mid_y = fn( max_mid_y, -Math.abs( min_y - max_y ), 'max' );
			this.setup_viewbox();
		}
		...
		fn2 = function( wrap_bcr, _left, _width, _right, minx, maxx, scroll ){
			...
			if( this_width >= wrap_width ){
				var originValue = origin[_left],
				mid = this_bcr[ _left ] + ( originValue != undefined ? originValue : this_width * 0.5 ),
				wrap_mid = wrap_bcr[ _left ] + ( originValue != undefined ? 0 : wrap_width * 0.5 );//movable element
		...
		if( !ignore_min && !data.ltPropAvoidCenter ){
		...
		if( !render_arrange && to_check.length ){
			// fastdom.mutate( function(){
				this.arrange( !data.ltPropSequentialFlow && to_check );
				data.ltPropSequentialFlow || this.resetQueue();
			// }.bind( this ));
		} 
	...
	mousemove : function( evt ){
		...
		moving.forEach( function( item ){
			var elem = item.get(0);
			this.commonConnectorActions({type: 'mouseleave', target: elem});
	...
	set_detail : function ( obj, param, pos, elem ){
		...
		if( __data.ltPropPreviewImage && __details == obj ){
			...
			$L.fastdom.clear(origin_elem.fastdom);
			origin_elem.fastdom = $L.fastdom.measure( function(){
				delete origin_elem.fastdom;
				$L.screenGrab({
					dom : origin_elem,
	...
	hover_fn : function( _this, evt, addClass, cb, fn_name, __class ){
		...
		this.commonConnectorActions( evt );
	...
	actions : {
		...
		mousedown : function( evt ){
			...
			// $elem = $L( element ),
			__ns = '__selected_con',
			sequential = __data.ltPropSequentialFlow,
			readonly = __data.ltPropReadonly || sequential,
			...
			this[__ns] && this.getMethods('onConnectionUnselect') && this.executeMethod('onConnectionUnselect', this[__ns], evt, this.$node);
			delete this[ __ns ];
			...
			if( $connection.hasClass( 'lyteConnectionContainer' ) ){
				...
				this.focus();
				this.getMethods('onConnectionSelect') && this.executeMethod('onConnectionSelect', $connection, evt, this.$node);
			...
			if (sequential && document.activeElement !== this.$node) {
	...
	arrange : function( frm_didConnect ){
		...
		elems.forEach( function( item, index ){
			...
			if (data.ltPropSequentialFlow) {
				var from = elems[index + 1],
					to = elems[index - 1];
				from && (new_obj[item.id].from = [from.id], new_obj[item.id].from_position = [{ x: 0.5, y: 1 }]);
				to && (new_obj[item.id].to = [to.id], new_obj[item.id].to_position = [{ x: 0.5, y: 0 }]);
			}
		...
		fastdom.measure( function(){
			this._boundary();
			if(!data.ltPropSequentialFlow && !data.ltPropAvoidCenter){
				fastdom.mutate( function(){
					this.moveToCenter();
				}.bind( this ));
			}
	...
	set_individual_position : function( obj, _dim, ranges, key, ignore, is_render_arrange ){
		...
		if( !cur.modified ){
			...
			if( !is_same || this.__allow_same ){
				...
				if( !is_same ){
					!this.data.ltPropSequentialFlow && this.pushToQueue({
						type : "positionUpdate",
						id : key,
	...
	filterSubParent: function (connection) {
		var isDirectChildren = function (src, target) {
				var y1 = src.y,
					y2 = target.y,
					modify_vert1 = y1 > 0 && y1 < 1,
					modify_vert2 = y2 > 0 && y2 < 1;

				if (modify_vert1 && modify_vert2) {
					return false;
				}
				return true;
			},
			sortConnections = function (_conn, _mainConn, _subConn, hasMain) {
				var trgConn = _conn.target;
				for (var i = 0; i < trgConn.length; i++) {
					var item = trgConn[i],
						_data = item.data();
					if (isDirectChildren(_data.src_position, _data.target_position)) {
						if (hasMain) {
							return true;
						}
						_mainConn.push(item);
			...
			subConn = [], mainConn = [], srcConn = [];

		sortConnections(connection, mainConn, subConn);
		connection.src.forEach(function (item) {
			var _data = item.data();
			if (isDirectChildren(_data.src_position, _data.target_position) || !sortConnections($L(this.$node).connection('getConnections', _data.target), [], [], true)) {
				srcConn.push(item);
			}
		}.bind(this));

		return { target: mainConn.length ? mainConn : subConn, src: srcConn };
	},
	...
	formatting : function( item, obj, __level ){
		...
		$node = $L( _this.$node ),
		__obj = { id : id },
		isReverseValid = function(parentElem){
			if(parentElem && _this.data.ltPropCenterCommonItems && _this.data.ltPropIgnoreSibling){
				return _this.$node.getNodesInSeries(parentElem, {outgoing: false}).every(function(item){
					return !obj[item];
				});
			}
			return true;
		...
		fn = function( arr, push, position_arr, name, other, to_level ){
			$L.each( arr, function( index, __item ){
				...
				//ltPropCenterCommonItems feature allows side connections and lines without trgPos, so filterSubParent won't work

				// if( _this.data.ltPropIgnoreSibling && !_this.data.ltPropFixedArrangeLevels ){
				// 	connection = _this.filterSubParent( connection );
				// }
				...
				connection[ name ].forEach( function( in_item ){
					...
					if( !obj[ __id ] && (name == 'target' ? isReverseValid(__other) : true)){
	...
	selectup : function( evt ){
		...
		if( elem && !( _this.getMethods( cb ) && _this.executeMethod( cb, evt, shapes, _this.$node ) == false ) ){
			...
			if( !evt.shiftKey ){
				_this.resetSelected( void 0, evt );
			}
...
// 			}
	...
	moveToShape : function( elem, check_min_max, hori, vert ){
		...
		if( !current ){
			if(this.data.ltPropSequentialFlow){
				var par = elem.closest('.parentNode'),
					parBcr = par.getBoundingClientRect(),
					childBcr = elem.getBoundingClientRect();
				current = details[ fn( par.id ) ].position;
				current = {
					position: {
						left: current.left + (childBcr.left - parBcr.left),
						top: current.top + (childBcr.top - parBcr.top),
						width: childBcr.width,
						height: childBcr.height
					}
				};
			} else {
				var parent = this.getItem(elem),
					parentDetails = parent && details[fn(this.getItem(elem).parentNode.id)];
				if (parentDetails) {
					parentDetails.data.children.every(function (child) {
						if (child.id == id) {
							current = child;
						} else {
							return true;
	...
	keydown : function( evt ){
		...
		if (this.data.ltPropSequentialFlow) {
			prevent = this.handleSequentialKeyEvents(evt);
			if (prevent) {
				this.stopevt(evt);
			}
			return;
   ...
   onConnectionUpdate : function( svg ){
	...
	if( __data.ltPropPreview && __data.ltPropRenderConnectorsInPreview && !this.__seq_trans_happening ){
	  ...
	  if( this.data.ltPropAllowConnectionBoundary ){
		$L.fastdom.clear( this._cnbmeasure );
		this._cnbmeasure = $L.fastdom.measure( this.updateConnBound.bind( this ) );
	  }
   ...
   updateConnBound : function(){
		var boundary = {};
		this.$node.getAllConnections().forEach(function(elem){
			$L(elem.connection_elem).data('absolute_points').forEach(function(item){
				if(boundary.min_x == undefined || boundary.min_x > item.x){
					boundary.min_x = item.x;
				}
				if(boundary.max_x == undefined || boundary.max_x < item.x){
					boundary.max_x = item.x;
				}
				if(boundary.min_y == undefined || boundary.min_y > item.y){
					boundary.min_y = item.y;
				}
				if(boundary.max_y == undefined || boundary.max_y < item.y){
					boundary.max_y = item.y;
				}
			});
		});
		this.setData('connectionBoundary', boundary);
		delete this._cnbmeasure;
		this._boundary(true);
   },
   ...
   show_adjust_pts : function( elem, __pts ){
	  ...
	  find_opt_pos = function( first, second, is_hori ){
			...
			textBoxAvail && this.$node.getAllConnections().forEach( function( item ){
				...
				__width = text_box.prop( "offsetWidth" ),
				__height = text_box.prop( "offsetHeight" );
				text_box && text_box.length && this.split_ranges( ranges, {
					position : {
						left : position.x - __width / 2,
						top : position.y - __height / 2
					},
   ...
   adjust_drag : function( evt ){
		...
		this.commonConnectorActions({type: 'mouseleave', target: __connector});
   ...
   toggle_hover : function( evt, connection ){
		...
		this.commonConnectorActions(evt, connection);
...
}, { mixins : [ 'lyte-connect-positioning', 'lyte-connect-magnetiser', 'lyte-smartguide-utils', 'lyte-connect-animation', 'lyte-connect-undo', 'lyte-shape-positioning', 'lyte-connect-suggestion', "lyte-connect-group-sort", "lyte-connect-circle-positioning", "lyte-connect-level-arrange", "lyte-connect-sequential-flow" ] });
