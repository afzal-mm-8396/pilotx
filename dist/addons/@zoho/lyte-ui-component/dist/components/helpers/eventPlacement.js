...
var cloneEventTag = function( event_tag, scheduler ){
    ...
    cloneTag.setAttribute('id',event_tag.getAttribute('id'));
    cloneTag.setAttribute('tabindex',event_tag.getAttribute('tabindex'));
    cloneTag.setData('ltPropEvent',event_tag.getData('ltPropEvent'));
    cloneTag.setAttribute('class', event_tag.getAttribute('class'));

    event_tag.children.forEach( function( child ){
        var clonedChild = child.cloneNode( true );
        cloneTag.appendChild( clonedChild );
    });

    cloneTag.setAttribute('style', event_tag.getAttribute('style'));
...
function isAllDayEvent( tag ){
 ...
 eventPlacement.setTimelinePosition = function(eventData,scheduler,event_tag){
    ...
    var pos = _lyteUiUtils.eventPlacement.fixTimeLineEvent(eventData,startDate,format,scheduler,view);
    event_tag.style.top = pos.top + 'px';
    event_tag.style.height = pos.height + 'px';
    return pos.height;
}
eventPlacement.fixTimeLineEvent = function(eventData,startDate,format,scheduler,view,customTimeline){
    var endDate = $L.moment(eventData.end,format);
    var timelineInterval = scheduler.getData('ltPropTimeLine'); 
    if( customTimeline && (customTimeline.start || customTimeline.end) ){
        var customStart = customTimeline.start;
        var customEnd = customTimeline.end;
        if( startDate.get('hours') < customStart.get('hours') || ( startDate.get('hours') === customStart.get('hours') && startDate.get('minutes') < customStart.get('minutes') ) ){
            startDate.set('hours', customStart.get('hours'));
            startDate.set('minutes', customStart.get('minutes'));
        }
    }
    ...
    var isUserBasedView = view == 'day' && isMultiView && isUserBased;
    if(startTimeDiv){
        ...
        var header_height = 0;
        if(view == 'day' && (!scheduler.getData('ltPropUserBasedView') || !scheduler.getData('ltPropMultiUserView'))){
            header_height = scheduler.$node.querySelector('.lyteSchedulerDayViewAllDayRow').offsetHeight;
        }
        var endDate = $L.moment(eventData.end,format);
        if( customEnd && ( customEnd.get('hours') < endDate.get('hours') || ( customEnd.get('hours') === endDate.get('hours') && customEnd.get('minutes') < endDate.get('minutes') ) ) ){
            endDate.set('hours', customEnd.get('hours'));
            endDate.set('minutes', customEnd.get('minutes') + timelineInterval);
        }
        var diffMs = startDate.fromNow(endDate).timestamp;
        var diff =  Math.round((diffMs / 1000) / 60);
        diff = diff < (timelineInterval / 4) ? (timelineInterval / 4) : diff;
        var scrollTop = 0;
        if(isUserBasedView){
            scrollTop =  startTimeDiv.offsetParent.scrollTop;
        }
        top =  startTimeDivRect.top + stratminDiff - startTimeDiv.offsetParent.getBoundingClientRect().top + parseInt(window.getComputedStyle(startTimeDiv.querySelector('td')).borderTopWidth) - header_height + scrollTop ;
        height = diff  * interval ;
    }
    return { top : top, height: height};
}
eventPlacement.setAllDayPosition = function(eventData,scheduler,event_tag,dragDir, isCurrTag, lastStartDiv){
    var format = scheduler.getData('ltPropFormat') || 'YYYY-MM-DDTHH:mm:ss';
    var view = scheduler.getData('ltPropSchedulerView');
    var isMultiView = scheduler.getData('ltPropMultiUserView');
    ...
    if(eventData.end){
        ...
        if(isinBetween || (view !== 'day')){
            ...
            if( dragDir === 'left' && isCurrTag ){
                if( startDiv ){
                    var left_interval = scheduler.getEventInterval(divPosition,startDate.format('DD-MM-YYYY'), 'DD-MM-YYYY'); 
                    if( left_interval < 0 ){
                        left_interval *= -1;
...
eventPlacement.setDayMultiUserPosition = function(eventData,scheduler,event_tag){
        ...
        if(startTimeDiv){
            ...
            if( endDate.format('DD-MM-YYYY') != startDate.format('DD-MM-YYYY') && end_time == '12:00 AM' ){
                var lastDiv = user.querySelector('[data-time="11:00 PM"]');
                _this.$node.style.width =  ( lastDiv.getBoundingClientRect().right - _this.$node.getBoundingClientRect().left ) +  (endDate.get('minutes') / scheduler.getData('ltPropTimeLine')  ) * startTimeDiv_width + 'px';
            }
            else{
