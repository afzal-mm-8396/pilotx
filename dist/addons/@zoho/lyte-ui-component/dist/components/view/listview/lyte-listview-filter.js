...
_observedAttributes :["ltPropHeader","ltPropContent","ltPropRenderContent","ltPropFilteredContent","ltPropGroupedRow","ltPropCondition","ltPropPlaceHolder","ltPropTriggerEvent","ltPropNavigator","ltPropFilterContent","ltPropFormat","ltPropNestedGrouping","heading","inputPlaceHolder","ltPropSearchContent","renderContent","currentContent","currentFilterCount","focusedElem","showInput","containsSortBy","containsGroupBy","containsContains","conditions","curDate","inputValue","hideInput","ltPropFilterGrouped","sortBy","filterBy","textFilter","numberFilter","booleanFilter","dateFilter","timeFilter","dateTimeFilter","multiselectFilter","noResultMessage","headingsRemoved","ltPropPrevGroupIndex","headerMap","renderCustom","customValue"],
_observedAttributesType :["array","array","array","array","boolean","object","string","boolean","boolean","array","string","boolean","array","string","array","array","array","number","object","boolean","boolean","boolean","boolean","object","string","string","boolean","boolean","array","array","array","array","array","array","array","array","array","boolean","array","object","object","string","string"],
	...
	updateConditions: function (_filter , forApplying , forRemoving ) {
		var __this = this,
		__data = __this.data,
		// conditions = __this.constructCondition(),
		conditions = {},
		header = __data.heading,
		trigger = __data.ltPropTriggerEvent,
		filterContent = __data.ltPropSearchContent || [],
		__groups = [],
		__sort = [],
		__search = [],
		// __filter = [],
		...
		__header_map = __data.headerMap || {},
		is_multiple_filter = __data.ltPropMultipleFilter;

		if( __data.ltPropNestedGrouping ){
			if( !trigger ){
				__this.setData({
					ltPropFilterGrouped: false,
					ltPropFilteredContent: undefined	
				});
			}
	
			filterContent.forEach( function( item ){
				var __filterName = item.name || "",
				isFullySpecified = __this.isFilterFullySpecified( item, true );
				...
				if( isFullySpecified ){
					switch( __filterName ){
						...
						break;
						case 'Filter by': {
							var datatype = __this.getDataType( item.column ), 
							condition = __this[ datatype.toLowerCase() + "Condition" ]( item ),
							heading = __header_map[ item.column ];
							// __filter.push( condition );
	
							if( is_multiple_filter ){
								var __arr = conditions[ heading ];
								if( __arr ){
									Lyte.arrayUtils( __arr, 'push', condition );
								} else {
			...
			var __grouped_columns = __groups.map( function( item ){
				return __header_map[ item.column ];
			} ),
			__sorted_columns = __sort.map( function( item ){
				var __map = __header_map[ item.column ];
				return {
					column : __map,
					condition : item.condition,
					index : __grouped_columns.indexOf( __map )
				};
			} ),
			cb = "onInlineFilterChange";
	
			if( this.getMethods( cb ) ){
				this.executeMethod( cb, {
					group : __grouped_columns,
					sort : __sorted_columns,
					contains : __search,
					filter : conditions
				}, forApplying, forRemoving );
			}
		} else {
			var conditions = this.constructCondition(),
			_this = this,
					...
					switch (filterName) {
						...
						case 'Group by':
							...
							if( !_this.data.ltPropTriggerEvent ){
								...
								var final = [],
								heading = _this.getHeaderValue( header.findIndex( function(item){ return item == filter.column } ) ).prop;

								if( _this.data.ltPropNestedGrouping ){
									final = _this.do_grouping( _this.data.ltPropHeader, _this.data.ltPropContent, [ heading ] )
								} else {
	...
	setFilterContent: function () {
		...
		if (this.data.containsSortBy || this.data.containsGroupBy || this.data.containsContains ) { 
			...
			!this.data.ltPropNestedGrouping && this.data.containsGroupBy && this.removeFromArray( data, 'Group by' );
