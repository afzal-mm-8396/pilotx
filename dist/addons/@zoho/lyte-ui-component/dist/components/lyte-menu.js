...
_observedAttributes :["ltPropContent","ltPropId","ltPropClass","ltPropQuery","ltPropEvent","ltPropYield","ltPropUserValue","ltPropSystemValue","ltPropCallout","ltPropPosition","ltPropDescription","ltPropTabIndex","ltPropFreeze","ltPropShow","ltPropWidth","ltPropHeight","ltPropQueryClass","ltPropBoundary","ltPropScope","ltPropPreventInsideClick","ltPropAnimate","ltPropSetCss","ltPropWrapperClass","ltPropWrapperId","ltPropBindToBody","ltPropAria","ltPropAriaAttributes","ltPropOffset","ltPropShadowRootCheck","ltPropActiveElement","ltPropHeightResetOnScroll","ltPropPreventKeyEvents","ltPropForceClick","ltPropPreventKeydown","ltPropPreventDefaultSelection","ltPropClearSelectionOnHoverClose","ltPropOriginPosition","ltPropForceNavigation","ltPropBoundaryRestore","ltPropShowDelay","ltPropSyncOpen","pos","parIndex","lyteUnbound","randomId","customPosition"],
_observedAttributesType :["array","string","string","string","string","boolean","string","string","boolean","string","string","number","boolean","boolean","string","string","string","object","string","boolean","boolean","boolean","string","string","boolean","boolean","object","object","boolean","string","boolean","boolean","boolean","boolean","boolean","boolean","object","boolean","boolean","number","boolean","string","number","boolean","string","string"],

    _lyteUtilFunctions : [ "setCss" ],
    init: function () {
        var event = this.getData('ltPropEvent');
        var evt = this.checkElementForMenu.bind(this);
        // this.setData('eventListeners.event', evt);
        this.__eventListeners = { event : evt };
        this._close = this.closing.bind(this);
        this._trsend = this.tranEnd.bind(this);
        this._hoverclose = this.hoverClose.bind(this);

        var is_shadow = _lyteUiUtils.isWidget;

        document[ "addEventListener" + ( is_shadow ? "Global" : "" ) ](/^(mouseenter|mousemove|mouseover|hover)$/.test(event) ? 'mousemove' : event, evt, true);
        ...
        if (!_lyteUiUtils._lyteMenu) {
            var origin_elem = is_shadow ? document : document.documentElement,
            ns = "addEventListener" + ( is_shadow ? "Global" : "" ); 

            origin_elem[ ns ]('click', lyteCloseMenu, !is_shadow );
            origin_elem[ ns ]('keydown', this.keydownCheck, true);
            origin_elem[ ns ]('keypress', menukeypress, true);
            window.addEventListener('orientationchange', this.resizeFunc, true);
            window.addEventListener('resize', this.resizeFunc, true);
            _lyteUiUtils._lyteMenu = {};
            _lyteUiUtils._lyteMenu.eventFlag = true;
        }
        /**
         * This method is called before the menu is rendered.
         * @method beforeRender
    ...
    appendFreeze: function (className) {
        ...
        return _lyteUiUtils.appendChild( document.body, freezeLayer, { referenceNode : this.$node } );
    },
    ...
    resizeFunc: function (event) {
        ...
        if( event.type == 'orientationchange' ){
            this._resizeTimeout = setTimeout( this.resizeFn , 500 )
        }
    ...
    open_submenu : function( evt ){
        ...
        if( !child_opened && _this.data.ltPropAria ){
            ...
            __fn = function(){
                ...
                if( submenu ){
                    var __sub = document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ]( submenu );
    ...
    keydownCheck: function (event) {
        ...
        if( __target && _lyteUiUtils.isWidget && /*!document.contains( __target )*/ __target.shadowRoot ){
            return;
        }

        var 
        /**
         * In writer's case they are opening the menu with origin position but the focus is in a hidden contenteditable element. So input check prevents the navigation because focus in in a input element. to overcome that i have added this property
         * They want to typing while menu opening but navigation part alone should in the menu. So can't shift focus to the menu element.
         */
        force_navigation,
        force_prevent = function(){
            if( force_navigation ){
        ...
        if (keyCode == 27) {
            lyteCloseMenu(event, undefined, true, true );
        } else if (/^(38|40|13|36|35)$/.test(keyCode)) {
            var menus = document.getElementsByTagName('lyte-menu');
            // if( menus.length ) {
            // event.preventDefault()
            for (var i = 0; i < menus.length; i++) {
                if (menus[i].classList.contains('lyteMenuClosed') || menus[i].getAttribute('lyte-rendered') == null || menus[i].ltProp('preventKeydown')) {
                    continue;
                }
                var menu = menus[i].component;

                force_navigation = menu.data.ltPropForceNavigation;

                if( !force_navigation && menu.is_input_element( __target, keyCode ) ){
                    continue;
                }
                ...
                if (!menu.childMenu) {
                    ...
                    flag && menu.traverseList( event, force_prevent );
            ...
            for (var i = 0; i < menus.length; i++) {
                ...
                if ( __cur_menu.classList.contains('lyteMenuClosed') || __cur_menu.getAttribute('lyte-rendered') == null || __cur_menu.ltProp('preventKeydown') ) {
                    continue;
                }
            ...
            menus.every( function( item ){
                ...
                if( !force_navigation && comp.is_input_element( __target, keyCode ) ){
                    return true;
                }
    ...
    scrIntoView: function (elem) {
        ...
        $L.fastdom.measure(function () {
            ...
            scr = this.findscrElem.call(this, elem);

            if( scr ){
                var hgt = scr.getBoundingClientRect();
    ...
    findscrElem: function (el) {
        ...
        while (nde && nde.nodeName != "LYTE-MENU-BODY") {
            var st = getComputedStyle(nde).getPropertyValue('overflow-y')
    ...
    didDestroy: function () {
        ...
        var removeEvents = /*this.getData('eventListeners')*/ this.__eventListeners, 
        event = this.getData('ltPropEvent'),
        data = this.data;

        if (document.querySelectorAll('lyte-menu').length == 0) {
            var freezeLayers = document.querySelectorAll('div.lytemenufreezelayer')
            for (var i = 0; i < freezeLayers.length; i++) {
                var __cur = freezeLayers[ i ];
                __cur.parentNode.removeChild( __cur );
            }
            if (_lyteUiUtils._lyteMenu) {
                delete _lyteUiUtils._lyteMenu
                var is_shadow = window.document != document,
                origin_elem = is_shadow ? document : document.documentElement,
                ns = "removeEventListener" + ( is_shadow ? "Global" : "" );

                origin_elem[ ns ]('keydown', this.keydownCheck, true);
                origin_elem[ ns ]('keypress', menukeypress, true)
                origin_elem[ ns ]('click', lyteCloseMenu, !_lyteUiUtils.isWidget );
            ...
            if (_lyteUiUtils.menu == this) {
                delete _lyteUiUtils.menu;
            }
            this.removeFreeze();
        }
        ...
        document[ "removeEventListener" + ( _lyteUiUtils.isWidget ? "Global" : "" ) ](/^(mouseenter|mousemove|mouseover|hover)$/.test(event) ? 'mousemove' : event, removeEvents.event, true);
        ...
        clearTimeout( this.__selection_time );
    ...
    ___closestFind: function (path, query) {
        var parIndex = this.getData('parIndex'),
            elements = this.arrayFrom.call(this, (parIndex != undefined ? (this.$node.parentNode.parentNode.querySelectorAll('lyte-menu-item:nth-of-type(' + ++parIndex + ')')) : document[  _lyteUiUtils.isWidget ? "querySelectorAllGlobal" : 'querySelectorAll' ](query.trim())));
    ...
    checkForShadowRoot: function( event ){
        if( event.target && event.target.shadowRoot ){

            if( !_lyteUiUtils.isWidget && event.target.closest( 'lyte-widget' ) ){
                return null;
            }

            let elem = event.target.shadowRoot.elementFromPoint( event.clientX, event.clientY ),
            query = this.data.ltPropQuery;

            if( elem && query && elem.matches( query ) ){
                return elem;
            }
        }
        return null;
    ...
    checkElementForMenu: function (event) {
        ...
        if( /*!document.contains( __target )*/ _lyteUiUtils.isWidget && __target.shadowRoot ){
            return;
        }

        if (!event.menuFlag) {
            var query = this.getData('ltPropQuery');
            var closetElem = query ? this.closestFind.call(this, event.path ? event.path : this.composePath.call(this, event), query) : null;

            if( !closetElem ){
                closetElem = this.checkForShadowRoot( event );
            }
    ...
    isInsideMenuItem : function( elem ){
        if (!elem) {
            return false;
        }
        
        while (elem && elem.tagName) {
            if (elem.tagName.toLowerCase() === 'lyte-menu-item') {
                return elem;
            }
    ...
    openingMenu: function (closetElem, event) {
        ...
        var menuItem = this.isInsideMenuItem( closetElem );
        if (!this.parentMenu) {
            if (event.type != "contextmenu") {
                var ret;
                ...
                if( menuItem ){
                    menuItem.__sub_open = true;
                    menuItem.__force_callback = this.data.ltPropForceClick;
                }

                if( this.data.ltPropSyncOpen && this.getMethods( 'onBeforeOpen' ) ){
                    ret = this.executeMethod( 'onBeforeOpen', this.$node, event, this.$node.element );
                }
    ...
    didConnect: function () {
        ...
        this.$node.toggle = function (event, flag) {
            ...
            if (this.childComp.classList.contains('lyteMenuHidden') || flag) {
                if (['mousedown', 'mouseup'].indexOf(event.type) > -1) {
                    _lyteUiUtils._lyteMenu.preventClick = false;
                }
                clearTimeout(this._showDelayTimeout);
                var showDelay = this.getData('ltPropShowDelay') || 0;
                if (showDelay > 0) {
                    this._showDelayTimeout = setTimeout(function() {
        ...
        this.$node.getMenuBox = function(){
            return this.childComp;
        }.bind(this);

        this.$node.highlightMenuItem = function( menuItem ){
            if( menuItem ){
                var __class = 'lyteMenuSelection',
                elem = this.childComp.querySelector( '.' + __class );

                elem && elem.classList.remove( __class );
                menuItem.classList.add( __class );
            } else {
                this.traverseList({ key : "Home" });
            }
        }.bind( this );
        ...
        if (this.getMethods('afterRender')) {
            ...
            /**
             * This method is called after rendering the menu
             * @method afterRender
             * @author ponkarthikeyan.t@zohocorp.com
             * @version 1.0.1
             * @param { object } menuElement
    ...
    bToBody: function () {
        if (this.data.ltPropBindToBody && this.childComp.parentNode == this.$node ) {
            ...
            _lyteUiUtils.appendChild(document.body, this.childComp, { referenceNode : this.$node });
    ...
    data: function () {
        var default_values = _lyteUiUtils.getDefault( 'lyte-menu' );
        return {
            // user data
            /**
            ...
            */
             ...
             * @input
             */
            ltPropQuery: Lyte.attr('string', { 'default': default_values.query || '', input : true }),
            /**
             ...
             * @input
             */
            ltPropEvent: Lyte.attr('string', { 'default': default_values.event || 'click', input : true }),
            /**
             ...
             * @input
             */
            ltPropUserValue: Lyte.attr('string', { 'default': default_values.userValue || '', input : true }),
            /**
             ...
             * @input
             */
            ltPropSystemValue: Lyte.attr('string', { 'default': default_values.systemValue || '', input : true }),
            /**
             ...
             * @input
             */
            ltPropFreeze: Lyte.attr('boolean', { 'default': default_values.freeze || true, input : true }),
            /**
             ...
             * @input
             */
            ltPropWidth: Lyte.attr('string', { 'default': default_values.width || 'auto', input : true }),
            /**
             * @componentProperty {string} ltPropHeight=auto
             * @version 1.0.0
             * @input
             */
            ltPropHeight: Lyte.attr('string', { 'default': default_values.height || 'auto', input : true }),
            /**
             * @componentProperty {string} ltPropQueryClass=lyteMenuSelected
             * @version 1.0.0
             * @input
             */
            ltPropQueryClass: Lyte.attr('string', { 'default': default_values.queryClass || 'lyteMenuSelected', input : true }),
            /**
             * @typedef {object} menuBoundary
             ...
             */
            /**
             * @componentProperty {menuBoundary} ltPropBoundary
             * @default {}
             * @version 1.0.0
             * @input
             */
             ...
             * @input
             */
            ltPropScope: Lyte.attr('string', { 'default': default_values.scope || '', input : true }),
            /**
            ...
            //  * @input
            //  */
            ltPropSetCss: Lyte.attr('boolean', { default: default_values.setCss == false ? false : true, input : true }),
            /**
             ...
             * @condition ltPropYield true
             */
            ltPropWrapperClass: Lyte.attr('string', { 'default': default_values.wrapperClass || '', input : true }),
            /**
             ...
             * @condition ltPropYield true
             */
            ltPropWrapperId: Lyte.attr('string', { default: default_values.wrapperId || "", input : true }),
            /**
             * @componentProperty {boolean} ltPropBindToBody=true
             * @version 2.1.0
             * @input
             */
            ltPropBindToBody: Lyte.attr('boolean', { default: default_values.bindToBody == false ? false : true, input : true }),
            /**
             ...
             * @input
             */
            ltPropAriaAttributes: Lyte.attr('object', { default: default_values.ariaAttributes || { role: "menu" }, watch : true, input : true }),
            /**
             ...
             * @input
             */
            ltPropOffset: Lyte.attr('object', { default: default_values.offset || {}, input : true }),

            // /**
            //  * @componentProperty {boolean} ltPropShadowRootCheck=false
            //  * @version 3.114.0
            //  * @description This property is used to check if the menu's origin element is inside a shadow root or not.
            //  * @input
            //  */

            ltPropShadowRootCheck : Lyte.attr( 'boolean', { default : default_values.shadowRootCheck || false } ),
            
            ltPropActiveElement : Lyte.attr( 'string', { input : true } ),
            /**
             * @experimental ltPropHeightResetOnScroll
             */
            ltPropHeightResetOnScroll: Lyte.attr('boolean', { default: default_values.heightResetOnScroll == false ? false : true }),

            /**
             * @experimental ltPropPreventKeyEvents
             */
            
            ltPropPreventKeyEvents : Lyte.attr( 'boolean', { default : default_values.preventKeys || false } ),

            /**
             * @experimental ltPropForceClick
             */

            ltPropForceClick : Lyte.attr( 'boolean', { default : default_values.forceClick || false } ),

            /**
             * @experimental ltPropPreventKeydown
             */

            ltPropPreventKeydown : Lyte.attr( 'boolean', { default : default_values.preventKeydown || false } ),

            /**
             * @experimental ltPropPreventDefaultSelection
            */

            ltPropPreventDefaultSelection : Lyte.attr( 'boolean', { default : default_values.preventDefaultSelection || false } ),

            /**
             * @experimental ltPropClearSelectionOnHoverClose
             */

            ltPropClearSelectionOnHoverClose : Lyte.attr( 'boolean', { default : default_values.clearSelectionOnHoverClose || false } ),

            /**
             * @experimental ltPropOriginPosition
             */

            ltPropOriginPosition : Lyte.attr( 'object', { default : void 0 } ),

            /**
             * @experimental ltPropForceNavigation
             */

            ltPropForceNavigation : Lyte.attr( 'boolean', { default : default_values.forceNavigation || false } ),

            /**
             * @experimental ltPropBoundaryRestore
             */

            ltPropBoundaryRestore : Lyte.attr( 'boolean', { default : default_values.boundaryRestore || false } ),

            /**
             * @experimental ltPropShowDelay
             */

            ltPropShowDelay : Lyte.attr( 'number', { default : default_values.showDelay || 0 } ),

            /**
            * @experimental ltPropSyncOpen
            */ 

            ltPropSyncOpen : Lyte.attr( 'boolean', { default : default_values.handle || false } ),
            ...
            customPosition : Lyte.attr( 'string' , { default : "" } )
        }
    },

    firePosCallBack: function () {
        /**
         * This method is called after the menu position is changed
    ...
    setContextCss: function (evt, position) {
        ...
        $L.fastdom.measure(function () {
            var evt1 = evt,
                nbr = this.data.ltPropOriginPosition || element.getBoundingClientRect();
            if (!evt || !evt.type) {
            ...
            var clientRect = menuBody.getBoundingClientRect(),
                ...
                menuBodystyle = getComputedStyle(this.menuBody),
            ...
            newClient = menuBody.getBoundingClientRect();
            
            if (newClient.left < lftoff) {
                menuBody.style[lT] = (cX + lftoff) + 'px';
            }
            else if (this.rtlfunc.call(this, 'right', newClient, innerWidtht) > (innerWidtht - rgtoff)) {
    ...
    setup_controls : function( prev_origin ){
        var origin_menu = this.get_origin_menu(),
        __$elem = $L( origin_menu.$node.element ),
        __attr = "aria-haspopup",
        __child_comp = this.childComp,
        current_origin = this.$node.element,
        label_attr =  "aria-labelledby",
        act_desc = "aria-activedescendant",
        cur_id = ( prev_origin || current_origin || {} ).id,
        top_most_origin = origin_menu.$node.element;

        __$elem.attr( {
            "aria-expanded" : "true",
            "aria-controls" : __child_comp.id
        });

        if( origin_menu != this ){
            $L( __child_comp ).attr( label_attr, cur_id );
        } else {
            $L( top_most_origin ).attr( act_desc, cur_id );
            $L( __child_comp ).removeAttr( label_attr ).attr( act_desc, cur_id );    
        }
    ...
    openMenu: function (event, flagg) {
        var onBeforeOpen, eventType = this.getData('ltPropEvent'), 
        targetDiv,origin_position = this.data.ltPropOriginPosition;
        event = event ? event : {};

        if (!flagg && !origin_position ) {
            targetDiv = this.targetElem.call(this, event.target);
            if (targetDiv[0]) {
                targetDiv[0].originMenu = targetDiv[1].component
            }
        }
        if (flagg && eventType != 'contextmenu' && !origin_position ) {
            if (!this.$node.element) {
                var query = this.getData('ltPropQuery');
                this.$node.element = document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ](query.trim());
            }
        ...
        /**
         * This method is called before opening the menu
         * @method onBeforeOpen
         * @author ponkarthikeyan.t@zohocorp.com
         * @version 1.0.0
         * @param { object } menuElement
        ...
        if ( ( !this.data.ltPropSyncOpen || !event.type ) && this.getMethods('onBeforeOpen')) {
            onBeforeOpen = this.executeMethod('onBeforeOpen', this.$node, event, this.$node.element);
        }

        if (!origin_position && targetDiv && targetDiv[0] && targetDiv[1].tagName == 'LYTE-MENU') {
        ...
        if (onBeforeOpen != false && ( this.$node.element || origin_position ) ) {

            _lyteUiUtils.dispatchEvent( 'beforeshow', this.$node, { originalEvent: event, component : this.$node, wormhole : this.childComp } );
            ...
            if (!origin_position && targetDiv[0]) {
            ...
            if( !this.parentMenu ){
                ...
                if( this.getData( 'ltPropFreeze' ) ){
                    ...
                    if( _lyteUiUtils.isWidget ){
                        _lyteUiUtils.addGlobalEventListener( 'scroll', this.addScrollPos, true );
                    } else {
                        window.addEventListener( 'scroll', this.addScrollPos, true );
                    }
                    _lyteUiUtils.menu = this;
                }
            }
            ...
            if ((/^(mouseenter|mousemove|mouseover|hover)$/.test(eventType)) && !this.childMenu && !this.parentMenu && !targetDiv[0]) {
                document[ "addEventListener" + ( _lyteUiUtils.isWidget ? "Global" : "" ) ]('mousemove', this._hoverclose);
            ...
            this._openFastdom = $L.fastdom.measure(function () {
                // if( origin_position && !origin_position.ignoreScrollPosition ){
                //     var /*mbcr = this.menuBody.getBoundingClientRect(),
                //         iH = window.innerHeight,*/
                //         yOff = window.pageYOffset || document.documentElement.scrollTop,
                //         xOff = window.pageXOffset || document.documentElement.scrollLeft;

                //         this.__initial_sL = xOff;
                //         this.__initial_sT = yOff;
                // }
                this._dontCall = true;
                ...
                !this.data.ltPropAnimate && this.traverseList({ key : "Home" });

                /**
                 * This method is called after opening the menu
                 * @method onOpen
                 * @author ponkarthikeyan.t@zohocorp.com
                 * @version 1.0.0
                 * @param { object } menuElement
            ...
            this._dontCall = true;
            !this.data.ltPropBoundaryRestore && delete this.$node.element;
            this.setData('ltPropShow', false)
    ...
    heightSet: function () {
        ...
        var style = getComputedStyle(this.menuBody);
    ...
    composePath: function (event) {
        ...
        shadowRoot = node && node.shadowRoot;

        if( this.data.ltPropShadowRootCheck && shadowRoot ){
            return this.getTargetFromShadowRoot( shadowRoot, event );
        }
    ...
    getTargetFromShadowRoot : function( shadowRoot, evt ){
        var elem = shadowRoot.elementFromPoint( evt.clientX, evt.clientY );

        if( elem && elem.shadowRoot ){
            return this.getTargetFromShadowRoot( elem.shadowRoot, evt );
        }

        return shadowRoot.elementsFromPoint( evt.clientX, evt.clientY );
    },
    ...
    hoverClose: function (event) {
        if (this.$node) {
            ...
            origin_elem = this.$node.element;

            if ( ( !origin_elem || elements.indexOf( origin_elem ) == -1 ) && elements.indexOf(this.childComp) == -1 && !this.eligibleForClose(this, elements[0])) {
                this.hideMenu(true, event);
            }
    ...
    mouseleave: function (event) {
        ...
        if (component.childMenu) {
            if (target[1] == component.$node && target[0] != component.isInsideMenuItem( component.childMenu.$node.element ) && target[0]) {
    ...
    hideMenu: function (flag, event, flag2) {
        ...
        if (this.childMenu) {
            if (!this.childMenu.childComp.contains(event.target) || flag2) {
                if (!this.childMenu.hideMenu.call(this.childMenu, flag, event, flag2)) {
                    return
                }else if(this.data.ltPropClearSelectionOnHoverClose){
                    let _selection = this.childComp.querySelector('.lyteMenuSelection')
                    if (_selection) {
                        _selection.classList.remove('lyteMenuSelection')
                    }
                }
            } else {
                return
        ...
        /**
         * This method is called before closing the menu
        ...
        if (onBeforeClose != false) {
            ...
            delete this.__cX; 
            delete this.__cY;
            var parent_menu = this.parentMenu;

            if( parent_menu ){
                delete parent_menu.childMenu
                delete this.parentMenu
                parent_menu.setup_controls( this.$node.element );
            }
            else {
                ...
                if( _lyteUiUtils.isWidget ){
                    _lyteUiUtils.removeGlobalEventListener( 'scroll', this.addScrollPos, true );
                } else {
                    window.removeEventListener('scroll', this.addScrollPos, true);
                }
            }
            ...
            if (/^(mouseenter|mousemove|mouseover|hover)$/.test(this.data.ltPropEvent) != -1) {
                ...
                document[ "removeEventListener" + ( _lyteUiUtils.isWidget ? "Global" : "" ) ]('mousemove', this._hoverclose);
            }
            this._hideStarts = true;
            // $L.fastdom.mutate( function(){
    ...
    closing: function () {
        ...
        if( this.data.ltPropAria ){
            ...
            $L( this.childComp ).removeAttr( 'aria-activedescendant aria-labelledby' );
        }
        
        delete this._dontCall;
        !this.data.ltPropBoundaryRestore && delete this.$node.element;
        delete this._evtadded;
        this.childComp.classList.remove('lyteAnimate');
        
        this.childComp.style.left = '';
        this.childComp.style.top = '';
        this.childComp.style.bottom = '';
        this.childComp.style.right = '';
        ...
        if( frm_direct && !promise_res && !this.data.ltPropAnimate ){
            ...
            /**
             * This method is called after closing the menu
             * @method onClose
             * @author ponkarthikeyan.t@zohocorp.com
             * @version 1.0.0
             * @param { object } menuElement
    ...
    optionSelect: function (event) {
        ...
        if( isGroupMenu && !this._ignore ){
            ...
            let menuBodystyle = getComputedStyle(this.menuBody);
            ...
            var menuGroupTransEnd = function( flag ){
                ...
                if( flag ){
                    // /**
                    //  * This method is called before opening a menu group
                    //  * @method onMenuGroupOpen
                    //  * @author ponkarthikeyan.t@zohocorp.com
                    //  * @version 1.0.0
                    //  * @param { string } valueOfTheSelectedItem
                    //  * @param { object } event
                    //  * @param { object } menuElement
                    //  * @param { object } menuOriginElement
                    //  * @param { object } subMenuDetails
                    //  */
                    if( this.getMethods('onMenuGroupOpen') ){
                        this.executeMethod('onMenuGroupOpen', value, event, this.$node, this.$node.element, _lyteUiUtils.isWidget ? nodeName : { element: nodeName, submenu: !!nodeName.originMenu } );
                    }
                    this.setAuto();
                }else{
                    // /**
                    //  * This method is called before closing a menu group
                    //  * @method onMenuGroupClose
                    //  * @author ponkarthikeyan.t@zohocorp.com
                    //  * @version 1.0.0
                    //  * @param { string } valueOfTheSelectedItem
                    //  * @param { object } event
                    ...
                    if( this.getMethods('onMenuGroupClose') ){
                        this.executeMethod('onMenuGroupClose', value, event, this.$node, this.$node.element,  _lyteUiUtils.isWidget ? nodeName : { element: nodeName, submenu: !!nodeName.originMenu } );
                    }
                }
            ...
            if( elem.hasClass('lyteMenuAccordionOpened') ){
                ...
                if( this.getMethods('onBeforeMenuGroupClose') ){
                    ret = this.executeMethod('onBeforeMenuGroupClose', value, event, this.$node, this.$node.element,  _lyteUiUtils.isWidget ? nodeName : { element: nodeName, submenu: !!nodeName.originMenu } );
                }
                ...
                if( this.getMethods('onBeforeMenuGroupOpen') ){
                    ret = this.executeMethod('onBeforeMenuGroupOpen', value, event, this.$node, this.$node.element,  _lyteUiUtils.isWidget ? nodeName : { element: nodeName, submenu: !!nodeName.originMenu } );
                }
        ...
        if (this.getMethods('onMenuClick') && nodeName) {
            ...
            flag = this.executeMethod('onMenuClick', value, event, this.$node, this.$node.element,  _lyteUiUtils.isWidget ? nodeName : { element: nodeName, submenu: !!nodeName.originMenu });
        }
    ...
    */
     ...
     */
    setCss: function (onlyScroll, _pos, maintainPos ) {
        ...
        if( arguments.length === 0 && this.menuBody.style.height.includes('px') ){ 
            this.menuBody.style.height = this.data.ltPropHeight || '';
        }
        ...
        origin_position = this.data.ltPropOriginPosition,
        query = this.getData('ltPropScope'), 
        flag;

        if( origin_position ){
            var cb = "onGetOriginPosition";
            if( this.getMethods( cb ) ){
                origin_position = this.executeMethod( cb, origin_position, this.$node ) || origin_position;
            }
        }

        if( query) {
            var temp = { target: par || this.$node };
            // var elemm = this.closestFind.call(this, this.composePath.call(this, temp), query);

            var elemm = query ? ( _lyteUiUtils.isWidget && Lyte.Component.closestGlobal ? Lyte.Component.closestGlobal( this.$node, query ) : this.closestFind.call(this, this.composePath( temp ), query ) ) : void 0;
        ...
        var wtop = 0,
            ...
            position = parentPos ? ( (parentCustomPos && parentPos != parentCustomPos) ? parentCustomPos : (_pos || this.getData('ltPropPosition')) ) : (_pos || this.getData('ltPropPosition')),
            offsets = origin_position || par.getBoundingClientRect(),
            ...
            if( origin_position ){
                offsets = $L.extend( {}, offsets );
            }

            if( offsets.right == void 0 ){
                offsets.right = offsets.left + offsets.width;
            }

            if( offsets.bottom == void 0 ){
                offsets.bottom = offsets.top + offsets.height;
            }

        if( menu_header ){
            $L( link ).addClass( 'lyteMenuWithHeader' );
        }else{
            $L( link ).removeClass( 'lyteMenuWithHeader' );
        }
        
        if( menu_footer ){
            $L( link ).addClass( 'lyteMenuWithFooter' );
        }else{
            $L( link ).removeClass( 'lyteMenuWithFooter' );
        }
        ...
        var checkPosition = function(){
            if( ( parentCustomPos == 'right' || parentCustomPos == 'left' ) && parentCustomPos != pos ){
                if( (pos == 'right' || pos == 'left')  ){
                    this.setData( 'customPosition', pos );
                }
        ...
        ['top','bottom','left','right'].forEach(function(item){
            body.style.setProperty(item, "unset");
        });
        ...
        if (position === 'down') {
            ...
            if (topPos || ( maintainPos && cusPos=='up') ) {
                ...
                if (callout) {
                    this.setCorrectClass('lyteArrowBottom');
                    tempStore = aHeight - (window.scrollY - ( wheight - offsets.top )) + 'px'
                    // tempStore = this.setTopAboveForDown(par, body, offsets, drop, y) - aHeight + 'px';
                }
                else {
                    body.style.bottom = ( (window.scrollY - ( wheight - offsets.top )) * -1 ) + 'px'
        ...
        if( isParentMenu && (pos == 'right' || pos == 'left')  ){
            this.setData( 'customPosition', pos );
        }else{
            this.setData( 'customPosition', '' );
        }
        ...
        let setMenuBodyHeight = function( height ){
            let propHeight = this.getData('ltPropHeight');
            if( /px$/.test( propHeight ) ){
                if( parseFloat( propHeight ) < height ){
                    this.menuBody.style.height = propHeight;
                    return true;
                }else{
        ...
        if (parseFloat(body.style.top) - y + bodyHeight > wheight && offsets.top <= wheight) {
            setMenuBodyHeight( (wheight - parseFloat(body.style.top) + y - padding) );
            bodyHeight = parseFloat(this.menuBody.style.height);
            this._dont_modify = true;
        }else if (parseFloat(body.style.top) - y < wtop && offsets.bottom >= wtop) {
            val = (parseFloat(body.style.top) - y + bodyHeight - wtop);
            ret = setMenuBodyHeight( val );
            body.style.top = (wtop + y + ( ret ? val - parseFloat(this.menuBody.style.height) : 0 )) + 'px';
            bodyHeight = parseFloat(this.menuBody.style.height);
            this._dont_modify = true;
        }else{
            let bottom = parseFloat(body.style.bottom);
            if( (bottom > 0) && bottom - y + bodyHeight > wheight && offsets.top <= wheight ){
                setMenuBodyHeight( (wheight - bottom + y - padding) );
                bodyHeight = parseFloat(this.menuBody.style.height);
                this._dont_modify = true;
            }else if( (bottom < 0) && y + bottom + bodyHeight > wheight && offsets.top >= wtop ){
                setMenuBodyHeight( wheight - (y + bottom) - padding );
                bodyHeight = parseFloat(this.menuBody.style.height);
                this._dont_modify = true;
            }
        }
    ...
    checkForBoundary: function (clientRect, ww, flag) {
        // var clientRect = this.$node.element.getBoundingClientRect(), ww = window.innerWidth;
        var boundary = this.getData('ltPropBoundary');

        var isOutside = (boundary.hasOwnProperty('left') ? (this.rtlfunc.call(this, 'left', clientRect, ww) < this.rtlfunc.call(this, 'left', boundary, ww)) : false) || (boundary.hasOwnProperty('right') ? (this.rtlfunc.call(this, 'right', clientRect, ww) > this.rtlfunc.call(this, 'right', boundary, ww)) : false) || (boundary.hasOwnProperty('top') ? (clientRect.top < boundary.top) : false) || (boundary.hasOwnProperty('bottom') ? (clientRect.bottom > boundary.bottom) : false);
        var restore = this.data.ltPropBoundaryRestore;

        if (isOutside && !flag) {
            this.hideMenu.call(this, true, {} );

            if( restore ){
                this._scrollFunc = this.callScrollFunc.bind(this);
                window.addEventListener('scroll', this._scrollFunc, true);
            }
        }else if( !isOutside && flag && restore ){
            this.openMenu({}, true);
            window.removeEventListener('scroll', this._scrollFunc, true);
        }
    },

    get_origin_menu : function(){
        var __menu = this;

        while( __menu.parentMenu ){
            __menu = __menu.parentMenu;
        }

        return __menu;
    },

    traverseList: function (event, force_prevent ) {
        var kc = event.keyCode || event.which,
        child_comp = this.childComp;

        if ( !child_comp || ( child_comp && child_comp.classList.contains('lyteMenuHidden')) || ( kc && kc != 13 && kc != 40 && kc != 38 && kc != 36 && kc != 35 ) ) {
            return
        }
        ...
        var fn = function( elem ){
            if( this.data.ltPropAria ){
                ...
                var __menu = this.get_origin_menu();
                $L( __menu.$node.element ).attr( 'aria-activedescendant', ( elem || {} ).id ).attr( "aria-controls", child_comp.id );
                $L( child_comp ).attr( 'aria-activedescendant', ( elem || {} ).id );
            }

            
            if( !this.data.ltPropPreventDefaultSelection ){
                elem.classList.add( 'lyteMenuSelection' );
            }
            this.scrIntoView( elem );

            /**
             * This method is called during the keyboard navigation of menu items
             * @method onNavigation
             * @author ponkarthikeyan.t@zohocorp.com
             * @version 3.96.0
             * @param { string } valueOfTheSelectedItem
             ...
             */
            var cb = "onNavigation";
            this.getMethods( cb ) && this.executeMethod( cb, elem.getAttribute( 'data-value' ), event, this.$node, this.$node.element,  _lyteUiUtils.isWidget ? elem : { element : elem, submenu : !!elem.originMenu } );
            
        }.bind( this ),

        __key = event.key; 

        var cursel = child_comp.querySelector('.lyteMenuSelection'),
        elements = Array.from( child_comp.getElementsByTagName( 'lyte-menu-item' ) ).filter( function( item ){
            var __$elem = $L( item );
            return !( __$elem.hasClass( 'lyteSearchHidden' ) || __$elem.hasClass( 'lyteMenuItemDisabled' ) );
        });
        ...
        var checkSelection = function(){
            ...
            let _elems = Array.from( child_comp.getElementsByTagName( 'lyte-menu-item' ) ).filter( function( item ){
                var __$elem = $L( item );
                return !( __$elem.hasClass( 'lyteSearchHidden' ) || __$elem.hasClass( 'lyteMenuItemDisabled' ) );
            });
        ...
        clearTimeout( this.__selection_time );
        this.__selection_time = setTimeout( checkSelection, 0 );
    ...
    setFreeze: function (nodeName) {
        var fz = document.querySelectorAll('.lytemenufreezelayer:not(.nogroup)')
        // freeze bound calculation
        $L.fastdom.measure(function () {
            var node = document.querySelector('.lytemenufreezelayer.left'), rect = nodeName.getBoundingClientRect(),
                iwdt = window.innerWidth
            $L.fastdom.mutate(function () {
                node.style.height = rect.height + "px"
                node.style.width = Math.max(rect.left, 0) + "px"
                node.style.top = rect.top + "px"
                node = document.querySelector('.lytemenufreezelayer.right')
                node.style.height = rect.height + "px"
                node.style.width = Math.max(iwdt - rect.right, 0) + "px"
                node.style.top = rect.top + "px"
                node = document.querySelector('.lytemenufreezelayer.top')
                node.style.height = rect.top + "px"
                node = document.querySelector('.lytemenufreezelayer.bottom')
    ...
    addScrollPos: function (event) {
        if (!_lyteUiUtils.menu) {
            return
        }
        if (event.target != window && ((_lyteUiUtils.menu.childComp.contains(event.target)) || ( (_lyteUiUtils.menu.childMenu && _lyteUiUtils.menu.childMenu.childComp.contains(event.target)) ) ) ) {
            return
        }
        var component = _lyteUiUtils.menu;
        component.callScrollFunc(event, true);

    },
    removeFreeze: function () {
        if (!_lyteUiUtils.menu || (this.data.ltPropAnimate && this.data.ltPropFreeze)) {
            var fz = document.querySelectorAll('.lytemenufreezelayer')
            for (var i = 0; i < fz.length; i++) {
    ...
    setZIndex: function (flag) {
        ...
        __fn = function(){
            var freezeLayer = document.querySelector('.lytemenufreezelayer.nogroup');
            freezeLayer.classList.remove('lyteMenuHidden');
            document.body.classList.add('lyteBodyWrapper');
        };
        if (nodeName) {
            while( nodeName && nodeName.tagName && nodeName.tagName != 'HTML') {
            ...
            if ( ( nodeName && nodeName.tagName == 'HTML' ) || !nodeName || nodeName.host ) {
                if (flag && (!_lyteUiUtils.menu || (_lyteUiUtils.menu && !_lyteUiUtils.menu.data.ltPropFreeze))) {
                    this.removeFreeze.call(this)
                }
                else {
                    __fn();
        ...
        } else if( this.data.ltPropOriginPosition ){
            __fn();
        }
    ...
    callScrollFunc: function (evt, callBoundary) {
        var comp = this,
            ...
            origin_element = comp.$node.element,
            origin_position = this.data.ltPropOriginPosition;

            if( origin_position ){
                var cb = "onGetOriginPosition";
                if( comp.getMethods( cb ) ){
                    origin_position = comp.executeMethod( cb, origin_position, comp.$node ) || origin_position;
                }
            }

            var bcr = (callBoundary || !show) && boundaryKeys && ( origin_element || origin_position ) ? ( origin_position || origin_element.getBoundingClientRect() ) : undefined,
            ww = (callBoundary || !show) && boundaryKeys && ( origin_element || origin_position ) ? window.innerWidth : undefined,
            given = comp.$node.ltProp('height'),
            allowreset = this.data.ltPropHeightResetOnScroll,
            isContext = comp.$node.ltProp('event') == 'contextmenu',
            show = this.data.ltPropShow;
        ...
        if (height && allowreset) {
            ...
            if( !show ){
                boundaryKeys && $L.fastdom.measure(comp.checkForBoundary.bind(comp, bcr, ww, true));
            }else{
    ...
    push_global_stack : function(){
        var arr = ( _lyteUiUtils.popupStack || {} ).globalStack;
    ...
    width_hgt_obs : function( arg ){
        if( arg.item == "ltPropWidth" ){
            this.childComp.style.width = arg.newValue || '';
        } else {
            this.menuBody.style.height = arg.newValue || '';
        }
    }.observes( 'ltPropWidth', 'ltPropHeight' )
});


var lyteCloseMenu = function( evt, element, flag, close_submenu_alone ){
    if( _lyteUiUtils._lyteMenu.preventClick != false || element ){
        if( ( evt && evt.button != 2 ) || element || flag ){
            var menus = document.getElementsByTagName( 'lyte-menu' ),
            target = ( evt || {} ).target;

            // if( target && target.offsetParent && !document.containsGlobal( target ) ){
            //     return;
            // }

            for( var i = 0; i < menus.length; i++ ){
                var cur_menu = menus[ i ];
                if( cur_menu.classList.contains( 'lyteMenuClosed' ) ||  !cur_menu.hasAttribute( 'lyte-rendered' ) || ( cur_menu.ltProp('preventKeydown') && evt.type === 'keydown' ) ){
                    continue;
                }
    ...
    if( evt && evt.type == 'click'){
        _lyteUiUtils._lyteMenu.preventClick = true;
    }
}
...
if (!_lyteUiUtils.registeredCustomElements['lyte-menu-item']) {
    ...
    function menukeypress(evt) {
        ...
        for (var i = 0; i < menus.length; i++) {
            ...
            if (!_this.childMenu) {
                ...
                if( _this.data.ltPropForceNavigation || _this.is_input_element( evt.target, keyCode ) ){
                    continue;
                }
    ...
    /**
     * @customElement lyte-menu-item It is used to create individual menu items in a menu.
     * @author Pon karthikeyan T <ponkarthikeyan.t@zohocorp.com>
     * @customAttribute {true | false} lt-prop-disabled=false It is used to disable the menu item.
     * @attribute lyte-shortcut It is used to set a keyboard shortcut for the menu item.
     */

    /**
     * @customElement lyte-menu-body It is used to create the body of the menu.
     * @author Pon karthikeyan T <ponkarthikeyan.t@zohocorp.com>
     */

    /**
     * @customElement lyte-menu-description It is used to create the description of the menu item.
     * @author Pon karthikeyan T <ponkarthikeyan.t@zohocorp.com>
     */

    /**
     * @customElement lyte-menu-label It is used to create the label of the menu item.
     * @author Pon karthikeyan T <ponkarthikeyan.t@zohocorp.com>
     */

    /**
     * @customElement lyte-menu-group It is used to create a group of menu items.
     * @author Pon karthikeyan T <ponkarthikeyan.t@zohocorp.com>
     */

    /**
     * @customElement lyte-menu-header It is used to create the header of the menu.
     * @author Pon karthikeyan T <ponkarthikeyan.t@zohocorp.com>    
     */
    ...
    Lyte.createCustomElement("lyte-menu-item", {
        ...
        connectedCallback: function () {
            ...
            parent = closestBody ? ( closestBody.parent || closestBody.closest( 'lyte-menu' ) ) : void 0;


            if( parent ) {
                var aria = parent.ltProp('aria');
                if (aria) {
...
/**
 * @syntax nonYielded
 * @dollar 0 [{"label" : "Open File", "description" : "Ctrl + O"},{"label" : "New File", "description" : "Ctrl + N"},{"label" : "Save File", "description" : "Ctrl + S"}]
 * <lyte-menu lt-prop-content='{{$0}}' lt-prop-user-value = 'label' lt-prop-description = 'description' lt-prop-event="click" ></lyte-menu>
 */
