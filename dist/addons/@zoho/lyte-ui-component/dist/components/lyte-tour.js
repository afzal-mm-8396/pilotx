...
_observedAttributes :["ltPropTakeTour","ltPropBindToBody","ltPropWrapperClass","ltPropParentWrapperClass","ltPropCreateDummy","ltPropHeight","ltPropWidth","ltPropArrowWidth","ltPropArrowHeight","ltPropArrowPosition","ltPropFixedArrow","ltPropType","ltPropMarginFromTarget","ltPropShortCut","ltPropFreezeLayer","ltPropSmartPlacement","ltPropResumeHint","ltPropResumeStep","ltPropIgnoreTabKey","ltPropPreventOutsideFocus","isStepClosed","isResizeEventOn","windowDimension","tourHintIndex","hintElements","tourStepIndex","totalHints","totalSteps","localSteps","scrollTop","closeStepFlag","arrowDiagonalLength","tourPaused","tourRunning"],
_observedAttributesType :["boolean","boolean","string","string","boolean","string","string","number","number","string","boolean","string","string","boolean","boolean","boolean","number","number","boolean","boolean","boolean","boolean","object","number","array","number","number","number","number","number","boolean","number","boolean","boolean"],

	_lyteUtilFunctions:['startTour','closeStep','nextStep','prevStep','goToStep','changeHint','skipTour','pauseTour','resumeTour','getTourStatus'],
	data : function(){
		return {
			...
			'ltPropShortCut'			: Lyte.attr('boolean' , { default : true }),
			...
			'tourPaused'				: Lyte.attr('boolean' , {default : false}),
	...
	setTourDimensions : function(){
		$L(this.actualTourDiv).find('.lyteTourContainer').css({
			   'width' : this.getData('ltPropWidth'),
			   'height' : this.getData('ltPropHeight')
		});
		if($L(this.actualTourDiv).find('.lyteTourArrow')[0]){
		   $L(this.actualTourDiv).find('.lyteTourArrow').css({
	...
	init : function(){
		...
		this.$node.moveContainer = function(){
			...
			currentStep.component.startLyteStep( currentStep  , true);

		}

		this.$node.alignStep = function(){
			setCurrentHint();
			setCurrentStep();

			if($L('.lyteTourContainer')[0]){
				$L('.lyteTourContainer')[0].style.transition = "unset"
			}
			currentStep.component.startLyteStep( currentStep , true);
			setTimeout(function(){
				if($L('.lyteTourContainer')[0]){
					$L('.lyteTourContainer')[0].style.transition = ""
				}
			},20)
		}
	...
	leftKeyFunction : function(){
		if(this.getData('tourRunning') && (!(this.getData('closeStepFlag')))&&(this.getData('ltPropBindToBody'))){
			this.$node.prevStep();
	  	}
	},
