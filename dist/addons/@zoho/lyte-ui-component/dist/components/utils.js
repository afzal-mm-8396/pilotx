if( window.document == document ){
	window._lyteUiUtils = window._lyteUiUtils || { version : "3.122.3" };
} else {
	var _lyteUiUtils = { version : "3.122.3" };
}
...
_lyteUiUtils.globalConfig = {};
_lyteUiUtils.setGlobalConfig = function( key, name ){
    if( typeof key == "string" ){
        let obj = {};
        obj[ key ] = name;
        key = obj;
    }  
    $L.extend( this.globalConfig, key );
};
...
_lyteUiUtils.convertHexToRgb = function(hex){
...
_lyteUiUtils.contains = function( element ) {
	var isWidget = _lyteUiUtils.isWidget;

	if( isWidget ) {
		return document.containsGlobal( element );
	}
	
	return document.contains( element );
}

// pass all values as date objects. today is always defined
_lyteUiUtils.getViewDate = function( start, end, today, current ) {
	var isStartDefined = ( start && start !== 'Invalid Date' ),
	isEndDefined = ( end && end !== 'Invalid Date' ),
	isCurrentDefined = ( current && current !== 'Invalid Date' ) ;

	start = isStartDefined ? start : new Date( '01/01/1900' );
	end = isEndDefined ? end : new Date( '01/01/2100' );

	if( isCurrentDefined && current >= start && current <= end ) {
		return current;
	}
...
_lyteUiUtils.getTodaysDateBasedOnTimeZone = function( comp_timezone, isHavingTimezone ) {
...
_lyteUiUtils.getImageFromUrl = function(url) {
	return new Promise( function( resolve, reject ) {
		var convertToFile = function( img ){
			...
			try{
				...
				canvas.toBlob( function( blob ){
					...
					const fileName = fileNameWithExt.substring( 0, lastDotIndex  ) || "image";
		...
		var imageFailure = function( error ){
			reject( error );
		};
		var __img = new Image();
		...
		__img.onerror = imageFailure;
		__img.onabort = imageFailure;
...
_lyteUiUtils.getZIndex = function(){
	...
	if(globalStack && globalStack.length>1){
		var prevEle = $L(globalStack[globalStack.length-2].childElement)
		var prevZ;
        for(var i=_lyteUiUtils.popupStack.globalStack.length-2;i>=0;i--){
			if(_lyteUiUtils.popupStack.globalStack[i].childElement.tagName ==='LYTE-WORMHOLE'){
				prevZ = $L(_lyteUiUtils.popupStack.globalStack[i].childElement).find('.lytePopupZI')
			}
		}
		if(prevZ[0]){
...
_lyteUiUtils.lyteUiFileSize = function( curr, def, dgt, frmt ){
	var sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'], idx = 0;
	var sizeValue = frmt && frmt === "binary" ? 1024 : 1000;
	if(curr == 0) {
		return "0 Byte";
	}
	if( def ) {
		...
		idx = Math.floor( Math.log( curr ) / Math.log( sizeValue ) )
	}
	if( idx == 0 && curr == 1 ){
		return "1 Byte";
	}
	return ( parseInt( curr / Math.pow( sizeValue , idx ) * Math.pow( 10, dgt ) ) / Math.pow( 10, dgt ) ) + ' ' + sizes[ idx ];

};

// _lyteUiUtils.getDeviceInfo = function () {
...
// 		// Special case: iPadOS 13+ reports as Mac
// 		if (os === 'mac' && maxTouchPoints > 1) os = 'ios';
// 	}
...
// 	if (isTouch) {
// 		if (width <= 599) deviceType = 'mobile';
// 		else if (width > 599 && width <= 1024) deviceType = 'tablet';
// 		else deviceType = 'desktop'; // large touchscreens (e.g., touchscreen laptops)
// 	}

// 	return { os, deviceType, isTouch }
// }

// _lyteUiUtils.devicePlatform = _lyteUiUtils.getDeviceInfo().os;

// _lyteUiUtils.deviceType = _lyteUiUtils.getDeviceInfo().deviceType;

// _lyteUiUtils.isTouchDevice = _lyteUiUtils.getDeviceInfo().isTouch;

// _lyteUiUtils.isMobile = _lyteUiUtils.deviceType === 'mobile';

// _lyteUiUtils.isTablet = _lyteUiUtils.deviceType === 'tablet';

// _lyteUiUtils.isIOS = _lyteUiUtils.devicePlatform === 'ios';

// _lyteUiUtils.isAndroid = _lyteUiUtils.devicePlatform === 'android';

// Accessibility
_lyteUiUtils.reduceAnimation = function(){
	$L('body').addClass('lyteUICompReducedAnim lyteUiCompReduceAnim');
}

_lyteUiUtils.enableAnimation = function(){
	$L('body').removeClass('lyteUICompReducedAnim lyteUiCompReduceAnim')
}
...
Lyte.Component.registerHelper('lyteUiColorboxAddClass',function(wrapperClass,elem){	
	var classStr = 'lyteColorbox lyteCBox ' + wrapperClass;
	if(elem.classList.length){
		if(elem.classList.contains(this.component._className)){
			classStr += ' ' + this.component._className;
		}
...
Lyte.Component.registerHelper('lyteUiMyFontClass',function( font, myFonts ){
	var name = font.name;

	for( var i = 0; i < myFonts.length; i++ ) {
		if( myFonts[ i ].name === name ){
			return 'lyteMyFont';
		}
	}
...
Lyte.Component.registerHelper("lyteUiTreeClassHelp",function(state,col,open,close,wrapopen,wrapclose,exclusive){
	if((state === "open")||!col && !exclusive){
...
Lyte.Component.registerHelper("lyteTreeClassStateHelp",function(item , exclusive){
	if(((!item.defaultState)||(item.defaultState === "open")) && !exclusive){
		return "lyteTreeOpened";
	} else if(item.defaultState === "close" || exclusive){
...
Lyte.Component.registerHelper("lyteTreeDataType" , function(elem){
	...
	if(elem && typeof elem === 'number'){
		...
		if(typeof elem === 'number' && Number.isFinite(elem) && !Number.isInteger(elem)){
			return 'float'						
		}
...
Lyte.Component.registerHelper('lyteJsonViewerIndentHelper' , function(rawString){
	const matches = rawString.match(/\\t/g); 
	const tabCount = matches ? matches.length : 0;
	return tabCount
})
Lyte.Component.registerHelper('lyteJsonViewerFunctionHelper' , function(rawString){
	const str = rawString.replace(/\\t/g, '\t') 
	return str
})
Lyte.Component.registerHelper('lyteJsonVrDatatypeObj' , function(str){
	return {dataType : str};
})
Lyte.Component.registerHelper('lyteJsonVrObjLengthHelper' , function(item){
	if(Object.keys(item).length < 1){
		return ' lyteJsonVrEmptyObject'
	}
	return '';
})
Lyte.Component.registerHelper('lyteJsonTreeWithoutKey' , function(key , value){
	if(key === 'lyteJsonRoot'){
		return true
	}
	return false
...
Lyte.Component.registerHelper('lyteJsonVrArrLengthHelper' , function(item){
	if(item.length < 1){
		return ' lyteJsonVrEmptyArray'
	}
	return '';
})
Lyte.Component.registerHelper('lyteJsonViewerCommaHelper' , function(actualValue , data , item , type){
	...
	switch(type){
		case 'array':
			...
			var arrayIndex = data.indexOf(item)
		...
		case 'object':
			...
			var objIndex = Object.keys(data).indexOf(item)
...
Lyte.Component.registerHelper('lyteJsonFunctionParser' , function(key , value){
	return (key +' : '+value).toString()
})
Lyte.Component.registerHelper('lyteTreeGetObjectLength' , function(data){
	return Object.keys(data).length
})
Lyte.Component.registerHelper('lyteTreeDataLengthCalculator' , function(data){
	var dataType = Object.prototype.toString.call(data)

    if(dataType.split(' ')[1].includes('Array')){
      return data.length
    } else if(dataType.split(' ')[1].includes('Object')){
      return Object.keys(data).length
    }
})
Lyte.Component.registerHelper('lyteTreeGetSmallArray', function (data, index) {
	return data.slice(index - 99, index + 1)
})
Lyte.Component.registerHelper('lyteJsonVrIsUrlHelper' , function(str){
	const urlHttpsRegex = /^(https?:\/\/[^\s]+)/i; 
	const urlHttpRegex = /^(https?:\/\/[^\s]+)/i; 
	const urlWWWRegex = /^(www?:\/\/[^\s]+)/i; 
	if(urlHttpsRegex.test(str) || urlHttpRegex.test(str) || urlWWWRegex.test(str)){
		return true
	} else {
		return false
	}
})
Lyte.Component.registerHelper('lyteTreeDatatypePredictor', function (data, superLongStringMinLength, returnBool) {
	if (returnBool) {
		return checkIfURL() ? true : false; 
	}

	if (checkIfURL()) {
		return "lyteURLJsonData"
	} else if (checkIfFunction()) {
		return "lyteFunctionJsonData"
	} else if (checkIfSuperLongString()) {
		return "lyteSuperLongStringJsonData"
	}else { 
		return "lyteJsonData"
	}
	...
	function checkIfSuperLongString(minLength = superLongStringMinLength) { 
...
Lyte.Component.registerHelper('lyteDataTreeObjLength' , function(data){
	return Object.keys(data).length
})
Lyte.Component.registerHelper('lyteDataTreeArrayLength' , function(data){
	return data.length
})
...
Lyte.Component.registerHelper('lyteUiProgressbarLabel',function(label,percentage,showPercentage,ShowValue){ //used in progressbar
 	if(label){
 		return label;
 	}
...
Lyte.Component.registerHelper( 'lyteUiAttribute', function( value, aria, original_value, tab_index ){ // Used in slider
...
Lyte.Component.registerHelper( "lyteNumericOnly", function( value ){ // Used in slider
	if( typeof value == "string" ){
		if( isNaN( value ) ){
			switch( this.tagName ){
				case "LYTE-SLIDER":
				case "LYTE-MULTISLIDER": {
					return ( this.component.data.ltPropContent || [] ).indexOf( value );
				}
				break;
			}
		}
...
Lyte.Component.registerHelper( 'lyteSliderBoundary', function( ns, preferred_value ) { // Used in slider
	var __data = this.component.data,
...
//scheduler Helper
Lyte.Component.registerHelper("lyteUiSchedulerUserBusinessHour",function( businessHour, curr_time ){
	if(!businessHour){
		return ' ';
	}
	curr_time = new Date("01/01/2007 " + curr_time);
	var curr = ((curr_time.getHours() * 60) + curr_time.getMinutes());
	var is_business_hour = ' ';
	for(var index = 0; index < businessHour.length; index++){
		if( businessHour[index].start && businessHour[index].end ){
			var timeStart = new Date("01/01/2007 " + businessHour[index].start);
			var timeEnd = new Date("01/01/2007 " + businessHour[index].end);
			...
			if( ( compareStartDate >= 0 )&& ( compareEndDate >= 0 ) ){
				is_business_hour = 'lyteSchedulerUserBusinessHour ' + (businessHour[index].class ? businessHour[index].class : '');	
				break;
			}
		}
	}
...
Lyte.Component.registerHelper("lyteUiSchedulerCombineTimeMinutes",function( ltPropDateObj, userid,time, minutes ){
	if(!time){
		return time || '';
	}
	var match = time.match(/^(\d+)(am|pm)$/i);
	if(!match){
		return time;
	}
	...
	var return_arr = [];
	if(ltPropDateObj){
		if(ltPropDateObj[userid]){
			if(ltPropDateObj[userid][hour + minutes + suffix]){
				return_arr = ltPropDateObj[userid][hour + minutes + suffix];
			}
		}
	}
	return return_arr;
...
Lyte.Component.registerHelper( "lyteUiAddDataTabIndexBasedOnSelectedAndIndex", function( index, radioObject, systemValue, selectedObject, dataTabindex ) {
	if( selectedObject && selectedObject[ systemValue] === radioObject[ systemValue ] ) {
		return dataTabindex;
	}
	else if( ( !selectedObject || $L.isEmptyObject( selectedObject ) ) && index === 0 ) {
		return dataTabindex;
	}
});
...
if(!LytePopup){
    var LytePopup = {
        ...
        onEscape : function(evt){
            ...
            if (isEscape) {
                if($L(document.activeElement).hasClass('lyteDummyEventContainer')){
        ...
        addPopup : function(component) {
            ...
            if(_lyteUiUtils.popupStack && _lyteUiUtils.popupStack.globalStack && _lyteUiUtils.popupStack.globalStack.length > 0){
                if(_lyteUiUtils.popupStack.betaModalStack.length > 0 || _lyteUiUtils.popupStack.betaPopoverStack.length > 0){
                    var zIndex = _lyteUiUtils.getZIndex()
                    if(component.$node.tagName == "LYTE-MODAL"){
                        thisPopup = '.modalWrapper';
                    }
                    else if(component.$node.tagName == "LYTE-POPOVER"){
            ...
            if(node.classList.contains('lyteModal') || node.classList.contains('lytePopover')){
                ...
                for(var i = 0; i<buttons.length; i++){
                    if(buttons[i].type == "accept"){
                        ...
                        if($L(node).closest('lyte-wormhole')[0].component.parent.tagName === 'LYTE-ALERT'){
                            var alert = $L(node).closest('lyte-wormhole')[0].component.parent
                            if(alert.getData().ltPropAriaAttributes && alert.getData().ltPropAriaAttributes !== {}){
                                var arDes = alert.getData().ltPropAriaAttributes['aria-describedby']
                                $L(node.querySelectorAll('button')[i]).attr('aria-describedby' , arDes)
                            }
                        }
        ...
        transitionEnd : function(evt){
            if(evt.target == this && LytePopup.components.length > 0){
                ...
                if(comp.$node.tagName === 'LYTE-MODAL'){
                    if( 
                        (comp.getData().ltPropTransition.animation === 'fadeIn' && evt.propertyName === 'opacity') 
                    || 
                        (comp.getData().ltPropTransition.animation.includes('slideFrom') && evt.propertyName === 'transform')
                    ||  
                        (comp.getData().ltPropTransition.animation === 'zoom' && evt.propertyName === 'transform')
                    ){
...
_template:"<template tag-name=\"lyte-wormhole\" lyte-wormhole=\"\"> <lyte-yield yield-name=\"lyte-content\"></lyte-yield> </template>",
...
_observedAttributesType :["string","boolean","boolean","boolean","boolean","object"],
	...
	handleRemovalFromStack: function( utilObj ){
		...
		if(_lyteUiUtils.popupStack.globalStack.length >= 1){
			...
			if(changeFocus && lastActiveElement && this.getData('ltPropFocusOnClose')){
				if( lastActiveElement.parentNode && lastActiveElement.parentNode.tagName == "LYTE-TEXTEDITOR" ){
