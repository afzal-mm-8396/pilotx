...
_observedAttributesType :["boolean","string","array","array","array","object","object","boolean","string","string","string","string","string","array","array","string","string","string","boolean","boolean","boolean"],
	...
	openWormhole: function (event) {
		...
		addClass.removeClass('lyteDMSWormholeClosed');
		// $L('.lyteDMSWormholeOpen')[0].style.display = "block";
		this.wormholePosition(event);
		// $L('.lyteDMSButton')[0].style.width =  $L('.lyteDmsCalander')[0].clientWidth-(10 + 10)+"px";
		if (ltPropType == "popup") {
			var getSelectedDays = this.getData('ltPropSelectedDates');
			let placePosition = $L(wormhole).find('.lyteDmsWrap')[0];
			var selectedDays = this.getData('itemsArray');
			if (getSelectedDays.length > 0 && placePosition.length) {
				this.setData("lyteSelectionContainer", true);
				if (selectedDays.length == 1 || selectedDays.length == 0) {
					$L(placePosition).find('.lyteDMSRemoveAll')[0].style.display = "none";
				}
	...
	open: function (event) {
		...
		button.forEach(function (button) {
			if (button.classList.contains('lyteDMSWormholeOpen') && button != _this.$node && allowMultiple == false) {
				$L(button).removeClass('lyteDMSWormholeOpen');
				$L(button).addClass('lyteDMSWormholeClosed');
	...
	close: function (event) {
		...
		let wormhole = this.childComp;
		let parentElement = this.$node;
		if (ret) {
			return;
		}
		if(parentElement.classList.contains('lyteDMSWormholeOpen')){
			parentElement.classList.add('lyteDMSWormholeClosed');
			parentElement.classList.remove('lyteDMSWormholeOpen');
		}
	...
	methods: {
		...
		handleAddition: function (event, date) {
			...
			var sliceMonth = date.slice(0, 2);
			var convertMonth = Number(sliceMonth);
			var sliceDate = date.slice(3, 5);
			var sliceYear = date.slice(6, 10);
			var convertDate = Number(sliceDate);
			var convertYear = Number(sliceYear);
			var ltPropFormat = this.getData('ltPropFormat');
			let selectedDays = this.getData('itemsArray');
			let formattedDate;
...
_lyteUiUtils.addGlobalEventListener('click', function (event) {
	var target = event.target,
		parent = $L(target).closest('.lyteDMSContainer, .lyteDMSButton, .lyteDmsCalander, .lyteDmsSelectionContainer, .lyteDmsDropBoxClass');
	if (!parent.get(0)) {
		_lyteDMS.closeDMS();
...
window.addEventListener('scroll', function (event) {
	...
	button.forEach(function (button) {
		...
		comp.forEach(function (button) {
			if (button.classList.length > 1 && allowMultiple == false && ele)  {
