...
_observedAttributesType :["string","string","string","string","string","string","string","string","string","object","array","boolean","boolean","boolean","string","string","string","string","string","string","string","string","boolean","boolean","boolean","boolean","boolean","boolean","boolean","boolean","boolean","string","array","string","string","string","string","string","string","boolean","boolean","boolean","boolean","boolean","boolean","string","string","boolean","boolean","number","number","number","number"],
	...
	data : function(){
		return {
			...
			ltPropFormat : Lyte.attr('string' , {default : '12',input:true,output:true}),

			/**
			 * @componentProperty { digital | analog } ltPropType
			 * @default digital
			...
			ltPropType : Lyte.attr('string' , {default : 'digital',input:true,output:false}),

			/**
			 * @componentProperty {watch | timer | stopWatch} ltPropMode
			 * @allowedValuesDepends ltPropType
			 * @map digital watch, timer, stopWatch
			 * @map analog watch
			 * @default watch
			 * @input
			 */

			ltPropMode : Lyte.attr('string' , {default : 'watch' , input:true,output:false}),

			/**
			 * @componentProperty {timeString} ltPropDuration
			 * @condition ltPropMode timer
			 * @condition ltPropEndTime 00:00:00, null,
			 * @condition ltPropEndDate 00/00/0000, null,
			 * @default 00:00:00
			 * @format default HH:mm:ss
			 * @preventEmptySet true
			 * @input
			 * @output
			 */

			ltPropDuration :Lyte.attr('string',{default : '00:00:00',input:true,output:true}),

			/**
			 * @componentProperty {dateString} ltPropEndDate
			 * @condition ltPropMode timer
			 * @condition ltPropDuration 00:00:00, null,
			 * @default 00/00/0000
			 * @preventEmptySet true
			 * @input
			 * @output
			...
			ltPropEndDate :Lyte.attr('string',{default : '00/00/0000',input:true,output:true}),

			/**
			 * @componentProperty {timeString} ltPropStartTime
			 * @default 00:00:00
			 * @input
			 * @output
			 */

			ltPropStartTime : Lyte.attr('string',{default : '00:00:00',input:true,output:true}),

			/**
			 * @componentProperty {timeString} ltPropEndTime
			 * @format default HH:mm:ss
			 * @condition ltPropDuration 00:00:00, null,
			 * @condition ltPropMode timer
			 * @default 00:00:00
			...
			ltPropEndTime : Lyte.attr('string',{default : '00:00:00',input:true,output:true}),

			/**
			 * @componentProperty {timeString} ltPropAlarm
			 * @condition ltPropMode watch
			 * @condition ltPropType digital
			 * @default 00:00:00
			 * @format default HH:mm:ss
			...
			ltPropAlarm : Lyte.attr('string' , {default : '00:00:00',input:true,output:false}),

			/**
			 * @componentProperty {object} ltPropUnitLabel
			 * @default {object} {year  : 'Yrs',month : 'Mths',day   : 'Days',hour	: 'Hrs',minute : 'Mins',second : 'Secs'}
			 * @input
			 */
			...
			/**
			 * @componentProperty {object} ltPropUnits
			 * @input
			 */
			...
			/**
			 * @componentProperty {boolean} ltPropShowSecondsDial=true
			 * @version 1.0.0
			 * @input
			 */
			 ltPropShowSecondsDial : Lyte.attr('boolean' , {
				default : true,input:true,output:false
			 }),

			/**
			 * @componentProperty {boolean} ltPropStatic=false
			 * @version 1.0.0
			 * @input
			 */
			 ltPropStatic : Lyte.attr('boolean' , {
				default : false,input:true,output:false
			 }),

			 ltPropShowLapButton : Lyte.attr('boolean' , {
				default : true
			 }),
			...
			stopWatchMilliSec : Lyte.attr('number' , {
				default : 0
			}),
			stopWatchSec : Lyte.attr('number' , {
				default : 0
			}),
			stopWatchMin : Lyte.attr('number' , {
				default : 0
			}),
			stopWatchHrs : Lyte.attr('number' , {
				default : 0
			})
	...
	init : function(){
		...
		this.$node.startStopWatch = function(){
			this.component.startStopWatchFun()
		}

		this.$node.pauseStopWatch = function(){
			this.component.pauseStopWatchFun()
		}

		this.$node.resetStopWatch = function(){
			this.component.resetStopWatchFun()
		}

		this.$node.setCustomStopTime = function(hrs,mins,secs,millisecs){
			this.component.setCustomStopWatchTime(hrs,mins,secs,millisecs)
		}
	...
	isValidTime : function(gt){
		...
		if(isNaN(parseInt(arr[0])) || isNaN(parseInt(arr[1])) || isNaN(parseInt(arr[2]))){
			return false
		}
		if( parseInt(arr[0]) > 23 || parseInt(arr[1]) > 59 || parseInt(arr[2]) >59 ){
			return false
		}
		return true
	},
	isValidDate : function(gd){
		if(gd === undefined){
			return false;
		}
		var arr = gd.split('/');
		...
		if(parseInt(arr[2]) >= ct.getFullYear()){
			if(parseInt(arr[0]) >= ct.getMonth()){
				return true;
			} else {
				if(parseInt(arr[1]) >= ct.getDate()){
					return true
				}
				return false
			}
		...
		// if(parseInt(arr[2]) >= ct.getFullYear() && parseInt(arr[0]) >= ct.getMonth() && parseInt(arr[1]) >= ct.getDate()){
		// 	return true;
		// }
		return false;
	},
	...
	lyteTimer : function(){
		...
		switch (timerType) {
			...
			{
				if(!this.isValidDate(this.getData('ltPropEndDate'))){
					return
				}
				if(!this.isValidTime(this.getData('ltPropEndTime'))){
					return
				}
				...
				var dayPassedFlag = false

				if(isFullDatePassed(givenDate)){
					break
				} else {
					dayPassedFlag = true
				}

				if(isFullTimePassed(givenDate) && !dayPassedFlag){
					return
				}
				...
				// if((!_this.getData('ltPropShowDays')) && (!_this.getData('ltPropShowMonths'))){
				// 	_this.setData('ltPropShowSeconds' , true)	
				// }
			...
			case 'T':
			{
				...
				if(!this.isValidTime(this.getData('ltPropDuration'))){
					return
				}
		...
		function getNumberOfDaysInAMonth(date){
			...
			var totalMonths = new Date(year, month+1, 0).getDate()
			return totalMonths ;
		}
		...
		function getDaysDiff(c,g){
			...
			if(c.getDate() > g.getDate()){
				...
				if((c.getDate() == g.getDate()) && (g.getMonth() > c.getMonth())){
					daysDiff = (getNumberOfDaysInAMonth(g) - c.getDate()) + g.getDate() -1
				} else {
		...
		function isFullDatePassed(g){
			var ct = new Date();

			if(g.getFullYear() < ct.getFullYear()){
				return true
			} else if(g.getFullYear() == ct.getFullYear()){
				if(g.getMonth() < ct.getMonth()){
					return true
				} else if(g.getMonth() == ct.getMonth()){
					if(g.getDate() < ct.getDate()){
						return true
					} else if(g.getDate() == ct.getDate()){
		...
		function isFullTimePassed(t){
			var ct = new Date();
			if(t.getHours() > ct.getHours){
				return false
			} else if(t.getHours() === ct.getHours()){
				if(t.getMinutes() > ct.getMinutes){
					return false
				} else if(t.getMinutes() === ct.getMinutes()){
					if(t.getSeconds() > ct.getSeconds){
						return false
					} else if(t.getSeconds() === ct.getSeconds()){
						return true
					} else if(t.getSeconds() < ct.getSeconds()){
						return true
					}
		...
		function isSameDay(g){
			const today = new Date();
			var gd = new Date(dayTime)
    
			return (
				gd.getFullYear() === today.getFullYear() &&
	...
	setCustomStopWatchTime : function(hrs,mins,secs,millisecs){
		this.setData('ltPropStopMilliSeconds' , millisecs);
		this.setData('ltPropStopSeconds' , secs);
		this.setData('ltPropStopMinutes' , mins);
		this.setData('ltPropStopHours' , hrs);
		this.setData('stopWatchMilliSec' , millisecs);
		this.setData('stopWatchSec' , secs);
		this.setData('stopWatchMin' , mins);
		this.setData('stopWatchHrs' , hrs);
	},

	stopwatchIntervalFun : function(){
		var _this = this
		var stopWatchMilliSec = _this.getData('stopWatchMilliSec');
		var stopWatchSec = _this.getData('stopWatchSec');
		var stopWatchMin = _this.getData('stopWatchMin');
		var stopWatchHrs = _this.getData('stopWatchHrs');
		...
		if(!_this.getData('ltPropPause')){
			_this.setData('stopWatchMilliSec' , ++stopWatchMilliSec)
			if(stopWatchMilliSec === 100){
				_this.setData('stopWatchMilliSec',0)
				_this.setData('stopWatchSec' , ++stopWatchSec)
				_this.setData('ltPropStopSeconds' , _this.getData('stopWatchSec'));
			}
			_this.setData('ltPropStopMilliSeconds' , _this.getData('stopWatchMilliSec'));
			if(stopWatchSec === 60){
				_this.setData('stopWatchSec' , 0)
				_this.setData('stopWatchMin' , ++stopWatchMin)
				_this.setData('ltPropStopMinutes' , _this.getData('stopWatchMin'));
			}
			if(stopWatchMin === 60){
				_this.setData('stopWatchMin' , 0) 
				_this.setData('stopWatchHrs' , ++stopWatchHrs)
				_this.setData('ltPropStopHours' , _this.getData('stopWatchHrs'));
			}
			if(_this.getMethods('onStopWatchRunning')){
				_this.executeMethod("onStopWatchRunning" , {
					milliSec : _this.getData('ltPropStopMilliSeconds'),
					sec : _this.getData('ltPropStopSeconds'),
					min : _this.getData('ltPropStopMinutes'),
					hrs : _this.getData('ltPropStopHours')
				})
			}
		}
		if(stopWatchHrs > 1){
	...
	startStopWatchFun : function(){
		var _this = this;

		if(!_this.getData('stopWatchRunning')){
			_this.setData('ltPropPause' , false);
			_this.stopInterval = setInterval(_this.stopwatchIntervalFun.bind(this) , 10);
		}
	...
	pauseStopWatchFun : function(){
		var _this = this
		clearInterval(_this.stopInterval);
		_this.setData('stopWatchRunning' , false);
		_this.setData('ltPropPause' , true);
		_this.setData('ltPropStopHours' , _this.getData('ltPropStopHours'));
		_this.setData('ltPropStopMinutes' , _this.getData('ltPropStopMinutes'));
		_this.setData('ltPropStopSeconds' , _this.getData('ltPropStopSeconds'));
		_this.setData('ltPropStopMilliSeconds' , _this.getData('ltPropStopMilliSeconds'));
	...
	resetStopWatchFun : function(){
		...
		_this.setData('stopWatchMilliSec' , 0)
		_this.setData('stopWatchSec' , 1)
		_this.setData('stopWatchMin' , 1)
		_this.setData('stopWatchHrs' , 1)
	...
	lyteStopWatch : function(){
			...
			if($L(currentClock).find('.lyteClockSWStart')[0]){
				$L(currentClock).find('.lyteClockSWStart')[0].addEventListener('click' , stopWatchStart);
			}
	},
	lyteAnalogWatch : function(){
		if(!this.getData('ltPropStatic')){
			if(this.getData('ltPropShowSecondsDial')){
				var secondsDial = $L(this.$node).eq(0).find('.lyteClockSecDial')[0];
			}
	
			var minutesDial = $L(this.$node).eq(0).find('.lyteClockMinDial')[0];
			var hoursDial = $L(this.$node).eq(0).find('.lyteClockHrDial')[0];
			var time = new Date();
	...
	_showSecDial : function(){
		...
		this.lyteAnalogWatch();
	}.observes('ltPropShowSecondsDial'),
	_triggerClock : function(){
		this.lyteAnalogWatch();
	}.observes('ltPropStatic'),
	didDestroy : function(){
		function clearIntervalFun(fun){
			if(fun){
				clearInterval(fun);
