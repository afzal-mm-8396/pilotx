...
_observedAttributesType :["number","number","number","boolean","string","string","array","number","boolean","boolean","boolean","boolean","boolean","object","object","object","object","object","number","number","string","number","number"],
	...
	data: function () {
		return {
			 ...
			 * @input
			 */
			ltPropPerpage: Lyte.attr("number", {
				"default": _lyteUiUtils.resolveDefaultValue('lyte-navigator', 'perPage', 10),
				input: true
			}),
			/**
			 ...
			 * @input
			*/
			ltPropRecords: Lyte.attr("number", { "default": undefined, input: true }),
			/**
			 * @componentProperty {boolean} ltPropMoreRecords=false
			 * @input
			 */
			ltPropMoreRecords: Lyte.attr("boolean", { "default": false, input: true }),
			/**
			 * @componentProperty {string} ltPropMiddleText=to
			 * @condition ltPropType default
			 * @input
			 */
			ltPropMiddleText: Lyte.attr('string', {
				'default': _lyteUiUtils.resolveDefaultValue('lyte-navigator', 'middleText', 'to'),
				input: true
			}),
			/**
			 * @componentProperty {default | simple} ltPropType='default'
			 * @input
			 */
			ltPropType: Lyte.attr("string", {
				"default": _lyteUiUtils.resolveDefaultValue('lyte-navigator', 'type', 'default'),
				input: true
			}),
			paginationRange: Lyte.attr("array", { "default": undefined }),
			/**
			 * @componentProperty {number} ltPropSelected=0
			 * @condition ltPropType simple
			 * @input
			 * @output
			 */
			ltPropSelected: Lyte.attr("number", { "default": 0, input: true, output: true }),
			/**
			 * @componentProperty {boolean} ltPropShowOnlyIcon=false
			 * @condition ltPropType simple
			 * @input
			 */
			ltPropShowOnlyIcon: Lyte.attr("boolean", { "default": _lyteUiUtils.resolveDefaultValue('lyte-navigator', 'showOnlyIcon', false), input: true }),
			/**
			 * @componentProperty {boolean} ltPropShowText=false
			 * @condition ltPropType simple
			 * @input
			 */
			ltPropShowText: Lyte.attr("boolean", {
				"default": _lyteUiUtils.resolveDefaultValue('lyte-navigator', 'showText', false),
				input: true
			}),
			/**
			 ...
			 * @input
			 */
			ltPropAria: Lyte.attr('boolean', { 'default': true, input: true }),
			/**
			...
			'ltPropEndText': Lyte.attr('string', { 'default': _lyteUiUtils.resolveDefaultValue('lyte-navigator', 'endText', 'pages') }),
			totalPages: Lyte.attr('number', { 'default': 0 }),
			currentPage: Lyte.attr('number', { 'default': 1 })
		}
	},
	init: function () {
	...
	didConnect: function () {
		 ...
		 * @utility goToPage
		 */
		this.$node.goToPage = function (pageNo) {
			this.selectPage(pageNo)
		}.bind(this)
		$L.fastdom.measure(function () {
			var fg = getComputedStyle(this.$node).getPropertyValue('direction') == 'rtl';
	...
	didDestroy: function () {
		if (this.getData('ltPropAria')) {
			const div = this.$node.querySelector('.lyteNavigator')
			div.removeEventListener('keydown', this._clickEvents)
			delete this._clickEvents
		}
	},
	...
	initalizeOrReset: function () {
		this.checkButton()
		this.setInitialValue()
		if (this.getData('ltPropAria')) {
			const div = this.$node.querySelector('.lyteNavigator')
			this._clickEvents = this.clickEventFunc.bind(this)
			div.addEventListener('keydown', this._clickEvents)
		}
	},
	...
	setInitialValue: function () {
		...
		if (this.getData("ltPropType").toLowerCase() === "simple") {
			...
			if (!this.getData('ltPropAria')) {
				...
				var ele = this.$node.querySelectorAll('.lyteNavigatorPageNo');
				for (var i = 0; i < ele.length; i++) {
					const value = ele[i].getAttribute('data-value')
					if (value === '...') {
						ele[i].classList.add("lyteDisabled");
						ele[i].classList.add("dots3");
						this.setAriaDisabled(ele[i])
					} else {
						ele[i].setAttribute('tabindex', this.getData('ltPropTabIndex'))
					}
	...
	selectPage: function (pageNo, evt) {
		var firstIndex = this.getData('ltPropValue'), perPage = this.getData('ltPropPerpage'), MaxRecords = this.getData('ltPropRecords')

		firstIndex += perPage
		var x = (parseInt(pageNo) * perPage) - perPage >= MaxRecords ? MaxRecords - perPage : (parseInt(pageNo) * perPage) - perPage;
	...
	onSelect: function (evt) {
		const pageNo = evt.target.getAttribute('data-value')
		this.selectPage(pageNo, evt)
	},
	pagination: function () {
	...
	clickEventFunc: function (evt) {
		if (evt && evt.type == 'keydown' && !this.checkForEnter(evt)) {
			return
		}
		var elm = evt.target;
...
* @attribute ltPropType=simple 
* @attribute ltPropYield=true
*<lyte-navigator lt-prop-yield=true lt-prop-value="0" lt-prop-records="12" lt-prop-perpage="10" lt-prop-type="simple">
* <template is = "registerYield" yield-name = "navigatorYield">
*      <div class="lyteDoubleBack"> Home </div>
*      <div class="lyteSingleBack"> Prev </div>
*		<template lyte-for="{{paginationRange}} as column index">
