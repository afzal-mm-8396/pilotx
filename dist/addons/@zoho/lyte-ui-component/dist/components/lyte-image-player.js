...
_observedAttributesType :["array","boolean","number","boolean","boolean","object","number","number","boolean","number","number","number","string","object","boolean","string","boolean","boolean","boolean","object","boolean","array","string","number","string","string","string","string","array","array","boolean"],
	...
	update_width : function( current, total, dur ){
		return `--lyte-image-player-fill-width:${ current / total * 100 }%;transition-duration:${ dur / this.getPlaybackRate() }ms`;
	},
	...
	pause_obs : function( arg ){
		this.pause_observer( arg );
	}.observes( 'ltPropPause' ),

	pause_observer : function( arg ){
		var cb,
		fn = "remove",
		ns = "lyteImagePlayer";
	...
	getPlaybackRate : function(){
		var rate = this.data.ltPropPlayBackRate
		return rate > 0 ? rate : 1;
	},

	playback_obs : function( arg ){
		if( !this.data.ltPropPause ){
			this.pause_observer({ newValue : true });
			this.pause_observer({ newValue : false });
		}

		var cb = "onPlaybackRateChange";

		this.getMethods( cb ) && this.executeMethods( cb, arg, this.$node );

	}.observes( 'ltPropPlayBackRate' ),
	...
	initiate_play : function(){
		...
		if( elem ){
			...
			}, ( dur - ( this.__remaining_dur || 0 ) ) / rate );
		}
	},
