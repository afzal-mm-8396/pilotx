...
var LyteColorbox = {
	 ...
	 *
	 */
	getSelector : function(compSelectors,elemClasses,elem){
		for(var i = 0; i<elemClasses.length; i++){
			if(compSelectors.indexOf('.'+elemClasses[i]) != -1){
				return elemClasses[i];
			}
			if(compSelectors.indexOf(elem.tagName) != -1){
				return elem.tagName;
			}
		}
		return false;
	...
	getRelated : function(rel) {
		...
		if (rel && rel !== false && rel !== 'nofollow') {
			var elements = Array.from(_lyteUiUtils.querySelectorAll('.' + LyteColorbox._boxEle)).filter(function (node) {
				return (LyteColorbox.getSelector(LyteColorbox._component.getData('ltPropSelectors'),Array.from(node.classList),{ tagName: node.tagName.toLowerCase()}) === rel);
			});
	...
	getProperties : function(element){
		...
		obj.classList = Array.from(element.classList);
	...
	launch : function(element,event) {
		if(this.data){
			...
			this._component.data.ltPropShow = true;
		}
		if(!element){
			element = this.getSelectorElements()[0];
		}
		if(!element){
			if(this.getMethods('onLaunchFailure')){
				this.executeMethod('onLaunchFailure',this.$node);
			}
		...
		if (!LyteColorbox._closing) {
			if(element && Array.isArray(element)){
				...
				if(element.nodeType){
					...
					LyteColorbox.getRelated(LyteColorbox.getSelector(LyteColorbox._component.getData('ltPropSelectors'),Array.from(element.classList),element));
				}
			...
			if (!LyteColorbox._open) {
				...
				// },100)
				if( !LyteColorbox._component.getData( 'ltPropPreventFocus' ) ){
	...
	load : function(element, init){
		...
		var crop = $L('.lyteColorboxCropperIcon',this.childComp);
		if(crop[0] && element.type == 'photo'){
			crop.removeClass('lyteColorboxCropDisable');
		}else{
			crop.addClass('lyteColorboxCropDisable');
		}
		...
		if(element.type == "custom"){
			...
			var type = LyteColorbox._component.getData( 'ltPropType' ),
				...
				withCredentials = LyteColorbox._component.getData('ltPropWithCredentials');
	...
	setImagePostion : function(){
		if(LyteColorbox._component.getData('ltPropAnimation') == "slide"){
			var itemDiv = $L('.lyteCBoxCurrentItem',LyteColorbox._component.childComp)[0] ? $L('.lyteCBoxCurrentItem',LyteColorbox._component.childComp)[0] : LyteColorbox._content;
		}else{
			var itemDiv = $L('.lyteCBoxCurrentElem',LyteColorbox._component.childComp)[0] ? $L('.lyteCBoxCurrentElem',LyteColorbox._component.childComp)[0] : LyteColorbox._content;
		}
		var cropper=  $L('lyte-colorbox-crop',itemDiv)[0];
		if(cropper){
			var isCropper = window.getComputedStyle($L('lyte-colorbox-crop')[0]).display === 'none' ? true : false;
			if(isCropper){
				var photo = $L( '.lyteCropArea' , itemDiv);
				itemDiv.style.top = (LyteColorbox._content.offsetHeight - photo[0].offsetHeight)/2 + 'px';
				itemDiv.style.left = (LyteColorbox._content.offsetWidth - photo[0].offsetWidth)/2 + 'px';
			}
		}
	},
	computeOffsetImpl : function(event,photo,preventCheckAspectRatio){
		...
		type = comp.getData( 'ltPropType' ),
		header = type === 'custom' ? document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ]('.lyteColorboxHeader') : comp._header;
		if( LyteColorbox._open && LyteColorbox._overlay) {
			...
			if(LyteColorbox._crop){
				var isCropper = window.getComputedStyle(LyteColorbox._crop).display === 'none' ? true : false;
				...
				if(isCropper){
					...
					var photo = $L( '.lyteCBoxVisible' , itemDiv);
					itemDiv.style.top = (LyteColorbox._content.offsetHeight - photo[0].offsetHeight)/2 + 'px';
					itemDiv.style.left = (LyteColorbox._content.offsetWidth - photo[0].offsetWidth)/2 + 'px';
				}
			}
	...
	moveToNext: function( event, stopMethod ) {
		...
		if (!LyteColorbox._active && LyteColorbox._related[1] && (LyteColorbox._component.getData('ltPropLoop') || LyteColorbox._related[(LyteColorbox._index + 1)])) {
			if(LyteColorbox._component.getData('ltPropAnimation') != "slide"){
				LyteColorbox.hideElement();
				// LyteColorbox.toggleLoadingIcon();
				// document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ]('.lyteCBoxPhoto').remove();
			}else{
	 ...
	 */
	close : function () {
		this._component.data.ltPropShow = false;
	...
	hideCropper : function(){
		LyteColorbox._crop.classList.add('lyteColorboxHideBtn');
		if(LyteColorbox._download){
			LyteColorbox._download.classList.add('lyteColorboxHideBtn');
		}
		if(LyteColorbox._title){
			LyteColorbox._title.classList.add('lyteColorboxHideBtn');
		}
		if(LyteColorbox._zoomIn){
			LyteColorbox._zoomIn.classList.add('lyteColorboxHideBtn');
		}
		if(LyteColorbox._zoomOut){
			LyteColorbox._zoomOut.classList.add('lyteColorboxHideBtn');
		}
		if(LyteColorbox._reset){
			LyteColorbox._reset.classList.add('lyteColorboxHideBtn');
		}
		if(LyteColorbox._prev){
			LyteColorbox._prev.classList.add('lyteColorboxHideBtn');
		}
		if(LyteColorbox._next){
			LyteColorbox._next.classList.add('lyteColorboxHideBtn');
		}
	},

	crop : function(){
		if(LyteColorbox._component.getMethods('onCropOpen')){
			LyteColorbox._component.executeMethod('onCropOpen',LyteColorbox._component);
		}
		if(LyteColorbox._component.getData('ltPropAnimation') == "slide"){
			var itemDiv = $L('.lyteCBoxCurrentItem',LyteColorbox._component.childComp)[0] ? $L('.lyteCBoxCurrentItem',LyteColorbox._component.childComp)[0] : LyteColorbox._content;
		}else{
		...
		if(LyteColorbox._component.getData('ltPropYield')){
			this.hideCropper();
			var cropperbtns = $L('.lyteColorboxCropDisable',LyteColorbox._component.childComp);
			cropperbtns.removeClass('lyteColorboxCropDisable');
			cropperbtns.addClass('lyteColorboxCropEnable'); 
		}
		var width =   photo[0].offsetWidth;
		var height = photo[0].offsetHeight;
		if(photo[0].offsetWidth < photo[0].offsetHeight){
			width = height;
		}
		itemDiv.style.top = (LyteColorbox._content.offsetHeight - height)/2 + 'px';
		itemDiv.style.left = (LyteColorbox._content.offsetWidth - width)/2 + 'px';
		itemDiv.style.width = width + 'px';
		itemDiv.style.height = height + 'px';
		if(LyteColorbox._thumbDiv){
			LyteColorbox._thumbDiv.style.display = 'none';
		}
		photo[0]._imgDimension = { width : photo[0].offsetWidth , height : photo[0].offsetHeight };
		photo.cropper({ 
			'aspectRatio' : 'n:n' , 
			cropDrag : function(){
				var cropper = $L('.lyteCropDivImage',itemDiv)[0];
				if(LyteColorbox._component.getMethods('onCrop')){
					LyteColorbox._component.executeMethod('onCrop',cropper.offsetWidth,cropper.offsetHeight);
				}
			}
		});
		this.addCropEvent(itemDiv);
		$L('.lyteCropArea',this.childComp)[0].style.position = 'absolute';
	},
	addCropEvent : function(itemDiv){
		var _this = this;
		var cropBtns  =  LyteColorbox._component.childComp.querySelectorAll( '.lyteColorboxCrop' );
		cropBtns.forEach(function(cropBtn){
			cropBtn.addEventListener('click',function(event){
				var target = event.target.classList.contains('lyteColorboxCrop') ? event.target : event.target.closest('.lyteColorboxCrop'); 
				var ratio = target.dataset.ratio;
				var img  = $L('.lyteCBoxVisible' ,itemDiv).data('cropper');
				var selected_ratio = $L('.lyteCBoxSelectedRatio',itemDiv);
				if(selected_ratio){
					selected_ratio.removeClass('lyteCBoxSelectedRatio');
				}
				$L(target).addClass('lyteCBoxSelectedRatio');
				if(ratio){
					$L('.lyteCBoxVisible' ,itemDiv).cropper({
						'aspectRatio' : ratio,
						cropDrag : function(){
							var cropper = $L('.lyteCropDivImage',itemDiv)[0];
							if(LyteColorbox._component.getMethods('onCrop')){
					...
					var cropper = $L('.lyteCropDivImage',itemDiv)[0];
					if(LyteColorbox._component.getMethods('onRatioSet')){
						LyteColorbox._component.executeMethod('onRatioSet',cropper.offsetWidth,cropper.offsetHeight);
					}
				}else{
					...
					if(rotate){
						img.rotate();
						var cropper = $L('.lyteCropDivImage',itemDiv)[0];
						if(LyteColorbox._component.getMethods('onRotate')){
							LyteColorbox._component.executeMethod('onRotate',cropper.offsetWidth,cropper.offsetHeight);
						}
					}
				}	
				$L('.lyteCropArea',itemDiv)[0].style.position = 'absolute';					
			});
		});
	},
	...
	download : async function(){
		...
		if(LyteColorbox._component.getMethods('onDownload')){
			/**
			 * It is triggered when the download icon is clicked.
			 * @method onDownload
		...
		if(LyteColorbox._el.lytecboxDlink){
			var response = await fetch(LyteColorbox._el.lytecboxDlink);
		}else{
			if(LyteColorbox._el.type == "photo"){
				var response = await fetch(LyteColorbox._el.lytecboxHref);
			}
		}
		...
		a.href = url;
		a.download = LyteColorbox.createName(LyteColorbox._el.lytecboxTitle);
		a.click();
	...
	wheel : function( evt ){
		...
		var minZoom = Math.max(LyteColorbox._component.data.ltPropMinScale, 1);
		var maxZoom = Math.min(LyteColorbox._component.data.ltPropMaxScale, 6.0);
		var curZoom = LyteColorbox.getCurrentZoom(image);
	
		var newZoom = Math.min(Math.max(curZoom + (y / 1000), minZoom), maxZoom);
		...
		this.zoomTo(newZoom, { left: evt.clientX, top: evt.clientY }, image);

		LyteColorbox._zoom = false;
		LyteColorbox._currentZoomLevel = LyteColorbox.getCurrentZoom(LyteColorbox._domEle);
		var zoomBy = parseFloat(LyteColorbox._component.getData('ltPropZoomBy')) / 100;
var currentZoom = LyteColorbox.getCurrentZoom(LyteColorbox._domEle);

// Update internal zoom level tracker
LyteColorbox._currentZoomLevel = currentZoom;

// Update _clickNumber based on new scale
LyteColorbox._clickNumber = Math.round((currentZoom - 1) / zoomBy);
	...
	zoomIn: function () {
		...
		if(LyteColorbox._clickNumber <= LyteColorbox._component.getData('ltPropZoomClickNumber')){
			...
			$L.fastdom.measure(function () {
				...
				var maxZoom = Math.min(LyteColorbox._component.data.ltPropMaxScale, Infinity);
				var minZoom = Math.max(LyteColorbox._component.data.ltPropMinScale || 1, 1);
				var newZoom = Math.min(curZoom + zoomBy, maxZoom);
		
				if (newZoom === curZoom) {
					// Already at or above maxZoom
					LyteColorbox._zoomIn?.classList.add('lyteColorboxDisabled');
					LyteColorbox._zoomIn?.setAttribute('aria-disabled', true);
					return;
				}
		
				// LyteColorbox._clickNumber += 1;
		
				this.zoomTo(newZoom, {}, LyteColorbox._domEle);
				LyteColorbox.checkAndChangeStatus(LyteColorbox._domEle);
		
				// Enable zoom out and reset
				if (newZoom > minZoom) {
					LyteColorbox._zoomOut?.classList.remove('lyteColorboxDisabled');
					LyteColorbox._zoomOut?.setAttribute('aria-disabled', false);
					LyteColorbox._reset?.classList.remove('lyteColorboxDisabled');
				...
				// Disable zoom in if at max
				if ( LyteColorbox._clickNumber == LyteColorbox._component.getData('ltPropZoomClickNumber')) {
					LyteColorbox._zoomIn?.classList.add('lyteColorboxDisabled');
					LyteColorbox._zoomIn?.setAttribute('aria-disabled', true);
				}
		
				LyteColorbox.setPercentage(undefined, newZoom);
				/**
				 * It is triggered when we zoom the image.
				 * @method onZoomin
				 * @author santhoshraj.s@zohocorp.com
				 * @param { number } value
				 * @param { object } component
				*/
				LyteColorbox._component.getMethods("onZoomin") &&
					LyteColorbox._component.executeMethod('onZoomin', (Math.round(newZoom * 100) - 100), LyteColorbox._component.$node);
			}.bind(this));
		}else{
			LyteColorbox._clickNumber -= 1;
		}
	...
	zoomOut : function(){
		...
		if(LyteColorbox._zoom){		//if the picture is zoomed using the util function then reset it.
			var orientation = LyteColorbox._domEle.getAttribute('lytecbox-rotate');
			LyteColorbox._zoom = false;
			LyteColorbox._domEle.style.transform = orientation || "";
			LyteColorbox._domEle.style.top = "0px";
			LyteColorbox._domEle.style.left = "0px";
			LyteColorbox._domEle.style.transformOrigin = "";
			LyteColorbox.checkAndChangeStatus( LyteColorbox._domEle );
			...
			if(LyteColorbox._zoomIn?.classList.contains('lyteColorboxDisabled')){
				LyteColorbox._zoomIn.classList.remove('lyteColorboxDisabled');
			...
			/**
			 * It is triggered when the image is zoomed out.
			 * @method onZoomout
			 * @author santhoshraj.s@zohocorp.com
			 * @param { number } value
			 * @param { object } component
			*/
			LyteColorbox._component.getMethods("onZoomout") &&
				LyteColorbox._component.executeMethod('onZoomout', 0, LyteColorbox._component.$node);
		} 
		else {
			var transform = LyteColorbox._domEle.style.transform;
			var currentScaleMatch = transform.match(/scale\(([\d.]+)\)/);
			let currentScale = currentScaleMatch ? parseFloat(currentScaleMatch[1]) : 1;
		
			var minScale = Math.max(LyteColorbox._component.data.ltPropMinScale || 1, 1);
			var newScale = Math.max(currentScale - 0.25, minScale);  // Will not go below 1 now
		
			// Apply new scale
			// LyteColorbox._domEle.style.transform = `scale(${newScale})`;
			LyteColorbox._clickNumber -= 1;
		
			// Disable zoom out if we've hit the minimum
			...
			// Re-enable zoom in if not at max
			if (newScale < LyteColorbox._component.getData('ltPropMaxScale')) {
				LyteColorbox._zoomIn?.classList.remove('lyteColorboxDisabled');
				LyteColorbox._zoomIn?.setAttribute('aria-disabled', false);
			}
			...
			if (LyteColorbox._clickNumber >= 0 && LyteColorbox._clickNumber < LyteColorbox._component.getData('ltPropZoomClickNumber')) {
				...
				$L.fastdom.measure(function () {
					...
					var curZoom = LyteColorbox.getCurrentZoom(LyteColorbox._domEle);
					var minZoom = Math.max(LyteColorbox._component.data.ltPropMinScale || 1, 1);
					var newZoom = Math.max(curZoom - zoomBy, minZoom);
					...
					LyteColorbox.setPercentage(undefined, newZoom);
					LyteColorbox._component.getMethods("onZoomout") &&
						LyteColorbox._component.executeMethod('onZoomout', (Math.round(newZoom * 100) - 100), LyteColorbox._component.$node);
				}.bind(this));
			} else {
				LyteColorbox._clickNumber += 1;
			}
	...
	reset : function( elementToBeReset , onclick ) {
		...
		if( elementToBeReset.classList.contains(LyteColorbox._prefix+"Photo" ) && ( LyteColorbox._clickNumber > 0 || LyteColorbox._zoomedThroughUtil ) ){
			...
			/**
			 * It is triggered when we reset the image.
			 * @method onReset
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } component
			*/
		...
		if(LyteColorbox._domEle && LyteColorbox._domEle.classList.contains('lyteCBoxPhoto') && (value >= 0 || value === "fitWidth" || value === 'actual' || value === 'reset' )){
			...
			if (value === "fitWidth") {
				var parentWidth = parseFloat(getComputedStyle(LyteColorbox._content).width),
					...
					naturalSize =
						orientation && orientation.indexOf("90") !== -1
							? LyteColorbox._domEle.naturalHeight
							: LyteColorbox._domEle.naturalWidth;
	
				zoomBy = parentWidth / naturalSize;
				value = zoomBy * 100;
				LyteColorbox.setPercentage(value.toFixed(2));
				LyteColorbox._zoomedThroughUtil = true;
			} else if (value === "reset" || value === "actual") {
				zoomBy = 1;
				value = 100;
	...
	setElementWidthAndHeight : function(element,referenceOffsets){
		...
		if(elementOffset.width > elementOffset.height && elementOffset.width > referenceOffsets.width){
			...
			if(height > referenceOffsets.height){
				width = elementh(height,width,referenceOffsets);
				height = referenceOffsets.height;
				element.style.height = height + "px";
				element.style.width = width + "px";
			}
			...
			if(width > referenceOffsets.width){
				element.style.width = referenceOffsets.width + "px";
				element.style.height = LyteColorbox.getHeight(height,width,referenceOffsets) + "px";
			}
		}
	},
...
_template:"<template tag-name=\"lyte-colorbox\" lyte-colorbox=\"\"> <template is=\"if\" value=\"{{expHandlers(ltPropType,'===','custom')}}\"><template case=\"true\"> <template is=\"if\" value=\"{{ltPropYield}}\"><template case=\"true\"> <lyte-yield yield-name=\"colorBoxYield\" lt-cb-content-type=\"{{ltPropContent.lytecboxType}}\"></lyte-yield> </template><template case=\"false\"> <div id=\"lyteColorbox\" class=\"{{lyteUiColorboxAddClass(ltPropWrapperClass,this)}}\"> <div class=\"lyteColorboxFreezeLayer lyteCBoxOverlay\"></div> <div class=\"lyteColorboxCloseIcon lyteCBoxClose\"></div> <template is=\"if\" value=\"{{ltPropNavIcon}}\"><template case=\"true\"> <div class=\"lyteColorboxIconDiv lyteCBoxPrevious\"> <div class=\"lyteColorboxPreviousIcon\"></div> </div> <div class=\"lyteColorboxIconDiv lyteCBoxNext\"> <div class=\"lyteColorboxNextIcon\"></div> </div> </template></template> <div class=\"lyteColorboxWrapper\"> <template is=\"if\" value=\"{{expHandlers(isIframe,'!')}}\"><template case=\"true\"> <div class=\"lyteColorboxUtilDiv\"> <span class=\"lyteCBoxZoomOut lyteColorboxZoomOut lyteColorboxDisabled\" lt-prop-title=\"Zoom Out\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;appearance&quot; : &quot;box&quot;,&quot;margin&quot; : 5}\"></span> <span class=\"lyteCBoxReset lyteColorboxReset lyteColorboxDisabled\" lt-prop-title=\"Reset Zoom\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;appearance&quot; : &quot;box&quot;,&quot;margin&quot; : 5}\"></span> <span class=\"lyteCBoxZoomIn lyteColorboxZoomIn\" lt-prop-title=\"Zoom In\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;appearance&quot; : &quot;box&quot;,&quot;margin&quot; : 5}\"></span> </div> </template></template> <div class=\"lyteColorboxContent lyteCBoxContent\"> <span class=\"lyteColorboxLoadingImg\"></span> </div> <div class=\"lyteColorboxHeader\"> <div class=\"lyteColorboxTitle lyteCBoxTitle\"></div> <template is=\"if\" value=\"{{download}}\"><template case=\"true\"> <div class=\"lyteColorboxDownloadDiv lyteCBoxDownload\"> <span class=\"lyteColorboxDownloadImg\"></span> </div> </template></template> </div> </div> <template is=\"if\" value=\"{{ltPropThumbnail}}\"><template case=\"true\"> <div class=\"lyteColorboxThumbOuterWrapper\"> <div class=\"lyteColorboxThumbnails\" onclick=\"{{action(&quot;clickOnThumbnails&quot;,event)}}\"> <div class=\"lyteColorboxThumbInnerWrapper\"> <template is=\"for\" items=\"{{thumbnails}}\" item=\"item\" index=\"index\"> <div class=\"{{lyteUiAddShowClass(ltPropAddOrientation,'lyteColorboxThumb','addedOrientation')}}\" data-thumb-val=\"{{lyteUiSetIndexString(index,thumbnails.length)}}\"> <template is=\"if\" value=\"{{ifEquals(item.type,&quot;photo&quot;)}}\"><template case=\"true\"> <img src=\"{{item.lytecboxHref}}\"> </template><template case=\"false\"><template is=\"if\" value=\"{{lyteUiIfEqualsAny(item.type,&quot;iframe&quot;,&quot;custom&quot;)}}\"><template case=\"true\"><template is=\"if\" value=\"{{item.lytecboxThumbnail}}\"><template case=\"true\"> <img class=\"lyteCBoxFavImg\" src=\"{{item.lytecboxThumbnail}}\"> </template><template case=\"false\"> <div class=\"lyteCBoxIframeWrap\"> <div class=\"lyteCBoxIframeIcon\"></div> <div class=\"lyteCBoxIframeLabel\">{{item.format}}</div> </div> </template></template> </template></template></template></template> </div> </template> </div> </div> </div> </template></template> </div> </template></template> </template><template case=\"false\"> <template is=\"if\" value=\"{{ltPropYield}}\"><template case=\"true\"> <lyte-yield yield-name=\"colorBoxYield\"></lyte-yield> </template><template case=\"false\"> <lyte-colorbox-container> <lyte-colorbox-header> <lyte-colorbox-title></lyte-colorbox-title> <template is=\"if\" value=\"{{download}}\"><template case=\"true\"> <lyte-colorbox-download> <span class=\"lyteColorboxDownloadImg\"></span> </lyte-colorbox-download> </template></template> <lyte-colorbox-close></lyte-colorbox-close> </lyte-colorbox-header> <lyte-colorbox-content> </lyte-colorbox-content> <template is=\"if\" value=\"{{ltPropNavIcon}}\"><template case=\"true\"> <lyte-colorbox-previous> <div class=\"lyteColorboxPreviousIcon\"></div> </lyte-colorbox-previous> <lyte-colorbox-next> <div class=\"lyteColorboxNextIcon\"></div> </lyte-colorbox-next> </template></template> <lyte-colorbox-description> </lyte-colorbox-description> <div class=\"lyteColorboxUtilDiv\"> <lyte-colorbox-zoomin lt-prop-title=\"Zoom In\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;appearance&quot; : &quot;box&quot;,&quot;margin&quot; : 5}\"></lyte-colorbox-zoomin> <lyte-colorbox-reset class=\"lyteColorboxDisabled\" lt-prop-title=\"Reset Zoom\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;appearance&quot; : &quot;box&quot;,&quot;margin&quot; : 5}\"></lyte-colorbox-reset> <lyte-colorbox-zoomout class=\"lyteColorboxDisabled\" lt-prop-title=\"Zoom Out\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;appearance&quot; : &quot;box&quot;,&quot;margin&quot; : 5}\"></lyte-colorbox-zoomout> </div> <template is=\"if\" value=\"{{ltPropThumbnail}}\"><template case=\"true\"> <lyte-colorbox-thumbnail> <div class=\"lyteColorboxThumbnails\" onclick=\"{{action(&quot;clickOnThumbnails&quot;,event)}}\"> <div class=\"lyteColorboxThumbInnerWrapper\"> <template is=\"for\" items=\"{{thumbnails}}\" item=\"item\" index=\"index\"> <div class=\"{{lyteUiAddShowClass(ltPropAddOrientation,'lyteColorboxThumb','addedOrientation')}}\" data-thumb-val=\"{{lyteUiSetIndexString(index,thumbnails.length)}}\"> <template is=\"if\" value=\"{{ifEquals(item.type,&quot;photo&quot;)}}\"><template case=\"true\"> <img src=\"{{item.lytecboxHref}}\"> </template><template case=\"false\"><template is=\"if\" value=\"{{lyteUiIfEqualsAny(item.type,&quot;iframe&quot;,&quot;custom&quot;)}}\"><template case=\"true\"><template is=\"if\" value=\"{{item.lytecboxThumbnail}}\"><template case=\"true\"> <img class=\"lyteCBoxFavImg\" src=\"{{item.lytecboxThumbnail}}\"> </template><template case=\"false\"> <div class=\"lyteCBoxIframeWrap\"> <div class=\"lyteCBoxIframeIcon\"></div> <div class=\"lyteCBoxIframeLabel\">{{item.format}}</div> </div> </template></template> </template></template></template></template> </div> </template> </div> </div> </lyte-colorbox-thumbnail> </template></template> </lyte-colorbox-container> </template></template> </template></template> </template>",
_dynamicNodes : [{"type":"attr","position":[1]},{"type":"if","position":[1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"if","position":[1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"insertYield","position":[1]}]},"false":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"attr","position":[1,5]},{"type":"if","position":[1,5],"cases":{"true":{"dynamicNodes":[]}},"default":{}},{"type":"attr","position":[1,7,1]},{"type":"if","position":[1,7,1],"cases":{"true":{"dynamicNodes":[]}},"default":{}},{"type":"attr","position":[1,7,5,3]},{"type":"if","position":[1,7,5,3],"cases":{"true":{"dynamicNodes":[]}},"default":{}},{"type":"attr","position":[1,9]},{"type":"if","position":[1,9],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1]},{"type":"attr","position":[1,1,1,1]},{"type":"for","position":[1,1,1,1],"dynamicNodes":[{"type":"attr","position":[1]},{"type":"attr","position":[1,1]},{"type":"if","position":[1,1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]}]},"false":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"if","position":[0],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"if","position":[0],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]}]},"false":{"dynamicNodes":[{"type":"text","position":[1,3,0]}]}},"default":{}}]}},"default":{}}]}},"default":{}}]}]}},"default":{}}]}},"default":{}}]},"false":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"if","position":[1],"cases":{"true":{"dynamicNodes":[{"type":"insertYield","position":[1]}]},"false":{"dynamicNodes":[{"type":"componentDynamic","position":[1,1,1]},{"type":"attr","position":[1,1,3]},{"type":"if","position":[1,1,3],"cases":{"true":{"dynamicNodes":[{"type":"componentDynamic","position":[1]}]}},"default":{}},{"type":"componentDynamic","position":[1,1,5]},{"type":"componentDynamic","position":[1,1]},{"type":"componentDynamic","position":[1,3]},{"type":"attr","position":[1,5]},{"type":"if","position":[1,5],"cases":{"true":{"dynamicNodes":[{"type":"componentDynamic","position":[1]},{"type":"componentDynamic","position":[3]}]}},"default":{}},{"type":"componentDynamic","position":[1,7]},{"type":"componentDynamic","position":[1,9,1]},{"type":"componentDynamic","position":[1,9,3]},{"type":"componentDynamic","position":[1,9,5]},{"type":"attr","position":[1,11]},{"type":"if","position":[1,11],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1]},{"type":"attr","position":[1,1,1,1]},{"type":"for","position":[1,1,1,1],"dynamicNodes":[{"type":"attr","position":[1]},{"type":"attr","position":[1,1]},{"type":"if","position":[1,1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]}]},"false":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"if","position":[0],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"if","position":[0],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]}]},"false":{"dynamicNodes":[{"type":"text","position":[1,3,0]}]}},"default":{}}]}},"default":{}}]}},"default":{}}]},{"type":"componentDynamic","position":[1]}]}},"default":{}},{"type":"componentDynamic","position":[1]}]}},"default":{}}]}},"default":{}}],
_observedAttributes :["ltPropHeight","ltPropWidth","ltPropOverlayClose","ltPropYield","ltPropSelectors","ltPropLoop","ltPropArrowKey","ltPropZoomBy","ltPropEscKey","ltPropImgError","ltPropAnimation","ltPropThumbnail","ltPropThumbnailNumber","ltPropZoomClickNumber","ltPropType","ltPropMinScale","ltPropMaxScale","ltPropWheelZoom","ltPropZoomPosition","ltPropAddOrientation","ltPropWithCredentials","ltPropWrapperClass","ltPropPreventFocus","ltPropCrop","ltPropShow","ltPropContent","ltPropNavIcon","currentEle","thumbnails","triggerSetThumbnail","isIframe","download"],
_observedAttributesType :["string","string","boolean","boolean","array","boolean","boolean","string","boolean","string","string","boolean","number","number","string","number","number","boolean","string","boolean","boolean","string","boolean","boolean","boolean","object","boolean","object","array","number","boolean","boolean"],

	_lyteUtilFunctions : [ 'launch', 'push', 'open', 'zoomBy', 'replace', 'close', 'delete'],
	data : function(){
		return {
			 ...
			 * @input
			 */
			'ltPropHeight' : Lyte.attr("string",{default : '', 'input' : 'true', 'output' : 'false'}),
			 ...
			 * @input
			 */
			'ltPropWidth' : Lyte.attr("string",{default : '', 'input' : 'true', 'output' : 'false'}),

			/**
             * @componentProperty {boolean} ltPropOverlayClose
             * @version 1.0.0
             * @default true
			 * @condition ltPropType custom
             * @input
             */
			'ltPropOverlayClose' : Lyte.attr("boolean",{"default":true, 'input' : 'true', 'output' : 'false' }),
			 ...
			 * @input
             */
			'ltPropSelectors' : Lyte.attr("array",{"default" : [],  'input' : 'true', 'output' : 'false'}),
             ...
             * @input
             */
			'ltPropLoop' : Lyte.attr("boolean",{"default":true, 'input' : 'true', 'output' : 'false'}),
             ...
             * @input
             */
			'ltPropArrowKey' : Lyte.attr("boolean",{"default":true,  'input' : 'true', 'output' : 'false'}),
			 ...
			 * @input
             */
			'ltPropZoomBy' : Lyte.attr("string",{"default":"25%", 'input' : 'true', 'output' : 'false'}),
             ...
             * @input
             */
			'ltPropEscKey' : Lyte.attr("boolean",{"default":true, 'input' : 'true', 'output' : 'false'}),
			 ...
			 * @input
             */
			'ltPropImgError' : Lyte.attr("string",{"default":"This image failed to load.", 'input' : 'true', 'output' : 'false'}),

			/**
             * @componentProperty {default|slide} ltPropAnimation=default
             * @version 1.0.0
			 * @input
             */
			'ltPropAnimation' : Lyte.attr("string",{"default":"default",  'input' : 'true', 'output' : 'false'}),   //default,slide
             ...
             * @input
             */
			'ltPropThumbnail' : Lyte.attr("boolean",{"default":true,  'input' : 'true', 'output' : 'false'}),
			 ...
			 * @input
             */
			'ltPropThumbnailNumber' : Lyte.attr("number",{"default":7,  'input' : 'true', 'output' : 'false'}),
			 ...
			 * @input
             */
			'ltPropZoomClickNumber' : Lyte.attr('number',{'default':5,  'input' : 'true', 'output' : 'false'}),
			 ...
			 * @input
             */
			'ltPropType': Lyte.attr( 'string', { 'default': 'custom',  'input' : 'true', 'output' : 'false' } ),
			 ...
			 * @input
             */
			'ltPropMinScale' : Lyte.attr( 'number', { default : 0.01,  'input' : 'true', 'output' : 'false' } ),
			 ...
			 * @input
             */
			'ltPropMaxScale' : Lyte.attr( 'number', { default : Infinity,  'input' : 'true', 'output' : 'false' } ),
             ...
             * @input
             */
			'ltPropWheelZoom' : Lyte.attr( 'boolean', { default : false,  'input' : 'true', 'output' : 'false' } ),
			 ...
			 * @input
             */
			'ltPropZoomPosition' : Lyte.attr( 'string', { default : 'center',  'input' : 'true', 'output' : 'false' } ),	//center, imageCenter
             ...
             * 
             */
			'ltPropAddOrientation' : Lyte.attr('boolean', { default : false,  'input' : 'true', 'output' : 'false'} ),
			/**
             * @componentProperty {boolean} ltPropAddOrientation
             * @version 3.92.0
             * @default false
             ...
             * 
             */
			'ltPropWithCredentials' : Lyte.attr('boolean',{ default : false,  'input' : 'true', 'output' : 'false'}),
			/**
			...
			'ltPropCrop' : Lyte.attr('boolean',{default : false}),
	...
	trapFocus: function() {
		...
		if( isTrapFocusPresent && !this.getData('ltPropPreventFocus') ) {
			content.setAttribute( 'id', id );
			$L(LyteColorbox._box).trapFocus();
		}
	},
	hideShowColorbox : function(){
	...
	CropperObserver : function(){
		if(this.getData('ltPropCrop')){
			LyteColorbox._crop =  this.childComp.querySelector('lyte-colorbox-crop');
			LyteColorbox._crop.addEventListener('click',function(){
				LyteColorbox.crop();
			},true);
		}
	}.observes('ltPropCrop'),
	...
	init : function(){
		LyteColorbox._currentZoomLevel = 1;
		/**
		 * @utility launch
		...
		var _this = this;
		this.$node.reinitialize = function(){
			_this.recalculateSelectors();
		}
	...
	didConnect : function(){
		var _this = this;
		this.$node.crop = function(crop){
			_this.removecropper(crop);
		}
		if(this.getData('ltPropAnimation') == "slide"){
			var itemDiv = $L('.lyteCBoxCurrentItem',this.$node)[0];
		}else{
			var itemDiv = $L('.lyteCBoxCurrentElem',this.$node)[0];
		}
		itemDiv = itemDiv ? itemDiv :  $L('.lyteColorboxContent',this.$node)[0];
		this.$node.swapAspectRatio =  function(){
			if(this.getData('ltPropAnimation') == "slide"){
				var itemDiv = $L('.lyteCBoxCurrentItem',this.$node)[0];
			}else{
				var itemDiv = $L('.lyteCBoxCurrentElem',this.$node)[0];
			...
			itemDiv = itemDiv ? itemDiv :  $L('.lyteColorboxContent',this.$node)[0];
			var cropperData = $L( '.lyteCBoxVisible',itemDiv).data( 'cropper' );
			cropperData.swapAspectRatio(); 
			if(_this.getMethods('onCrop')){
				var img =  $L( '.lyteCropDivImage',itemDiv)[0];
				_this.executeMethod('onCrop',img.offsetWidth,img.offsetHeight);
			}
		}
		this.$node.cropperWidth =   function( changeValue  ){
			if(this.getData('ltPropAnimation') == "slide"){
				var itemDiv = $L('.lyteCBoxCurrentItem',this.$node)[0];
			}else{
				var itemDiv = $L('.lyteCBoxCurrentElem',this.$node)[0];
			...
			itemDiv = itemDiv ? itemDiv :  $L('.lyteColorboxContent',this.$node)[0];	
			var cropperData = $L( '.lyteCBoxVisible',itemDiv).data( 'cropper' );
			cropperData.changeByWidth( changeValue ); 
		}
		this.$node.cropperHeight = function(changeValue ) {
			if(this.getData('ltPropAnimation') == "slide"){
				var itemDiv = $L('.lyteCBoxCurrentItem',this.$node)[0];
			}else{
				var itemDiv = $L('.lyteCBoxCurrentElem',this.$node)[0];
			...
			itemDiv = itemDiv ? itemDiv :  $L('.lyteColorboxContent',this.$node)[0];
			var cropperData = $L( '.lyteCBoxVisible',itemDiv ).data( 'cropper' );
			cropperData.changeByHeight( changeValue );
		}
	...
	intialFunc : function(){
		...
		if( type === 'custom' ) {
			if(!this._box){
				...
				this._crop = comp.$node.querySelector('.lyteCBoxCrop');
			...
			if (document.body && ((this._box.parentElement && this._box.parentElement.tagName != "BODY") || !this._box.parentElement) ) {
				this._box.setAttribute('lyte-colorbox', '')
	...
	setupElements: function() {
		...
		this._crop = comp.$node.querySelector('lyte-colorbox-content');
		this._title = comp.$node.querySelector( 'lyte-colorbox-title' );
		this._crop = comp.$node.querySelector('lyte-colorbox-crop');
	...
	utilDivEvents : function(){
		if(!this.getData('isIframe')){
			...
			if(LyteColorbox._zoomIn){
				...
				LyteColorbox._zoomIn.setAttribute('tabindex',0);
			...
			if(LyteColorbox._zoomOut){
				...
				LyteColorbox._zoomOut.setAttribute('tabindex',0);
			...
			if(LyteColorbox._reset){
				...
				LyteColorbox._reset.setAttribute('tabindex',0);
	...
	disableCropper : function(){
		var hidedBtn = $L('.lyteColorboxHideBtn',LyteColorbox._component.childComp);
		hidedBtn.removeClass('lyteColorboxHideBtn');
	},
	removecropper : function(crop){
		if(LyteColorbox._component.getData('ltPropAnimation') == "slide"){
			var itemDiv = $L('.lyteCBoxCurrentItem',LyteColorbox._component.childComp)[0];
		}else{
			var itemDiv = $L('.lyteCBoxCurrentElem',LyteColorbox._component.childComp)[0];
		}
		itemDiv = itemDiv ? itemDiv :  LyteColorbox._content;
		itemDiv.style.width = '100%';
		itemDiv.style.height = '100%';
		itemDiv.style.left = 'unset';
		itemDiv.style.top = 'unset';
		if(LyteColorbox._thumbDiv){
			LyteColorbox._thumbDiv.style.display = 'flex';
		}
		this.disableCropper();
		var cropperbtn = $L('.lyteColorboxCropEnable');
		cropperbtn.removeClass('lyteColorboxCropEnable');
		cropperbtn.addClass('lyteColorboxCropDisable');	
		if(this.getMethods('onCropClose')){
		 	this.executeMethod( 'onCropClose', LyteColorbox._component.$node );
		}
		if(crop == 'cancel'){
			this.setData('ColorboxCropImg',false);
			$L('.lyteCBoxVisible',itemDiv).cropper("destroy");
			$L('.lyteCBoxVisible' ,itemDiv)[0].style.display = '';	
		}else{
			var cropElem = $L('.lyteCBoxVisible',itemDiv);
			var cropperData = cropElem.data('cropper');
			var canvas = cropperData.getCroppedImage();
			cropElem.cropper("destroy");
			cropElem[0].style.display = 'none';
			LyteColorbox.toggleLoadingIcon();
			var imageExtension = 'image/'+ LyteColorbox._related[LyteColorbox._index].lytecboxHref.split('.').pop();
			var returnVal;
			if(this.getMethods('onImageCropped')){
				returnVal = this.executeMethod( 'onImageCropped', canvas );
			}
			if(returnVal == false){
				this.setData('ColorboxCropImg',false);
				$L('.lyteCBoxVisible' ,itemDiv)[0].style.display = 'inline-block';
			}
			else if(returnVal && returnVal.then){
				returnVal.then( function(src) {
					if(typeof src == 'string'){
						cropElem[0].style.display = 'block';
						cropElem.attr({'src' : src , 'style' : 'width:'+cropperData.cropperDimensions.width+'px;height:' +cropperData.cropperDimensions.height + 'px;'});
						LyteColorbox._related[LyteColorbox._index].lytecboxHref = src;
					}else{
						cropElem[0].style.display = 'block';
						LyteColorbox.toggleLoadingIcon();
						cropElem.attr({'src' : canvas.toDataURL(imageExtension) , 'style' : 'width:'+cropperData.cropperDimensions.width+'px;height:' +cropperData.cropperDimensions.height + 'px;'});
						LyteColorbox._related[LyteColorbox._index].lytecboxHref = canvas.toDataURL(imageExtension);
					}
		...
		if(this.getData('ltPropType') == 'custom'){
			LyteColorbox._el.lytecboxHref = canvas ? canvas.toDataURL(imageExtension) : LyteColorbox._el.lytecboxHref;
		}else{
			LyteColorbox._el.lytecboxHref = canvas ? canvas.toDataURL(imageExtension) : LyteColorbox._el.lytecboxHref;
		}
	...
	callOnBeforeOpen : function(arrayObj,event){
		var returnVal;
		_lyteUiUtils.dispatchEvent('beforeshow', this.$node,  { originalEvent: event, component : this.$node, wormhole : this._box } );
		if(this.getMethods('onBeforeOpen')){
			/**
			 * It is invoked before the colorbox is going to open. It can return true or false or nothing ie. undefined. If false is returned then the colorbox won't open.
			 * @method onBeforeOpen
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } imgObjects
			 * @param { object } component
			*/
	...
	callOnLoad : function(){
		if(this.getMethods('onLoad')){
			/**
			 * It is invoked when the image or the file to be displayed in the colorbox is loaded.
			 * @method onLoad
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } image
			 * @param { number } index
	...
	callOnComplete : function(){
		if(this.getMethods('onComplete')){
			/**
			 * It is invoked when all the calcultaions and styling is completed and the image or file is appended to the colorbox.
			 * @method onComplete
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } image
			 * @param { number } index
	...
	callOnFailure : function(){
		if(this.getMethods('onFailure')){
			/**
			 * It will be triggered when the image fails to load.
			 * @method onFailure
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } image
			 * @param { number } index
	...
	getSelectorElements : function(){
		...
		for(var i = 0; i<selectors.length; i++){
			Lyte.arrayUtils(ele,'push',Array.from(_lyteUiUtils.querySelectorAll(selectors[i])));
		}
		return ele;
	},

	unbindEventsForSelectors: function(){
		var elements = _lyteUiUtils.querySelectorAll('.'+this._boxElement);
	...
	clickHandler : function(e) {
		if (!(e.which > 1 || e.shiftKey || e.altKey || e.metaKey || e.ctrlKey)) {
			...
			LyteColorbox.setupForBindings(document[  _lyteUiUtils.isWidget ? "querySelectorGlobal" : 'querySelector' ]('.'+this._className)._callee.component);
	...
	checkBeforeClose : function(event){
		var returnVal;
		_lyteUiUtils.dispatchEvent('beforeclose', this.$node,  { originalEvent: event, component : this.$node, wormhole : this._box } );
		if(this.getMethods('onBeforeClose')){
			/**
			 * It is invoked when the colorbox is going to get closed. If false is returned then the colorbox won't be closed.
	...
	addBindings : function() {
		...
		if (LyteColorbox._box) {
			...
			if(!this._init){
				...
				if(LyteColorbox._crop && this.getData('ltPropCrop')){
					LyteColorbox._crop.addEventListener('click',function(){
						LyteColorbox.crop();
					},true);
					//LyteColorbox._crop = null;
				}
				if(LyteColorbox._download){
					LyteColorbox._download.addEventListener('click',function(){
						LyteColorbox.download();
				...
				if(!LyteColorbox._boundKeydown){
					...
					document.body.addEventListener('keydown', function (e) {
						...
						if (LyteColorbox._open && LyteColorbox._component.getData('ltPropEscKey') && key === 27) {
							if( LyteColorbox._crop && LyteColorbox._component.getData('ltPropCrop') ){
								var isCropper = window.getComputedStyle(LyteColorbox._crop).display === 'none' ? true : false;
								if(isCropper){
									LyteColorbox._component.$node.crop('cancel');
								}else{
									e.preventDefault();
							...
							if( !this.getData( 'ltPropPreventFocus' ) ){
								LyteColorbox._content.focus();
							}
						}
	...
	recalculateSelectors: function() {
		if (!this._box) {
			console.warn("Colorbox is not initialized yet.");
			return;
		}
	...
	moveToImage: function( event ) {
		...
		if(target.classList.contains('lyteColorboxThumb')){
			...
			/**
			 * It is triggered when navigate from one file to another file.
			 * @method onNavigate
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } event
			 * @param { object } component
	...
	/**
	 * @customElement lyte-colorbox-header
	 */
	/**
	 * @customElement lyte-colorbox-title
	 */
	/**
	 * @customElement lyte-colorbox-content
	 */
	/**
	 * @customElement lyte-colorbox-previous
	 */
	/**
	 * @customElement lyte-colorbox-next
	 */
	/**
	 * @customElement lyte-colorbox-description
	 */
	/**
	 * @customElement lyte-colorbox-loading-icon
	 */
	actions : {
