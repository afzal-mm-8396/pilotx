...
_observedAttributesType :["string","string","string","string","array","object","string","string","string","string","boolean","boolean","boolean","boolean","boolean","object","boolean","boolean","string","boolean","object","boolean","object","object","boolean","number","boolean","boolean","boolean","object","boolean","string","string","boolean","array","array","array","array","object","object","object","number","string","boolean","boolean"],
	...
	data: function () {
		return {
			...
			ltPropPreventInvalid: Lyte.attr("boolean", { default: true }),
	...
	validateTimeInterval: function (curTime, getInfo) {
		var data = this.data,
			...
			hasMilliSec = curFormat.indexOf("S") != -1,
			...
			startTime = startMoment && startMoment.fromNow(curTimeMoment, undefined, hasMilliSec),
			endTime = endMoment && endMoment.fromNow(curTimeMoment, undefined, hasMilliSec),
			ret;
		if (startMoment && endMoment && startMoment.fromNow(endMoment, undefined, hasMilliSec).past) {
	...
	inputKeys: function (evt, fromCounter) {
		var activeElement = document.activeElement,
			...
			nextSelectionStart, nextSelectionEnd, nextNotAvail, prevSelectionStart, prevSelectionEnd, prevNotAvil, selectedFormat,
			selectedVal, meriAvail, updatedTime, curFormatType, preventAdj, hh, S, formatLimit, prvEvt = false, milliSec = {},
			...
			getUpdVal = function (func, val, nearest) {
				...
				if (intervalRet == true) {
					...
					if (nearest) {
						...
						if (!getUpdVal("set", intervalRet[selectionStart] + (S ? intervalRet[selectionStart + 1] : '') + intervalRet[selectionEnd - 1])) {
			...
			getLastSplit = function () {
				var lastSplit = split[split.length - 1],
					milliBefore = false,
					prevMilli = false;
				prevSelectionStart = lastSplit.index;
				if (!(milliBefore = lastSplit.format.val == "S") && (curFormat.indexOf("S") != -1)) {
					prevMilli = true;
				}
				prevSelectionStart += (prevMilli && !milliBefore) ? 2 : 0;
				prevSelectionEnd = prevSelectionStart + (milliBefore ? 3 : 2);
			};
		...
		for (var i = 0; split && (i < split.length); i++) {
			var splitFormat = split[i].format.val,
				isMilli = splitFormat == "S",
				hasMilli = isMilli || milliSec.prevMilli,
				prevMilli = milliSec.prevMilli,
				curSplitIndex = split[i].index + (prevMilli ? 2 : 0);
			milliSec.isMilli = isMilli;
			if (selectionStart == curSplitIndex && selectionEnd == (curSplitIndex + (isMilli ? 3 : 2))) {
				curFormatType = splitFormat;
				selectedVal = moment.format(curFormatType);
				selectedFormat = split[i].format.type + "s";
				meriAvail = selectedFormat == "meridians";
				var milliNext, milliBefore;
				if (split[i + 1]) {
					nextSelectionStart = split[i + 1].index;
					milliNext = split[i + 1].format.val == "S";
					nextSelectionStart += hasMilli ? 2 : 0;
				} else {
					nextSelectionStart = split[0].index;
					nextNotAvail = true;
					milliNext = split[0].format.val == "S";
				}
				nextSelectionEnd = nextSelectionStart + (milliNext ? 3 : 2);
				if (split[i - 1]) {
					prevSelectionStart = split[i - 1].index;
					milliBefore = split[i - 1].format.val == "S";
					prevSelectionStart += (prevMilli && !milliBefore) ? 2 : 0;
					prevSelectionEnd = prevSelectionStart + (milliBefore ? 3 : 2);
				} else {
					prevNotAvil = true;
			...
			milliSec.prevMilli = milliSec.prevMilli || isMilli;
		}

		hh = curFormatType == 'hh';
		S = curFormatType == 'S';
		formatLimit = ((curFormatType == 'mm' || curFormatType == 'ss') && 60) || (hh && 12) || (curFormatType == 'HH' && 24) || (S && 1000);
			...
			switch (key) {
				...
				case 'Enter': {
					if (isButton) {
						...
						var selInd, counterMilli = {};
						split.forEach(function (item) {
							var index = item.index + (counterMilli.has ? 2 : 0),
								curMilli = item.format.val == "S";
							counterMilli.has = counterMilli.has || curMilli;
							if (item.format.val != "A") {
								selInd = index;
								counterMilli.is = curMilli;
							}
						});
						if (selInd) {
							inputElem.setSelectionRange(selInd, selInd + (counterMilli.is ? 3 : 2));
							inputElem.focus();
		...
		if (!data.ltPropReadonly && ((keycode >= 48 && keycode <= 57) || (keycode >= 96 && keycode <= 105))) {
			if (S) {
				if (!getUpdVal("set", selectedVal[1] + selectedVal[2] + key)) {
					if (!getUpdVal("set", '0' + selectedVal[2] + key)) {
						getUpdVal("set", '00' + key);
					}
				}
			} else {
			...
			fn = function (moment, split) {
				...
				for (var i = 0; i < len; i++) {
					var cur = split[i],
						...
						__end = __start + /*converted.length*/(format == "S" ? 3 : 2);
	...
	setUnitValues: function () {
		...
		if (refresh) {
			...
			availTimeUnits.meri && (availTimeUnits.hour == "hh") && this.setData("meridiems", [{ index: 0, value: i18n ? _lyteUiUtils.i18n(am) : am, selected: "false", disabled: "false", class: "lyteTimePickerMeridiem", format: availTimeUnits.meri, engVal: am }, { index: 1, value: i18n ? _lyteUiUtils.i18n(pm) : pm, selected: "false", disabled: "false", class: "lyteTimePickerMeridiem", format: availTimeUnits.meri, engVal: pm }]);
