...
/**
 * This callback is fired when an accordion item is opened or closed.
 * @method onChanged
...
/**
 * This callback is fired when an accordion item is opened.
 * @method onOpen
 * @author ananthapadmanaban.n@zohocorp.com
 * @param { object } event
 * @param { object } accordionItem
...
/**
 * This callback is fired when an accordion item is closed.
 * @method onClose
 * @author ananthapadmanaban.n@zohocorp.com
 * @param { object } event
 * @param { object } accordionItem
...
/**
 * This callback is fired just before an accordion is opened.
 * @method onBeforeOpen
 * @author ananthapadmanaban.n@zohocorp.com
 * @param { object } event
 * @param { object } accordionItem
...
/**
 * This callback is fired just before an accordion is closed.
...
/**
 * This callback is fired after the accordion is rendered.
 * @method afterRender
 * @author ananthapadmanaban.n@zohocorp.com
 * @param { boolean } accordionContext
 */
...
_template:"<template tag-name=\"lyte-accordion\" lyte-accordion=\"\"> <lyte-yield yield-name=\"yield\"> </lyte-yield> </template>",
_dynamicNodes : [{"type":"insertYield","position":[1]}],
_observedAttributes :["ltPropDuration","ltPropHeight","ltPropExclusive","ltPropYield","ltPropDynamic","ltPropNested","ltPropPreventAutoScroll"],
_observedAttributesType :["string","string","boolean","boolean","boolean","boolean","boolean"],
	...
	data: function() {
		return {
			...
			'ltPropHeight': Lyte.attr( 'string', { 'input': true } ),
			...
			'ltPropDynamic': Lyte.attr( 'boolean', {
				'default': _lyteUiUtils.resolveDefaultValue( 'lyte-accordion', 'dynamic', true ),
				'input': true 
			} ),
			...
			'ltPropPreventAutoScroll': Lyte.attr( 'boolean', {
				'default': false
			} )
		}
	},
	...
	doAfterCloseWork: function( accordionBody ) {
	...
	doBeforeOpenWork: function( accordionBody ) {
	...
	initiateOpen: function( sibling, configuration, event ) {
		...
		var accordionItem = sibling.parentElement,
		isImmediate = accordionItem._immediate;
	...
	calculateHeight: function( body, index ) {
		...
		if( !isStyle ) {
			this.getData( 'heights' )[ index ].height = body.getBoundingClientRect().height - parseFloat( getComputedStyle( body ).paddingTop ) - parseFloat( getComputedStyle( body ).paddingBottom ) + 'px';
	...
	didConnect: function() {
		this.setData( 'rAFMap', {} );		
		this.getAllHeights()
		this.$node.querySelector( 'lyte-yield' )._accordionsYield = true;
	},
...
var _lyteAccordion = {
	...
	findProperParentS: function( elm ) {
		...
		while( elm && !elm._accordionsYield && elm.tagName != 'HTML' ) {
			properparent = elm;
			stack.push( elm );
			elm = elm.parentNode;
	...
	findProperParent: function( element ) {
		...
		while( !element._accordionsYield && element.tagName != 'HTML' ) {
...
_lyteUiUtils.addGlobalEventListener( 'click', function( event ) {
	...
	itsStyle = getComputedStyle( sibling ).height;
 	if( transition ) {
...
if( !_lyteUiUtils.registeredCustomElements[ 'lyte-accordion-item' ] ) {
	 ...
	 * @customAttribute lyte-shortcut This is used to provide shortcut key for the accordion item
	 * @utility openOnly, open, close
	 */
	...
	Lyte.createCustomElement( "lyte-accordion-item", {
		...
		"connectedCallback": function() {
			...
			// This is a safety check because it is generally guaranteed to be a LYTE-ACCORDION Element
			if( comp.tagName == 'HTML' || comp.nodeType === 9 ) {
				return ;
			}

			comp = comp.component;

			this.openOnly = function() {
				var exclusive = comp.getData( 'ltPropExclusive' );

				if( exclusive ) {
					that.open();
				}
				else {
					var yieldElement = comp.getYieldElement(),
					items = Array.from( yieldElement.children );
