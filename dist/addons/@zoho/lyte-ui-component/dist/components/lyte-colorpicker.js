...
if(!ColorPicker_Util){
	var ColorPicker_Util = {
		...
		baseConverter : function(numberToConvert, oldBase, newBase) {
	        ...
	        if (newBase == 16) {
	            return ('0'+parseInt(numberToConvert).toString(16)).slice(-2);
	        }
...
if(!ColorPicker_EventUtil){
	var ColorPicker_EventUtil = {
		...
		__initOpacityMove: function(e,_this) {
			if(_this){
				if(e.type == "mousedown"){
					_lyteUiUtils.addEventListenerGlobal('mousemove',ColorPicker_EventUtil.__mm = function(event){
						ColorPicker_EventUtil.__moveOnOpacityTrack(event,_this);
					});
					_lyteUiUtils.addEventListenerGlobal('mouseup',ColorPicker_EventUtil.__mu = function(event){
						ColorPicker_EventUtil.__endDrag(event,_this);
					});
				}
				if(e.type == "touchstart"){
					_lyteUiUtils.addEventListenerGlobal('touchmove',ColorPicker_EventUtil.__tm = function(event){
						ColorPicker_EventUtil.__moveOnOpacityTrack(event,_this);
					});
					_lyteUiUtils.addEventListenerGlobal('touchend',ColorPicker_EventUtil.__te = function(event){
						ColorPicker_EventUtil.__endDrag(event,_this);
					});
				}
		...
		__initPaletteMove : function(e,_this) {
	    	if(_this){
	    		_this.setData('setPrevCmyk',false);
	    		if(e.type == "mousedown"){
					var popOver = _this.$node.closest('lyte-wormhole');
					if(!_this.getData('cpInline') && popOver && popOver.component.parent && popOver.component.parent.closest('lyte-colorpicker').getData('ltPropCloseOnBodyClick')){
						popOver.component.parent.setData('ltPropCloseOnBodyClick',false);
					}
					_lyteUiUtils.addEventListenerGlobal('mousemove',ColorPicker_EventUtil.__mm = function(event){
	    ...
	    __endDrag : function(e,_this) {
	    	if(_this && _this.$node){
	    		...
	    		if(e.type == "mouseup"){
					_lyteUiUtils.removeEventListenerGlobal('mousemove',ColorPicker_EventUtil.__mm);
					_lyteUiUtils.removeEventListenerGlobal('mouseup',ColorPicker_EventUtil.__mu);
					ColorPicker_EventUtil.__mm = undefined;
					ColorPicker_EventUtil.__mu = undefined;
				}
				if(e.type == "touchend"){
					_lyteUiUtils.removeEventListenerGlobal('touchmove',ColorPicker_EventUtil.__tm);
					_lyteUiUtils.removeEventListenerGlobal('touchend',ColorPicker_EventUtil.__te);
			...
			if(!_this.getData('cpInline') && popOver && popOver.component.parent && popOver.component.parent.closest('lyte-colorpicker').getData('ltPropCloseOnBodyClick')){
				setTimeout(function(){
					popOver.component.parent.setData('ltPropCloseOnBodyClick',true);
				},10)
...
_template:"<template tag-name=\"colorpicker-ui\"> <template is=\"if\" value=\"{{cpBasicColorPicker}}\"><template case=\"true\"> <div class=\"lyteColorPicker__default\"> <template is=\"if\" value=\"{{cpDefaultColorEnable}}\"><template case=\"true\"> <div class=\"lyteColorPicker__palettecontainer LCP_DefaultColorCont default_color\" tabindex=\"0\" aria-label=\"{{lyteUiI18n(&quot;Default.Colors&quot;)}}\" onclick=\"{{action(&quot;selectColor&quot;,event,cpDefaultColor)}}\"> <div class=\"lyteColorPicker__palette\"> {{log(cpDefaultColor)}} <span class=\"lyteColorPicker__colorpan cpDefault__colors\" onmouseenter=\"{{action('colorhover',event,color,this,'DefaultColor')}}\" lt-prop-title=\"{{lyteUiRgbToHex(cpDefaultColor)}}\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;showdelay&quot; : &quot;1000&quot;}\" data-col=\"{{indexVal}}\" tabindex=\"-1\" aria-label=\"{{lyteUiRgbToHex(cpDefaultColor)}}\" aria-selected=\"false\">{{cpDefaultColor}}</span> </div> <span class=\"lyteColorPicker__defaultpaletteheading\">Default Color</span> </div> </template></template> <template is=\"if\" value=\"{{cpYield}}\"><template case=\"true\"> <lyte-yield yield-name=\"colorpickeryield\"></lyte-yield> </template></template> <template is=\"if\" value=\"{{cpNoFillButton}}\"><template case=\"true\"> <div class=\"lyteColorPicker__nocolorbutton\" onclick=\"{{action(&quot;noFillExecute&quot;,event)}}\"> <span class=\"lyteColorPicker__noFill\"> </span> <span class=\"lyteColorPicker__text\">{{cpNoFillLabel}}</span> </div> </template></template> <div class=\"lyteColorPicker__palettecontainer LCP_predefinedPallete\" tabindex=\"0\" aria-label=\"{{cpPaletteLabel}}\"> <div class=\"lyteColorPicker__paletteheading\">{{cpPaletteLabel}}</div> <div class=\"lyteColorPicker__palette {{if(cpAccent,'lyteColorPickerAccentColorPalette','')}}\"> <div role=\"grid\"> <template is=\"for\" items=\"{{cpAvailableColors}}\" item=\"colorArray\" index=\"rowindexVal\"> <ul class=\"lyteColorPicker__shades default__colors {{if(cpShowHiddenColor,'lyteColorPickerShowHiddenColor','')}}\" role=\"row\"> <template is=\"for\" items=\"{{colorArray}}\" item=\"color\" index=\"indexVal\"> <li class=\"lyteColorPicker__colorpan {{if(cpAccent,'lyteColorPickerAccentColor','')}}\" onmouseenter=\"{{action('colorhover',event,color,this,'themeColor')}}\" lt-prop-title=\"{{if(cpAccent,lyteUiColorpickerAccentTooltip(cpAccentColor[indexVal],rowindexVal),lyteUiRgbToHex(color))}}\" data-color=\"{{lyteUiRgbToHex(color)}}\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;showdelay&quot; : &quot;1000&quot;}\" onclick=\"{{action(&quot;selectColor&quot;,event,color)}}\" tabindex=\"-1\" data-row=\"{{rowindexVal}}\" data-col=\"{{indexVal}}\" aria-label=\"{{lyteUiRgbToHex(color)}}\" role=\"cell\" aria-selected=\"false\">{{color}}</li> </template> </ul> </template> </div> </div> </div> <template is=\"if\" value=\"{{cpStandardColors}}\"><template case=\"true\"> <div class=\"lyteColorPicker__palettecontainer LCP_standardColorCont\" tabindex=\"0\" aria-label=\"{{lyteUiI18n(&quot;Standard.Colors&quot;)}}\"> <div class=\"lyteColorPicker__paletteheading\">{{lyteUiI18n(\"Standard.Colors\")}}</div> <div class=\"lyteColorPicker__palette\"> <ul class=\"standard__colors\"> <template is=\"for\" items=\"{{cpStandardColorArray}}\" item=\"color\" index=\"indexVal\"> <li class=\"lyteColorPicker__colorpan\" onmouseenter=\"{{action('colorhover',event,color,this,'standardColor')}}\" lt-prop-title=\"{{lyteUiRgbToHex(color)}}\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;showdelay&quot; : &quot;1000&quot;}\" onclick=\"{{action(&quot;selectColor&quot;,event,color)}}\" data-col=\"{{indexVal}}\" tabindex=\"-1\" aria-label=\"{{lyteUiRgbToHex(color)}}\" aria-selected=\"false\" style=\"background: {{color}};\">{{color}}</li> </template> </ul> </div> </div> </template></template> <template is=\"if\" value=\"{{cpUsedColors}}\"><template case=\"true\"> <div class=\"lyteColorPicker__palettecontainer usedColor__container\" tabindex=\"0\" aria-label=\"{{cpUsedColorLabel}}\" data-color=\"{{cpUsedColorLabel}}\"> <div class=\"lyteColorPicker__paletteheading\">{{cpUsedColorLabel}}</div> <div class=\"lyteColorPicker__palette\"> <ul class=\"lyteColorPickerUsedColors used__colors\"> <template is=\"for\" items=\"{{usedColors}}\" item=\"color\" index=\"indexVal\"> <li class=\"lyteColorPicker__colorpan {{lyteUiAddPE(color)}}\" onmouseenter=\"{{action('colorhover',event,color,this,'usedColor')}}\" lt-prop-title=\"{{lyteUiRgbToHex(color)}}\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;showdelay&quot; : &quot;1000&quot;}\" onclick=\"{{action(&quot;selectColor&quot;,event,color)}}\" tabindex=\"-1\" data-col=\"{{indexVal}}\" aria-label=\"{{lyteUiRgbToHex(color)}}\" aria-selected=\"false\">{{color}}</li> </template> </ul> </div> </div> </template></template> <template is=\"if\" value=\"{{cpAdvancedColorButton}}\"><template case=\"true\"> <div class=\"lyteColorPicker__navigatable lyteColorPicker__morecolorbutton\" onclick=\"{{action(&quot;goToAdvancedCP&quot;,event)}}\" role=\"button\" tabindex=\"0\"> <span class=\"lyteColorPicker__icon\"></span> <span class=\"lyteColorPicker__text\">{{lyteUiI18n(\"Advanced.Colors\")}}</span> <span class=\"lyteColorpickerForwardArrow h-alignright LCP_rightArrow\"></span> </div> </template></template> </div> </template><template case=\"false\"> <div class=\"lyteColorPicker--advanced\"> <template is=\"if\" value=\"{{expHandlers(cpNoFillButton,'&amp;&amp;',cpAdvancedNoFillButton)}}\"><template case=\"true\"> <div class=\"lyteColorPicker__nocolorbutton\" onclick=\"{{action(&quot;noFillExecute&quot;,event)}}\"> <span class=\"lyteColorPicker__noFill\"> </span> <span class=\"lyteColorPicker__text\">{{cpNoFillLabel}}</span> </div> </template></template> <div class=\"lyteColorPicker__maparea\"> <div class=\"colorDiv\" id=\"colorDiv\"> <div id=\"lyteCPImgDiv\"></div> <div class=\"colorSlider_palette_circle colorSlider_palette_circleBlack\"></div> </div> </div> <div class=\"lyteColorPicker__previewDiv lyteColorPicker__sliderDiv\"> <div> <div class=\"colorSlider_hue\"> <div class=\"colorSlider_sliderHandle\" role=\"slider\" tabindex=\"0\" aria-valuemin=\"0\" aria-valuemax=\"360\" aria-valuenow=\"{{currentHue}}\" aria-label=\"hue\" aria-orientation=\"horizontal\"> </div> <div class=\"colorSlider_hueBar_border\"> <div class=\"colorSlider_hueBar\"></div> </div> </div> <div class=\"lyteColorPicker__opacityslider\"> <div class=\"opacityslider__circlethumb\" role=\"slider\" tabindex=\"0\" aria-valuemin=\"0\" aria-valuemax=\"100\" aria-valuenow=\"{{opacity}}\" aria-label=\"Opacity\" aria-orientation=\"horizontal\"></div> <div class=\"lyteColorPicker__transparentbg\"> <div class=\"opacityslider__track\"></div> </div> </div> </div> <div class=\"lyteCP_PreviewWrapper {{setDynamicColorClass(this,cpDisplayPreviousColor)}}\"> <div class=\"lyteCP__transparentbg\"> <div class=\"previewDiv\" onmouseover=\"{{action(&quot;onOverColorPreviewDiv&quot;,event)}}\" onclick=\"{{action(&quot;copyValueToClipboard&quot;)}}\" lt-prop-title=\"{{lyteUiI18n('Copy.Color.value.to.Clipboard')}}\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;}\"></div> </div> <template is=\"if\" value=\"{{cpEnableEyeDropper}}\"><template case=\"true\"><div class=\"lyteCP__eyeDropper\" id=\"lyteCP_eyeDropper\"> </div></template></template> <template is=\"if\" value=\"{{cpDisplayPreviousColor}}\"><template case=\"true\"><div class=\"lyteCP__transparentbg\"> <div class=\"dynamicDiv\" onclick=\"{{action(&quot;changeValueOfDynamicColor&quot;)}}\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;}\"></div> </div></template></template> </div> </div> <div class=\"lyteColorPicker__previewDiv\"> <div class=\"selectFormat\"> <lyte-dropdown lt-prop-box-class=\"{{cpDropBoxClass}}\" lt-prop-selected=\"{{cpDropdownType}}\" class=\"cPDropDown\" lt-prop-yield=\"true\" lt-prop-freeze=\"false\" lt-prop-tabindex=\"1\" on-option-selected=\"{{method(&quot;changeFormatView&quot;)}}\" tabindex=\"0\"> <template is=\"registerYield\" yield-name=\"yield\"> <lyte-drop-button class=\"colorPickerDD\"> <span class=\"lyteMarginRight lyteDropdownLabel\">{{dropButtonValue}}</span> <lyte-icon class=\"dropdown\"></lyte-icon> </lyte-drop-button> <lyte-drop-box class=\"lyteCPDropbox {{cpDropBoxClass}}\"> <lyte-drop-body> <template is=\"for\" items=\"{{cpColorFormats}}\" item=\"item\" index=\"indexVal\"> <lyte-drop-item data-value=\"{{item}}\"> {{item}} </lyte-drop-item> </template> </lyte-drop-body> </lyte-drop-box> </template> </lyte-dropdown> </div><div class=\"showValue\"> <lyte-table lt-prop-yield=\"true\" class=\"cpValFormatIp\" lt-prop-prevent-scrollbar=\"true\" lt-prop-role=\"colorpicker\"> <template is=\"registerYield\" yield-name=\"yield\"> <lyte-table-structure> <lyte-tbody> <template is=\"if\" value=\"{{ifEquals(dropButtonValue,lyteUiI18n(&quot;HEX&quot;))}}\"><template case=\"true\"> <lyte-tr> <lyte-td> <lyte-input id=\"lyteCPShowValue\" lt-prop-type=\"text\" lt-prop-appearance=\"box\" lt-prop-maxlength=\"7\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;HEX&quot;)}}\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;HEX&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;HEX&quot;)}}\" lt-prop-direction=\"vertical\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','HEX')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_HEX_label&quot;}\"> </lyte-input> </lyte-td> <lyte-td> <lyte-input id=\"lyteCP__A\" lt-prop-type=\"text\" lt-prop-appearance=\"box\" lt-prop-maxlength=\"3\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;A&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;A&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;A&quot;)}}\" lt-prop-direction=\"vertical\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','A')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_HEX_alpha_label&quot;}\"> </lyte-input> </lyte-td> </lyte-tr> <lyte-tr class=\"lyteCPHexText\" role=\"row\"> <lyte-td id=\"lytecp_HEX_label\">HEX</lyte-td> <lyte-td id=\"lytecp_HEX_alpha_label\" class=\"lyteColorPicker_alpha\">{{lyteUiSetAlphaLabel(cpOpacityLabel)}}</lyte-td> </lyte-tr> </template><template case=\"false\"><template is=\"if\" value=\"{{ifEquals(dropButtonValue,lyteUiI18n(&quot;RGB&quot;))}}\"><template case=\"true\"> <lyte-tr role=\"row\"> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"255\" id=\"lyteCP__R\" lt-prop-type=\"text\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" lt-prop-maxlength=\"3\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;R&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;R&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;R&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','R')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_R_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"255\" id=\"lyteCP__G\" lt-prop-type=\"text\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" lt-prop-maxlength=\"3\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;G&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;G&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;G&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','G')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_G_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"255\" id=\"lyteCP__B\" lt-prop-type=\"text\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" lt-prop-maxlength=\"3\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;B&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;B&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;B&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','B')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_B_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__A\" lt-prop-type=\"text\" lt-prop-appearance=\"box\" lt-prop-maxlength=\"3\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;A&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;A&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;A&quot;)}}\" lt-prop-direction=\"vertical\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','A')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_RGB_alpha_label&quot;}\"> </lyte-number> </lyte-td> </lyte-tr> <lyte-tr> <lyte-td id=\"lytecp_R_label\">R</lyte-td> <lyte-td id=\"lytecp_G_label\">G</lyte-td> <lyte-td id=\"lytecp_B_label\">B</lyte-td> <lyte-td id=\"lytecp_RGB_alpha_label\" class=\"lyteColorPicker_alpha\">{{lyteUiSetAlphaLabel(cpOpacityLabel)}}</lyte-td> </lyte-tr> </template><template case=\"false\"><template is=\"if\" value=\"{{ifEquals(dropButtonValue,lyteUiI18n(&quot;HSV&quot;))}}\"><template case=\"true\"> <lyte-tr> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"360\" id=\"lyteCP__H\" lt-prop-type=\"text\" lt-prop-maxlength=\"3\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;H&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;H&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;H&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','H')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_H_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__S\" lt-prop-type=\"text\" lt-prop-maxlength=\"3\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;S&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;S&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;S&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','S')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_S_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__V\" lt-prop-type=\"text\" lt-prop-maxlength=\"3\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;V&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;V&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;V&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','V')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_V_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__A\" lt-prop-type=\"text\" lt-prop-appearance=\"box\" lt-prop-maxlength=\"3\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;A&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;A&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;A&quot;)}}\" lt-prop-direction=\"vertical\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','A')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_HSV_alpha_label&quot;}\"> </lyte-number> </lyte-td> </lyte-tr> <lyte-tr> <lyte-td id=\"lytecp_H_label\">H</lyte-td> <lyte-td id=\"lytecp_S_label\">S</lyte-td> <lyte-td id=\"lytecp_V_label\">V</lyte-td> <lyte-td id=\"lytecp_HSV_alpha_label\" class=\"lyteColorPicker_alpha\">{{lyteUiSetAlphaLabel(cpOpacityLabel)}}</lyte-td> </lyte-tr> </template><template case=\"false\"><template is=\"if\" value=\"{{ifEquals(dropButtonValue,lyteUiI18n(&quot;CMYK&quot;))}}\"><template case=\"true\"> <lyte-tr> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__C\" class=\"lyteCP_CMYK\" lt-prop-type=\"text\" lt-prop-maxlength=\"3\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;C&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;C&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;C&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','C')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_C_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__M\" class=\"lyteCP_CMYK\" lt-prop-type=\"text\" lt-prop-maxlength=\"3\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;M&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;M&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;M&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','M')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_M_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__Y\" class=\"lyteCP_CMYK\" lt-prop-type=\"text\" lt-prop-maxlength=\"3\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;Y&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;Y&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;Y&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','Y')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_Y_label&quot;}\"> </lyte-number> </lyte-td> <lyte-td> <lyte-number lt-prop-min=\"0\" lt-prop-max=\"100\" id=\"lyteCP__K\" class=\"lyteCP_CMYK\" lt-prop-type=\"text\" lt-prop-maxlength=\"3\" lt-prop-appearance=\"box\" lt-prop-direction=\"vertical\" onkeydown=\"{{action(&quot;inputKeyDown&quot;,event,this,&quot;K&quot;)}}\" on-value-change=\"{{method(&quot;inputValueChanged&quot;,&quot;K&quot;)}}\" on-blur=\"{{method(&quot;onInputBlur&quot;,&quot;K&quot;)}}\" lt-prop-aria=\"true\" after-render=\"{{method('afterInputRender','K')}}\" lt-prop-aria-attributes=\"{&quot;aria-labelledby&quot; : &quot;lytecp_K_label&quot;}\"> </lyte-number> </lyte-td> </lyte-tr> <lyte-tr> <lyte-td id=\"lytecp_C_label\">C</lyte-td> <lyte-td id=\"lytecp_M_label\">M</lyte-td> <lyte-td id=\"lytecp_Y_label\">Y</lyte-td> <lyte-td id=\"lytecp_K_label\">K</lyte-td> </lyte-tr> </template></template></template></template></template></template></template></template> </lyte-tbody> </lyte-table-structure> </template> </lyte-table> <input id=\"lyteCPHiddenInput\" tabindex=\"-1\" aria-hidden=\"true\"> </div> </div> <template is=\"if\" value=\"{{expHandlers(cpUsedColors,'&amp;&amp;',cpAdvancedUsedColor)}}\"><template case=\"true\"> <div class=\"lyteColorPicker__palettecontainer usedColor__container\" tabindex=\"0\" aria-label=\"{{cpUsedColorLabel}}\" data-color=\"{{cpUsedColorLabel}}\"> <div class=\"lyteColorPicker__paletteheading\">{{cpUsedColorLabel}}</div> <div class=\"lyteColorPicker__palette\"> <ul class=\"lyteColorPickerUsedColors used__colors\"> <template is=\"for\" items=\"{{usedColors}}\" item=\"color\" index=\"indexVal\"> <li class=\"lyteColorPicker__colorpan {{lyteUiAddPE(color)}}\" onmouseenter=\"{{action('colorhover',event,color,this,'usedColor')}}\" lt-prop-title=\"{{lyteUiRgbToHex(color)}}\" lt-prop-tooltip-config=\"{&quot;position&quot; : &quot;bottom&quot;,&quot;showdelay&quot; : &quot;1000&quot;}\" onclick=\"{{action(&quot;selectColor&quot;,event,color)}}\" tabindex=\"-1\" data-col=\"{{indexVal}}\" aria-label=\"{{lyteUiRgbToHex(color)}}\" aria-selected=\"false\">{{color}}</li> </template> </ul> </div> </div> </template></template> <template is=\"if\" value=\"{{cpInline}}\"><template case=\"true\"> <template is=\"if\" value=\"{{cpMoreColorOptionSelected}}\"><template case=\"true\"> <div class=\"lyteColorPicker__commandbar\"> <lyte-button class=\"LCP_backbtn\" lt-prop-size=\"small\" lt-prop-appearance=\"default\" onclick=\"{{action(&quot;fromAdvCPtoBasic&quot;,event)}}\"> <template is=\"registerYield\" yield-name=\"text\">{{lyteUiI18n(cpBackBtnText)}}</template> </lyte-button> <template is=\"if\" value=\"{{cpInlineDoneButton}}\"><template case=\"true\"><lyte-button class=\"LCP_done\" lt-prop-size=\"small\" lt-prop-appearance=\"primary\" onclick=\"{{action(&quot;callOnSelect&quot;,event,true)}}\"> <template is=\"registerYield\" yield-name=\"text\">{{lyteUiI18n(cpDoneBtnText)}}</template> </lyte-button></template></template> </div> </template></template> </template><template case=\"false\"> <div class=\"lyteColorPicker__commandbar\"> <template is=\"if\" value=\"{{cpMoreColorOptionSelected}}\"><template case=\"true\"> <lyte-button class=\"LCP_backbtn\" lt-prop-size=\"small\" lt-prop-appearance=\"default\" onclick=\"{{action(&quot;fromAdvCPtoBasic&quot;,event)}}\"> <template is=\"registerYield\" yield-name=\"text\">{{lyteUiI18n(cpBackBtnText)}}</template> </lyte-button> <lyte-button class=\"LCP_done\" lt-prop-size=\"small\" lt-prop-appearance=\"primary\" onclick=\"{{action(&quot;callOnSelect&quot;,event,true)}}\"> <template is=\"registerYield\" yield-name=\"text\">{{lyteUiI18n(cpDoneBtnText)}}</template> </lyte-button> </template><template case=\"false\"> <lyte-button class=\"LCP_cancel\" lt-prop-size=\"small\" style=\"margin-left: auto; margin-right: 7px;\" lt-prop-appearance=\"default\" onclick=\"{{action(&quot;closeColorPicker&quot;,event)}}\"> <template is=\"registerYield\" yield-name=\"text\">{{lyteUiI18n(cpCancelBtnText)}}</template> </lyte-button> <lyte-button class=\"LCP_done\" lt-prop-size=\"small\" lt-prop-appearance=\"primary\" onclick=\"{{action(&quot;callOnSelect&quot;,event,true)}}\"> <template is=\"registerYield\" yield-name=\"text\">{{lyteUiI18n(cpApplyBtnText)}}</template> </lyte-button> </template></template> </div> </template></template> </div> </template></template> </template>",
_dynamicNodes : [{"type":"attr","position":[1]},{"type":"if","position":[1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1]},{"type":"if","position":[1,1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,1,1]},{"type":"attr","position":[1,1,3]},{"type":"text","position":[1,1,3,0]}]}},"default":{}},{"type":"attr","position":[1,3]},{"type":"if","position":[1,3],"cases":{"true":{"dynamicNodes":[{"type":"insertYield","position":[1]}]}},"default":{}},{"type":"attr","position":[1,5]},{"type":"if","position":[1,5],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,3,0]}]}},"default":{}},{"type":"attr","position":[1,7]},{"type":"text","position":[1,7,1,0]},{"type":"attr","position":[1,7,3]},{"type":"attr","position":[1,7,3,1,1]},{"type":"for","position":[1,7,3,1,1],"dynamicNodes":[{"type":"attr","position":[1]},{"type":"attr","position":[1,1]},{"type":"for","position":[1,1],"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,0]}]}]},{"type":"attr","position":[1,9]},{"type":"if","position":[1,9],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,1,0]},{"type":"attr","position":[1,3,1,1]},{"type":"for","position":[1,3,1,1],"dynamicNodes":[{"type":"attr","position":[1],"attr":{"style":{"name":"style","helperInfo":{"name":"concat","args":["'background: '","color","';'"]}}}},{"type":"text","position":[1,0]}]}]}},"default":{}},{"type":"attr","position":[1,11]},{"type":"if","position":[1,11],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,1,0]},{"type":"attr","position":[1,3,1,1]},{"type":"for","position":[1,3,1,1],"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,0]}]}]}},"default":{}},{"type":"attr","position":[1,13]},{"type":"if","position":[1,13],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,3,0]}]}},"default":{}}]},"false":{"dynamicNodes":[{"type":"attr","position":[1,1]},{"type":"if","position":[1,1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,3,0]}]}},"default":{}},{"type":"attr","position":[1,5,1,1,1]},{"type":"attr","position":[1,5,1,3,1]},{"type":"attr","position":[1,5,3]},{"type":"attr","position":[1,5,3,1,1]},{"type":"attr","position":[1,5,3,3]},{"type":"if","position":[1,5,3,3],"cases":{"true":{"dynamicNodes":[]}},"default":{}},{"type":"attr","position":[1,5,3,5]},{"type":"if","position":[1,5,3,5],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[0,1]}]}},"default":{}},{"type":"attr","position":[1,7,1,1]},{"type":"registerYield","position":[1,7,1,1,1],"dynamicNodes":[{"type":"text","position":[1,1,0]},{"type":"componentDynamic","position":[1,3]},{"type":"componentDynamic","position":[1]},{"type":"attr","position":[3]},{"type":"attr","position":[3,1,1]},{"type":"for","position":[3,1,1],"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,1]},{"type":"componentDynamic","position":[1]}]},{"type":"componentDynamic","position":[3,1]},{"type":"componentDynamic","position":[3]}]},{"type":"componentDynamic","position":[1,7,1,1]},{"type":"registerYield","position":[1,7,2,1,1],"dynamicNodes":[{"type":"attr","position":[1,1,1]},{"type":"if","position":[1,1,1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1,1]},{"type":"componentDynamic","position":[1,1,1]},{"type":"componentDynamic","position":[1,1]},{"type":"attr","position":[1,3,1]},{"type":"componentDynamic","position":[1,3,1]},{"type":"componentDynamic","position":[1,3]},{"type":"componentDynamic","position":[1]},{"type":"componentDynamic","position":[3,1]},{"type":"text","position":[3,3,0]},{"type":"componentDynamic","position":[3,3]},{"type":"componentDynamic","position":[3]}]},"false":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"if","position":[0],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1,1]},{"type":"componentDynamic","position":[1,1,1]},{"type":"componentDynamic","position":[1,1]},{"type":"attr","position":[1,3,1]},{"type":"componentDynamic","position":[1,3,1]},{"type":"componentDynamic","position":[1,3]},{"type":"attr","position":[1,5,1]},{"type":"componentDynamic","position":[1,5,1]},{"type":"componentDynamic","position":[1,5]},{"type":"attr","position":[1,7,1]},{"type":"componentDynamic","position":[1,7,1]},{"type":"componentDynamic","position":[1,7]},{"type":"componentDynamic","position":[1]},{"type":"componentDynamic","position":[3,1]},{"type":"componentDynamic","position":[3,3]},{"type":"componentDynamic","position":[3,5]},{"type":"text","position":[3,7,0]},{"type":"componentDynamic","position":[3,7]},{"type":"componentDynamic","position":[3]}]},"false":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"if","position":[0],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1,1]},{"type":"componentDynamic","position":[1,1,1]},{"type":"componentDynamic","position":[1,1]},{"type":"attr","position":[1,3,1]},{"type":"componentDynamic","position":[1,3,1]},{"type":"componentDynamic","position":[1,3]},{"type":"attr","position":[1,5,1]},{"type":"componentDynamic","position":[1,5,1]},{"type":"componentDynamic","position":[1,5]},{"type":"attr","position":[1,7,1]},{"type":"componentDynamic","position":[1,7,1]},{"type":"componentDynamic","position":[1,7]},{"type":"componentDynamic","position":[1]},{"type":"componentDynamic","position":[3,1]},{"type":"componentDynamic","position":[3,3]},{"type":"componentDynamic","position":[3,5]},{"type":"text","position":[3,7,0]},{"type":"componentDynamic","position":[3,7]},{"type":"componentDynamic","position":[3]}]},"false":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"if","position":[0],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1,1]},{"type":"componentDynamic","position":[1,1,1]},{"type":"componentDynamic","position":[1,1]},{"type":"attr","position":[1,3,1]},{"type":"componentDynamic","position":[1,3,1]},{"type":"componentDynamic","position":[1,3]},{"type":"attr","position":[1,5,1]},{"type":"componentDynamic","position":[1,5,1]},{"type":"componentDynamic","position":[1,5]},{"type":"attr","position":[1,7,1]},{"type":"componentDynamic","position":[1,7,1]},{"type":"componentDynamic","position":[1,7]},{"type":"componentDynamic","position":[1]},{"type":"componentDynamic","position":[3,1]},{"type":"componentDynamic","position":[3,3]},{"type":"componentDynamic","position":[3,5]},{"type":"componentDynamic","position":[3,7]},{"type":"componentDynamic","position":[3]}]}},"default":{}}]}},"default":{}}]}},"default":{}}]}},"default":{}},{"type":"componentDynamic","position":[1,1]},{"type":"componentDynamic","position":[1]}]},{"type":"componentDynamic","position":[1,7,2,1]},{"type":"attr","position":[1,9]},{"type":"if","position":[1,9],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,1,0]},{"type":"attr","position":[1,3,1,1]},{"type":"for","position":[1,3,1,1],"dynamicNodes":[{"type":"attr","position":[1]},{"type":"text","position":[1,0]}]}]}},"default":{}},{"type":"attr","position":[1,11]},{"type":"if","position":[1,11],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"if","position":[1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1,1]},{"type":"registerYield","position":[1,1,1],"dynamicNodes":[{"type":"text","position":[0]}]},{"type":"componentDynamic","position":[1,1]},{"type":"attr","position":[1,3]},{"type":"if","position":[1,3],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[0]},{"type":"registerYield","position":[0,1],"dynamicNodes":[{"type":"text","position":[0]}]},{"type":"componentDynamic","position":[0]}]}},"default":{}}]}},"default":{}}]},"false":{"dynamicNodes":[{"type":"attr","position":[1,1]},{"type":"if","position":[1,1],"cases":{"true":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"registerYield","position":[1,1],"dynamicNodes":[{"type":"text","position":[0]}]},{"type":"componentDynamic","position":[1]},{"type":"attr","position":[3]},{"type":"registerYield","position":[3,1],"dynamicNodes":[{"type":"text","position":[0]}]},{"type":"componentDynamic","position":[3]}]},"false":{"dynamicNodes":[{"type":"attr","position":[1]},{"type":"registerYield","position":[1,1],"dynamicNodes":[{"type":"text","position":[0]}]},{"type":"componentDynamic","position":[1]},{"type":"attr","position":[3]},{"type":"registerYield","position":[3,1],"dynamicNodes":[{"type":"text","position":[0]}]},{"type":"componentDynamic","position":[3]}]}},"default":{}}]}},"default":{}}]}},"default":{}}],
_observedAttributes :["cpDraggable","cpCloseOnBodyClick","cpCloseOnSelection","cpAdvancedColorButton","cpAppearance","cpAccent","cpAccentColor","cpStandardColors","cpDefaultColorEnable","cpUsedColors","cpBasicColorPicker","cpSelectedColor","cpDynamicColor","cpOriginElement","cpWrapperClass","cpCurrentRgbCode","cpBoundary","cpScrollable","cpYield","cpNoFillButton","cpNoFillLabel","cpPaletteLabel","cpInline","cpAdvancedNoFillButton","cpColorFormats","cpDoneBtnText","cpCancelBtnText","cpBackBtnText","cpApplyBtnText","cpUsedColorLabel","cpStandardColorArray","cpDefaultColor","cpDuration","cpBoardColor","cpAvailableColors","cpMoreColorOptionSelected","cpOpacityLabel","cpUsedColorsNumber","cpDropBoxClass","cpDisplayPreviousColor","cpPreventInitialFocus","stopObserverOnBoardColorChange","usedColors","polygonPoints","noFillPath","moreColorsPath","currentHue","currentBrightness","currentSaturation","paletteSize","circleOffsetSize","circleOffsetBecauseOfWinWidget","posdivElPalette","dragStatus","paletteMaxX","paletteMaxY","clickOnPaletteInProgress","component","hueStatus","poxYHue","poxXHue","dropButtonValue","calledForHueMove","changedColor","tooltipTextChanged","inputChangeEvnt","hueBarSize","opacityStatus","opacityTrackSize","posXOpacity","opacity","sliderOffset","dragged","counter","cmykColors","setPrevCmyk","restrictOnChange","prevColor","cpShowHiddenColor","colorFormat"],
_observedAttributesType :["boolean","boolean","boolean","boolean","string","boolean","array","boolean","boolean","boolean","boolean","object","object","string","string","string","object","boolean","boolean","boolean","string","string","boolean","boolean","array","string","string","string","string","string","array","string","number","string","array","boolean","string","number","string","boolean","boolean","boolean","array","string","string","string","number","number","number","object","number","number","object","number","number","number","boolean","object","number","number","number","string","boolean","object","boolean","boolean","number","number","number","number","number","number","boolean","number","array","boolean","boolean","object","boolean","object"],
	...
	data : function(){
		return {
			...
			"cpAccent" : Lyte.attr('boolean',{"default": false}),
			"cpAccentColor" : Lyte.attr('array',{"default": []}),
			// "cpColorPreview" : Lyte.attr("boolean",{"default" : true}),
			"cpStandardColors" : Lyte.attr("boolean",{"default" : true}),
			"cpDefaultColorEnable" : Lyte.attr("boolean",{"default" : false}),
			...
			"cpDynamicColor" : Lyte.attr("object",{"default" : null}),
			...
			"cpAdvancedNoFillButton" : Lyte.attr("boolean",{"default": false}),
			"cpColorFormats" : Lyte.attr("array"),
			"cpDoneBtnText" : Lyte.attr("string",{"default" : "lyte.colorpicker.done"}),
			"cpCancelBtnText" : Lyte.attr("string",{"default" : "lyte.colorpicker.cancel"}),
			"cpBackBtnText" : Lyte.attr("string",{"default" : "lyte.colorpicker.back"}),
			"cpApplyBtnText" : Lyte.attr("string",{"default" : "lyte.colorpicker.apply"}),
			"cpUsedColorLabel" : Lyte.attr("string"),
			"cpStandardColorArray" : Lyte.attr("array",{"default" : []}),
			"cpDefaultColor" : Lyte.attr("string",{'default' : "rgb(0,0,0)"}),
			...
			"cpDropBoxClass" : Lyte.attr("string",{"default":''}),
			"cpDisplayPreviousColor" : Lyte.attr("boolean",{"default" : false}),
			"cpPreventInitialFocus" : Lyte.attr("boolean", {"default" : false}),
			//Local variables
			...
			"cpShowHiddenColor" : Lyte.attr('boolean',{"default": false}),
	...
	didConnect : function(){
		var self = this;
		this.$node.selectColor = function( selectedColor ){
			if(selectedColor){
				if( !(selectedColor.rgb && selectedColor.rgb.length) && !(selectedColor.rgba && selectedColor.rgba.length) && !(selectedColor.hex && selectedColor.hex.length)  &&  !(selectedColor['hex-alpha'] && selectedColor['hex-alpha'].length) && !(selectedColor.cmyk && selectedColor.cmyk.length)   ){
					self.nofillFunc(undefined);
					return;
				}
			...
			self.setData('cpSelectedColor',selectedColor);
			self.setData('cpDynamicColor',selectedColor);
			var usedColor  = self.getData('usedColors');
			var selectedColor = self.getData('cpSelectedColor');
			if(!selectedColor.nofill && (!usedColor[0] || !usedColor[0].hex || usedColor[0].hex !== selectedColor.hex)){
				var rgbCode = self.getData('cpCurrentRgbCode');
				var rgbColor = ColorPicker_Util.getRgbColorsByRgbCode(rgbCode);
				var cmykColors = self.getData('cmykColors');
				var type = self.getData('opacity') < 100 ? 'advanced' : 'basic'; 
				var selectedColor = {
					"hex" : "#"+rgbCode,
					"rgb" : "rgb("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+")",
					"opacity" : self.getData('opacity'),
					"hex-alpha" : "#"+rgbCode+ColorPicker_Util.getDecimalToHexAlphaCode(self.getData('opacity')),
					"rgba" : "rgba("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+", "+(self.getData('opacity')/100)+")",
					"cmyk" : cmykColors,
					"type" : self.getData('cpSelectedColor') ? self.getData('cpSelectedColor').type : type,
					"nofill" : self.getData('cpSelectedColor') ? self.getData('cpSelectedColor').nofill : null
				};
				self.setData('cpSelectedColor',selectedColor);
				self.setData('cpDynamicColor',selectedColor);
				self.addedUsedColor();
		...
		this.$node.addEventListener('keydown',function(event){
			...
			if(key === 9){
				if($L('.lyteColorPicker__colorpan[tabindex="0"]',self.$node)[0]){
					$L('.lyteColorPicker__colorpan[tabindex="0"]',self.$node)[0].setAttribute('tabindex',-1);
				}
				...
				if(focusedItem.classList.contains('lyteColorPicker__morecolorbutton')){
					focusedItem.click();
					self.$node.focus();
				}else if(focusedItem.classList.contains('LCP_predefinedPallete') || focusedItem.classList.contains('LCP_standardColorCont') || focusedItem.classList.contains('LCP_defaultColorCont') || focusedItem.classList.contains('usedColor__container')){
				...
				if(focusedItem.classList.contains('lyteColorPicker__colorpan')){
					...
					var col = parseInt(focusedItem.dataset.col);
					row -= 1;
					var top_color = focusedItem.parentNode.parentNode.querySelector('[data-row="' + ( row ) + '"][ data-col="' + col + '"]');
					if(top_color){
						var isVisible = window.getComputedStyle(top_color.parentNode).display == 'none' ? false : true;
						while(!isVisible){
							row -= 1;
							top_color = top_color.parentNode.parentNode.querySelector('[data-row="' + ( row ) + '"][ data-col="' + col + '"]');
							isVisible = window.getComputedStyle(top_color.parentNode).display == 'none' ? false : true;
						}
						focusedItem.setAttribute('tabindex',-1);
				...
				var col = parseInt(focusedItem.dataset.col);
				row += 1;
				var down_color = focusedItem.parentNode.parentNode.querySelector('[data-row="' + ( row ) + '"][ data-col="' + col + '"]');
				if(down_color){
					var isVisible = window.getComputedStyle(down_color.parentNode).display == 'none' ? false : true;
					while(!isVisible){
						row += 1;
						down_color = down_color.parentNode.parentNode.querySelector('[data-row="' + ( row ) + '"][ data-col="' + col + '"]');
						isVisible = window.getComputedStyle(down_color.parentNode).display == 'none' ? false : true;
					}
					focusedItem.setAttribute('tabindex',-1);
	...
	usedColorsObserver : function(newVal,oldVal){
		this.updateUsedColors(this.$node);
	}.observes('usedColors'),
	...
	cpAvailableColorsObserver : function(){
		this.initializeBasicColorPicker(undefined,this.$node);
	}.observes('cpAvailableColors'),
	...
	initializeAdvColorPicker : function(event,ele,restrictCmykChange) {
		...
		if(!cpDiv.addedEvents){
			...
			ele.querySelector('.opacityslider__circlethumb').addEventListener('keydown', function(event) {
				ColorPicker_Util.component = ColorPicker_Util.getColorPicker(event.target).component;
				var opacity = ColorPicker_Util.component.getData('opacity');
	            if(event.keyCode == 37){
					opacity = Math.max(opacity - 1,0);
					ColorPicker_Util.component.setData('opacity', opacity );
				}else if(event.keyCode == 39){
					opacity = Math.min(opacity + 1,100);
					ColorPicker_Util.component.setData('opacity', opacity );
				}
				ColorPicker_Util.component.__updateOpacityInForm();
	        });
			ele.querySelector('.colorSlider_sliderHandle').addEventListener('keydown', function(event) {
				ColorPicker_Util.component = ColorPicker_Util.getColorPicker(event.target).component;
				var currentHue = ColorPicker_Util.component.getData('currentHue');
				var sliderOffset = ColorPicker_Util.component.getData('sliderOffset');
				var barSize = ColorPicker_Util.component.getData('hueBarSize');
				var singlehueSize = ( barSize / 360 );
		        var diff;
		        diff = event.target.getBoundingClientRect().left - event.target.offsetParent.getBoundingClientRect().left;
		        
				if(event.keyCode == 37){
					diff = diff - singlehueSize;
					currentHue = Math.min(currentHue + 1,360);
					ColorPicker_Util.component.setData('currentHue', currentHue );
				}else if(event.keyCode == 39){
					diff = diff + singlehueSize;
					currentHue = Math.max(currentHue - 1,0);
					ColorPicker_Util.component.setData('currentHue', currentHue );
				}
				if (diff > barSize - sliderOffset){
		            diff = barSize - sliderOffset;
		        }
		        if (diff < 0 - sliderOffset){
		            diff = 0 - sliderOffset;
		        }
	        	event.target.style.left = diff + 'px';
				ColorPicker_Util.component.__setCurrentRgbCode();
		        ColorPicker_Util.component.__setPaletteBgColor();
		        ColorPicker_Util.component.__updateRgbInForm();
		        ColorPicker_Util.component.__changeColorInOpacityTrack();
				ColorPicker_Util.component.__setOpacityColorPreviewDiv();
			})
		...
		if( this.getData('cpAdvancedUsedColor') ){
			this.updateUsedColors(ele);
		}
		if( this.getData('cpEnableEyeDropper') ){
			$L.eyeDropper.initialize({
				'query': '#lyteCP_eyeDropper',
				'onPick': function (colorValue) {
					this.setData("cpCurrentRgbCode", colorValue.slice(1));
					this.setRgbColor(colorValue);
				}.bind(this)
			});
		}
	...
	initializeBasicColorPicker : function(event,ele){
		...
		var first_focus = ele.querySelector('[tabindex="0"]');
		if(first_focus && (this.rendered || (!this.getData('cpInline') && !this.getData('cpPreventInitialFocus') ) ) ){
			first_focus.focus();
		}
		...
		// 	}
		// }
		if(this.getData('cpDefaultColor') && this.getData('cpDefaultColorEnable')){
			liELes = ele.querySelectorAll('.cpDefault__colors');
			liELes[0].style.background = liELes[0].textContent;
		}
		this.updateUsedColors(ele);
		...
		$L.fastdom.mutate(function(){
			var selectColor = _this.getData('cpSelectedColor');
			var prev_element = $L('.lyteCPSelectedColor');
			if(prev_element.length){
				prev_element.removeClass('lyteCPSelectedColor');
			}
			if( selectColor && !selectColor.nofill && Object.keys(selectColor).length && !_this.isColorObjectEmpty(selectColor)){
				var ele;
				if(_this.getData('cpAccent')){
					ele = _this.$node.querySelector('[data-color="'+selectColor.hex+'"]');
				}else{
					ele = _this.$node.querySelector('[lt-prop-title="'+selectColor.hex.toUpperCase()+'"]');
				}
				var alpha = ColorPicker_Util.getDecimalToHexAlphaCode(_this.getData('opacity'));
				ele = _this.getHexAlphaSelectColor(ele, selectColor.hex, alpha, selectColor);
				ele && ele.classList.add('lyteCPSelectedColor');
			}
		})
	...
	updateUsedColors : function(ele){
		if(this.getData('cpUsedColors')){
			var liELes = ele.querySelectorAll('.used__colors > li');
			for(var v =0; v<liELes.length ; v++){
				liELes[v].style.background = liELes[v].textContent;
			}
		}
	...
	getHexAlphaSelectColor : function( ele, color, alpha, selectColor ){
		if(!ele || ( color + alpha !== selectColor['hex-alpha'] && selectColor['hex-alpha'] )  ){
			ele = this.$node.querySelector('[lt-prop-title="'+selectColor['hex-alpha'].toUpperCase()+'"]');
		}else if (color + alpha === selectColor['hex-alpha']  &&  this.getData('opacity') < 100 ){
			ele = this.$node.querySelector('[lt-prop-title="'+selectColor['hex-alpha'].toUpperCase()+'"]');
		}
		return ele;
	},
	addedUsedColor : function(){
		...
		$L.fastdom.mutate(function(){
			setTimeout(function(){
				var selectColor = _this.getData('cpSelectedColor');
				var alpha = ColorPicker_Util.getDecimalToHexAlphaCode(_this.getData('opacity'));
				if(selectColor && !selectColor.nofill && Object.keys(selectColor).length){
					var ele;
					var color;
					if(_this.getData('cpAccent')){
						ele = _this.$node.querySelector('[data-color="'+selectColor.hex+'"]');
						color = ele ? ele.dataset.color : '';
					}else{
						ele = _this.$node.querySelector('[lt-prop-title="'+selectColor.hex+'"]');
						color = ele ? ele.getAttribute('lt-prop-title') : '';
					}
					var prev_element = _this.$node.querySelector('.lyteCPSelectedColor');
						prev_element && prev_element.classList.remove('lyteCPSelectedColor');
					ele = _this.getHexAlphaSelectColor(ele, selectColor.hex, alpha, selectColor);
					ele && ele.classList.add('lyteCPSelectedColor');
				}
	...
	executeOnSelect : function(event, close, element){
		var colors = this.getData('usedColors');
		if(this.data.cpSelectedColor){
			this.data.cpSelectedColor.nofill =  false;
		} 
		this.__setSelectedColor(element);
		this.addedUsedColor();
	...
	executeOnCancelButton : function(event){
		if(this.getMethods('onCancelButton')){
			this.executeMethod('onCancelButton',event);
		}
	},
  ...
  //       this.setData("cpSelectedColor",selectedColor);
	...
	__setSelectedColor : function( element ) {
		...
		var type = this.getData('opacity') < 100 ? 'advanced' : 'basic'; 
        var selectedColor = {
        	"hex" : "#"+rgbCode,
        	"rgb" : "rgb("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+")",
        	"opacity" : this.getData('opacity'),
        	...
        	"rgba" : "rgba("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+", "+(this.getData('opacity')/100)+")",
        	"cmyk" : cmykColors,
			"type" : this.getData('cpSelectedColor') ? this.getData('cpSelectedColor').type : type,
			"nofill" : this.getData('cpSelectedColor') ? this.getData('cpSelectedColor').nofill : null

        };
		if(this.getData('cpAccent') && element && element.classList.contains('lyteColorPickerAccentColor') && element.dataset.row){
			var accentColor = this.getData('cpAccentColor');
			var colorTone = accentColor[element.dataset.col].colorTone[element.dataset.row - 1];
			if(colorTone){
				selectedColor.colorTone = {
					toneType : colorTone.toneType,
					tonePercent : colorTone.tonePercent
				}
			}else if(accentColor){
				selectedColor.colorTone = {
					colorType : accentColor[element.dataset.col].colorType
				}
			}
		}
		var dropdown = $L('.cPDropDown',this.$node)[0];
		if(this.getData('cpDropdownType') && dropdown){
			this.setData('cpDropdownType',dropdown.getData('ltPropSelected'))
		}
        this.setData("cpSelectedColor",selectedColor);
		this.setData("cpDynamicColor",selectedColor);
	},
    ...
    __changeViewAfterColorChange : function(restrictCmykChange) {
		...
		this.__setOpacityColorDynamicDiv();
		this.__updateOpacityInForm();
    },
	...
	__setOpacityColorDynamicDiv : function(){
		if(this.data.cpDisplayPreviousColor){
			var ele = this.$node /*this.getData('cpInline') ? this.$node : this.childComp*/;
			var rgbColor =  ColorPicker_Util.getRgbColorsByRgbCode(this.getData("cpCurrentRgbCode"));
			ele.querySelector('.dynamicDiv').style.backgroundColor = "rgba("+rgbColor.red+","+rgbColor.green+","+rgbColor.blue+","+(this.getData('opacity')/100)+")";
		}
    },
	...
	defaultSelectedColor : function(event,color,element){
		...
		this.setData('currentBrightness', hsv.brightness * 100);
		this.setData('currentSaturation', hsv.saturation * 100);
		this.executeOnSelect(event,undefined,element);
	...
	isColorObjectEmpty : function(selectColor) {
		var selectColor = selectColor || this.getData("cpSelectedColor");
		var keys = Object.keys(selectColor);
		var flag = true;
		for(var i=0; i<keys.length; i++) {
			if(selectColor[keys[i]]){
				flag = false;
				break;
			}
		}
	...
	selectedColor : function() {
		...
		if(selectedColor &&  Object.keys(selectedColor).length && !this.isColorObjectEmpty(selectedColor)) {
			var _this = this;
			var rgb; 
			var count = 1;
			var rgbColor;
			
			if(selectedColor.rgb && selectedColor.rgb.length) {
				rgb = selectedColor.rgb;
			}else if(selectedColor.rgba && selectedColor.rgba.length) {
				var rgbaString = selectedColor.rgba;
				var regex = /rgba\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(0|1|0?\.\d+)\s*\)/;
				var color = rgbaString.match(regex);
				selectedColor.opacity = color[4]* 100;
				this.setData('opacity',color[4]*100);
				rgb = "rgb("+color[1]+", "+color[2]+", "+color[3]+")";
			}
			else if(selectedColor.hex && selectedColor.hex.length) {
			    var color = selectedColor.hex.slice(1);
				if(color.length == 3) {
					color = ColorPicker_Util.convert3DigitTo6DigitRgbCode(color);
				}
				rgbColor = ColorPicker_Util.getRgbColorsByRgbCode(color);
				
				rgb = "rgb("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+")";
			}
			else if(selectedColor['hex-alpha'] && selectedColor['hex-alpha'].length){
				var color = selectedColor['hex-alpha'].slice(1);
				if(color.length == 3) {
					color = ColorPicker_Util.convert3DigitTo6DigitRgbCode(color);
				}
				rgbColor = ColorPicker_Util.getRgbColorsByRgbCode(color);
				var opacity = ColorPicker_Util.getHexToDecimalAlphaCode(color.substr(6, 4));
				selectedColor.opacity = opacity;
				this.setData('opacity',opacity);
				rgb = "rgb("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+")";
			}
			else if(selectedColor.cmyk && selectedColor.cmyk.length) {
				var color = selectedColor.cmyk;
				rgbColor = ColorPicker_Util.getRgbColorsByCmyk(color[0],color[1],color[2],color[3]);
				rgb = "rgb("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+")";
			}
			if(selectedColor.opacity !== undefined){
				this.setData('opacity',selectedColor.opacity);
			}else{
				this.setData('opacity', 100);
			}

			if( !selectedColor.nofill && rgb && (selectedColor.type == "basic" || selectedColor.type == undefined)) {
				rgbCode =  rgb.replace(/[^\d,]/g, '').split(',');
				
				var color =  ColorPicker_Util.getRgbCodeByRgbColors(rgbCode[0],rgbCode[1],rgbCode[2]);
				var element;
				if(this.getData('cpAccent')){
					element = this.$node.querySelector('[data-color= "#' + color + '"]');
				}else{
					element = this.$node.querySelector('[lt-prop-title= "#' + color + '"]');
				}
				var alpha = ColorPicker_Util.getDecimalToHexAlphaCode(this.getData('opacity'));
				
				if(selectedColor['hex-alpha'] && selectedColor.hex + alpha !== selectedColor['hex-alpha'] || !element ){
					element = this.$node.querySelector('[lt-prop-title="' + selectedColor['hex-alpha'] +'"]') ;
					
				}else if(selectedColor.hex + alpha === selectedColor['hex-alpha'] && _this.getData('opacity') < 100 ){
					element = this.$node.querySelector('[lt-prop-title="' + selectedColor.hex + alpha.toUpperCase() +'"]') ;
				}

				if(element && !element.classList.contains('lyteCPSelectedColor')){
					var rgbColor = ColorPicker_Util.getRgbColorsByRgbCode(color+alpha);
					var opacity = selectedColor.opacity;
					
					if(opacity){
						color = "rgba("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+", "+this.getData('opacity')/100+")";
					...
					this.data.cpSelectedColor.rgba = "rgba("+rgbColor.red+", "+rgbColor.green+", "+rgbColor.blue+", "+(this.getData('opacity')/100)+")";
				}
				...
				if(selectedColor){
					...
					rgbCode = ('rgb(0,0,0)').replace(/[^\d,]/g, '').split(',');
				}
	...
	nofillFunc : function(event){
		...
		var advancedInput = $L('#lyteCPShowValue',this.$node)[0];
		if(advancedInput){
			advancedInput.setData('ltPropValue','#000000');
		}
		$L('.lyteCPSelectedColor',this.childComp).removeClass('lyteCPSelectedColor');
		this.setData('cpSelectedColor',selectedColor);
		this.setData('cpDynamicColor',selectedColor);
	...
	fromAdvCPtoBasic : function(event){
		...
		if( this.rendered ||  ( !this.getData('cpInline') && !this.getData('cpPreventInitialFocus') ) ){
			this.$node.focus();
		}
	...
	basicColorpickerObs : function(){
		if(this.getData('cpBasicColorPicker')){
			this.fromAdvCPtoBasic();
		}else{
			this.goToAdvancedCP();
		}
	}.observes('cpBasicColorPicker'),
	methods : {
		...
		afterInputRender : function(type,input){
			var self = this;
			input.querySelector('input').addEventListener('paste', function(e){
				self.pasteFunc(e,type);
			});
		},
		changeFormatView : function(e,value){
			var currentTarget = _lyteUiUtils.getCurrentTarget( e );
			var dropDown = $L('.cPDropDown',this.$node)[0];
			if(this.getData('cpDropdownType') && dropDown){
				this.setData('cpDropdownType',dropDown.getData('ltPropSelected'))
			}
			if( currentTarget.tagName == 'LYTE-DROP-BOX' ){
	    		this.__updateRgbInForm(true);
			}
	    }
	},
	actions : {		
		colorhover : function(event,color,elem,colorPallete){
			if(this.getMethods('onColorHover')){
				this.executeMethod('onColorHover',color,elem,colorPallete);
			}
		},
		...
		callOnSelect : function(event,close){
			if(this.getData('cpInline') && this.getData('cpInlineDoneButton')){
				this.setData('cpBasicColorPicker',true);
			}
		...
		changeValueOfDynamicColor: function (params) {
			this.$node.querySelector('.previewDiv').style.backgroundColor = this.$node.querySelector('.dynamicDiv').style.backgroundColor;
		}
	}
});
Lyte.Component.registerHelper("setDynamicColorClass", function (_this,cpDisplayPreviousColor) {
	if (cpDisplayPreviousColor) {
		return "lyteCP__dynamicColor";
	}
});
...
_observedAttributes :["ltPropShow","ltPropDraggable","ltPropCloseOnBodyClick","ltPropCloseOnSelection","ltPropAdvancedColorButton","ltPropAppearance","ltPropStandardColors","ltPropDefaultColorEnable","ltPropUsedColors","ltPropBasicColorPicker","ltPropSelectedColor","ltPropOriginElement","ltPropWrapperClass","ltPropCurrentRgbCode","ltPropBoundary","ltPropScrollable","ltPropNoFillButton","ltPropNoFillLabel","ltPropPaletteLabel","ltPropInline","ltPropColorFormats","ltPropDoneBtnText","ltPropCancelBtnText","ltPropBackBtnText","ltPropApplyBtnText","ltPropStandardColorArray","ltPropDefaultColor","ltPropDuration","ltPropBoardColor","ltPropOpacityLabel","ltPropPopover","ltPropUsedColorsNumber","ltPropUsedColorsList","ltPropOpacity","ltPropFreeze","ltPropEnableEyeDropper","ltPropAdvancedUsedColor","ltPropPreventInitialFocus","ltPropUsedColorLabel","ltPropDisplayPreviousColor","ltPropAccent","ltPropAccentColor","ltPropDropBoxClass","ltPropFocusOnClose","ltPropShowHiddenColor","ltPropAdvancedNoFillButton","ltPropColorFormatDropdown","ltPropInlineDoneButton","moreColorOptionSelected","availableColors","usedColors","colorpicker","dropButtonValue","ltPropYield"],
_observedAttributesType :["boolean","boolean","boolean","boolean","boolean","string","boolean","boolean","boolean","boolean","object","string","string","string","object","boolean","boolean","string","string","boolean","array","string","string","string","string","array","string","number","string","string","object","number","array","number","boolean","boolean","boolean","boolean","string","boolean","boolean","array","string","boolean","boolean","boolean","string","boolean","boolean","array","array","object","string","boolean"],

	_lyteUtilFunctions: ['selectColor'],
	data: function () {
		return {
			//Component Variables
			...
			/**
						 ...
						 * @input
						 */
			"ltPropDraggable": Lyte.attr("boolean", { "default": false, 'input' : 'true', 'output' : 'false' }),

			/**
						 * @componentProperty {boolean} ltPropCloseOnBodyClick
						 * @version 1.0.0
						 * @default true
						 * @condition ltPropInline false
						 * @input
						 */
			"ltPropCloseOnBodyClick": Lyte.attr("boolean", { "default": true, 'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropCloseOnSelection": Lyte.attr("boolean", { "default": false, 'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropAdvancedColorButton": Lyte.attr("boolean", { "default": true, 'input' : 'true', 'output' : 'false' }),

			/**
						 * @componentProperty {callout|box} ltPropAppearance
						 * @version 1.0.0
						 * @default callout
						 * @condition ltPropInline false
						 * @options callout,box
						 * @input
						 */
			"ltPropAppearance": Lyte.attr("string", { "default": "callout", 'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropStandardColors": Lyte.attr("boolean", { "default": true, 'input' : 'true', 'output' : 'false' }),
			"ltPropDefaultColorEnable": Lyte.attr("boolean", { "default": false }),
			...
			"ltPropUsedColors": Lyte.attr("boolean", { "default": true, 'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropBasicColorPicker": Lyte.attr("boolean", { "default": true,'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropOriginElement": Lyte.attr("string", { "default": "",'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropWrapperClass": Lyte.attr("string", { "default": "lyteColorPickerPopover",'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropBoundary": Lyte.attr("object", { "default": null,'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropScrollable": Lyte.attr("boolean", { "default": true,'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropNoFillButton": Lyte.attr("boolean", { "default": false,'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropNoFillLabel": Lyte.attr("string", { "default": "",'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropPaletteLabel": Lyte.attr("string", { "default": "",'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropInline": Lyte.attr("boolean", { "default": false,'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropColorFormats": Lyte.attr("array", { "default": ["HEX", "RGB", "HSV", "CMYK"],'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropDoneBtnText": Lyte.attr("string", { "default": "lyte.colorpicker.done",'input' : 'true', 'output' : 'false' }),

			/**
						 * @componentProperty {string} ltPropCancelBtnText
						 * @condition ltPropBasicColorpicker false
						 * @condition moreColorOptionSelected false
						 * @version 2.0.0
						 * @default Cancel
						 * @input
						 */
			"ltPropCancelBtnText": Lyte.attr("string", { "default": "lyte.colorpicker.cancel",'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropBackBtnText": Lyte.attr("string", { "default": "lyte.colorpicker.back",'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropApplyBtnText": Lyte.attr("string", { "default": "lyte.colorpicker.apply",'input' : 'true', 'output' : 'false' }),

			/**
						 * @componentProperty {array} ltPropStandardColorArray
						 * @version 2.0.0
						 * @condition ltPropStandardColors true
						 * @input
						 */
			"ltPropStandardColorArray": Lyte.attr("array", { "default": [],'input' : 'true', 'output' : 'false' }),
			"ltPropDefaultColor": Lyte.attr("string", { "default": 'rgb(0,0,0)'}),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropDuration": Lyte.attr("number", { "default": 600,'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropBoardColor": Lyte.attr("string", { "default": "#90c3d4",'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropOpacityLabel": Lyte.attr("string",{'input' : 'true', 'output' : 'false'}),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropPopover": Lyte.attr("object", { "default": {},'input' : 'true', 'output' : 'false' }),

			/**
						 * @componentProperty {number} ltPropUsedColorsNumber
						 * @version 3.8.0
						 * @default 10
						 * @condition ltPropUsedColors true
						 * @input
						 */
			"ltPropUsedColorsNumber": Lyte.attr("number", { "default": 10,'input' : 'true', 'output' : 'false' }),
			...
			/**
						 ...
						 * @condition ltPropUsedColors true
						 */
			"ltPropUsedColorsList": Lyte.attr("array", { "default": [],'input' : 'true', 'output' : 'true' }),
			...
			/**
						 ...
						 * @input
						 */
			"ltPropOpacity": Lyte.attr("number", { "default": 100,'input' : 'true', 'output' : 'false' }),
			/**
						 ...
						 * @input
						 */
			"ltPropFreeze": Lyte.attr("boolean", { "default": false,'input' : 'true', 'output' : 'false' }),
			"ltPropEnableEyeDropper" : Lyte.attr("boolean", {default : false}),
			"ltPropAdvancedUsedColor" : Lyte.attr("boolean", {default : false}),
			"ltPropPreventInitialFocus": Lyte.attr("boolean", { "default": false }),
			"ltPropUsedColorLabel": Lyte.attr("string", { "default": _lyteUiUtils.i18n('Recently.Used.Colors')}),
			"ltPropDisplayPreviousColor": Lyte.attr("boolean", { "default": false }),
			...
			"ltPropColorFormatDropdown": Lyte.attr('string', { "default": '' }),
			"ltPropInlineDoneButton" : Lyte.attr('boolean', { "default": false }),
			//Local variables
	...
	didConnect: function () {
		...
		/**
		 * @utility selectColor
		 * @author santhosraj.s <santhoshraj.s@zohocorp.com>
		 * @version 1.0.0
		 * @param { object } selectcolor
		 */
		this.$node.selectColor = function(selectColor){
			var cp_com = $L('colorpicker-ui', _this.$node)[0];
			cp_com = cp_com ? cp_com : $L('colorpicker-ui', _this.childComp)[0];
			if(cp_com){
				cp_com.selectColor(selectColor);
			}else{
				_this.setData('ltPropSelectedColor',selectColor)
			}
		} 
	},
	updateUsedColors: function(){
		...
		var used__colors = usedColorsList.slice(0, len);
		this.data.ltPropUsedColorsList = used__colors;
		this.childComp && this.childComp.querySelector('colorpicker-ui').setData('usedColors', used__colors);
	}.observes('ltPropUsedColorsList','ltPropUsedColorsNumber').on('init'),
	...
	showColorPicker: function () {
		...
		if(this.getData('colorpicker') && !this.getData('colorpicker').rendered){
			this.getData('colorpicker').rendered = true;
		}

	}.observes('ltPropShow', 'ltPropInline', 'ltPropBasicColorPicker').on('didConnect'),
	observerForWrapperClass : function(){
		var popOver = this.$node.querySelector('.popColorPicker');
		popOver.ltProp({ "wrapperClass": this.getData("ltPropWrapperClass") });
	}.observes('ltPropWrapperClass'),
	...
	executeOnBeforeOpen: function () {
		/**
		 * Triggered before the color picker is opened.
		 * @method onBeforeOpen
		 * @condition ltPropInline false
	...
	executeOnOpen: function () {
		/**
		 * Triggered when the color picker is opened.
		 * @method onOpen
		 * @author santhoshraj.s@zohocorp.com
		 * @condition ltPropInline false
	...
	executeOnCloseFn: function (event) {
		/**
		 * It is invoked when the colorpicker is closed.
		 * @method onClose
		 * @author santhoshraj.s@zohocorp.com
		 * @condition ltPropInline false
		 * @param { object } event
	...
	methods: {
		...
		showColorPickerPopover: function () {
			...
			var ele = this.childComp.querySelector('.popoverWrapper');
			var popoverNode = $L('.lyteColorPickerPopover').find('.lytePopover')[0];
		...
		executeOnSelect: function (event, close, calledForHueMove) {
			 ...
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } event
			 * @param { object } selectedColorObject
			*/
		...
		executeOnChange: function (event) {
			/**
			 * Triggered when the user moves the pointer on advanced colorpicker palette to change from one color to another.
			 * @method onChange
			 * @author santhoshraj.s@zohocorp.com
			 * @param { object } event
			 * @param { object } changedColorObject
			 * @param { object } component
			*/
			if (this.getMethods('onChange')) {
		...
		executeOnInputError: function (type) {
			/**
			 * Triggered when the user enters invalid value in any of the color input fields in advanced colorpicker. This method can be utilised by the developer to show an error message for invalid value entered by the user.
			 * @method onInputError
			 * @author santhoshraj.s@zohocorp.com
			 * @condition ltPropAdvancedColorButton true
			 * @param { string } type
			*/
			if (this.getMethods('onInputError')) {
